#!/bin/bash
# Perform install for hccl package
...
##########################################################################
log "INFO" "step into run_hccl_install.sh ......"
...
new_install() {
    ...
    sh "$curpath/install_common_parser.sh" --copy_all --package="hccl" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall \
