#!/bin/bash
# Perform install/upgrade/uninstall for hccl package
...
hccl_func_path="${curpath}/hccl_func.sh"
...
update_install_info_feature() {
    ...
    if [ "$featuremode" = "all" ] || [ "$operation" = "Upgrade" ]; then
        update_install_param "Hccl_Feature_Type" "$featuremode" "$install_info"
        return
    fi
    local current_featuremode=$(get_install_param "Hccl_Feature_Type" "$install_info")
...
update_install_info() {
    ...
    update_install_param "Hccl_Install_Type" "$installmode" "$install_info"
    update_install_param "Hccl_Chip_Type" "$chipmode" "$install_info"
    update_install_info_feature
    update_install_param "Hccl_UserName" "$username" "$install_info"
    update_install_param "Hccl_UserGroup" "$usergroup" "$install_info"
    update_install_param "Hccl_Install_Path_Param" "$install_path_param" "$install_info"
    update_install_param "Hccl_Arch_Linux_Path" "$arch_linux_path" "$install_info"
    update_install_param "Hccl_Hetero_Arch_Flag" "$hetero_arch" "$install_info"
}
...
install_run() {
    ...
    local hccl_install_path_param=""
    update_install_path
    update_install_info
    local hccl_input_install_path=$(get_install_param "Hccl_Install_Path_Param" "${install_info}")
    local hccl_install_type=$(get_install_param "Hccl_Install_Type" "${install_info}")
    if [ "$is_docker_install" = "y" ]; then
        hccl_install_path_param=$(concat_docker_install_path "${docker_root}" "${hccl_input_install_path}")
    else
        hccl_install_path_param="${hccl_input_install_path}"
    fi
    if [ "$1" = "install" ]; then
        unchattr_files
        chmod_start
        ...
        if [ $? -eq 0 ]; then
            ...
            log "INFO" "Hccl package installed successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "Hccl package install failed, please retry after uninstall!"
...
upgrade_run() {
    ...
    if [ -f "$install_info" ]; then
        local hccl_input_install_path=$(get_install_param "Hccl_Install_Path_Param" "${install_info}")
        local hccl_install_type=$(get_install_param "Hccl_Install_Type" "${install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i hccl_install_path_param "$install_info_old") -ne 0 ]; then
        local hccl_input_install_path="$(grep -iw hccl_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local hccl_install_type="$(grep -iw hccl_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ "$1" = "upgrade" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            log "INFO" "Hccl package upgraded successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "Hccl package upgrade failed, please retry after uninstall!"
            log_operation "${operation}" "failed!"
            exit_install_log 1
        fi
    fi
...
uninstall_run() {
    ...
    if [ -f "$upgrade_install_info" ]; then
        local hccl_input_install_path=$(get_install_param "Hccl_Install_Path_Param" "${upgrade_install_info}")
        local hccl_install_type=$(get_install_param "Hccl_Install_Type" "${upgrade_install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i hccl_install_path_param "$install_info_old") -ne 0 ]; then
        local hccl_input_install_path="$(grep -iw hccl_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local hccl_install_type="$(grep -iw hccl_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ ! -f "$upgrade_default_dir/script/run_hccl_uninstall.sh" ]; then
        log "WARNING" "run_hccl_uninstall.sh not found."
        return $?
    fi
    if [ "$1" = "uninstall" ]; then
        chmod_start "$upgrade_default_dir"
        new_echo "INFO" "uninstall ${hccl_install_path_param} ${hccl_install_type}"
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $(id -u) -eq 0 ]; then
                if [ "$uninstall" = "y" ] && [ -f "$install_info_old" ] && [ $(grep -c -i hccl_install_path_param "$install_info_old") -ne 0 ]; then
                    sed -i '/hccl_install_path_param=/Id' "$install_info_old" 2> /dev/null
                    sed -i '/hccl_install_type=/Id' "$install_info_old" 2> /dev/null
                fi
            fi
            new_echo "INFO" "Hccl package uninstalled successfully! Uninstallation takes effect immediately."
            log "INFO" "Hccl package uninstalled successfully! Uninstallation takes effect immediately."
            log_operation "${operation}" "succeeded"
        else
            log "ERROR" "Hccl package uninstall failed!"
            log_operation "${operation}" "failed!"
            exit_uninstall_log 1
        fi
    fi
...
judgmentpath() {
    ...
    if [ $? -ne 0 ]; then
        log "ERROR" "The Hccl install_path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exit 1
    fi
}
...
migrate_user_assets_v2() {
    if [ "$pkg_is_multi_version" = "true" ] && [ "$hetero_arch" != "y" ]; then
        get_package_last_installed_version_dir "last_installed_dir" "$pkg_install_path" "hccl"
        if [ -n "$last_installed_dir" ]; then
            ...
            local data_dir="hccl/data/fusion_strategy"
...
if [ "$featuremode" != "all" ]; then
    ...
    if [ "$ret" = "false" ]; then
        log "WARNING" "Hccl package doesn't contain features $featuremode, skip installation."
        exit 0
    fi
fi
...
if [ "$pkg_is_multi_version" = "true" ] && [ "$hetero_arch" != "y" ]; then
    default_dir="${pkg_install_path}/$pkg_version_dir/share/info/hccl"
else
    default_dir="${pkg_install_path}/share/info/hccl"
fi
...
uninstall_none_multi_version "$pkg_install_path/share/info/hccl"
check_install_for_all
create_default_install_dir_for_common_user
log_base_version
...
if [ "x$version_installed" != "x" -a "$version_installed" != "none" ] || [ -f "${install_info}" ]; then
    ...
    # 升级场景
        ...
        if [ -n "$pkg_version_dir" ]; then
            if [ "$hetero_arch" = "y" ]; then
                ...
                get_package_upgrade_install_info "upgrade_install_info" "$pkg_install_path" "hccl"
            fi
    ...
    # 安装场景
        ...
        if [ "$is_quiet" = "n" ]; then
            log "INFO" "Hccl package has been installed on the path ${pkg_install_path}, the version is ${version_installed}, and the version of this package is ${version_of_package}, do you want to continue? [y/n]"
            while true
            do
                read yn
                if test "$yn" = n; then
