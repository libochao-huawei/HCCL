    ...
    AlgResourceCtx& resCtxHost, void **resCtxSequence, uint64_t *ctxSize)
{
    ...
    // === 打印 resCtxHost channels 信息 ===
    printf("[DEBUG] resCtxHost.channels size: %lu\n", resCtxHost.channels.size());
    for (size_t i = 0; i < resCtxHost.channels.size(); i++) {
        const auto& ch = resCtxHost.channels[i];
    ...
    if (resCtxHost.channels.size() != resCtxVerify.channels.size()) {
        ...
        for (size_t i = 0; i < resCtxHost.channels.size(); i++) {
            const auto& chHost = resCtxHost.channels[i];
            const auto& chVerify = resCtxVerify.channels[i];
            if (chHost.remoteRank != chVerify.remoteRank) {
                printf("[DEBUG] Verify FAILED: channels[%lu] remoteRank mismatch: %u vs %u\n",
                    i, chHost.remoteRank, chVerify.remoteRank);
                verifyPass = false;
            }
            if (chHost.notifyNum != chVerify.notifyNum) {
                printf("[DEBUG] Verify FAILED: channels[%lu] notifyNum mismatch: %u vs %u\n",
                    i, chHost.notifyNum, chVerify.notifyNum);
                verifyPass = false;
            }
            if (chHost.handle != chVerify.handle) {
                printf("[DEBUG] Verify FAILED: channels[%lu] handle mismatch: %lu vs %lu\n",
                    i, chHost.handle, chVerify.handle);
                verifyPass = false;
            }
            if (chHost.remoteCclMem.addr != chVerify.remoteCclMem.addr) {
                printf("[DEBUG] Verify FAILED: channels[%lu] remoteCclMem.addr mismatch: %p vs %p\n",
                    i, chHost.remoteCclMem.addr, chVerify.remoteCclMem.addr);
                verifyPass = false;
            }
            if (chHost.remoteCclMem.size != chVerify.remoteCclMem.size) {
                printf("[DEBUG] Verify FAILED: channels[%lu] remoteCclMem.size mismatch: %lu vs %lu\n",
                    i, chHost.remoteCclMem.size, chVerify.remoteCclMem.size);
                verifyPass = false;
            }
        }
    }
