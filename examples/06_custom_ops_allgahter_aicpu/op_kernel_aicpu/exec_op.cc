...
HcclResult ExecOp(OpParam &param, AlgResourceCtx &resCtx)
{
    ...
    if (param.rankSize == 1) {
        CHK_RET(static_cast<HcclResult>(HcommLocalCopyOnThread(AlgResourceCtx.threads[0], param.outputPtr, param.inputPtr, size)));
        return HCCL_SUCCESS;
    }
    ...
    for (uint64_t loop = 0; loop < loopCount; loop++) {
        ...
        CHK_RET(static_cast<HcclResult>(HcommLocalCopyOnThread(AlgResourceCtx.threads[0], curCclBuffAddr, curInputAddr, sliceSize)));

        // 前同步
        for(uint32_t i = 1; i < param.rankSize; i++) {
            // 主thread通知其他thread开始工作
            CHK_RET(static_cast<HcclResult>(HcommThreadNotifyRecordOnThread(AlgResourceCtx.threads[0], AlgResourceCtx.threads[i], 0)));
            // 其他thread等待主thread通知
            CHK_RET(static_cast<HcclResult>(HcommThreadWaitRecordOnThread(AlgResourceCtx.threads[i], 0, CUSTOM_TIMEOUT)));
        }

        // 交换数据
        for(uint32_t i = 0; i < param.rankSize; i++) {
            // thread通知对端数据准备完成，可以开始传输
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(AlgResourceCtx.threads[i], channelMap[i].channelHandle, NOTIFY_IDX_ACK)));
            // thread等待对端确认数据准备完成
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(AlgResourceCtx.threads[i], channelMap[i].channelHandle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT)));
            ...
            // 将本地数写到对端cclBuff
            CHK_RET(static_cast<HcclResult>(HcommWriteOnThread(AlgResourceCtx.threads[i], channelMap[i].channelHandle, remoteCclBuffAddr, curCclBuffAddr, sliceSize)));

            // 告诉对端执行完成, 同时等待对端完成
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(AlgResourceCtx.threads[i], channelMap[i].channelHandle, NOTIFY_IDX_DATA_SIGNAL)));
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(AlgResourceCtx.threads[i], channelMap[i].channelHandle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT)));
        }

        // 后同步, 从cclbuff拷贝到outputPtr
        for(uint32_t rankId = 0; i < param.rankSize; i++) {
            ...
            CHK_RET(static_cast<HcclResult>(HcommLocalCopyOnThread(AlgResourceCtx.threads[0], curOutputAddr, curCclBuffAddr, sliceSize)));
        }
