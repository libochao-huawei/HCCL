...
#ifndef OPS_HCCL_DOUBLE_ALLGATHER_LOG_H
#define OPS_HCCL_DOUBLE_ALLGATHER_LOG_H
...
#define CHK_PTR_NULL(ptr) do { if (UNLIKELY((ptr) == nullptr)) { HCCL_ERROR("[%s] ptr [%s] is nullptr", __func__, #ptr); return HCCL_E_PTR; } } while (0)
#define CHK_PRT_RET(result, exeLog, retCode) do { if (UNLIKELY(result)) { exeLog; return retCode; } } while (0)
#define CHK_RET(call) do { int32_t hcclRet = (call); if (UNLIKELY(hcclRet != HCCL_SUCCESS)) { HCCL_ERROR("[%s] call trace: hcclRet -> %d", __func__, hcclRet); return static_cast<HcclResult>(hcclRet); } } while (0)
#define ACLCHECK(cmd) do { aclError ret = (cmd); if (UNLIKELY(ret != ACL_SUCCESS)) { HCCL_ERROR("acl interface return err %s:%d, retcode: %d.", __FILE__, __LINE__, ret); return HCCL_E_RUNTIME; } } while (0)
