...
void Log(int rank, const char* format, const Args&... args) {
    struct timeval tv;
    ...
    if constexpr (sizeof...(args) > 0) {
        ...
        std::strncpy(buffer, format, sizeof(buffer) - 1);
...
int main(int argc, char* argv[]) {
    ...
    if (PrepareData(rank, size, totalCount, sendBytes, recvBytes, stream, 
        ...
        Log(rank, "Starting HcclAlltoAllVCustomAicpu...");
        
        auto run_alltoallv = [&]() -> int {
            HCCLCHECK(HcclAlltoAllVCustomAicpu(sendBuf, sendCounts, sdispls, recvBuf, 
        ...
        if (run_alltoallv() == 0) {
            Log(rank, "HcclAlltoAllVCustomAicpu completed and synchronized");
