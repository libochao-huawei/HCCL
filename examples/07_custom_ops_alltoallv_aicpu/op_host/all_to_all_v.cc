...
static HcclResult PrepareResources(HcclComm comm, OpParam& param, uint32_t rank, uint32_t rankSize, aclrtStream stream)
{
    ...
    void *recvCounts, void *rdispls, HcclDataType dataType, HcclComm comm, aclrtStream stream)
{
    ...
    CHK_RET(PrepareResources(comm, param, rank, rankSize, stream));
