...
本样例展示如何使用 `HcclAllGather()` 接口执行 AllGather 操作，包含以下功能点：
...
- 调用 `HcclAllGather()` 接口，将通信域内所有 rank 的输入数据按照 rank_id 顺序拼接起来，再将结果发送到所有 rank，并打印结果。
...
每个 rank 的数据初始化为对应的 rank id，经过 AllGather 操作后，每个 rank 的结果是所有 rank 数据的拼接。
