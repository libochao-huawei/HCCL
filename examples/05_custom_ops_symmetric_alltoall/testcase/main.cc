...
int Sample(void *arg)
{
    ...
    void* sendBuf = AllocSymmetricMem(hcclComm, dataSize, &sendWin, device);
    if (sendBuf == nullptr) {
        printf("Device %d: AllocSymmetricMem sendBuf failed\n", device);
        HCCLCHECK(HcclCommDestroy(hcclComm));
        return -1;
    }
    void* recvBuf = AllocSymmetricMem(hcclComm, dataSize, &recvWin, device);
    if (recvBuf == nullptr) {
        printf("Device %d: AllocSymmetricMem recvBuf failed\n", device);
        FreeSymmetricMem(sendBuf, sendWin);
        HCCLCHECK(HcclCommDestroy(hcclComm));
        return -1;
    }
    ...
    if (ret != ACL_SUCCESS) {
        ...
        FreeSymmetricMem(recvBuf, recvWin);
        FreeSymmetricMem(sendBuf, sendWin);
        HCCLCHECK(HcclCommDestroy(hcclComm));
        return -1;
    }

    hcclRet = HcclAllToAllCustom(sendBuf, count, HCCL_DATA_TYPE_FP32, recvBuf, hcclComm, stream);
    if (hcclRet != HCCL_SUCCESS) {
        printf("Device %d: HcclAllToAllCustom failed, ret=%d\n", device, hcclRet);
        FreeSymmetricMem(recvBuf, recvWin);
        FreeSymmetricMem(sendBuf, sendWin);
        HCCLCHECK(HcclCommDestroy(hcclComm));
        return -1;
    ...
    uint64_t chunkSize = count / devCount;
    bool pass = true;
    for (uint32_t peerRank = 0; peerRank < devCount; peerRank++) {
        for (uint64_t i = 0; i < chunkSize; i++) {
            uint64_t idx = peerRank * chunkSize + i;
            if (recvData[idx] != static_cast<float>(peerRank)) {
                printf("Device %d: Verification failed at index %lu, expected %f, got %f\n",
                       device, static_cast<unsigned long>(idx), static_cast<float>(peerRank), recvData[idx]);
                pass = false;
                break;
            }
        }
