...
HcclResult ExecOp(OpParam &param, AlgResourceCtx* resCtx)
{
    ...
    if (count % rankSize != 0) {
        HCCL_ERROR("[ExecOp] count must be divisible by rankSize, count=%lu, rankSize=%u",
                   static_cast<unsigned long>(count), rankSize);
        return HCCL_E_PARA;
    }
    ...
    for (uint32_t peerRank = 0; peerRank < rankSize; peerRank++) {
        ...
        // 使用 LocalCopy 直接写入对端内存（load/store 通信）
        ret = HcommLocalCopyOnThread(resCtx->threadHandle, peerRecvBuf, localData, dataSize);
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[ExecOp] LocalCopy to peer failed, peerRank=%u, ret=%d", peerRank, ret);
            return static_cast<HcclResult>(ret);
        }

        HCCL_DEBUG("[ExecOp] Copied %llu bytes to rank %u, peerRecvBuf=%p", dataSize, peerRank, peerRecvBuf);
    }
