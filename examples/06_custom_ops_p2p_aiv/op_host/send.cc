...
extern "C" HcclResult HcclSendCustomAiv(
    void *sendBuf, uint64_t count, HcclDataType dataType, uint32_t destRank, HcclComm comm, aclrtStream stream)
{
    ...
    CHK_RET(PrepareP2pAivResource(comm, destRank, "hccl_send_custom_aiv", stream, &resource));
    return ExecuteSend(resource, sendBuf, lenBytes, dataType, stream);
}
