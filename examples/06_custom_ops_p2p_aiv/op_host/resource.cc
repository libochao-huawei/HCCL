...
HcclResult PrepareSyncContext(HcclComm comm, P2pAivResource *resource, HcclMemHandle *memHandle)
{
    CHK_PTR_NULL(resource);
    CHK_PTR_NULL(memHandle);

    void *syncCtx = nullptr;
    CHK_RET(HcclEngineCtxCreate(comm, resource->tag, COMM_ENGINE_AIV, resource->syncCtxSize, &syncCtx));
    ACLCHECK(aclrtMemset(syncCtx, resource->syncCtxSize, 0, resource->syncCtxSize));

    CommMem regMem {COMM_MEM_TYPE_DEVICE, syncCtx, resource->syncCtxSize};
    CHK_RET(HcclCommMemReg(comm, resource->tag, &regMem, memHandle));
    resource->syncCtx = syncCtx;
    return HCCL_SUCCESS;
}
...
HcclResult ValidateTransferLength(const P2pAivResource &resource, uint64_t lenBytes)
{
    ...
    CHK_PRT_RET(resource.syncCtx == nullptr || resource.remoteSyncCtx == nullptr,
        HCCL_ERROR("local/remote sync ctx not ready"), HCCL_E_INTERNAL);
    ...
    uint64_t inputAddr, uint64_t outputAddr, P2pAivKernelParam *param)
{
    *param = {};
    ...
    param->localSyncAddr = reinterpret_cast<uint64_t>(resource.syncCtx);
    param->remoteSyncAddr = reinterpret_cast<uint64_t>(resource.remoteSyncCtx);
}
    ...
    HcclComm comm, uint32_t peerRank, const char *opName, aclrtStream stream, P2pAivResource *resource)
{
    ...
    CHK_RET(HcclThreadAcquire(comm, COMM_ENGINE_AIV, 1, 0, &thread));
    resource->threadHandle = thread;

    HcclMemHandle memHandle = nullptr;
    CHK_RET(PrepareSyncContext(comm, resource, &memHandle));
    ...
    CHK_PRT_RET(memNum != 1 || remoteMems == nullptr, HCCL_ERROR("unexpected remote mem num=%u", memNum),
        HCCL_E_INTERNAL);
    resource->remoteSyncCtx = remoteMems[0].addr;
    return HCCL_SUCCESS;
}
    ...
    aclrtStream stream)
{
    ...
    ACLCHECK(aclrtMemset(resource.syncCtx, resource.syncCtxSize, 0, resource.syncCtxSize));
