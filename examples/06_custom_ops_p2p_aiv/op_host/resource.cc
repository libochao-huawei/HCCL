...
CommProtocol NormalizeChannelProtocol(CommProtocol protocol)
{
    if (protocol == COMM_PROTOCOL_HCCS || protocol == COMM_PROTOCOL_ROCE) {
        return protocol;
    }
...
HcclResult TryFillChannelDescFromRankGraph(HcclComm comm, uint32_t rank, uint32_t peerRank, HcclChannelDesc *desc)
{
    ...
    for (uint32_t netLayer = 0; netLayer < MAX_NET_LAYER; ++netLayer) {
        ...
        desc->channelProtocol = NormalizeChannelProtocol(links[0].linkAttr.linkProtocol);
        return HCCL_SUCCESS;
    }
