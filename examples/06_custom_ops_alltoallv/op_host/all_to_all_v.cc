...
static HcclResult SetupExtraArgs(const void* sendCounts, const void* sdispls, 
                                 const void* recvCounts, const void* rdispls,
                                 uint32_t rankSize, ExtraArgs& extraArgs) {
    const uint64_t* sendCountsPtr = static_cast<const uint64_t*>(sendCounts);
    const uint64_t* sdisplsPtr = static_cast<const uint64_t*>(sdispls);
    const uint64_t* recvCountsPtr = static_cast<const uint64_t*>(recvCounts);
    const uint64_t* rdisplsPtr = static_cast<const uint64_t*>(rdispls);
    
    for (uint32_t i = 0; i < rankSize && i < MAX_RANK_SIZE; i++) {
        extraArgs.sendCounts[i] = sendCountsPtr[i];
        extraArgs.sendDispls[i] = sdisplsPtr[i];
        extraArgs.recvCounts[i] = recvCountsPtr[i];
        extraArgs.recvDispls[i] = rdisplsPtr[i];
    }
    return HCCL_SUCCESS;
}
...
extern 'C' HcclResult HcclAlltoAllVCustom(void *sendBuf, void *sendCounts, void *sdispls, void *recvBuf, 
    ...
    for (uint32_t i = 0; i < rankSize; i++) {
        totalSendCount += sendCountsPtr[i];
        totalRecvCount += recvCountsPtr[i];
    }
    param.len = totalSendCount; 
    
    CHK_RET(SetupExtraArgs(sendCounts, sdispls, recvCounts, rdispls, rankSize, param.extraArgs));
