...
HcclResult LaunchKernel(OpParam &param, aclrtStream stream, KernelLaunchMode mode)
{
    ...
    // 根据配置选择调用方式
    switch (mode) {
        case KernelLaunchMode::BINARY:
            {
                HCCL_INFO("Launching kernel using binary <<<>>> mode");
                // 使用二进制 <<<>>> 调用方式
                constexpr uint32_t blockDim = 1;
                HcclLaunchP2PAicpuKernelAsc<<<blockDim, nullptr, stream>>>(&param, sizeof(OpParam));
                // 检查内核启动错误
                aclError aclRet = aclrtGetLastError();
                if (aclRet != ACL_SUCCESS) {
                    HCCL_ERROR("Failed to launch kernel with binary mode, ret[%d]", aclRet);
                    return HCCL_E_INTERNAL;
                }
            }
            break;
        
        case KernelLaunchMode::ACL_API:
        default:
            {
                HCCL_INFO("Launching kernel using ACL API mode");
                // 执行device侧的算法编排
