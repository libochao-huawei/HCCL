...
HcclResult AcquireChannel(HcclComm comm, uint32_t srcRank, uint32_t dstRank, ChannelHandle* channel) {
        ...
        if (!protocolExists) {
            HCCL_ERROR("[AcquireChannel] Protocol %d not found between rank %u and rank %u", protocol, rank, destRank);
