...
- **针对Atlas A3 训练系列产品/Atlas A3 推理系列产品**：支持的配置如下，若设置了不支持的环境变量，使用默认值。
  ...
  - **AIV**：代表通信算子在Vector Core展开，执行也在Vector Core。
    ...
    - 该配置项下，集合通信支持控核能力，建议业务根据实际使用场景中计算算子与通信算子的并发情况进行Vector Core核数的配置。
      ...
      - 针对AllGather、非确定性ReduceScatter算子，建议最少分配max\(2, ranksize/20\(向上取整\)\)个vector核。
      - 针对AllReduce、确定性ReduceScatter、AlltoAll、AlltoAllV、AlltoAllVC算子，建议最少分配max\(2, ranksize/20\(向上取整\)\)个vector核，且核数需为偶数（若计算结果为奇数则向上取整至下一个偶数）。
