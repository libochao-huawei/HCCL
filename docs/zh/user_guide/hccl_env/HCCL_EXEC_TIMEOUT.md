...
- **针对Ascend 950PR/Ascend 950DT**： 
  ...
  - “AIV”模式下：单位为s，默认值为1800，支持十毫秒级精度配置（例如，需要50毫秒超时，则配置为0.05）。取值范围为：\[interval, interval\*254\]，其中interval为硬件支持的算子超时最短时间间隔（可通过aclrtGetOpTimeoutInterval接口获取，默认为4.2秒）。若设置的值小于interval则设置为interval，若设置的值大于interval\*254，则设置为interval\*254。
