...
本项目支持源码构建。源码编译前，需确保已具备CPU环境并安装Ascend-cann-toolkit软件包。如需进行上板测试（执行通信算子），还需要具备NPU环境，并安装固件、驱动及Ascend-cann-ops软件包。
...
安装完成后，用户编译生成的HCCL软件包会替换已安装CANN toolkit软件包中的HCCL相关软件。

### 卸载

若您想卸载编译生成的HCCL软件包，恢复到安装完CANN toolki软件包的状态，可参考如下命令：
