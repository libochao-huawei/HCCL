...
{
    HCCL_COMPAT_DEBUG("[HcclThreadResAcquireTimeOut] enter.");
    if (!HcommIsSupportHcommThreadResAcquireTimeOut()) {
        return HCCL_E_NOT_SUPPORT;
    }
    HCCL_COMPAT_DEBUG("[HcclThreadResAcquireTimeOut] hcomm fun start.");
    // return static_cast<HcclResult>(HcommThreadResAcquireTimeOut(timeout));
    auto ret = static_cast<HcclResult>(HcommThreadResAcquireTimeOut(timeout));
    HCCL_COMPAT_DEBUG("[HcclThreadResAcquireTimeOut] hcomm fun end.");
    return HCCL_SUCCESS;
}
