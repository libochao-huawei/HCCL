...
#ifdef __cplusplus
typedef int32_t (*HcclHcommBatchTransferOnThreadFunc)(ThreadHandle, ChannelHandle, void *, uint32_t);

static inline HcclHcommBatchTransferOnThreadFunc HcclResolveHcommBatchTransferOnThread()
{
    HcclHcommBatchTransferOnThreadFunc func = reinterpret_cast<HcclHcommBatchTransferOnThreadFunc>(
        dlsym(RTLD_DEFAULT, "HcommBatchTransferOnThread"));
    if (func != nullptr) {
        return func;
    }

    void *libCclKernelHandle = dlopen("libccl_kernel.so", RTLD_NOW);
    if (libCclKernelHandle == nullptr) {
        return nullptr;
    }
    return reinterpret_cast<HcclHcommBatchTransferOnThreadFunc>(
        dlsym(libCclKernelHandle, "HcommBatchTransferOnThread"));
}

static inline int32_t HcclHcommBatchTransferOnThread(ThreadHandle thread, ChannelHandle channel,
    void *transferDescs, uint32_t transferDescNum)
{
    static HcclHcommBatchTransferOnThreadFunc hcommBatchTransferOnThread =
        HcclResolveHcommBatchTransferOnThread();
