...
{
    std::string multipleDimensionSplitRatioEnv = GetEnv(MM_ENV_HCCL_ALG_MULTIPLE_DIMENSION_SPLIT_RATIO);
    if (multipleDimensionSplitRatioEnv == "EmptyString") {
        g_algEnvConfig.multipleDimensionSplitRatioSet = false;
        g_algEnvConfig.multipleDimensionSplitRatio = 0;
        return HCCL_SUCCESS;
    }

    if (!IsValidTimeoutFormat(multipleDimensionSplitRatioEnv)) {
        HCCL_WARNING("[ParseExecTimeout] HCCL_ALG_MULTIPLE_DIMENSION_SPLIT_RATIO[%s] format is invalid, use default.",
            multipleDimensionSplitRatioEnv.c_str());
        g_algEnvConfig.multipleDimensionSplitRatioSet = false;
        g_algEnvConfig.multipleDimensionSplitRatio = 0;
        return HCCL_E_PARA;
    }
    ...
    if (SalStrToDouble(multipleDimensionSplitRatioEnv, multipleDimensionSplitRatio) != HCCL_SUCCESS) {
...
HcclResult InitEnvConfig()
{
    ...
    RPT_ENV_ERR(ret != HCCL_SUCCESS, "EI0001", std::vector<std::string>({"value", "env", "expect"}),
        std::vector<std::string>({GetEnv(MM_ENV_HCCL_ALG_MULTIPLE_DIMENSION_SPLIT_RATIO), "HCCL_ALG_MULTIPLE_DIMENSION_SPLIT_RATIO",
        "a non-negative number with up to 2 decimals"}));
    CHK_PRT_RET(ret != HCCL_SUCCESS,
