...
HcclResult ParseExecTimeout()
{
    ...
    if (g_algEnvConfig.aivMode) {
        ...
        if (execTimeOut == 0 || execTimeOut > AIV_TIMEOUT_MAX_SEC) {
            HCCL_WARNING("[ParseExecTimeout] HCCL_EXEC_TIMEOUT[%s] is adjusted to default[%.0f] in AIV mode.",
                execTimeOutEnv.c_str(), AIV_TIMEOUT_MAX_SEC);
        ...
        double intPart = 0;
        if (std::modf(execTimeOut, &intPart) != 0.0) {
