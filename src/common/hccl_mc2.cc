...
HcclResult HcclAllocComResourceByTiling(HcclComm comm, void *stream, void* mc2Tiling, void** opResCtx)
{
    ...
    for (uint32_t i = 0U; i < tilingNum; ++i) {
        ...
        CHK_RET(GetOpParam(comm, stream, topoTag[i].c_str(), ccTilingList[i], opParamVec[i]));
    }

    // TODO: 根据ctxTag 申请通信资源 ，并返回OpResCtx的地址
    CHK_RET(HcclAllocOpResCtx(comm, ctxTag, opParamVec.data(), mc2Tiling, ccTilingList, opResCtx));
    
    // 记录退出日志和性能统计信息
    CHK_RET(LogHcclExit("HcclAllocComResourceByTiling", ctxTag, startut));
