...
static int safe_open_dir_chain(const char* path) {
    ...
    if (safe_strcpy(buf, sizeof(buf), path) != 0) {
        return -1;
    }

    dirfd = open("/", O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
    if (dirfd < 0) {
        HCCL_ERROR("Cannot open root directory: %s", strerror(errno));
        return -1;
    }

    comp = strtok_r(buf, "/", &saveptr);
    while (comp != NULL) {
        struct stat st;
        ...
        if (fstatat(dirfd, comp, &st, AT_SYMLINK_NOFOLLOW) != 0) {
            HCCL_ERROR("fstatat '%s' failed: %s", comp, strerror(errno));
            goto fail;
        }
        if (S_ISLNK(st.st_mode)) {
            HCCL_ERROR("Component '%s' is a symbolic link, refusing", comp);
            goto fail;
        }
        if (!S_ISDIR(st.st_mode)) {
            HCCL_ERROR("Component '%s' is not a directory", comp);
            goto fail;
        }
...
static FILE* open_target_secure(const char* target_path, int* file_exists) {
    ...
    if (safe_strcpy(dir_buf, sizeof(dir_buf), target_path) != 0) {
        return NULL;
    }

    last_slash = strrchr(dir_buf, '/');
    if (last_slash == NULL) {
        HCCL_ERROR("Target path '%s' has no '/'", target_path);
        return NULL;
    }
    if (last_slash == dir_buf) {
        /* 目标位于根目录下，父目录退化为 "/" */
        dir_buf[1] = '\0';
    } else {
        *last_slash = '\0';
    ...
    if (basename == NULL || *basename == '\0') {
        HCCL_ERROR("Target path '%s' missing basename", target_path);
        return NULL;
    }

    parent_fd = safe_open_dir_chain(dir_buf);
    if (parent_fd < 0) {
        HCCL_ERROR("Cannot prepare parent dir for '%s'", target_path);
        return NULL;
    }
    ...
    if (fd >= 0) {
        ...
        /* 不存在则原子创建：O_EXCL 保证只有一个进程赢得创建竞争 */
        fd = openat(parent_fd, basename,
                    O_CREAT | O_EXCL | O_RDWR | O_NOFOLLOW | O_CLOEXEC, 0644);
        if (fd < 0 && errno == EEXIST) {
            /* 并发场景：另一进程刚创建，重试以已存在方式打开 */
            fd = openat(parent_fd, basename,
                        O_RDWR | O_NOFOLLOW | O_CLOEXEC);
            if (fd >= 0 && file_exists != NULL) {
                *file_exists = 1;
            }
        }
    } else if (errno == ELOOP) {
        HCCL_ERROR("Target '%s' is a symbolic link, refusing", target_path);
    }
    close(parent_fd);

    if (fd < 0) {
        HCCL_ERROR("Cannot open target '%s': %s",
                target_path, strerror(errno));
        return NULL;
    }
