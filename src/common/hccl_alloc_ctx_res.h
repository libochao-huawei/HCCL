...
{
    OpResCtx resCtx;
    Mc2InitTilingInner *initTiling = static_cast_cast<Mc2InitTilingInner *>(mc2Tiling);
    ...
    for (uint32_t i = 0U; i < opParamVec.size(); ++i) {
        ...
        const Mc2CcTilingInner *ccTiling = static_cast<const Mc2CcTilingInner *>(ccTilingList[i]);
        if (HcclEngineCtxGet(comm, &tagParam, ccTiling->commEngine, &opParamSize, &opParamPtr) == HCCL_SUCCESS) {
            ...
            CHK_RET(HcclEngineCtxCreate(comm, &tagParam, ccTiling->commEngine, &opParamSize, &opParamPtr));
            opParamAddr[i] = reinterpret_cast<uint64_t>(opParamPtr);
        }
    ...
    if (HcclEngineCtxGet(comm, &tagWorkSpace, ccTiling0->commEngine, &memSize, &workSpacePtr) == HCCL_SUCCESS) {
        ...
        CHK_RET(HcclEngineCtxCreate(comm, &tagWorkSpace, ccTiling0->commEngine, &memSize, &workSpacePtr));
        resCtx.workSpace = reinterpret_cast<uint64_t>(workSpacePtr);
    }
    ...
    if (HcclEngineCtxGet(comm, &tagOpResCtx, ccTiling0->commEngine, &opResCtxSize, opResCtxPtr) == HCCL_SUCCESS) {
        ...
        CHK_RET(HcclEngineCtxCreate(comm, &tagOpResCtx, ccTiling0->commEngine, &opResCtxSize, opResCtxPtr));
    }
...
HcclResult GetOpParam(HcclComm comm, const char *topoTag, const Mc2CcTilingInner *ccTiling, OpParam &opParam)
{
    ...
    CHK_RET(ops_hccl::SetCommEngine(opParam));
    // AIV_ONLY 模式下禁止回退到非退到非 AIVari引警，未选中 AIV 时直接返回不支持。
    if (ops_hccl::GetExternalInputHcclAivOnlyMode() && opParam.engine != COMM_ENGINE_AIV) {
        HCCL_ERROR("[HcclExecOp] opType[%d] currently do not select aiv mode, aiv only not support.",
            static_cast<int>(opParam.opType));
        return HCCL_E_NOT_SUPPORT;
    }
    ...
    if ((opParam.engine == COMM_ENGINE_AICPU_TS) || (opParam.engine == COMM_ENGINE_CPU)) {
        HCCL_DEBUG("[Selector] is aicpu mode");
        CHK_RET(ops_hccl::LoadAICPUKernel()); // 该函数内部有防止重复加载的逻辑
    }
    CHK_RET(ops_hccl::SetOpParamAlgTag(opParam, algName));

    if (userRankSize == 1) {
        HCCL_WARNING("[%s] ranksize == 1, enter SingleRankProc", __func__);
        CHK_RET(ops_hccl::SingleRankProc(opParam));
        return HCCL_SUCCESS;
    }
    ...
    std::unique_ptr<ops_hccl::InsCollAlgBase> executor = ops_hccl::CollAlgExecRegistryV2::Instance().GetAlgExec(opParam.opType, algName);
    CHK_PRT_RET(
        executor.get() == nullptr, HCCL_ERROR("Fail to find executor for algName[%s]", algName.c_str()), HCCL_E_PARA);

    // 资源结构体
    std::unique_ptr<ops_hccl::AlgResourceCtxSerializable> resCtxHost = std::make_unique<ops_hccl::AlgResourceCtxSerializable>();
    // 资源序列化结果
    void *resCtxSequence = nullptr;
    bool isResourceReused = false;
    ...
    CHK_RET(ops_hccl::HcclGetAlgRes(comm, opParam, executor, topoInfo.get(), resCtxHost, &resCtxSequence, isResourceReused));
    opParam.resCtx = resCtxSequence;
    return HCCL_SUCCESS;
}
    CHK_RET(SetCommEngine(opParam));
