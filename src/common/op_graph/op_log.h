...
#define OP_LOG_FMT_IDX 3
#define OP_LOG_AGR_IDX 4
void DlogInner(int moduleId, int level, const char* fmt, ...)
    __attribute__((format(printf, OP_LOG_FMT_IDX, OP_LOG_AGR_IDX)));
...
#define GE_OP_LOGE_WITH_REPORT(opname, format, ...)                                                   \
  do {                                                                                                \
    GE_OP_LOGE(opname, format, ##__VA_ARGS__);                                                        \
    REPORT_INNER_ERR_MSG("E19999", "[OpName:%s]" format, get_cstr(get_op_info(opname)), ##__VA_ARGS__); \
  } while (0)
...
#define FUSION_PASS_LOGD_IF(cond, ...) \
  if ((cond)) {               \
    FE_LOGD(__VA_ARGS__);     \
  }
#endif
  ...
  do {                                                                                                                  \
      if (AlogCheckDebugLevel(static_cast<int>(moduleId), level) == 1) {                                                \
              AlogRecord(static_cast<int>(moduleId), DLOG_TYPE_DEBUG, level,                                            \
                  ...
                  __FILE__, __LINE__, OPPROTO_SUBMOD_NAME, __FUNCTION__,                                                \
...
#define D_FUSION_PASS_LOGI(fmt, ...)                                                                                    \
  do {                                                                                                                  \
    if (AlogCheckDebugLevel(FE, DLOG_INFO) == 1) {                                                                      \
            AlogRecord(FE, DLOG_TYPE_DEBUG, DLOG_INFO,                                                                  \
                "[%s:%d][%s]"  "%s:%d " #fmt,                                                                           \
                __FILE__, __LINE__, OPPROTO_SUBMOD_NAME, __FUNCTION__, __LINE__, ##__VA_ARGS__);                        \
    }                                                                                                                   \
  } while (0)
#define D_FUSION_PASS_LOGW(fmt, ...)                                                                                    \
  do {                                                                                                                  \
    if (AlogCheckDebugLevel(FE, DLOG_WARN) == 1) {                                                                      \
            AlogRecord(FE, DLOG_TYPE_DEBUG, DLOG_WARN,                                                                  \
                "[%s:%d][%s]"  "%s:%d " #fmt,                                                                           \
                __FILE__, __LINE__, OPPROTO_SUBMOD_NAME, __FUNCTION__, __LINE__, ##__VA_ARGS__);                        \
    }                                                                                                                   \
  } while (0)
#define D_FUSION_PASS_LOGE(fmt, ...)                                                                                    \
  do {                                                                                                                  \
    if (AlogCheckDebugLevel(FE, DLOG_ERROR) == 1) {                                                                      \
            AlogRecord(FE, DLOG_TYPE_DEBUG, DLOG_ERROR,                                                                  \
                "[%s:%d][%s]"  "%s:%d " #fmt,                                                                           \
                __FILE__, __LINE__, OPPROTO_SUBMOD_NAME, __FUNCTION__, __LINE__, ##__VA_ARGS__);                        \
    }                                                                                                                   \
  } while (0)
#define D_FUSION_PASS_LOGD(fmt, ...)                                                                                    \
  do {                                                                                                                  \
    if (AlogCheckDebugLevel(FE, DLOG_DEBUG) == 1) {                                                                      \
            AlogRecord(FE, DLOG_TYPE_DEBUG, DLOG_DEBUG,                                                                  \
                "[%s:%d][%s]"  "%s:%d " #fmt,                                                                           \
                __FILE__, __LINE__, OPPROTO_SUBMOD_NAME, __FUNCTION__, __LINE__, ##__VA_ARGS__);                        \
    }                                                                                                                   \
  } while (0)
...
#define OP_CHECK_NOTNULL(val)                           \
  do {                                                  \
    if ((val) == nullptr) {                             \
      OP_LOGE("CHECK_NOTNULL", "Parameter[%s] must not be null.", #val); \
