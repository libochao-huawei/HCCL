...
#define AICPU_INFER_SHAPE_CALL_ERR_REPORT(op_name, err_msg)                                      \
  do {                                                                                           \
    ...
    REPORT_INNER_ERR_MSG(GetViewErrorCodeStr(ViewErrorCode::AICPU_INFER_SHAPE_ERROR).c_str(), "%s", \
...
#define VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op_name, err_msg)                                \
  do {                                                                                       \
    OP_LOGE_WITHOUT_REPORT(op_name, "%s", get_cstr(err_msg));                                \
    REPORT_INNER_ERR_MSG(GetViewErrorCodeStr(ViewErrorCode::VECTOR_INNER_ERROR).c_str(), "%s", \
                       ConcatString("op[", op_name, "], ", err_msg).c_str());                \
  } while (0)
...
#define INFER_AXIS_TYPE_ERR_REPORT(op_name, err_msg, ...)                                            \
  do {                                                                                               \
    ...
    REPORT_INNER_ERR_MSG("EZ9999", "op[%s], " err_msg, get_cstr(get_op_info(op_name)), ##__VA_ARGS__); \
  } while (0)

#define VECTOR_FUSION_INNER_ERR_REPORT(op_name, err_msg, ...)                                              \
  do {                                                                                                     \
    OP_LOGE_WITHOUT_REPORT(op_name, err_msg, ##__VA_ARGS__);                                               \
    REPORT_INNER_ERR_MSG(GetViewErrorCodeStr(ViewErrorCode::VECTOR_INNER_ERROR).c_str(), "op[%s], " err_msg, \
...
#define CUBE_INNER_ERR_REPORT(op_name, err_msg, ...)                                                 \
  do {                                                                                               \
    OP_LOGE_WITHOUT_REPORT(op_name, err_msg, ##__VA_ARGS__);                                         \
    REPORT_INNER_ERR_MSG("E69999", "op[%s], " err_msg, get_cstr(get_op_info(op_name)), ##__VA_ARGS__); \
  } while (0)
...
#define CUBE_INNER_ERR_REPORT_PLUGIN(op_name, err_msg, ...)                                          \
  do {                                                                                               \
    OP_LOGE_WITHOUT_REPORT(op_name, err_msg, ##__VA_ARGS__);                                         \
    REPORT_INNER_ERR_MSG("E59999", "op[%s], " err_msg, get_cstr(get_op_info(op_name)), ##__VA_ARGS__); \
  } while (0)
...
#define REPORT_GET_SHAPE_ERR(op_name, index, wrong_shape, correct_shape, ...)                                       \
  do {                                                                                                              \
    ...
    ge::ReportPredefinedErrMsg("EZ0001",                                                                            \
                      {"op_name", "index", "incorrect_shape", "correct_shape"},                                     \
                      {get_cstr(get_op_info(op_name)),                                                              \
...
#define REPORT_GET_ATTR_VALUE_ERR(op_name, attr_name, wrong_val, correct_val, ...)                                  \
  do {                                                                                                              \
    ...
    ge::ReportPredefinedErrMsg("EZ0002",                                                                            \
                       {"op_name", "attr_name", "incorrect_val", "correct_val"},                                    \
                       {get_cstr(get_op_info(op_name)),                                                             \
                                                 attr_name.c_str(),                                                 \
                                                 wrong_val.c_str(),                                                 \
                                                 correct_val.c_str()});                                             \
...
#define REPORT_GET_ATTR_SIZE_ERR(op_name, attr_name, wrong_size, correct_size, ...)                                 \
  do {                                                                                                              \
    ...
    ge::ReportPredefinedErrMsg("EZ0003",                                                                            \
                       {"op_name", "attr_name", "incorrect_size", "correct_size"},                                  \
                       {get_cstr(get_op_info(op_name)),                                                             \
                                                 attr_name.c_str(),                                                 \
                                                 wrong_size.c_str(),                                                \
                                                 correct_size.c_str()});                                           \
...
#define REPORT_GET_INPUT_ERR(op_name, param_name, ...)                                                            \
  do {                                                                                                            \
    ...
    ge::ReportPredefinedErrMsg("EZ0004",                                                                          \
                       {"op_name", "param_name"},                                                                 \
                       {get_cstr(get_op_info(op_name)), param_name.c_str()});                                     \
  } while (0)
...
#define REPORT_GET_SHAPE_SIZE_ERR(op_name, index, wrong_shape_size, correct_shape_size, ...)                    \
  do {                                                                                                          \
    ...
    ge::ReportPredefinedErrMsg("EZ0005",                                                                        \
                       {"op_name", "index", "incorrect_size", "correct_size"},                                  \
                       {get_cstr(get_op_info(op_name)),                                                         \
                                                 std::to_string(index).c_str(),                                 \
                                                 wrong_shape_size.c_str(),                                      \
                                                 correct_shape_size.c_str()});                                 \
  } while (0)
...
#define REPORT_GET_INPUT_FORMAT_ERR(op_name, param_name, data_format, expected_format_list, ...)                            \
  do {                                                                                                                      \
    ...
    ge::ReportPredefinedErrMsg("EZ0006",                                                                                    \
                       {"op_name", "param_name", "data_format", "expected_format_list"},                                    \
                       {get_cstr(get_op_info(op_name)),                                                                     \
                                                 param_name.c_str(),                                                        \
                                                 data_format.c_str(),                                                       \
                                                 expected_format_list.c_str()});                                            \
  } while (0)
...
#define REPORT_GET_INPUT_DTYPE_ERR(op_name, param_name, data_dtype, expected_dtype_list, ...)                               \
  do {                                                                                                                      \
    ...
    ge::ReportPredefinedErrMsg("EZ0007",                                                                                    \
                       {"op_name", "param_name", "data_dtype", "expected_dtype_list"},                                      \
                       {get_cstr(get_op_info(op_name)),                                                                     \
                                                 param_name.c_str(),                                                        \
                                                 data_dtype.c_str(),                                                        \
                                                 expected_dtype_list.c_str()});                                             \
  } while (0)
