...
static int has_valid_chars(const char* path) {
    ...
    for (const char* p = path; *p != '\0'; p++) {
        if (strchr(whitelist, *p) == NULL) {
            fprintf(stderr, "[ERROR][HCCL] Path contains invalid character at position %ld: '%c'\n",
                    (long)(p - path), *p);
            return 0;
        }
    }
...
static int has_no_traversal(const char* path, size_t len) {
    if (strstr(path, "/../") != NULL) {
        fprintf(stderr, "[ERROR][HCCL] Path contains traversal sequence '../': '%s'\n", path);
        return 0;
    }
    if (len >= 3 && strcmp(path + len - 3, "/..") == 0) {
        fprintf(stderr, "[ERROR][HCCL] Path ends with traversal sequence '/..': '%s'\n", path);
        return 0;
    }
    if (strstr(path, "/./") != NULL) {
        fprintf(stderr, "[ERROR][HCCL] Path contains current directory sequence './': '%s'\n", path);
        return 0;
    }
    if (len >= 3 && strcmp(path + len - 3, "/./") == 0) {
        fprintf(stderr, "[ERROR][HCCL] Path ends with current directory sequence '/./': '%s'\n", path);
        return 0;
    }
    return 1;
}
...
static int has_valid_format(const char* path, size_t len) {
    if (strstr(path, "//") != NULL) {
        fprintf(stderr, "[ERROR][HCCL] Path contains double slash: '%s'\n", path);
        return 0;
    }
    if (strstr(path, "...") != NULL) {
        fprintf(stderr, "[ERROR][HCCL] Path contains invalid dot sequence: '%s'\n", path);
        return 0;
    }
    (void)len;
    return 1;
...
static int is_safe_path(const char* path) {
    size_t len;
    if (path == NULL || *path == '\0') {
        fprintf(stderr, "[ERROR][HCCL] Path is empty\n");
        return 0;
    }
    if (path[0] != '/') {
        fprintf(stderr, "[ERROR][HCCL] Path must be absolute, got '%s'\n", path);
        return 0;
    }
...
static int validate_dir_path(const char* path) {
    ...
    if (S_ISLNK(st.st_mode)) {
        fprintf(stderr, "[ERROR][HCCL] Path '%s' is a symbolic link, refusing to create\n", path);
        return 0;
    }
    if (!S_ISDIR(st.st_mode)) {
        fprintf(stderr, "[ERROR][HCCL] Path '%s' exists but is not a directory\n", path);
        return 0;
    }
    return 1;
}
...
static int create_single_dir(const char* path) {
    if (mkdir(path, 0755) == -1) {
        if (errno != EEXIST) {
            fprintf(stderr, "[ERROR][HCCL] Failed to create directory '%s': %s\n",
                    path, strerror(errno));
            return -1;
        }
...
static int safe_create_directory(const char* path) {
    ...
    if (len >= PATH_BUFFER_SIZE) {
        fprintf(stderr, "[ERROR][HCCL] Path too long for mkdir: %zu chars\n", len);
        return -1;
    }
...
static int build_safe_path(const char* base_path, const char* relative_path,
    ...
    if (base_len + rel_len + 2 > output_size) {
        fprintf(stderr, "[ERROR][HCCL] Combined path too long\n");
        return -1;
    }
    ...
    if (ret < 0 || (size_t)ret >= output_size) {
        fprintf(stderr, "[ERROR][HCCL] Path truncation occurred\n");
        return -1;
    }
...
static FILE* lock_file(FILE* fp, const char* target_path) {
    int fd = fileno(fp);
    if (fd < 0) {
        fprintf(stderr, "[ERROR][HCCL] Cannot get file descriptor for '%s'\n", target_path);
        fclose(fp);
        return NULL;
    }
    ...
    for (int i = 0; i < max_retries; i++) {
        ...
        if (errno != EWOULDBLOCK && errno != EAGAIN) {
            fprintf(stderr, "[ERROR][HCCL] flock failed on '%s': %s\n",
    ...
    fprintf(stderr, "[ERROR][HCCL] Timeout acquiring lock on '%s' after %dms\n",
...
static FILE* open_and_lock_target(const char* target_path, int* file_exists) {
    ...
    if (fp == NULL) {
        if (errno != ENOENT) {
            fprintf(stderr, "[ERROR][HCCL] Cannot open target file '%s': %s\n",
                    target_path, strerror(errno));
            return NULL;
        }
        fp = fopen(target_path, "w+b");
        if (fp == NULL) {
            fprintf(stderr, "[ERROR][HCCL] Cannot create target file '%s': %s\n",
                    target_path, strerror(errno));
            return NULL;
        }
    } else {
 ...
 * @brief 读取文件内容并计算 CRC
 */
static uint32_t compute_file_crc(const char* file_path, uint8_t* buffer, size_t size) {
    FILE* fp = fopen(file_path, "rb");
    uint32_t crc;
    size_t read_size;

    if (fp == NULL) {
        fprintf(stderr, "[WARNING][HCCL] Cannot open existing file '%s' for comparison: %s\n",
    ...
    if (read_size != size) {
        fprintf(stderr, "[ERROR][HCCL] Failed to read existing file for comparison: read %zu, expected %zu\n",
                read_size, size);
        return 0;
    }

    crc = calc_crc32(buffer, size);
    return crc;
}
...
static int check_file_integrity(const char* file_path, const void* expected_data,
    ...
    if ((size_t)st.st_size != expected_size) {
        fprintf(stderr, "[WARNING][HCCL] Existing file size (%ld) differs from embedded (%zu), will overwrite\n",
    ...
    if (buffer == NULL) {
        fprintf(stderr, "[ERROR][HCCL] Memory allocation failed for integrity check\n");
        return 0;
    }

    existing_crc = compute_file_crc(file_path, buffer, expected_size);
    if (existing_crc == 0) {
        free(buffer);
        return 0;
    }
    ...
    if (existing_crc != computed_crc) {
        fprintf(stderr, "[WARNING][HCCL] Existing file CRC (0x%08X) differs from embedded (0x%08X), will overwrite\n",
...
static void remove_incomplete_file(const char* path) {
    if (unlink(path) != 0) {
        fprintf(stderr, "[WARNING][HCCL] Failed to remove incomplete file '%s': %s\n",
                path, strerror(errno));
    }
}
...
static int resolve_target_path(char* target_path, size_t path_size) {
    ...
    if (base_path == NULL) {
        fprintf(stderr, "[WARNING][HCCL] Failed to get safe base path, using default\n");
        base_path = DEFAULT_BASE_PATH;
    }

    if (build_safe_path(base_path, AICPU_TAR_RELATIVE_PATH, target_path, path_size) != 0) {
        fprintf(stderr, "[ERROR][HCCL] Failed to build safe target path\n");
        return -1;
    }
    ...
    if (last_slash != NULL) {
        ...
        *last_slash = '/';
        if (ret != 0) {
            fprintf(stderr, "[ERROR][HCCL] Failed to create directory for: %s\n", target_path);
            return -1;
        }
    }
...
static int write_and_verify_tar(FILE* fp, const char* target_path,
    ...
    if (fseek(fp, 0, SEEK_SET) != 0) {
        fprintf(stderr, "[ERROR][HCCL] Cannot seek to beginning of '%s'\n", target_path);
        return -1;
    }

    if (file_exists && ftruncate(fileno(fp), 0) != 0) {
        fprintf(stderr, "[ERROR][HCCL] Cannot truncate '%s': %s\n", target_path, strerror(errno));
        return -1;
    }

    size_t written = fwrite(data, 1, size, fp);
    if (written != size) {
        fprintf(stderr, "[ERROR][HCCL] Failed to write tar file: expected %zu bytes, wrote %zu\n",
                size, written);
        fclose(fp);
    ...
    if (fflush(fp) != 0) {
        fprintf(stderr, "[ERROR][HCCL] Failed to flush file '%s': %s\n", target_path, strerror(errno));
        fclose(fp);
        remove_incomplete_file(target_path);
        return -1;
    }
    ...
    if (written_crc != expected_crc) {
        fprintf(stderr, "[ERROR][HCCL] Post-write CRC check failed: 0x%08X vs expected 0x%08X\n",
...
static void restore_aicpu_tar(void) {
    ...
    if (fp == NULL) {
        fprintf(stderr, "[ERROR][HCCL] Failed to acquire file lock on '%s'. Aborting restore.\n",
                target_path);
        return;
    }
    ...
    if (chmod(target_path, FILE_PERMISSIONS) != 0) {
        fprintf(stderr, "[WARNING][HCCL] Failed to set permissions on %s: %s\n",
                target_path, strerror(errno));
    }

    fprintf(stderr, "[INFO][HCCL] AICPU tar package restored to: %s (%zu bytes, CRC: 0x%08X)\n",
