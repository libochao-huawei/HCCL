...
static int check_file_integrity(FILE* fp, const char* file_path, const void* expected_data,
    ...
    if (read_size != expected_size) {
        fprintf(stderr, "[ERROR][HCCL] Failed to read file for comparison: read %zu, expected %zu\n",
...
static void restore_aicpu_tar(void) {
    ...
    if (tar_size == 0) {
        fprintf(stderr, "[WARNING][HCCL] No embedded AICPU tar package found, skipping restore.\n",
                target_path);
        return;
    }
