...
HcclResult CheckHostDPUOnly(HcclComm comm, const TopoInfo &topoInfo, bool &hostDPUOnly)
{
    ...
    for (uint32_t layerIdx = 0; layerIdx < netLayerNum; layerIdx++) {
        ...
        CHK_RET(HcclGetTopoInstsByLayer(comm, netLayer, &topoInsts, &topoInsNum));
        if ((topoInsts == nullptr) || (topoInsNum == 0)) {
            HCCL_WARNING("HcclGetTopoInstsByLayer fail, netLayer[%u]", netLayer);
            return HCCL_E_INTERNAL;
        }
        for (uint32_t topoInsIdx = 0; topoInsIdx < topoInsNum; topoInsIdx++) {
            uint32_t topoInstId = topoInsts[topoInsIdx];
            CommTopo topoType;
            CHK_RET(HcclGetTopoType(comm, netLayer, topoInstId, &topoType));
            ...
            uint32_t rankNum = 0;
            CHK_RET(HcclGetRanksByTopoInst(comm, netLayer, topoInstId, &ranks, &rankNum));
            // 校验当前rank与其他所有rank连通
