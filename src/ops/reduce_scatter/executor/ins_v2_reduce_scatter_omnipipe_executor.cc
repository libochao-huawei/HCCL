    ...
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    HCCL_INFO("rankSizeLevel0_[%u], rankSizeLevel1_[%u]", rankSizeLevel0_, rankSizeLevel1_);
...
InsV2ReduceScatterOmniPipeExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1,
                                   InsAlgTemplate2>::GenTemplateAlgParamsByDimData(TemplateDataParams& tempAlgParams,
                                                                                   const StepSliceInfo& stepSliceInfo, bool isFirstStep)
{
    ...
    std::map<u32, std::shared_ptr<InsAlgTemplateBase>> tempMap)
{
    ...
    HCCL_INFO("[InsV2ReduceScatterOmniPipeExecutor][OrchestrateLoop] bw0[%f], bw1[%f], bw2[%f]", bw0, bw1, bw2);
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        for (int i = 0; i < level2StepCount; i++) {
            ...
            if (rankSizeLevel2_ > 1) {
                bool isFirstStepZ = (i == 0) ? true : false;
                // 初始化机间template param
                GenTemplateAlgParamsByDimData(tempAlgParamMap[OMNIPIPE_LEVEL2], omnipipeSliceInfo.dataSliceLevel2[i], isFirstStepZ);
                // level2前同步
            ...
            for (int j = 0; j < level0StepCount; j++) {
                bool isFirstStepXYZ = (i == 0 && j == 0) ? true : false;
                // level0、1前同步
                CHK_RET(PreSyncInterThreads(controlThread_, tempMainThreadsLevel01_, notifyIdxCtrlToTempLevel01_));
                // 初始化并执行机内template任务
                if (rankSizeLevel0_ > 1) {
                    ...
                    CHK_RET(GenTemplateAlgParamsByDimData(tempAlgParamMap[0],
                                                          omnipipeSliceInfo.dataSliceLevel0[i * level0StepCount + j], isFirstStepXYZ));
                    CHK_RET(
                ...
                if (rankSizeLevel1_ > 1) {
                    ...
                    CHK_RET(GenTemplateAlgParamsByDimData(tempAlgParamMap[1],
                                                          omnipipeSliceInfo.dataSliceLevel1[i * level0StepCount + j], isFirstStepXYZ));
                    CHK_RET(
