...
HcclResult ReduceScatterExecutorBase::RunLoop(const OpParam &param)
{
    ...
    HCCL_DEBUG("[ReduceScatterExecutorBase][RunLoop]tag[%s], userRankSize is [%u], "
               "maxCountPerLoop is [%llu], totalRecvCount is [%llu]",
        tag_.c_str(), topoInfo_->userRankSize, maxCountPerLoop, totalRecvCount);

    // for (u64 countLeft = totalRecvCount, curRecvCount = 0; countLeft > 0; countLeft -= curRecvCount) {
        u64 curRecvCount = totalRecvCount; //(countLeft > maxCountPerLoop ? maxCountPerLoop : countLeft);
