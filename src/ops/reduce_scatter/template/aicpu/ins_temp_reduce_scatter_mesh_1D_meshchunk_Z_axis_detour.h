...
class InsTempReduceScatterMesh1DMeshChunkZAxisDetour : public InsTempReduceScatterMesh1DMeshChunk {
public:
    InsTempReduceScatterMesh1DMeshChunkZAxisDetour() = default;
    explicit InsTempReduceScatterMesh1DMeshChunkZAxisDetour(
        const OpParam& param, const u32 rankId,
        const std::vector<std::vector<u32>> &subCommRanks);

    ~InsTempReduceScatterMesh1DMeshChunkZAxisDetour() override;

    std::string Describe() const override
    {
        std::string info = "Template of reduce scatter Mesh Chunk Z axis detour with tempRankSize ";
...
#endif // INS_TEMP_REDUCE_SCATTER_MESH_1D_MESH_CHUNK_Z_AXIS_DETOUR_H
