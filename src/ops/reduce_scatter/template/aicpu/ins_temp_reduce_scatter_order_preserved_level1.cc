...
HcclResult InsTempReduceScatterOrderPreservedLevel1::PreSyncThreads(const std::vector<ThreadHandle> &threads)
{
    if (threadNum_ > 1) {
        ...
        GetNotifyIdxMainToSub(notifyIdxMainToSub);
...
HcclResult InsTempReduceScatterOrderPreservedLevel1::PostSyncThreads(const std::vector<ThreadHandle> &threads)
{
    if (threadNum_ > 1) {
        ...
        GetNotifyIdxSubToMain(notifyIdxSubToMain);
    ...
    const TemplateDataParams &tempAlgParams, const std::vector<ThreadHandle> &threads)
{
    ...
    u64 sliceSize = memBlockInfo.size[myRank_];
    if (sliceSize == 0) {
        return HCCL_SUCCESS;
    }

    u64 srcOffset = memBlockInfo.inputOffsets[myRank_];
    u32 outputIndex = CalcOutputIndex(myRank_);
    u64 dstOffset = memBlockInfo.outputOffsets[outputIndex];

    HCCL_INFO("[LocalCopy] myRank[%u], sliceSize[%llu], srcOffset[%llu], dstOffset[%llu], outputIndex[%u]",
        myRank_, sliceSize, srcOffset, dstOffset, outputIndex);
    ...
    CHK_RET(ops_hccl::LocalCopy(threads[0], srcSlice, dstSlice));
    ...
    const std::vector<ThreadHandle> &threads, const TemplateDataParams &tempAlgParams)
{
    ...
    for (u32 round = 0; round < templateRankSize_; round++) {
        ...
        u64 srcOffset = memBlockInfo.inputOffsets[round];
        u32 outputIndex = CalcOutputIndex(round);
        u64 dstOffset = memBlockInfo.outputOffsets[outputIndex];

        for (const auto &channelInfo : channelInfos) {
            DataSlice txSrcSlice(tempAlgParams.buffInfo.inputPtr, srcOffset, sliceSize);
            DataSlice txDstSlice(channelInfo.remoteCclMem.addr, srcOffset, sliceSize);
            DataSlice rxSrcSlice(channelInfo.remoteCclMem.addr, dstOffset, sliceSize);
            DataSlice rxDstSlice(tempAlgParams.buffInfo.outputPtr, dstOffset, sliceSize);
            ...
            SendRecvInfo sendRecvInfo{
                TxRxChannels{channelInfo, channelInfo},
    ...
    const std::vector<ThreadHandle> &threads, const TemplateDataParams &tempAlgParams)
{
    ...
    u32 rankIdx = myRank_;
    u64 dstOffset = memBlockInfo.outputOffsets[CalcOutputIndex(rankIdx)];
    ...
    for (u32 step = 0; step < reduceStep; step++) {
        ...
        u32 inputIndex = CalcOutputIndex(peerRank);
        u64 peerOffset = memBlockInfo.outputOffsets[inputIndex];

        DataSlice srcSlice(tempAlgParams.buffInfo.outputPtr, peerOffset, peerSliceSize);
        DataSlice dstSlice(tempAlgParams.buffInfo.outputPtr, dstOffset, sliceSize);

        HCCL_INFO("[RunLocalReduce] step[%u], peerRank[%u], peerSliceSize[%llu], dstOffset[%llu], peerOffset[%llu]",
            step, peerRank, peerSliceSize, dstOffset, peerOffset);
    ...
    const TemplateDataParams &tempAlgParams, const std::vector<ThreadHandle> &threads)
{
    ...
    if (sliceSize > 0) {
        DataSlice srcSlice(tempAlgParams.buffInfo.outputPtr, srcOffset, sliceSize);
        DataSlice dstSlice(tempAlgParams.buffInfo.outputPtr, dstOffset, sliceSize);
        CHK_RET(ops_hccl::LocalCopy(threads[0], srcSlice, dstSlice));
    }
