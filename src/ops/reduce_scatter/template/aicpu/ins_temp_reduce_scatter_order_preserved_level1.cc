...
HcclResult InsTempReduceScatterOrderPreservedLevel1::RunLocalReduce(
    const std::vector<ThreadHandle> &threads, const TemplateDataParams &tempAlgParams)
{
    HCCL_INFO("[RunLocalReduce] Start, templateRankSize[%u]", templateRankSize_);
    ...
    while (remainingBlocks > 1) {
        ...
        for (u32 srcVirtualIdx = M; srcVirtualIdx < remainingBlocks; srcVirtualIdx++) {
            ...
            u64 srcSliceSize = memBlockInfo.size[srcPeerRank];
            u64 dstSliceSize = memBlockInfo.size[dstPeerRank];

            // 跳过零大小数据块
            if (srcSliceSize == 0 || dstSliceSize == 0) {
                continue;
            }

            // 计算元素个数（LocalReduce需要count参数）
            u64 srcCount = srcSliceSize / DATATYPE_SIZE_TABLE[dataType_];
            u64 dstCount = dstSliceSize / DATATYPE_SIZE_TABLE[dataType_];

            // 创建数据切片描述符（4参数版本，包含count）
            DataSlice srcSlice(tempAlgParams.buffInfo.hcclBuff.addr, srcOffset, srcSliceSize, srcCount);
            DataSlice dstSlice(tempAlgParams.buffInfo.hcclBuff.addr, dstOffset, dstSliceSize, dstCount);
