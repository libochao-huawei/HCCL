...
{
    deterministicStrict_ = (param.deterministicConfig == static_cast<u8>(DeterministicEnableLevel::DETERMINISTIC_STRICT));
    all2allOffset_ = 0;
}
...
{
    const MemBlockInfo &memBlockInfo = tempAlgParams.memBlockInfo;
    ...
    const std::vector<ThreadHandle> &threads, const TemplateDataParams &tempAlgParams)
{
    ...
    for (u32 step = 0; step < reduceStep; step++) {
        ...
        void *dstMemPtr = static_cast<u8 *>(buffInfo_.outputMem.addr) + dstOffset;
