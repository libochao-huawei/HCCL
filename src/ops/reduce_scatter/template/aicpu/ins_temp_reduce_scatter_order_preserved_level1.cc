...
//     HCCL_INFO("[RunLocalReduce] lgx 2 Start, deterministicStrict[%d], templateRankSize[%u]",
//         deterministicStrict_, templateRankSize_);
...
//         // 获取对端rank的数据块大小（用于判断是否为空）
//         u64 peerSliceSize = sliceSize;
//         if (peerSliceSize == 0) {
//             continue;
//         }
...
//         // 创建源数据切片（对端数据在临时缓冲区中）
//         // 注意：srcSlice大小应该等于dstSlice大小（sliceSize），而不是peerSliceSize
//         // 因为对端发送的是属于本rank的那一部分数据，大小等于本rank的数据大小
//         DataSlice srcSlice(tempAlgParams.buffInfo.hcclBuff.addr, peerOffset, sliceSize, count);
...
//         // 分开打印 srcSlice 和 dstSlice
//         HCCL_INFO("[RunLocalReduce] myAlgRank[%u], peerRank[%u], peerOutputIndex[%u]",
//             myAlgRank, peerRank, peerOutputIndex);
//         HCCL_INFO("zdy [RunLocalReduce] srcSlice: %s", srcSlice.Describe().c_str());
//         HCCL_INFO("zdy [RunLocalReduce] dstSlice: %s", dstSlice.Describe().c_str());
