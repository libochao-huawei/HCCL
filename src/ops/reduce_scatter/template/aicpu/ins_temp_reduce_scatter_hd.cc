...
HcclResult InsTempReduceScatterHD::CalcRes(HcclComm comm, const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
                                            AlgResourceRequest& resourceRequest)
{
    // HD算法需要与每个对端建立channel，对端通过步进方式确定
    std::vector<HcclChannelDesc> channels;

    u32 stepNum = GetHDStepNum(templateRankSize_);
    u32 myAlgIdx = 0;
    ...
    for (u32 step = 0; step < stepNum; ++step) {
        u32 stride = 1 << step;
        u32 remoteIdx = myAlgIdx ^ stride;
        u32 remoteRank = subCommRanks_[0][remoteIdx];

        // 为每个对端创建channel
        CHK_RET(CreateChannelRequestByRankId(comm, param, myRank_, remoteRank, channels));
    }

    resourceRequest.channels.push_back(channels);
    u32 channelsPerRank = CalcChannelsPerRank(channels);
    HCCL_INFO("[InsTempReduceScatterHD][CalcRes] channelsPerRank: [%u].", channelsPerRank);
    channelsPerRank_ = channelsPerRank;
    GetRes(resourceRequest);
    HCCL_INFO("[InsTempReduceScatterHD][CalcRes] slaveThreadNum: [%u], notifyNumOnMainThread: [%u].",
        resourceRequest.slaveThreadNum, resourceRequest.notifyNumOnMainThread);
    return HcclResult::HCCL_SUCCESS;
}

HcclResult InsTempReduceScatterHD::GetRes(AlgResourceRequest& resourceRequest) const
{
...
u64 InsTempReduceScatterHD::GetThreadNum() const
{
    return 1 * channelsPerRank_;
}

HcclResult InsTempReduceScatterHD::KernelRun(const OpParam& param,
                                              const TemplateDataParams& tempAlgParams,
                                              TemplateResource& templateResource)
{
    HCCL_INFO("[InsTempReduceScatterHD] KernelRun start");
    if (tempAlgParams.sliceSize == 0 && tempAlgParams.tailSize == 0) {
        HCCL_INFO("[InsTempReduceScatterHD] sliceSize and tailSize are both 0, skip");
        return HCCL_SUCCESS;
    }
    ...
    HCCL_DEBUG("[InsTempReduceScatterHD] Use Dma Read[%d]", isDmaRead_);
    ...
    for (u32 channelIdx = 0; channelIdx < channelsPerRank_; channelIdx++) {
        CHK_PRT_RET(channelIdx >= sizeOut.size() || channelIdx >= elemOffset.size(),
                    HCCL_ERROR("[InsTempReduceScatterHD] channelIdx[%u] out of bounds", channelIdx), HCCL_E_INTERNAL);
        CHK_RET(LocalDataCopy(templateResource.threads, channelIdx));
        if (templateRankSize_ <= 1) {
            CHK_RET(PostLocalCopy(templateResource.threads, channelIdx));
            return HcclResult::HCCL_SUCCESS;
        }
        CHK_RET(RunHD(templateResource.threads, channelIdx));
        CHK_RET(PostLocalCopy(templateResource.threads, channelIdx));
    }
...
HcclResult InsTempReduceScatterHD::LocalDataCopy(const std::vector<ThreadHandle> &threads, u32 channelIdx)
{
    CHK_PRT_RET(threads.empty(),
        HCCL_ERROR("[InsTempReduceScatterHD][LocalDataCopy] empty threads"), HcclResult::HCCL_E_INTERNAL);
...
HcclResult InsTempReduceScatterHD::PostLocalCopy(const std::vector<ThreadHandle> &threads, u32 channelIdx)
{
    CHK_PRT_RET(threads.empty(),
        HCCL_ERROR("[RS-HD][PostLocalCopy] empty threads"), HcclResult::HCCL_E_INTERNAL);
...
HcclResult InsTempReduceScatterHD::RunHD(const std::vector<ThreadHandle> &threads, u32 channelIdx)
{
    CHK_PRT_RET(threads.empty(),
        HCCL_ERROR("[RS-HD][RunHD] empty threads"), HcclResult::HCCL_E_INTERNAL);
    ...
    std::vector<u64> elemOffset;
    // 预计算步骤列表（算法序）
    std::vector<AicpuHDStepInfo> steps;
    CHK_RET(GetStepInfoList(steps));
    u32 nSteps = steps.size();
    for (u32 s = 0; s < nSteps; ++s) {
        ...
        CHK_PRT_RET(recvFromRank == static_cast<u32>(-1) || sendToRank == static_cast<u32>(-1),
            HCCL_ERROR("[RS-HD][RunHD] rank map failed: from[%u] to[%u]", st.fromRank, st.toRank),
        ...
        CHK_PRT_RET(channels_.count(recvFromRank) == 0 || channels_.count(sendToRank) == 0 ||
                    ...
                    HCCL_ERROR("[RS-HD][RunHD] channel missing: recvFrom=%d sendTo=%d", recvFromRank, sendToRank),
            HcclResult::HCCL_E_INTERNAL);
        ...
        if (isDmaRead_) {
            CHK_PRT_RET(SendRecvBatchReadReduce(info, threads[channelIdx]),
                HCCL_ERROR("[RS-HD][RunHD] SendRecvBatchReadReduce failed (step=%u)",
                    st.step),
                HcclResult::HCCL_E_INTERNAL);
        } else {
            CHK_PRT_RET(SendRecvBatchWriteReduce(info, threads[channelIdx]),
                HCCL_ERROR("[RS-HD][RunHD] SendRecvBatchWriteReduce failed (step=%u)",
                    st.step),
                HcclResult::HCCL_E_INTERNAL);
        }
    }
...
u64 InsTempReduceScatterHD::CalcScratchMultiple(BufferType inBuffType, BufferType outBuffType)
{
    (void) inBuffType;
    (void) outBuffType;
    HCCL_INFO(
        "[InsTempReduceScatterHD][CalcScratchMultiple] templateScratchMultiplier[%llu]", templateRankSize_);
    return templateRankSize_;
}

//  计算每轮收发的对端以及slice编号
HcclResult InsTempReduceScatterHD::GetStepInfoList(std::vector<AicpuHDStepInfo> &stepInfoList)
{
    ...
    u32 nSteps = GetHDStepNum(templateRankSize_);
    stepInfoList.resize(nSteps);
    for (u32 step = 0; step < nSteps; step++) {
        // 计算通信对象
        u32 stride = 1 << (nSteps - step - 1);
        u32 remoteIdx = u32x ^ stride;
        u32 txSliceIdx = remoteIdx / stride * stride;
        u32 rxSliceIdx = u32x / stride * stride;
        u32 nSlices = stride;

        AicpuHDStepInfo &currStepInfo = stepInfoList[step];
        currStepInfo.step = step;
        currStepInfo.myRank = u32x;
        currStepInfo.nSlices = nSlices;
        currStepInfo.toRank = remoteIdx;
        currStepInfo.fromRank = remoteIdx;

        // 计算本rank在每轮收/发中的slice编号
        currStepInfo.txSliceIdxs.reserve(nSlices);
        currStepInfo.rxSliceIdxs.reserve(nSlices);
        for (u32 i = 0; i < nSlices; i++) {
            currStepInfo.txSliceIdxs.push_back(txSliceIdx + i);
            currStepInfo.rxSliceIdxs.push_back(rxSliceIdx + i);
            HCCL_DEBUG("[InsTempReduceScatterHD][GetStepInfoList] i[%u] txSliceIdx[%u] rxSliceIdx[%u]", i, txSliceIdx, rxSliceIdx);
        }
    }
    return HcclResult::HCCL_SUCCESS;
}

void InsTempReduceScatterHD::GetNotifyIdxMainToSub(std::vector<u32> &notifyIdxMainToSub)
{
...
void InsTempReduceScatterHD::GetNotifyIdxSubToMain(std::vector<u32> &notifyIdxSubToMain)
{
