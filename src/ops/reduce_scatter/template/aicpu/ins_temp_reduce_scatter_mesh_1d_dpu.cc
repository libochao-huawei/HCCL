 ...
 * Description: 算法模板InsTempReduceScatterMesh1dDpu类实现
 * Author:
 * Create:
 */
                                                    ...
                                                    const TemplateResource& templateResource)
{
    ...
    // if (channel.location == Device) {
    //     // trans with AICPU
    //     HCCL_ERROR("Reduce scatter sequence is not implemented for Device");
    ...
    const std::map<u32, std::vector<ChannelInfo>>& channels, const u32 myRank, const std::vector<std::vector<uint32_t>>& subCommRanks)
{
    ...
    // for (u32 rankIdx = 0; rankIdx < rankIds.size() - 1; rankIdx++) {
    //     u32 nextRank = (myAlgRank + 1 + rankIdx) % rankIds.size(); // 这里取的虚拟rankId
    //     u32 remoteRank = rankIds[nextRank];
    //     const ChannelInfo &linkSend = channels.at(remoteRank)[0];
    //     const ChannelInfo &linkRecv = channels.at(remoteRank)[0];
    //     CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecord(linkSend.handle, 0)));
    //     HCCL_INFO("rank %u send record to %u with notify %u", myRank, remoteRank, myAlgRank);
    ...
    //     const ChannelInfo &linkSend = channels.at(remoteRank)[0];
    //     const ChannelInfo &linkRecv = channels.at(remoteRank)[0];
    //     CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWait(linkRecv.handle, 0, CUSTOM_TIMEOUT)));
    //     HCCL_INFO("rank %u send wait from %u with notify %u", myRank, remoteRank, nextRank);
    // }
    ...
    //     ChannelInfo linkSend = channels.at(remoteRank)[0];
    //     ChannelInfo linkRecv = channels.at(remoteRank)[0];
    //     linkSend.myRank = myAlgRank;
    //     linkRecv.myRank = myAlgRank;
    //     std::vector<DataSlice> txSrcSlices;
    //     std::vector<DataSlice> txDstSlices;
    //     std::vector<DataSlice> rxSrcSlices;
    //     std::vector<DataSlice> rxDstSlices;
    ...
    //         DataSlice rxDstSlice = DataSlice(tempAlgParams.buffInfo.hcclBuff.addr,
    //             tempAlgParams.buffInfo.hcclBuffBaseOff +  repeatIdx * tempAlgParams.outputRepeatStride +
    //             nextRank * tempAlgParams.outputSliceStride, tempAlgParams.sliceSize, tempAlgParams.count); // 接收目标
    //         DataSlice txSrcSlice = DataSlice(tempAlgParams.buffInfo.inputPtr, tempAlgParams.buffInfo.inBuffBaseOff +
    //             repeatIdx * tempAlgParams.inputRepeatStride + nextRank * tempAlgParams.inputSliceStride,
    //             tempAlgParams.sliceSize, tempAlgParams.count); // 发送源
    //         // TODO 需要调整成对端的ccl-in
    //         DataSlice txDstSlice = DataSlice(remoteCclBuffAddr, tempAlgParams.buffInfo.hcclBuffBaseOff +
    //             repeatIdx * tempAlgParams.outputRepeatStride + myAlgRank * tempAlgParams.outputSliceStride,
    //             tempAlgParams.sliceSize, tempAlgParams.count);  // 发送目标
    ...
    //         txSrcSlices.push_back(txSrcSlice);
    //         txDstSlices.push_back(txDstSlice);
    //         HCCL_INFO("Send from src rank %u to dst rank %u, src offset %u, dst offset %u, size %u",
    //             myRank, remoteRank,
    //             tempAlgParams.buffInfo.inBuffBaseOff + repeatIdx * tempAlgParams.inputRepeatStride + nextRank * tempAlgParams.inputSliceStride,
    //             tempAlgParams.buffInfo.hcclBuffBaseOff + repeatIdx * tempAlgParams.outputRepeatStride + myAlgRank * tempAlgParams.outputSliceStride,
    //             tempAlgParams.sliceSize);
    //         HCCL_INFO("Send from buf %p, to buf %p", tempAlgParams.buffInfo.inputPtr, remoteCclBuffAddr);
    //     }
    ...
    //     const ChannelInfo &linkSend = channels.at(remoteRank)[0];
    //     const ChannelInfo &linkRecv = channels.at(remoteRank)[0];
    //     CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWait(linkRecv.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT)));
    //     HCCL_INFO("rank %u send wait from %u with notify %u", myRank, remoteRank, nextRank);
    // }
    ...
    //     const ChannelInfo &linkSend = channels.at(remoteRank)[0];
    //     const ChannelInfo &linkRecv = channels.at(remoteRank)[0];
    //     CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecord(linkSend.handle, 2)));
    //     HCCL_INFO("rank %u send record to %u with notify %u", myRank, remoteRank, myAlgRank);
    ...
    //     const ChannelInfo &linkSend = channels.at(remoteRank)[0];
    //     const ChannelInfo &linkRecv = channels.at(remoteRank)[0];
    //     CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWait(linkRecv.handle, 2, CUSTOM_TIMEOUT)));
    //     HCCL_INFO("rank %u send wait from %u with notify %u", myRank, remoteRank, nextRank);
    // }
