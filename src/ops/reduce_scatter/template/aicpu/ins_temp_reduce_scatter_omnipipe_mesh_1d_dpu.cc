 ...
 * Description: 算法模板InsTempReduceScatterOmniPipeMesh1dDpu类实现
 */
        ...
        const std::vector<std::vector<uint32_t>> &subCommRanks)
{
    ...
    for (u32 rankIdx = 0; rankIdx < rankIds.size(); rankIdx++) {
        ...
        // u32 nextRank = (myAlgRank + 1 + queIdx) % templateRankSize_;  // 这里取的虚拟rankId , z轴的时候templateRankSize_=2
        ...
        // HCCL_INFO("[InsTempReduceScatterOmniPipeMesh1dDpu][RunReduceScatter] tempAlgParam.inputOmniPipeSliceStride.size()=%u", tempAlgParam.inputOmniPipeSliceStride.size());
        // 按照偏移数组边计算位置
        for (u32 repeatIdx = 0; repeatIdx < tempAlgParam.stepSliceInfo.inputOmniPipeSliceStride[myAlgRank].size(); repeatIdx++) {
            ...
            // HCCL_INFO("[InsTempReduceScatterOmniPipeMesh1dDpu][RunReduceScatter] rxSrcCurrent=%u,rxDstCurrent=%u,txSrcCurrent=%u,txDstCurrent=%u,processSize_=%u,count_=%u", rxSrcCurrent, rxDstCurrent, txSrcCurrent, txDstCurrent, processSize_, count_);
