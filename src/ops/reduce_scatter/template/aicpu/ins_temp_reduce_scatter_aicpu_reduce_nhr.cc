...
HcclResult InsTempReduceScatterAicpuReduceNHR::PostLocalReduce(const std::vector<ThreadHandle> &threads)
{
    ...
    for (u64 rpt = 0; rpt < rptNum; ++rpt) {
        ...
        // 增加地址重叠判断优化
        if (tempAlgParams_.buffInfo.hcclBuff.addr != tempAlgParams_.buffInfo.outputPtr || myScratchOff != outBaseOff) {
...
{
    const u32 nSteps = templateRankSize_ - 1;
    ...
    for (u32 rpt = 0; rpt < rptNum; ++rpt) {
        ...
        for (u32 step = 0; step < nSteps; ++step) {
            ...
            for (u32 i = 0; i < stepInfo.nSlices; ++i) {
                ...
                // 发送：从本Rank的input读取txIdx分片，写入目标Rank的hcclBuffer中本Rank对应的槽位
                const u64 txInOff = inBaseOff + tempAlgParams_.inputSliceStride * txIdx;
                const u64 txScratchOff = scratchBase + tempAlgParams_.sliceSize * myAlgIdx;

                // 接收：从对端读取数据到本端hcclBuffer对应的Slot
                const u64 rxScratchOff = scratchBase + tempAlgParams_.sliceSize * stepInfo.fromRank;
                ...
                HCCL_DEBUG("[InsTempReduceScatterAicpuReduceNHR] i[%u] txIdx[%u] fromRank[%u] txInOff[%llu] txScratchOff[%llu] rxScratchOff[%llu]",
                    i, txIdx, stepInfo.fromRank, txInOff, txScratchOff, rxScratchOff);
            }
...
HcclResult InsTempReduceScatterAicpuReduceNHR::GetStepInfo(u32 step, u32 nSteps, AicpuNHRStepInfo &stepInfo)
{
    ...
    // N-1 步模式：每一步向不同的 Rank 发送它需要的分片
    // step 从 0 到 N-2
    u32 sendTo = (myAlgRank + step + 1) % templateRankSize_;
    u32 recvFrom = (myAlgRank + templateRankSize_ - (step + 1)) % templateRankSize_;

    // 每一轮只发送 1 个分片，即目标 Rank 所需的那个分片
    stepInfo.nSlices = 1;
    stepInfo.toRank = sendTo;
    stepInfo.fromRank = recvFrom;

    // 发送目标 Rank 需要的分片
    stepInfo.txSliceIdxs.push_back(sendTo);
    // 接收逻辑对应：recvFrom 发送给我的分片（即 myAlgRank 对应的分片）
    stepInfo.rxSliceIdxs.push_back(myAlgRank);

    HCCL_DEBUG("[InsTempReduceScatterAicpuReduceNHR][GetStepInfo] step[%u] sendTo[%u] recvFrom[%u] txSliceIdx[%u]", 
        step, sendTo, recvFrom, sendTo);
