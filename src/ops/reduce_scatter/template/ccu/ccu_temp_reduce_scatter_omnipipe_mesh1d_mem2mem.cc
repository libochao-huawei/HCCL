    ...
    const TopoInfoWithNetLayerDetails *topoInfo, AlgResourceRequest &resourceRequest)
{
    ...
    kernelInfo.creator = [](const hcomm::CcuKernelArg &arg) {
        return std::make_unique<CcuKernelReduceScatterOmniPipeMesh1DMem2Mem>(arg);
    };
    std::vector<HcclChannelDesc> channelDescs;
    // for(int i=0;i<subCommRanks_[0].size();i++){
    //     HCCL_DEBUG("[jjy]subCommRanks_[0][%d]:%d,myRank:%d",i,subCommRanks_[0][i],myRank_);
    // }
    if(topoInfo->level0Topo != Level0Shape::MESH_1D_CLOS) {
        ...
        for(auto channel : channelDescs){
            // HCCL_INFO("[jjy][CalcRes]myRank_:%d,channel.remoteRank:%d",myRank_,channel.remoteRank);
    ...
    kernelInfo.kernelArg = std::make_shared<CcuKernelArgReduceScatterOmniPipeMesh1DMem2Mem>(
    ...
    // HCCL_INFO("[jjy][CalcRes]myRank_:%d",myRank_);
    // for(HcclChannelDesc channel : channelDescs){
    //     HCCL_INFO("[jjy][CalcRes]myRank_:%d,channel.remoteRank:%d",myRank_,channel.remoteRank);
    // }
    ...
    const OpParam &param, const TemplateDataParams &templateDataParams, TemplateResource &templateResource)
{
    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            auto taskArg = std::make_unique<CcuTaskArgReduceScatterOmniPipeMesh1DMem2Mem>(inputAddr, outputAddr, scratchAddr, sliceSize,
...
//         auto taskArg = std::make_unique<CcuTaskArgReduceScatterOmniPipeMesh1DMem2Mem>(inputAddr, outputAddr, sliceSize,
