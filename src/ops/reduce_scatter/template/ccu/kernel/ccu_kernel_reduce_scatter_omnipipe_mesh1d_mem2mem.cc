    ...
    : CcuKernelAlgBase(arg)
{
    ...
    HCCL_INFO("[jjy1]subCommRanks_.size():%d,subCommRanks_[0].size():%d",kernelArg->subCommRanks_.size(),kernelArg->subCommRanks_[0].size());
    for(int i=0;i<kernelArg->subCommRanks_.size();i++){
        for(int j=0;j<kernelArg->subCommRanks_[0].size();j++){
            HCCL_INFO("subCommRanks_[%d][%d]:%d",i,j,kernelArg->subCommRanks_[i][j]);
        }
    }
}
...
void CcuKernelReduceScatterOmniPipeMesh1DMem2Mem::DoRepeatReduceScatter()
{
    ...
    CCU_IF(localCopyFlag_ == 0) {
        ...
        scratchOffset = 0;
        for (auto i = 0; i < rankSize_; ++i) {
            scratchMem[i].addr = scratch_; // [jjy][todo]scratch_待加载
        ...
        for (auto i = 0; i < rankSize_; ++i) {
            ...
            ReadNb(channels_[channelId], scratchMem[channelId], src[channelId], sliceSize_, event_);
            channelId++;
        }
        // 等读完所有对端
...
{
    HCCL_INFO("[noe]");
    uint32_t LOCAL_REDUCE_MS_PER_LOOP = 8;
    uint32_t CCU_MS_LOCAL_REDUCE_LOOP_COUNT = 8;
    // AllocGoResource(16);
    AllocGoResource(LOCAL_REDUCE_MS_PER_LOOP, CCU_MS_LOCAL_REDUCE_LOOP_COUNT);
