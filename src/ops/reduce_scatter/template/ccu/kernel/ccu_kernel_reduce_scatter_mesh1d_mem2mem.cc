...
void CcuKernelReduceScatterMesh1DMem2Mem::PairwiseLocalReduce(CcuRep::LocalAddr myOutput, std::vector<CcuRep::LocalAddr> &intputVec,
    CcuRep::Variable sliceSize, HcclDataType dataType, HcclDataType outputDataType, HcclReduceOp opType)
{
    ...
    while (remainPieces > 1) {
        ...
        uint32_t reducePieces = remainPiece / 2;
        ...
        event_[0].SetMask(1);
        LocalReduceNb(intputVec[0], intputVec[srcIdx], len, dataType, opType, event_[0]);
        WaitEvent(event_[0]);
    ...
    event_[0].SetMask(1);
    LocalCopyNb(myOutput, intputVec[0], sliceSize, event_[0]);
    WaitEvent(event_[0]);
}
