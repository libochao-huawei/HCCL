...
void CcuKernelReduceScatterOmniPipeMesh1D::LoadArgs()
{
    ...
    CcuRep::Variable repeatNumTmp = CreateVariable();
    repeatNumTmp = repeatNum_;

    CcuRep::Variable repeatNumAdd = CreateVariable();
    repeatNumAdd  = 1;
    uint64_t idx = 0;
    CCU_WHILE(repeatNumTmp != UINT64_MAX) {
        inputOmniPipeSliceStride_.emplace_back(CreateVariable());
        Load(inputOmniPipeSliceStride_[idx]);
        repeatNumTmp += repeatNumAdd;
        idx++;
    }
    HCCL_DEBUG("[%s] repeatNumDis[%llu]", __func__, idx); // 查看repeatNum
...
void CcuKernelReduceScatterOmniPipeMesh1D::DoRepeatReduceScatter()
{
    ...
    CCU_IF(localCopyFlag_ == 0) {
        ...
        CCU_WHILE(repeatNumTmp != UINT64_MAX)
        {
            ...
            dst.addr += inputOmniPipeSliceStride_[repeatIdx];
            ...
            for (auto i = 0; i < rankSize_; ++i) {
                ...
                src[idx].addr += inputOmniPipeSliceStride_[repeatIdx];
...
void CcuKernelReduceScatterOmniPipeMesh1D::DoLocalCopy()
{
    ...
    CcuRep::Variable inputSliceStride = CreateVariable();
    inputSliceStride = 0;
    for (auto i = 0; i < rankId_; ++i) {
        inputSliceStride += inputSliceStride_;
    }

    // myOutput.addr           =  output_[rankId_];
    myOutput.addr           =  output_;
    // myOutput.addr           += inputOmniPipeSliceStride_[0];
    // myOutput.addr           += outSliceStride;
    myInput.addr            =  input_[rankId_];
    myInput.addr            += inputSliceStride;
...
std::vector<uint64_t> CcuKernelReduceScatterOmniPipeMesh1D::GeneArgs(const CcuTaskArg &arg)
{
    ...
    std::vector<uint64_t> taskArgs = {inputAddr, outputAddr, sliceSize, offset, tokenInfo, repeatNumTmp, localCopyFlag,
        inputSliceStride, outputSliceStride, goSize[0], goSize[1], goSize[2], goSize[3]};
    HCCL_INFO(
        "[%s] rankId[%u] userRank[%u] TaskArgs: (0)inputAddr[%llu] (1)outputAddr[%llu] (2)sliceSize[%llu] "
        "(3)offset[%llu] (4)token[%llu] (5)repeatNum[%llu] (6)localCopyFlag[%llu] (7)inputSliceStride[%llu] "
        "(8)outputSliceStride[%llu]",
        __func__, rankId_, userRank_, inputAddr, outputAddr, sliceSize, offset, tokenInfo, repeatNumTmp, localCopyFlag,
        inputSliceStride, outputSliceStride);

    for (auto i = 0; i < repeatNum; ++i) {
        taskArgs.emplace_back(taskArg->inputOmniPipeSliceStride_[i]);
        HCCL_DEBUG("[%s] inputOmniPipeSliceStrie[%u]:%u", __func__, i, taskArg->inputOmniPipeSliceStride_[i]);
    }
