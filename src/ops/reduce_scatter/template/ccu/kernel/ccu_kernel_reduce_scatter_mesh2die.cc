...
static CcuResult InitResources(ReduceScatterMesh2DieContext &ctx)
{
    ...
    ctx.myOutput = ccu::GetResByChannel<ccu::Variable>(arg->channels[0], 1);
...
static CcuResult RmtReduce(ReduceScatterMesh2DieContext &ctx)
{
    ...
    if (ctx.rmtReduceWithMyRank) {
        CCU_CHK_RET(GroupReduce(ctx, arg->channels.data(), arg->channelCount, dst, src, localSrc, ctx.rmtReduceGoSize, ctx.dataType, ctx.outputDataType, ctx.reduceOp));
    } else {
        CCU_CHK_RET(GroupReduceWithoutMyRank(ctx, arg->channels.data(), arg->channelCount, dst, src, ctx.rmtReduceGoSize, ctx.dataType, ctx.outputDataType, ctx.reduceOp));
    }
    return CCU_SUCCESS;
}
