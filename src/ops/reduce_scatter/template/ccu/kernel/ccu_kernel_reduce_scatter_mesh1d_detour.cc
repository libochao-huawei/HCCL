    ...
    : CcuKernelAlgBase(arg)
{
    ...
    for (uint32_t i = 0; i < detourPathNum_; i++) {
        for (uint32_t j = 0; j < rankSize_ - 1; j++) {
            ...
            HCCL_DEBUG("[CcuKernelArgReduceScatterMeshDetour1D] detourChannels_ emplace_back directChannels[%llu], size[%zu]", i , detourChannels_[i].size());
            detourChannels_[i + directPathNum].emplace_back(channels_[(i + 1) * (rankSize_ - 1) + j + detourPathNum_ * (rankSize_ - 1)]);
        }
    }
}
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            HCCL_INFO("[CcuKernelReduceScatterMeshDetour1D] MyRank[%u], PeerId[%llu], ChannelId[%u]",
                ...
                HCCL_ERROR("[CcuKernelReduceScatterMeshDetour1D] Algorithm transport ptr is null"),);
                input_.push_back(CreateVariable((*detourChannels_[0][channelIdx]), INPUT_XN_ID));
...
HcclResult CcuKernelReduceScatterMeshDetour1D::Algorithm()
{
    ...
    GroupReduce(detourChannels_[0], tailDst, tailSrc, groupOpSize_, dataType_, outputDataType_, reduceOp);
