    ...
    : CcuKernelAlgBase(arg)
{
    ...
    if (outputDataType_ == HcclDataType::HCCL_DATA_TYPE_RESERVED) {
        ...
        HCCL_DEBUG(
            "[CcuKernelArgGatherOmniPipeMesh1D] outputDataType is [INVALID], set outputDataType to[%d]",
            outputDataType_);
    }
    HCCL_INFO(
        "[CcuKernelArgGatherOmniPipeMesh1D] Init, KernelArgs are rankIdx[%u], rankId[%u], rankSize_[%u], dataType[%d], "
        "outputDataType[%d]",
...
{
    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1D] [InitResource] begin");
    uint16_t channelIdx = 0;
    if (channels_.size() == 0) {
        HCCL_ERROR("[CcuKernelGatherOmniPipeMesh1D] channels is empty!");
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankIdx_) {
            ...
            HCCL_DEBUG("[CcuKernelGatherOmniPipeMesh1D] MyRank[%u], PeerId[%u], ChannelId[%u]", rankIdx_, peerId, channelIdx);
    ...
    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1D] [InitResource] end");
    return HcclResult::HCCL_SUCCESS;
}

void CcuKernelGatherOmniPipeMesh1D::LoadArgs() //TODO:change
{
    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1D] [LoadArgs] begin");
    ...
    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1D] [LoadArgs] end");
}
...
{
    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1D] [DoRepeatGather] begin");

    // 本地拷贝
    CCU_IF(localCopyFlag_ == 1) {
        HCCL_INFO("[DoRepeatGather] rankid %u, localcopy begin", rankId_);
        ...
        HCCL_DEBUG("[DoRepeatGather] rankid %u, localcopy end", rankId_);
        WaitEvent(event_);
    ...
    CCU_IF(localCopyFlag_ == 0) { // 由于 上面 return不能用，所以这边加了这个判断
        ...
        CCU_WHILE(repeatNum_ != UINT64_MAX) {
            ...
            idx ++;
            HCCL_DEBUG("[DoRepeatGather] GroupBroadcast begin, dst size %u", dst.size());
            ...
            // WaitEvent(event1_);
            HCCL_INFO("[DoRepeatGather] GroupBroadcast end");
        }
    }

    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1D] [DoRepeatGather] end");
...
{
    HCCL_INFO("[CcuKernelArgGatherOmniPipeMesh1D] GatherOmniPipeMesh1D run");
    ...
    HCCL_INFO("[CcuKernelArgGatherOmniPipeMesh1D] GatherOmniPipeMesh1D end");
...
{
    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1D] [GeneArgs] begin");
    ...
    HCCL_INFO("[CcuKernelArgGatherOmniPipeMesh1D] TaskArgs: rankIdx_[%u], rankId_[%u], inputAddr[%llu], outputAddr[%llu], "
    ...
    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1D] [GeneArgs] end");
    return taskArgs;
}
