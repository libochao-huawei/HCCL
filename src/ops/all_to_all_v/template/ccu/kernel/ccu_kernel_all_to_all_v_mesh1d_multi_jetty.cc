...
static CcuResult InitResource(AllToAllVMesh1DMultiJettyContext &ctx)
{
    ...
    for (uint64_t peerId = 0; peerId < arg->rankSize; peerId++) {
        if (peerId != arg->rankId) {
            ctx.input[peerId] = ccu::GetResByChannel<ccu::Variable>(arg->channels[channelIdx], OUTPUT_XN_ID);
    ...
    for (uint32_t i = 0; i < arg->jettyNums[rankIdx]; i++) {
        CCU_IF(ctx.sendRecvInfo[rankIdx].loopNum != UINT64_MAX - 1) {
            CCU_IF(ctx.sendRecvInfo[rankIdx].lastSliceSize == 0) {
                CHK_RET(ccu::EventRecord(ctx.eventList[rankIdx], 1 << i));
            }
            CCU_IF(ctx.sendRecvInfo[rankIdx].lastSliceSize != 0) {
    ...
    CCU_WHILE(ctx.completedRankCount != arg->rankSize) {
        for (uint32_t rankIdx = 0; rankIdx < arg->rankSize; rankIdx++) {
            ...
            CCU_IF(ctx.sendRecvInfo[rankIdx].loopNum == UINT64_MAX) {
                CHK_RET(ccu::EventRecord(ctx.eventList[rankIdx], (1 << arg->jettyNums[rankIdx]) - 1));
            }
            CCU_IF(ctx.sendRecvInfo[rankIdx].loopNum != UINT64_MAX) {
        ...
        CCU_IF(ctx.sendRecvInfo[arg->rankId].loopNum == UINT64_MAX) {
            CHK_RET(ccu::EventRecord(ctx.eventList[arg->rankId], (1 << arg->jettyNums[arg->rankId]) - 1));
        }
        ...
        for (uint32_t rankIdx = 0; rankIdx < arg->rankSize; rankIdx++) {
            CHK_RET(ccu::EventWait(ctx.eventList[rankIdx], (1 << arg->jettyNums[rankIdx]) - 1));
        }
    }
    return CCU_SUCCESS;
}
