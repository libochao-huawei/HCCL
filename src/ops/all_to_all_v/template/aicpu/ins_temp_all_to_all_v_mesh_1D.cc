    ...
    const u32 myAlgRank)
{
    ...
    for (u32 roundIdx = 0; roundIdx < commLoops && remainRankSize > 0; roundIdx++) {
        ...
        if (isDmaRead_) {
            ...
            CHK_RET(PreCopy(commRanks, templateResource.channels, templateResource.threads, tempAlgParams, myAlgRank));
            if (threadNum_ > 1) {
                std::vector<ThreadHandle> subThreads(templateResource.threads.begin() + 1, templateResource.threads.end());
        ...
        CHK_RET(RunSendRecvByLoop(commRanks, tempAlgParams, templateResource.channels, templateResource.threads));
        if (threadNum_ > 1) {
            std::vector<ThreadHandle> subThreads(templateResource.threads.begin() + 1, templateResource.threads.end());
            GetNotifyIdxSubToMain(notifyIdxSubToMain_);
            CHK_RET(PostSyncInterThreads(templateResource.threads[0], subThreads, notifyIdxSubToMain_));
    ...
    for (u32 rankIdx = 0; rankIdx < commRanks.size(); rankIdx++) {
        ...
        for (u32 channelId = 0; channelId < curChannels.size(); channelId++) {
            ...
            DataSlice rxDstSlice = DataSlice(tempAlgParams.buffInfo.outputPtr,
                tempAlgParams.rdispls[remoteRank] * dataTypeSize_ + recvOffsetSplit[channelId],
