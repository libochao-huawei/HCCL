    ...
    const ThreadHandle &thread, const u32 myAlgRank, const u32 queIdx) const
{
    ...
    HCCL_DEBUG("[InsTempAlltoAllVMesh1D][LocalCopyForMyRank] time: %ld us", localCopyDuration);
    return HCCL_SUCCESS;
}
    ...
    const std::vector<ThreadHandle> &threads)
{
    ...
    for (u32 rankIdx = 0; rankIdx < commRanks.size(); rankIdx++) {
        ...
        HCCL_DEBUG("[InsTempAlltoAllVMesh1D][RunSendRecvByLoop] remoteRank[%u] time: %ld us", remoteRank, rankDuration);
    }
    ...
    const std::vector<ThreadHandle> &threads, u32 &queIdx) const
{
    ...
    for (u32 channelId = 0; channelId < curChannels.size(); channelId++) {
        ...
        HCCL_DEBUG("[InsTempAlltoAllVMesh1D][RunSendRecvByChannel] channelId[%u] time: %ld us", channelId, chDuration);
        queIdx++;
    }
    ...
    HCCL_DEBUG("[InsTempAlltoAllVMesh1D][RunSendRecvByChannel] total time: %ld us", channelDuration);
    return HcclResult::HCCL_SUCCESS;
}
    ...
    const ThreadHandle& thread, const u32 channelId) const
{
    ...
    HCCL_DEBUG("[InsTempAlltoAllVMesh1D][RunSendRecv] channelId[%u] time: %ld us", channelId, sendRecvDuration);
    return HcclResult::HCCL_SUCCESS;
}
    ...
    const u64 &recvCount, const u64 &recvOffset) const
{
    ...
    HCCL_DEBUG("[InsTempAlltoAllVMesh1D][PostCopy] remoteRank[%u] recvSize[%llu] time: %ld us",
