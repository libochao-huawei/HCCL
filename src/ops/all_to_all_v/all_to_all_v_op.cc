    ...
    uint64_t recvCount, HcclDataType recvType, HcclComm comm, aclrtStream stream)
{
    ...
    /* 接口交互信息日志 */
    CHK_RET(AlltoAllEntryLog(sendBuf, recvBuf, sendCount, recvCount, sendType, recvType, stream, param.tag, "HcclAlltoAll"));

    // 底层走AlltoAllV
    bool useInnerOp = false;
    CHK_RET(AlltoAllVOutPlace(param, sendBuf, sendCounts.data(), sdispls.data(),
        recvBuf, recvCounts.data(), rdispls.data(), recvType, comm, stream,
        HcclCMDType::HCCL_CMD_ALLTOALL, rankSize, useInnerOp));
    
    CHK_RET(LogHcclExit("HcclAlltoAll", param.tag, startut));
    ...
    const void *recvBuf, const void *recvCounts, const void *rdispls, HcclDataType recvType, HcclComm comm, aclrtStream stream)
{
    ...
    /* 接口交互信息日志 */
    CHK_RET(AlltoAllVEntryLog(sendBuf, recvBuf, sendCounts, recvCounts, sdispls, rdispls, sendType, recvType, stream, param.tag, "HcclAlltoAllV"));

    // 底层走AlltoAllV
    bool useInnerOp = false;
    CHK_RET(AlltoAllVOutPlace(param, sendBuf, sendCounts, sdispls, recvBuf, recvCounts, rdispls, recvType, comm, stream,
        HcclCMDType::HCCL_CMD_ALLTOALLV, rankSize, useInnerOp));
    
    CHK_RET(LogHcclExit("HcclAlltoAllV", param.tag, startut));
    ...
    const void *recvBuf, HcclDataType recvType, HcclComm comm, aclrtStream stream)
{
    ...
    /* 接口交互信息日志 */
    CHK_RET(AlltoAllVCEntryLog(sendBuf, recvBuf, sendCountMatrix, sendType, recvType, stream, param.tag, "HcclAlltoAllVC"));

    // 底层走AlltoAllV
    bool useInnerOp = false;
    CHK_RET(AlltoAllVOutPlace(param, sendBuf, sendCounts.data(), sdispls.data(),
        recvBuf, recvCounts.data(), rdispls.data(), recvType, comm, stream,
        HcclCMDType::HCCL_CMD_ALLTOALLVC, rankSize, useInnerOp));

    CHK_RET(LogHcclExit("HcclAlltoAllVC", param.tag, startut));    
...
HcclResult AlltoAllVOutPlaceCommon(OpParam &param, const void *sendBuf, const void *sendCounts, const void *sdispls, const void *recvBuf,
    const void *recvCounts, const void *rdispls, HcclDataType dataType, HcclComm comm, aclrtStream stream,
    HcclCMDType opType, u32 rankSize, bool &useInnerOp, OpMode opMode, const ResPackGraphMode &resPack)
{
    ...
    CHK_RET(AlltoAllVConstructOpParam(sendBuf, sendCounts, sdispls, recvBuf, recvCounts, rdispls, dataType,
        comm, stream, opType, rankSize, opMode, varMemSize, param));
    ...
    const std::string &tag, HcclCMDType opType, u32 rankSize, const ResPackGraphMode &resPack)
{
    ...
    bool useInnerOp = false;
    CHK_RET(AlltoAllVOutPlaceCommon(param, sendBuf, sendCounts, sdispls, recvBuf, recvCounts, rdispls, dataType, comm, stream,
         opType, rankSize, useInnerOp, OpMode::OFFLOAD, resPack));
    if (useInnerOp) {
    ...
    HcclCMDType opType, u32 rankSize, bool &useInnerOp)
{
    ...
    CHK_RET(AlltoAllVOutPlaceCommon(param, sendBuf, sendCounts, sdispls, recvBuf, recvCounts, rdispls, dataType, comm, stream,
        opType, rankSize, useInnerOp, OpMode::OPBASE, ResPackGraphMode()));
...
HcclResult AlltoAllEntryLog(const void *sendBuf, const void *recvBuf, uint64_t sendCount, uint64_t recvCount,
    HcclDataType sendType, HcclDataType recvType, aclrtStream stream, const char *tag, const std::string &opName)
{
    if (GetExternalInputHcclEnableEntryLog()) {
        ...
        s32 ret = snprintf_s(stackLogBuffer, LOG_TMPBUF_SIZE, LOG_TMPBUF_SIZE - 1U,
            ...
            tag, sendBuf, recvBuf, sendCount, recvCount, GetDataTypeEnumStr(sendType).c_str(), GetDataTypeEnumStr(recvType).c_str(),
    ...
    const char *tag, const std::string &opName)
{
    if (GetExternalInputHcclEnableEntryLog()) {
        ...
        s32 ret = snprintf_s(stackLogBuffer, LOG_TMPBUF_SIZE, LOG_TMPBUF_SIZE - 1U,
            ...
            tag, sendBuf, recvBuf, sendCounts, recvCounts, sdispls, rdispls, GetDataTypeEnumStr(sendType).c_str(),
    ...
    HcclDataType sendType, HcclDataType recvType, aclrtStream stream, const char *tag, const std::string &opName)
{
    if (GetExternalInputHcclEnableEntryLog()) {
        ...
        s32 ret = snprintf_s(stackLogBuffer, LOG_TMPBUF_SIZE, LOG_TMPBUF_SIZE - 1U,
            ...
            tag, sendBuf, recvBuf, sendCountMatrix, GetDataTypeEnumStr(sendType).c_str(), GetDataTypeEnumStr(recvType).c_str(),
            streamId, deviceLogicId);
