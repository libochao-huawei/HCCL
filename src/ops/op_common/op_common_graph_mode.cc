                          ...
                          std::unique_ptr<AlgResourceCtxSerializable>& resCtxHost)
{
    ...
    for (u32 level = 0; level < resRequest.channels.size(); level++) {
        ...
        for (u32 idx = 0; idx < channelNum; idx++) {
            ...
            CHK_RET(HcclGetRemoteBuffGraphMode(comm, levelNChannels[idx], "InputBuffer", &remoteInputBufferAddr, &remoteInputBufferSize));
            ...
            CHK_RET(HcclGetRemoteBuffGraphMode(comm, levelNChannels[idx], "OutputBuffer", &remoteOutputBufferAddr, &remoteOutputBufferSize));
                                   ...
                                   std::unique_ptr<AlgResourceCtxSerializable>& resCtxHost)
{
    ...
    CHK_RET(HcclGetH2DNotify(resCtxHost));
    CHK_RET(HcclGetThread(comm, param, resRequest, resCtxHost));
    CHK_RET(HcclGetChannelForCcuGraphMode(comm, param, resRequest, resCtxHost));
    CHK_RET(HcclGetCcuKernelGraphMode(comm, param, resRequest, resCtxHost));
    return HCCL_SUCCESS;
}
