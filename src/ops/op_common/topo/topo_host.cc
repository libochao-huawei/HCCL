...
HcclResult CalcLevel0TopoShape(HcclComm comm, TopoInfoWithNetLayerDetails* topoInfo)
{
    ...
    if (topoInstNum == 1 && rankNumForTopoType[CommTopo::COMM_TOPO_1DMESH].size() == 1) {
        ...
        return HCCL_SUCCESS;
               ...
               rankNumForTopoType[CommTopo::COMM_TOPO_1DMESH].size() ==
               topoInstNum - rankNumForTopoType[CommTopo::COMM_TOPO_CLOS].size()) {
        if (!is_uniform(rankNumForTopoType[CommTopo::COMM_TOPO_1DMESH])) {
            HCCL_ERROR("[BaseSelector][CalcLevel0TopoShape] All mesh topos should have the same size!");
