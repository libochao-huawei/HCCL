...
{
    std::cout << "SelectChannelToVec: from " << myRankId << " to " << rmtRankId << std::endl;
    ...
    std::cout << "channelNum: " << rankIdToChannelDesc.at(rmtRankId).size() << std::endl;
    ...
    for (const HcclChannelDesc &channel: rankIdToChannelDesc.at(rmtRankId)) {
        ...
        std::cout << "dieid: " << tmpDieId << std::endl;
        if (tmpDieId == dieId) {
            channels.push_back(channel);
            std::cout << "channels.push_back" << std::endl;
    ...
    std::cout << "find channel on die["<< dieId << "] succeed." << std::endl;
    return HcclResult::HCCL_SUCCESS;
}
