...
HcclResult CcuAlgTemplateBase::GetDieInfoFromChannelDescs(HcclComm comm, 
    cosnt std::map<u32, std::vector<HcclChannelDesc>> &rankIdToChannelDesc, 
    u32 MyRankId, uint32_t &dieNum, uint32_t &dieId)
{
    ...
    // 遍历每个对端有几个channel
    for (const auto& [rmtRankId, channels]: rankIdToChannelDesc) {
        if (channels.size() == LINK_NUM_1) {
            dieNum = 1;
            GetChannelDieId(comm, myRank_, channels[0], dieId);
            HCCL_INFO("[CcuAlgTemplateBase::GetDieNumFromChannelDescs] only 1 channel, dieNum = 1, dieId = %u.", dieId);
            return HcclResult::HCCL_SUCCESS;
        }else if (channels.size() == LINK_NUM_2) {
            ...
            HCCL_ERROR("[CcuAlgTemplateBase::CalcRes] get channelDescs fail: there are [%u] link to rank [%u]",
                    firstVector.size(), channels.size());
            return HcclResult::HCCL_E_INTERNAL;
        }
    }
}
...
HcclResult CcuAlgTemplateBase::SelectChannelToVec(const HcclComm comm, const u32 myRankId, const u32 rmtRankId,
    ...
    std::map<u32, u32>& rank2ChannelIdx, std::vector<std::vector<HcclChannelDesc>>& channels)
{
    ...
    bool isFound = false;
    for (HcclChannelDesc channel: rankIdToChannelDesc.at(rmtRankId)) {
        uint32_t tmpDieId = 0;
