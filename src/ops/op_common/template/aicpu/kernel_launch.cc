...
// V2 executor path: uses InsCollAlgBase + AlgResourceCtxSerializable
static int LaunchAicpuKernelV2(OpParam *param, const std::string& algName)
{
    // 判断通信域状态
    ...
    // 栈溢出诊断：打印当前栈地址、栈边界和函数地址
    {
        void *frameAddr = __builtin_frame_address(0);
        unsigned long stackStart = 0;
        unsigned long stackEnd = 0;
        unsigned long ulFrame = reinterpret_cast<unsigned long>(frameAddr);
        std::string allStacks;
        ...
        if (maps.is_open()) {
            ...
            while (std::getline(maps, line)) {
                if (line.find("[stack") != std::string::npos) {
                    ...
                    if (!iss.fail()) {
                        allStacks += " [" + line + "]";
                        // 只取包含当前frameAddr的那个栈条目
                        if (ulFrame >= start && ulFrame < end) {
                            stackStart = start;
                            stackEnd = end;
                        }
                    }
        ...
        long remaining = (stackStart > 0) ? (ulFrame - stackStart) : -1;
        std::cerr << "[STACK PROBE V2] frame_addr=" << frameAddr
                  << " stack=[" << std::hex << stackStart << "-" << stackEnd << std::dec << "]"
                  << " stack_total=" << ((stackEnd > stackStart) ? (stackEnd - stackStart) : 0UL)
                  << " remaining=" << remaining
                  << " thread=" << thread
                  << " threads.size=" << resCtxPtr->threads.size()
                  << " maxNotifyNum=" << maxNotifyNum
                  << " func_ptr=" << reinterpret_cast<void*>(&HcommThreadNotifyWaitOnThread)
...
// V1 executor path: uses ExecutorBase + AlgResourceCtx
static int LaunchAicpuKernelV1(OpParam *param, const std::string& algName)
{
...
extern "C" unsigned int HcclLaunchAicpuKernel(OpParam *param)
{
    ...
    std::cerr << "[STACK PROBE main] &algName=" << &algName << std::endl;
    int ret;
    if (ops_hccl::IsOpsV2(param->algName, param->deviceType)) {
        ret = LaunchAicpuKernelV2(param, algName);
    } else {
        ret = LaunchAicpuKernelV1(param, algName);
    }
