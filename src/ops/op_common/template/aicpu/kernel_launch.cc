...
extern "C" unsigned int HcclLaunchAicpuKernel(OpParam *param)
{
    ...
    if (param->deviceType == DevType::DEV_TYPE_910_95) {
        ...
        auto statusRet = HcclCommGetStatusByName(param->commName, &commStatus);
        if (statusRet != HCCL_SUCCESS) {
            HCCL_ERROR("%s HcclCommGetStatusByName fail, commName[%s], ret = %d", __func__, param->commName, statusRet);
            return 1;
        }
