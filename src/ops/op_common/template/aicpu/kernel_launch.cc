...
extern "C" unsigned int HcclLaunchAicpuKernel(OpParam *param)
{
    ...
    if (param->deviceType == DevType::DEV_TYPE_910_95) {
        ...
        for (size_t i = 0; i < resCtx.threads.size(); ++i) {
            if (HcommBatchModeAddThread(resCtx.threads[i], param->algTag) != HCCL_SUCCESS) {
                HCCL_ERROR("failed to add thread %lu to batch mode, tag is %s.", resCtx.threads[i], param->algTag);
                return 1;   
            }
        }
