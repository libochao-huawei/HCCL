                                     ...
                                     HcclDataType outputDataType, HcclReduceOp opType)
{
    ...
    for (int32_t index = 0; index < 2; index++) { // 需要实现化2个Loop
        ...
        for (uint32_t i = 0; i < size; i++) {
            ...
            var.loopSrc[index].emplace_back(*reinterpret_cast<CcuRemoteAddr*>(&tmp));
...
CcuResult GroupReduce(CcuKernelCtxBase &ctx, const size_t channels[], uint32_t channelCount, CcuLocalAddr dst,
