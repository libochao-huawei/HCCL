                                     ...
                                     HcclDataType outputDataType, HcclReduceOp opType)
{
    ...
    for (int32_t index = 0; index < 2; index++) { // 需要实现化2个Loop
        ...
        for (uint32_t i = 0; i < size; i++) {
            ...
            var.loopSrc[index].emplace_back(*reinterpret_cast<ccu::RemoteAddr*>(&tmp));
            // CCU_CHK_RET(ccu::Alloc(&var.loopSrc[index][i]));
        }
        ...
        CCU_LOOP(ctx.loops[index]) {
            for (uint32_t i = 0; i < channelSize; i++) {
                loopEvt.mask = 1 << i;
                HCCL_INFO("xjhlog4, tmp handle[%llu]", var.loopSrc[index][i].addr.handle);
                ccu::ReadNb(channels[i], ctx.moRes.ccuBuf[bufBase + i], var.loopSrc[index][i], var.loopLen[index], loopEvt);
                HCCL_INFO("xjhlog5, tmp handle[%llu]", var.loopSrc[index][i].addr.handle);
            }

            ccu::LocalAddr &localSrc = *reinterpret_cast<ccu::LocalAddr*>(&var.loopSrc[index][channelSize]);
            loopEvt.mask = 1 << channelSize;
            ccu::LocalCopyNb(ctx.moRes.ccuBuf[bufBase + channelSize], localSrc, var.loopLen[index], loopEvt);
    ...
    for (uint32_t i = 0; i < size; i++) {
        HCCL_INFO("xjhlog2, tmp handle[%llu]", var.loopSrc[0][i].addr.handle);
    }
...
CcuResult GroupReduce(CcuKernelCtxBase &ctx, const size_t channels[], uint32_t channelCount, ccu::LocalAddr dst,
                        std::vector<ccu::RemoteAddr> src, GroupOpSizeVars goSize, HcclDataType dataType,
                        HcclDataType outputDataType, HcclReduceOp opType)
{
    GroupReduceVar var;
    ...
    CCU_IF_ONLY(goSize.loopParam != 0)
    {
        ...
        for (uint32_t i = 0; i < size; ++i) {
            HCCL_INFO("xjhlog1, loopSrc handle[%llu]", var.loopSrc[0][i].addr.handle);
            var.loopSrc[0][i].addr = src[i].addr;   // GSA[0] + GSA[400] to GSA[0] | GSA[1] + GSA[400] to GSA[0]
            var.loopSrc[0][i].token = src[i].token; // Xn[8] + Xn[413] to Xn[0] | Xn[9] + Xn[413] to Xn[0]
        }
