...
std::vector<uint64_t> CcuKernelAlgBase::CalGoSize(uint64_t size)
{
    ...
    //     Hccl::THROW<Hccl::CcuApiException>("Please Check Configure, loopCount = %u, memSlice = %u", moConfig.loopCount,
    //                            moConfig.memSlice);
    // }
    // if (size > maxSize) {
    //     Hccl::THROW<Hccl::CcuApiException>("Too Large Size, size = %lu, maxSize = %lu", size, maxSize);
    // }
                             ...
                             HcclDataType outputDataType, HcclReduceOp opType)
{
    ...
    CCU_IF(goSize.loopParam != 0)
    {
        ...
        LoopGroup({lc}, {loopParam}, paraCfg, offsetCfg);
        //AddCcuProfiling(goSize, channels, dataType, outputDataType, opType);
    }
