                                ...
                                const size_t channels[], uint32_t channelCount)
{
    ...
    for (int32_t index = 0; index < 2; index++) { // 需要实现化2个Loop
        ...
        CCU_LOOP(ctx.loops[index]) {
            ...
            for (uint32_t i = 0; i < channelSize; i++) {
                loopEvt.mask = 1 << i;
                ccu::WriteNb(channels[i], var.loopRemoteDst[index][i], ctx.moRes.ccuBuf[bufBase], var.loopLen[index], loopEvt);
            }

            // 向本地目标地址写入数据
            loopEvt.mask = 1 << channelSize;
            ccu::LocalCopyNb(var.loopLocalDst[index], ctx.moRes.ccuBuf[bufBase], var.loopLen[index], loopEvt);

            // 等待所有操作完成
            loopEvt.mask = (1 << (channelSize + 1)) - 1;
            ccu::WaitEvent(loopEvt);
        }
    }
