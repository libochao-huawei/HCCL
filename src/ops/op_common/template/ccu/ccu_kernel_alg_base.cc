                                                 ...
                                                 HcclReduceOp opType)
{
    ...
    for (int32_t index = 0; index < 2; index++) {
        ...
        for (uint32_t i = 0; i < channels.size(); i++) {
            if (i < rankId) {
                CHK_RET(MsWriteNb(channels[i], bufs[0], bufs[rankId], len, ckeIdx, WRITE_DONE_MASK));
            } else {
                CHK_RET(MsWriteNb(channels[i], bufs[0], bufs[rankId + 1], len, ckeIdx, WRITE_DONE_MASK));
            }
        }
