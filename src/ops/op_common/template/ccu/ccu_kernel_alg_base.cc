...
CcuResult GroupReduce(CcuKernelCtxBase &ctx, const std::vector<ChannelHandle> &channels, CcuLocalAddr dst,
                        std::vector<CcuRemoteAddr> src, GroupOpSizeVars goSize, HcclDataType dataType,
                        HcclDataType outputDataType, HcclReduceOp opType)
{
    GroupReduceVar var;
    CCU_CHK_RET(CreateMultiOpReduce(ctx, var, channels, dataType, outputDataType, opType));
