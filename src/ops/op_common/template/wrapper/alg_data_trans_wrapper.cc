...
HcclResult SendWrite(const DataInfo &sendInfo, const ThreadHandle &thread)
{
    ...
    u32 customTimeout = GetExternalInputHcclCustomTimeout();
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyWaitOnThread(thread, sendChannel.handle, NOTIFY_IDX_ACK, customTimeout)));
...
HcclResult RecvWrite(const DataInfo &recvInfo, const ThreadHandle &thread)
{
    ...
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyWaitOnThread(thread, recvChannel.handle, NOTIFY_IDX_DATA_SIGNAL, customTimeout)));
    return HCCL_SUCCESS;
}
...
HcclResult SendRead(const DataInfo &sendInfo, const ThreadHandle &thread)
{
    ...
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyWaitOnThread(thread, sendChannel.handle, NOTIFY_IDX_DATA_SIGNAL, customTimeout)));
    return HCCL_SUCCESS;
}
...
HcclResult RecvRead(const DataInfo &recvInfo, const ThreadHandle &thread)
{
    ...
    u32 customTimeout = GetExternalInputHcclCustomTimeout();
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyWaitOnThread(thread, recvChannel.handle, NOTIFY_IDX_ACK, customTimeout)));
    for (int i = 0; i < repeatNum; i++) {
        const DataSlice srcSlice = srcSlices[i];
        const DataSlice dstSlice = dstSlices[i];
        if (srcSlice.size_ == 0) {
    ...
    const std::vector<u32> &notifyIdxMainToSub)
{
    ...
    for (u32 tidx = 0; tidx < subThreads.size(); tidx++) {
        CHK_RET(static_cast<HcclResult>(
            HcommThreadNotifyWaitOnThread(subThreads[tidx], notifyIdxMainToSub[tidx], customTimeout)));
    }
    ...
    const std::vector<u32> &notifyIdxSubToMain)
{
    ...
    for (u32 tidx = 0; tidx < subThreads.size(); tidx++) {
        CHK_RET(static_cast<HcclResult>(
            HcommThreadNotifyWaitOnThread(mainThread, notifyIdxSubToMain[tidx], customTimeout)));
    }
