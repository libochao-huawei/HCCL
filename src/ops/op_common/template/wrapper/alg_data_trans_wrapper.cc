    ...
    const char *funcName, const char *transType, ProcessSliceFunc processSlice, FallbackFunc fallback)
{
    ...
    if (transferDescs.size() > 0) {
        TraceBatchSummary(funcName, transType, repeatNum, transferDescs.size(), sendChannel);
        CHK_RET(static_cast<HcclResult>(
    ...
    const char *funcName, const char *transType, ProcessSliceFunc processSlice, FallbackFunc fallback)
{
    ...
    if (transferDescs.size() > 0) {
        TraceBatchSummary(funcName, transType, repeatNum, transferDescs.size(), recvChannel);
        CHK_RET(static_cast<HcclResult>(
