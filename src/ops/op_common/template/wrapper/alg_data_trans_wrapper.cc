    ...
    HcclDataType dataType, HcclReduceOp reduceOp)
{
    HcommBatchTransferDesc desc = {};
    desc.transType = transType;
    desc.reduce.count = count;
    desc.reduce.dst = dst;
    desc.reduce.src = src;
    desc.reduce.dataType = static_cast<HcommDataType>(dataType);
    desc.reduce.reduceOp = static_cast<HcommReduceOp>(reduceOp);
    return desc;
}
}  // namespace
...
HcclResult SendRecvBatchWriteReduce(const SendRecvReduceInfo &sendRecvInfo, const ThreadHandle &thread)
{
    ...
    for (int i = 0; i < repeatNum; i++) {
        ...
        transferDescs.push_back(MakeBatchReduceDesc(HCOMM_TRANSFER_TYPE_REDUCE_WRITE, dst, src, srcSlice.count_,
...
HcclResult RecvBatchReadReduce(const DataReduceInfo &recvInfo, const ThreadHandle &thread)
{
    ...
    for (int i = 0; i < repeatNum; i++) {
        ...
        transferDescs.push_back(MakeBatchReduceDesc(HCOMM_TRANSFER_TYPE_REDUCE_READ, dst, src, srcSlice.count_,
