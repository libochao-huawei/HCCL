...
HcclResult GetTopoTypeByLink(HcclComm comm, uint32_t netLayer, CommLink &link, CommTopo &topoType)
{
    ...
    for (uint32_t topoInstIdx = 0; topoInstIdx < listSize; topoInstIdx++) { // 遍历topoInst
        ...
        if (endPointNum == 0) {
            HCCL_WARNING("[%s]EndpointNum is 0 on TopoInst[%u].", __func__, topoInstId);
            continue;
        }

        size_t allocSize = static_cast<size_t>(endPointNum) * sizeof(EndpointDesc);
        if (allocSize / sizeof(EndpointDesc) != endPointNum) {
            HCCL_ERROR("[%s]Integer overflow in allocation size calculation", __func__);
            return HCCL_E_INTERNAL;
        }

        EndpointDesc *endPointDescs = (EndpointDesc*)malloc(allocSize);
        if (endPointDescs == nullptr) {
            HCCL_ERROR("[%s]Failed to allocate memory for endpoint descriptors", __func__);
            return HCCL_E_INTERNAL;
        }
