...
HcclResult ProcessLinkForProtocol(HcclComm comm, const std::vector<CommProtocol>& expectedProtocols,
    const std::vector<CommLink>& linkList, u32 myRank, u32 rank, uint32_t netLayer,
        ...
        for (u32 idx = 0; idx < linkList.size(); idx++) {
            if (linkList[idx].linkAttr.linkProtocol == expectedProtocol) {
                CHK_RET(CreateChannelFromLink(comm, myRank, rank, netLayer, idx, linkList[idx], 
                    std::string("[CalcChannelRequestMesh1D]"), channels));
                protocolFound = true;
                break;
            }
        }
        ...
        for (u32 idx = 0; idx < linkList.size(); idx++) {
            if (linkList[idx].linkAttr.linkProtocol == expectedProtocol) {
                CHK_RET(CreateChannelFromLink(comm, myRank, remoteRank, netLayer, idx, linkList[idx], 
                    std::string("[CalcLevel1ChannelRequestNhr]"), channels));
                protocolFound = true;
                break;
            }
        }
