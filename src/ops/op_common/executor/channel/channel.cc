...
bool IsCommFaltten(HcclComm comm, const std::vector<std::vector<u32>>& subcommInfo, std::vector<uint32_t> netLayersVector, u32 myRank) {
    ...
    for (auto netLayer : netLayersVector) {
        ...
        for(u32 rank: subcommInfo[COMM_LEVEL0]){
            ...
            if (listSize == 0){
                isStainPath = false;
                HCCL_INFO("isflatten = %lld, isStainPath = %lld, myRank = %lld, rank = %lld, netLayer = %lld, listSize = %lld",isflatten,isStainPath,myRank,rank,netLayer,listSize);
                break;
            }
        }
        if(netLayer != 0 && isStainPath) {
            isflatten =true;
        }
        
    }
    HCCL_INFO("isflatten = %lld",isflatten);
    return isflatten;
}
...
HcclResult ProcessFlattenLink(HcclComm comm, u32 myRank, const std::vector<std::vector<u32>>& subcommInfo, std::vector<HcclChannelDesc> &channels)
{
    ...
    CHK_RET(ProcessMeshInfo(comm, subcommInfo, rank2ChannelIdx, myRank, enableDieNum, enableDieId, channelsPerDie, rankIdToChannelDesc));
