...
HcclResult CheckHostDPUOnly(const HcclComm comm, const TopoInfoWithNetLayerDetails* topoInfo, bool &hostDPUOnly)
{
    ...
    u32 rankSizeInNetLayer0 = std::accumulate(topoInfo->topoInstDetailsOfLayer[0].ranksInTopo.begin(),
        topoInfo->topoInstDetailsOfLayer[0].ranksInTopo.end(), 0, [](u32 rankSize, const auto &ranksInTopo) {
            return rankSize + ranksInTopo.size();
        });
    if (rankSizeInNetLayer0 == topoInfo->userRankSize) {
        HCCL_INFO("Not using hostdpu because netLayer0 rankSize equal to all rankSize");
        return HCCL_SUCCESS;
    }
