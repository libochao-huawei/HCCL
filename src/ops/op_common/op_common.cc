                                ...
                                AlgResourceCtxSerializable &resCtxHost)
{
    ...
    if (useCache && g_hcclCacheMap.find(cacheKey) != g_hcclCacheMap.end()) {
        ...
        for (auto& ins : *queue) {
            ...
            // Update aivCountTag (preserve sliceId from cached args)
            u32 oldSliceId = newArgs.aivCountTag & AIV_LOW_16_BITS;
            newArgs.aivCountTag = (static_cast<uint32_t>(param.aivCountTag) << AIV_TAG_MOVE_LEFT_BITS) | oldSliceId;
                      ...
                      std::unique_ptr<TopoInfoWithNetLayerDetails> &topoInfo, std::string &algName, const ResPackGraphMode &resPack)
{
    ...
    } else if (param.engine == COMM_ENGINE_CCU) {        if (isResourceReused) {
