...
HcclResult AicpuKernelLaunch(OpParam &param, ThreadHandle unfoldThread)
{
    ...
    // 通过Thread获取展开流stream
    HCCL_INFO("[AicpuKernelLaunch] unfoldThread [%llu]", unfoldThread);
    ...
    std::unique_ptr<AlgResourceCtxSerializable>& resCtxHost)
{
    ...
    if ((param.engine == COMM_ENGINE_AICPU_TS) || (param.engine == COMM_ENGINE_CPU)) {
        ...
        CHK_RET(SaveMainThreadInfo(comm, param, thread, resRequest.notifyNumOnMainThread + 1));
        // 申请展开流对应的Thread
...
HcclResult SaveUnfoldThreadInfo(HcclComm comm, const OpParam &param, ThreadHandle unfoldThread)
{
    ...
    int ret = snprintf(unfoldAlgTag, sizeof(unfoldAlgTag), "%s_unfold", param.algTag);
