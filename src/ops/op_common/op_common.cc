...
HcclResult SetOpParamAlgTag(OpParam &param, const std::string &algName)
{
    ...
    int len = snprintf_s(param.algTag, sizeof(param.algTag), "%s_%s_%s", param.tag, temp.c_str(), launchMode);
    ...
    if (param.engine == CommEngine::COMM_ENGINE_CCU) { 
        try{
            ...
            int len_ccu = snprintf_s(param.algTag + len, sizeof(param.algTag) - len, "%s", ccuExtraTag.c_str());
