...
HcclResult CaptureSlaveStreams(aclrtStream mainStream, const std::vector<ThreadHandle> &threads)
{
    ...
    HCCL_INFO("[%s]success, captured streams to rtmodel:[%u], streamNum:[%u]", __func__, rtModel, threads.size());
    return HCCL_SUCCESS;
}
    ...
    HcclComm comm, const OpParam &param, AlgResourceRequest &resRequest, AlgResourceCtxSerializable* resCtxHost)
{
    ...
    CHK_RET(hrtMemSyncCopy(resCtxHost->aivCommInfoPtr, MAX_RANK_SIZE * sizeof(void*), buffersIn, MAX_RANK_SIZE * sizeof(void*),
        HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_HOST_TO_DEVICE));
    CHK_RET(hrtMemSyncCopy(static_cast<u8*>(resCtxHost->aivCommInfoPtr) + AIV_TAG_ADDR_OFFSET, MAX_RANK_SIZE * sizeof(void*),
        buffersOut, MAX_RANK_SIZE * sizeof(void*), HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_HOST_TO_DEVICE));
...
HcclResult SingleRankProc(const OpParam &param)
{
    ...
    if (len > 0) {
        HcclResult ret = hrtMemSyncCopy(param.outputPtr, len, param.inputPtr, len, HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_DEVICE_TO_DEVICE);
        HCCL_DEBUG("Call hrtMemSyncCopy, return value[%d], para: dstAddr[%p], destMax[%llu], "
                "srcAddr[%p], count[%llu], rtKind[%d]", ret, param.outputPtr, len, param.inputPtr,
                len, HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_DEVICE_TO_DEVICE);
        if (ret != HCCL_SUCCESS) {
