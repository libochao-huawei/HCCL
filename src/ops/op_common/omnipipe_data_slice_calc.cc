...
std::vector<OmniPipeSplitSliceInfo> OmniPipeSplitSliceInfoListAssign(std::vector<u64> dataWholeSize, u64 rankSize,
                                                                     u64 dataTypeSize)
{
...
// 2DAG偏移计算,y轴快
// 参数,xAGOffect x轴偏移，yAGOffect y轴偏移，stepNum步数，xRankSize x轴大小，yRankSize y轴大小，xAGDataSize
...
void CalAllgather2DOffset(u64 *xAGOffect, u64 *yAGOffect, u64 stepNum, u64 xRankSize, u64 yRankSize, u64 *xAGDataSize,
                          u64 *yAGDataSize)
{
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgather2DOffset] start");
    ...
    for (int i = 0; i < stepNum; i++) {
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgather2DOffset] xAGOffect[%d]=[%u],yAGOffect[%d]=[%u]", i,
                  xAGOffect[i], i, yAGOffect[i]);
    }
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgather2DOffset] end");
}
...
{
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalReducescatter2DOffset] start");
    ...
    if (stepNum > finStepMark) {
        ...
        // 最后拆成两步了单独处理下
        yRSOffect[stepNum - finStepMark] = 0;
        yRSOffect[stepNum - 1] = yRSOffect[stepNum - finStepMark] + yRSDataSize[stepNum - finStepMark];
        xRSOffect[stepNum - 1] = 0;
        // 单独处理大于3步的情况
        if (stepNum > finStepMark + 1) {
            xRSOffect[stepNum - 1] = xRSOffect[stepNum - (finStepMark + 1)] + xRSDataSize[stepNum - (finStepMark + 1)];
        }
        xRSOffect[stepNum - finStepMark] = xRSOffect[stepNum - 1] + xRSDataSize[stepNum - 1];
        for (int i = 0; i < stepNum; i++) {
            HCCL_INFO("[omnipipe_data_slice_calc.cc][CalReducescatter2DOffset] xRSOffect[%d]=[%u],yRSOffect[%d]=[%u]",
                      i, xRSOffect[i], i, yRSOffect[i]);
        }
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalReducescatter2DOffset] end");
    }
}
...
{
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSizeRatio2D] start");
    u64 step = 1;
    if (yRankSize == 1) {
        ...
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSizeRatio2D] "
                  "bandwidthRatio=[%f],omniPipeRatio=[%f],scale=[%f],step=[%u]",
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSizeRatio2D] step=[%u]", step);
    for (int i = 0; i < step; i++) {
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSizeRatio2D] "
                  "xStepP2pDataSize[%d]=[%f],yStepP2pDataSize[%d]=[%f],",
                  i, xStepP2pDataSize[i], i, yStepP2pDataSize[i]);
    }
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSizeRatio2D] end");
    return step;
}
...
{
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSize2D] start");
    u64 step = 1;
    ...
    if (yRankSize == 1) {
        ...
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSize2D] "
                  "bandwidthRatio=[%f],omniPipeRatio=[%f],scale=[%f],step=[%u]",
                  bandwidthRatio, omniPipeRatio, scale, step);
        // 1. 计算第一步的通信数据
        // step为2单独处理一下
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSize2D] step=[%u]", step);
    for (int i = 0; i < step; i++) {
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSize2D] "
                  "xStepP2pDataSize[%d]=[%u],yStepP2pDataSize[%d]=[%u],",
                  i, xStepP2pDataSize[i], i, yStepP2pDataSize[i]);
    }
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalAllgatherDataSize2D] end");
    return step;
}
...
u64 CalReducescatterDataSize2D(u64 *xStepP2pDataSize, u64 *yStepP2pDataSize, double xB, double yB, u64 xRankSize,
                               u64 yRankSize, u64 dataSizeEachRank, u64 maxStep)
{
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalReducescatterDataSize2D] start");
    u64 step = 1;
    u64 justifyLen = HCCL_MIN_SLICE_ALIGN;
    if (yRankSize == 1) {
        xStepP2pDataSize[0] = dataSizeEachRank;
        ...
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalReducescatterDataSize2D] "
                  "bandwidthRatio=[%f],omniPipeRatio=[%f],scale=[%f],step=[%u]",
                  bandwidthRatio, omniPipeRatio, scale, step);
        // 1. 计算第一步的通信数据
        if (scale > 1) {
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalReducescatterDataSize2D] step=[%u]", step);
    for (int i = 0; i < step; i++) {
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalReducescatterDataSize2D] "
                  "xStepP2pDataSize[%d]=[%u],yStepP2pDataSize[%d]=[%u],",
                  i, xStepP2pDataSize[i], i, yStepP2pDataSize[i]);
    }
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalReducescatterDataSize2D] end");
    return step;
}
...
{
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcBandwidth2D] start");
    if (yRankSize == 1) {
        return xB;
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcBandwidth2D] xB=[%f]", xB);
    } else if (xRankSize == 1) {
        return yB;
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcBandwidth2D] yB=[%f]", yB);
    } else {
        ...
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcBandwidth2D] Bandwidth2D=[%f]", xB * ds / xds);
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcBandwidth2D] end");
        return xB * ds / xds;
    }
}
...
{
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] start");
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] "
              "justifyLen=[%u],transportBoundDataSize=[%u],maxStepNum=[%u],",
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] xRankSize=[%u],yRankSize=[%u],zRankSize=[%u],",
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] "
              "dataSize=[%u],dataTypeSize=[%u],maxTmpMemSize=[%u],opMode=[%u],engine=[%u],levelAlgType.size()=[%u]",
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] xB=[%f],yB=[%f],zB=[%f],xyB=[%f]", xB, yB, zB,
              xyB);
    ...
    if (zB > xyB) {
        outerStepNum = CalReducescatterDataSize2D(xyRSDataSize, zRSDataSize, xyB, zB, xRankSize * yRankSize, zRankSize,
                                                  dataSize, maxStepNum);
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] zB>xyB,outerStepNum=[%u]", outerStepNum);
    } else {
    ...
    for (u64 i = 0; i < outerStepNum; i++) {
        ...
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] innerStepNum=[%u]", innerStepNum);
    }
    ...
    if (zB > xyB) {
        ...
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] zB>xyB,scratchSize=[%u]", scratchSize);
    } else {
        if (outerStepNum > finStepMark) {
            zConnerStep = 1;
        }
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] allCclBufferSize=[%u],bufferRatio=[%f],",
    ...
    // 校验
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] start check");
    while ((allCclBufferSize != 0) && (allCclBufferSize > maxTmpMemSize)) {
        ...
        allCclBufferSize = allCclBufferSize + scratchSize[OmniPipeLevel::OMNIPIPE_LEVEL0] +
                           scratchSize[OmniPipeLevel::OMNIPIPE_LEVEL1] + scratchSize[OmniPipeLevel::OMNIPIPE_LEVEL2];
        HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] allCclBufferSize=[%u],", allCclBufferSize);
        if (allCclBufferSize > maxTmpMemSize) {
            maxDataSizePerLoop = maxDataSizePerLoop * LOOP_SCALING_FACTOR;  // 大了就小一点
            maxDataSizePerLoop = maxDataSizePerLoop / justifyLen * justifyLen;
        }
    }
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] end check");
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] maxDataCountPerLoop=[%u],loopTimes=[%u],",
              maxDataCountPerLoop, loopTimes);
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalcOmniPipeScratchInfo] end");
    return scratchInfo;
}
...
{
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalScratchSize] start");
    // 返回3个值，xBuffer,yBuffer,zBuffer,大小
    std::vector<u64> scratchSize = {0, 0, 0};
    u64 xRankSize = levelRankSize[OmniPipeLevel::OMNIPIPE_LEVEL0];  // x轴卡数
    u64 yRankSize = levelRankSize[OmniPipeLevel::OMNIPIPE_LEVEL1];  // y轴卡数
    u64 zRankSize = levelRankSize[OmniPipeLevel::OMNIPIPE_LEVEL2];  // z轴卡数
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalScratchSize] xRankSize=[%u],yRankSize=[%u],zRankSize=[%u],", xRankSize,
              yRankSize, zRankSize);
    ...
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalScratchSize] xTopo=[%u],yTopo=[%u],zTopo=[%u],", xTopo, yTopo, zTopo);
    ...
    HCCL_INFO(
        "[omnipipe_data_slice_calc.cc][CalScratchSize] scratchSize[0]=[%u],scratchSize[1]=[%u],scratchSize[2]=[%u]",
        scratchSize[OmniPipeLevel::OMNIPIPE_LEVEL0], scratchSize[OmniPipeLevel::OMNIPIPE_LEVEL1], scratchSize[OmniPipeLevel::OMNIPIPE_LEVEL2]);
    HCCL_INFO("[omnipipe_data_slice_calc.cc][CalScratchSize] end");
    return scratchSize;
}
                                                ...
                                                u64 innerStepNum, u64 maxStepNum)
{
    ...
    u64 yRankSize = levelRankSize[OmniPipeLevel::OMNIPIPE_LEVEL1];  // y轴卡数
    u64 zRankSize = levelRankSize[OmniPipeLevel::OMNIPIPE_LEVEL2];  // z轴卡数
    HCCL_INFO("[CalRSDataSizeStep] xRankSize=[%u],yRankSize=[%u],zRankSize=[%u],",
              xRankSize, yRankSize, zRankSize);
    ...
    HCCL_INFO("[CalRSDataSizeStep] xInCornerStep=[%u],yInCornerStep=[%u],cornerStep=[%u],",
    ...
    for (int i = 0; i < outerStepNum; i++) {
        HCCL_INFO("[CalRSDataSizeStep] rsStepDataSize[2][%d]=[%u],", i,
                  rsStepDataSize[OmniPipeLevel::OMNIPIPE_LEVEL2][i]);
    }
    for (int i = 0; i < outerStepNum * innerStepNum; i++) {
        HCCL_INFO("[CalRSDataSizeStep] rsStepDataSize[0][%d]=[%u],", i,
                  rsStepDataSize[OmniPipeLevel::OMNIPIPE_LEVEL0][i]);
    }
    for (int i = 0; i < outerStepNum * innerStepNum; i++) {
        HCCL_INFO("[CalRSDataSizeStep] rsStepDataSize[1][%d]=[%u],", i,
...
OmniPipeSliceInfo CalcAGOmniPipeSliceInfo(OmniPipeSliceParam &omniPipeSliceParam)
{
    ...
    HCCL_INFO("[CalcAGOmniPipeSliceInfo] xRankSize=[%u],yRankSize=[%u],zRankSize=[%u],",
              xRankSize, yRankSize, zRankSize);
    ...
    HCCL_INFO("[CalcAGOmniPipeSliceInfo] xAxis=[%u],yAxis=[%u],zAxis=[%u],rankid=[%u]",
    ...
    u64 zAGOffect[rankSize][maxStepNum];  // z轴偏移
    u64 xAGOffect[rankSize][maxStepNum][maxStepNum];  // x轴偏移
    u64 yAGOffect[rankSize][maxStepNum][maxStepNum];  // y轴偏移
    u64 xyAGOffect[rankSize][maxStepNum];  // xy整体偏移
    ...
    if (xyB > zB) {
        ...
        for (int rs = 0; rs < rankSize; rs++) {
            ...
            // 计算2d数据片的偏移，下面变成3d时用
            CalAllgather2DOffset(zAGOffect[rs], xyAGOffect[rs], outerStepNum, zRankSize, xRankSize * yRankSize,
        ...
        for (int rs = 0; rs < rankSize; rs++) {
            ...
            // 计算2d数据片的偏移，下面变成3d时用
            CalAllgather2DOffset(xyAGOffect[rs], zAGOffect[rs], outerStepNum, xRankSize * yRankSize, zRankSize,
    ...
    for (u64 osn = 0; osn < zConnerStep; osn++) {
        ...
        for (int oneDid = 0; oneDid < zRankSize; oneDid++) {
            ...
            u64 inputPieceIdOffect = zAGOffect[pieceId][osn];
            sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
            sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
            inputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
            outputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
        ...
        for (int oneDid = 0; oneDid < zRankSize; oneDid++) {
            ...
            for (u64 connerDataSlice = 0; connerDataSlice < xRankSize * yRankSize; connerDataSlice++) {
                ...
                if (connerDataSlice != yAxis * xRankSize + xAxis) {
                    ...
                    u64 sliceSizeOnePiece = zAGDataSize[pieceId][osn];
                    u64 inputPieceIdOffect = zAGOffect[pieceId][osn] + omniPipeSplitSliceInfoListTotal[pieceId].offset;
                    sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
                    sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
                    inputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
                    outputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
    ...
    for (int rs = 0; rs < rankSize; rs++) {
        for (u64 osn = 0; osn < outerStepNum; osn++) {
            CalAllgather2DOffset(xAGOffect[rs][osn], yAGOffect[rs][osn], innerStepNum, xRankSize, yRankSize,
    ...
    for (u64 osn = 0; osn < xyConnerStep; osn++) {
        ...
        for (u64 isn = 0; isn < xInCornerStep; isn++) {
            ...
            for (int oneDid = 0; oneDid < xRankSize; oneDid++) {
                ...
                u64 inputPieceIdOffect = xyAGOffect[pieceId][osn] + xAGOffect[pieceId][osn][isn];
                sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
                sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
                inputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
                outputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
    ...
    for (u64 osn = 0; osn < xyConnerStep; osn++) {
        ...
        for (u64 isn = 0; isn < yInCornerStep; isn++) {
            ...
            for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                ...
                u64 inputPieceIdOffect = xyAGOffect[pieceId][osn] + yAGOffect[pieceId][osn][isn];
                sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
                sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
                inputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
                outputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
...
u64 DataSliceCut(u64 originSliceSize, u64 originSliceOffset, u64 stopOffect)
{
    ...
    if (originSliceSize + originSliceOffset > stopOffect) {
        if (originSliceOffset < stopOffect) {
            cuttedSliceSize = stopOffect - originSliceOffset;
        } else {
            cuttedSliceSize = 0;
        }
    }
...
{
    if (originOffset > stopOffect) {
        originOffset = stopOffect;
    }
    return originOffset;
}
...
OmniPipeSliceInfo CalcRSOmniPipeSliceInfo(OmniPipeSliceParam &omniPipeSliceParam)
{
    ...
    u64 outerStepNum;  // 机内机间步数
    u64 innerStepNum;  // 机内两轴步数
    u64 zRSOffect[rankSize][maxStepNum];  // z轴偏移
    u64 xRSOffect[rankSize][maxStepNum][maxStepNum];  // x轴偏移
    u64 yRSOffect[rankSize][maxStepNum][maxStepNum];  // y轴偏移
    u64 xyRSOffect[rankSize][maxStepNum];  // xy整体偏移
    ...
    if (xyB > zB) {
        ...
        for (int rs = 0; rs < rankSize; rs++) {
            ...
            CalReducescatter2DOffset(zRSOffect[rs], xyRSOffect[rs], outerStepNum, zRankSize, xRankSize * yRankSize,
        ...
        for (int rs = 0; rs < rankSize; rs++) {
            ...
            CalReducescatter2DOffset(xyRSOffect[rs], zRSOffect[rs], outerStepNum, xRankSize * yRankSize, zRankSize,
    ...
    if (omniPipeSliceParam.opMode == OpMode::OPBASE && omniPipeSliceParam.engine == CommEngine::COMM_ENGINE_AICPU_TS) {
        ...
        HCCL_INFO("xCclBufferBaseOff=%u, dataSizePerLoop=%u, xRankSize=%u, yRankSize=%u, zRankSize=%u",
            ...
            for (u64 connerDataSlice = 0; connerDataSlice < xRankSize * yRankSize; connerDataSlice++) {
                ...
                if (connerDataSlice != yAxis * xRankSize + xAxis) {
                    ...
                    u64 sliceSizeOnePiece = DataSliceCut(zRSDataSize[maxDataPieceId][osn],
                                                         zRSOffect[maxDataPieceId][osn],
                                                         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                    u64 inputPieceIdOffect = sliceOffsetCut(zRSOffect[maxDataPieceId][osn],
                                                            omniPipeSplitSliceInfoListPerLoop[pieceId].size) +
                                             omniPipeSplitSliceInfoListTotal[pieceId].offset;
                    sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
                    sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
    ...
    for (u64 osn = zConnerStep; osn < outerStepNum; osn++) {
        ...
        for (int oneDid = 0; oneDid < zRankSize; oneDid++) {
            ...
            std::vector<u64> outputOmniPipeSliceStrideMultRankPiece;
            u64 pieceId = oneDid * xRankSize * yRankSize + yAxis * xRankSize + xAxis;
            u64 sliceSizeOnePiece = DataSliceCut(zRSDataSize[maxDataPieceId][osn], zRSOffect[maxDataPieceId][osn],
                                                 omniPipeSplitSliceInfoListPerLoop[pieceId].size);
            u64 inputPieceIdOffect =
                sliceOffsetCut(zRSOffect[maxDataPieceId][osn], omniPipeSplitSliceInfoListPerLoop[pieceId].size) +
                omniPipeSplitSliceInfoListTotal[pieceId].offset;
            sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
            sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
            inputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffect);
    ...
    for (int rs = 0; rs < rankSize; rs++) {
        for (u64 osn = 0; osn < outerStepNum; osn++) {
            CalReducescatter2DOffset(xRSOffect[rs][osn], yRSOffect[rs][osn], innerStepNum, xRankSize, yRankSize,
                            ...
                            if (connerDataSlice != yAxis && yRankSize > 1) {
                                ...
                                u64 sliceSizeOnePiece =
                                    DataSliceCut(xRSDataSize[maxDataPieceId][osn][isn],
                                                 xyRSOffect[maxDataPieceId][osn] + xRSOffect[maxDataPieceId][osn][isn],
                                                 omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                                u64 inputPieceIdOffect =
                                    sliceOffsetCut(
                                        xyRSOffect[maxDataPieceId][osn] + xRSOffect[maxDataPieceId][osn][isn],
                    ...
                    if (outSliceNum != zAxis) {
                        ...
                        u64 sliceSizeOnePiece =
                            DataSliceCut(xRSDataSize[maxDataPieceId][osn][isn],
                                         ...
                                         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                        u64 inputPieceIdOffect =
                            sliceOffsetCut(xyRSOffect[maxDataPieceId][osn] + xRSOffect[maxDataPieceId][osn][isn],
                                           omniPipeSplitSliceInfoListPerLoop[pieceId].size) +
                            omniPipeSplitSliceInfoListTotal[pieceId].offset;
                        sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
                        sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
                            ...
                            if (connerDataSlice != xAxis || xRankSize == 1) {
                                ...
                                u64 sliceSizeOnePiece =
                                    DataSliceCut(yRSDataSize[maxDataPieceId][osn][isn],
                                                 xyRSOffect[maxDataPieceId][osn] + yRSOffect[maxDataPieceId][osn][isn],
                                                 omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                                u64 inputPieceIdOffect =
                                    sliceOffsetCut(
                                        xyRSOffect[maxDataPieceId][osn] + yRSOffect[maxDataPieceId][osn][isn],
                    ...
                    if (outSliceNum != zAxis) {
                        ...
                        u64 sliceSizeOnePiece =
                            DataSliceCut(yRSDataSize[maxDataPieceId][osn][isn],
                                         ...
                                         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                        u64 inputPieceIdOffect =
                            sliceOffsetCut(xyRSOffect[maxDataPieceId][osn] + yRSOffect[maxDataPieceId][osn][isn],
                                           omniPipeSplitSliceInfoListPerLoop[pieceId].size) +
                            omniPipeSplitSliceInfoListTotal[pieceId].offset;
                        sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
                        sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
