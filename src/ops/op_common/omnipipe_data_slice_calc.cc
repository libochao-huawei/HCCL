...
OmniPipeSliceInfo CalcScatterOmniPipeSliceInfo(OmniPipeSliceParam &omniPipeSliceParam, uint32_t root)
{
    ...
    if (xyB > zB) {
        ...
        xyzDataSizeStep = CalRSDataSizeStep((u64 *)xSDataSize[maxDataPieceId], (u64 *)ySDataSize[maxDataPieceId],
    ...
    // 辅助函数：处理y轴斜对角通信（内层）
    auto processYDiagonalInnerComm = [&](StepSliceInfo& stepSliceInfotmp, u64 osn, u64 isn) {
        if ((isRoot && osn >= 0) || (isSameAxisAsRoot && osn > 0)) {
            for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                std::vector<u64> sliceSizeMultRankPiece;
                std::vector<u64> sliceCountMultRankPiece;
                ...
                for (u64 outSliceNum = 0; outSliceNum < zRankSize; outSliceNum++) {
                    ...
                    for (u64 connerDataSlice = 0; connerDataSlice < xRankSize; connerDataSlice++) {
                        u64 currentInnerStepDataSliceId = outSliceNum * xRankSize * yRankSize + oneDid * xRankSize + connerDataSlice;
                        u64 pieceId = currentInnerStepDataSliceId;
    ...
    // 辅助函数：处理y轴同轴通信（内层）
    auto processYCoaxialInnerComm = [&](StepSliceInfo& stepSliceInfotmp, u64 osn, u64 isn) {
        if ((isRoot && osn >= 0) || (isSameAxisAsRoot && osn > 0)) {
            for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                std::vector<u64> sliceSizeMultRankPiece;
                std::vector<u64> sliceCountMultRankPiece;
                ...
                for (u64 outSliceNum = 0; outSliceNum < zRankSize; outSliceNum++) {
                    ...
                    u64 sliceSizeOnePiece = DataSliceCut(ySDataSize[maxDataPieceId][osn][isn],
                                                         xySOffset[maxDataPieceId][osn] + ySOffset[maxDataPieceId][osn][isn],
                                                         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                    u64 inputPieceIdOffset = sliceOffsetCut(xySOffset[maxDataPieceId][osn] + ySOffset[maxDataPieceId][osn][isn],
                                                           omniPipeSplitSliceInfoListPerLoop[pieceId].size) +
                                             omniPipeSplitSliceInfoListTotal[pieceId].offset;
                    sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
                    sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
    ...
    // 辅助函数：处理y轴斜对角通信（外层）
    auto processYDiagonalOuterComm = [&](StepSliceInfo& stepSliceInfotmp, u64 osn, u64 isn) {
        if ((isRoot && osn >= 0) || (isSameAxisAsRoot && osn > 0)) {
            for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                std::vector<u64> sliceSizeMultRankPiece;
                std::vector<u64> sliceCountMultRankPiece;
    ...
    // 辅助函数：处理y轴同轴通信（外层）
    auto processYCoaxialOuterComm = [&](StepSliceInfo& stepSliceInfotmp, u64 osn, u64 isn) {
        if ((isRoot && osn >= 0) || (isSameAxisAsRoot && osn > 0)) {
            for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                std::vector<u64> sliceSizeMultRankPiece;
                std::vector<u64> sliceCountMultRankPiece;
    ...
    for (u64 osn = 0; osn < xyConnerStep; osn++) {
        for (u64 isn = 0; isn < yInCornerStep; isn++) {
            auto stepSliceInfotmp = createStepSliceInfo(yCclBufferBaseOff);
            processYDiagonalInnerComm(stepSliceInfotmp, osn, isn);
            dataSliceLevely.insert(dataSliceLevely.end(), stepSliceInfotmp);
        }
        for (u64 isn = yInCornerStep; isn < innerStepNum; isn++) {
            auto stepSliceInfotmp = createStepSliceInfo(yCclBufferBaseOff);
            processYCoaxialInnerComm(stepSliceInfotmp, osn, isn);
            dataSliceLevely.insert(dataSliceLevely.end(), stepSliceInfotmp);
        }
    }
    for (u64 osn = xyConnerStep; osn < outerStepNum; osn++) {
        for (u64 isn = 0; isn < yInCornerStep; isn++) {
            auto stepSliceInfotmp = createStepSliceInfo(yCclBufferBaseOff);
            processYDiagonalOuterComm(stepSliceInfotmp, osn, isn);
            dataSliceLevely.insert(dataSliceLevely.end(), stepSliceInfotmp);
        }
        for (u64 isn = yInCornerStep; isn < innerStepNum; isn++) {
            auto stepSliceInfotmp = createStepSliceInfo(yCclBufferBaseOff);
            processYCoaxialOuterComm(stepSliceInfotmp, osn, isn);
            dataSliceLevely.insert(dataSliceLevely.end(), stepSliceInfotmp);
        }
    }
