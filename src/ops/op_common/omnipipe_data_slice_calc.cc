...
OmniPipeSliceInfo CalcScatterOmniPipeSliceInfo(OmniPipeSliceParam &omniPipeSliceParam, uint32_t root)
{
    u64 processedDataEachRank = 0;  // 预留偏移参数，现在填0
    // 公共拓扑参数
    int maxStepNum = MAX_STEP_NUM + 1;  // scatter多拆了一步出来所以是+1
    std::vector<u64> levelRankSize = omniPipeSliceParam.levelRankSize;
    std::vector<u64> dataSize = omniPipeSliceParam.dataWholeSize;
    std::vector<u64> dataSizePerLoop = omniPipeSliceParam.dataSizePerLoop;
    u64 maxDataPieceId = 0;
    ...
    bool isDiagonalToRoot;
    
    // 节点类型判断
    bool isRoot = (rankid == root);
    if (zRankSize > 1) {
        isSameAxisAsRoot = (xAxis == rootx || yAxis == rooty || zAxis == rootz) && !isRoot;
        isDiagonalToRoot = (xAxis != rootx && yAxis != rooty && zAxis != rootz) && !isRoot;
    } else {
        isSameAxisAsRoot = (xAxis == rootx || yAxis == rooty) && !isRoot;
        isDiagonalToRoot = (xAxis != rootx && yAxis != rooty) && !isRoot;
    }
    ...
    if (xyB > zB) {
        ...
        for (int rs = 0; rs < rankSize; rs++) {
            // 判断当前节点是否为root或同轴线节点
            bool ifroot = (rs == root);
            u64 current_x = rs % xRankSize;
            u64 current_y = (rs / xRankSize) % yRankSize;
            u64 current_z = rs / (xRankSize * yRankSize);
            bool isSameAxis;
            
            if (zRankSize == 1) {
                // 二维拓扑：只判断x和y坐标
                isSameAxis = (current_x == rootx || current_y == rooty) && !ifroot;
            } else {
                // 三维拓扑：判断x、y和z坐标
                isSameAxis = (current_x == rootx || current_y == rooty || current_z == rootz) && !ifroot;
            }
        ...
        // scatter不需要归约，所以scratch大小计算可以简化
        scratchSizexyz = {0, 0, 0}; // scatter不需要scratch内存
    ...
    for (u64 osn = 0; osn < zConnerStep; osn++) {
        ...
        if ((isRoot && osn >= 0) || (isSameAxisAsRoot && osn > 0)) {
            for (int oneDid = 0; oneDid < zRankSize; oneDid++) {
                u64 outputslicestride = 0;
                std::vector<u64> sliceSizeMultRankPiece;
                std::vector<u64> sliceCountMultRankPiece;
