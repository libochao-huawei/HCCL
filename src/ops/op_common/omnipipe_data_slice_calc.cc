        ...
        for (int rs = 0; rs < rankSize; rs++) {
            ...
            if (ifroot || isSameAxis) {
                ...
                if (yB >= xB) {
                    ...
                    for (u64 i = 0; i < outerStepNum; i++) {
                        innerStepNum = CalScatterDataSize2D(ySDataSize[rs][i], xSDataSize[rs][i], yB, xB, yRankSize,
                            xRankSize, xySDataSize[rs][i], maxStepNum);
                        CalScatter2DOffset(ySOffset[rs][i], xSOffset[rs][i], innerStepNum, yRankSize, xRankSize,
                            ySDataSize[rs][i], xSDataSize[rs][i]);
                    }
                    // 这里判断下，步数为1的时候只进下面的循环，否则这里走一步
                    if (innerStepNum > finStepMark) {
                        yInCornerStep = 1;                          // 步数为1的时候只走一步，否则走innerStepNum-1步
