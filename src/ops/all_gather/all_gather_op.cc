                         ...
                         aclrtStream stream, uint64_t strideCount)
{
    ...
    // 执行AllGather
    CHK_RET_AND_PRINT_IDE(AllGatherOutPlace(sendBuf, recvBuf, sendCount, dataType, comm, stream, opTag, stirdeCount), opTag.c_str());
                                      ...
                                      aclrtStream stream, const std::string &tag, const ResPackGraphMode &resPack)
{
    ...
    CHK_RET(AllGatherOutPlaceCommon(sendBuf, recvBuf, sendCount, dataType, comm, stream, tag, OpMode::OFFLOAD, resPack, 0));
