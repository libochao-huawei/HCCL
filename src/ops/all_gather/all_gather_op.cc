...
static void SaveDeviceBufferToFile(const char *tag, void *deviceBuf, uint64_t elemCount,
    ...
    aclrtError ret = aclrtMemcpy(hostBuf, totalBytes, deviceBuf, totalBytes, ACL_MEMCPY_DEVICE_TO_HOST);
    if (ret != ACL_SUCCESS) {
    ...
    // 获取 rankId
    uint32_t rankId = 0;
    if (HcclCommGetRank(comm, &rankId) != HCCL_SUCCESS) {
        rankId = 0;
        HCCL_WARNING("Failed to get rankId, use 0 for %s", tag);
    }
