                                                    ...
                                                    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    ...
    for (u32 threadIdx = 0; threadIdx < subCommRanks_[0].size() - 1; threadIdx++) {
        ...
        for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
            ...
            u64 txOutOffset = tempAlgParams_.outputSliceStride * myRank_ + outBaseOff;
            u64 rxOutOffset = tempAlgParams_.outputSliceStride * connectedRank + outBaseOff;
            ...
            txSrcSlicesAll.emplace_back(txSrcPtr, txOutOffset, sliceSize, sliceCount); // 就是LocalDataCopy拷贝到的outputPtr地址。
            txDstSlicesAll.emplace_back(txDstPtr, txOutOffset, sliceSize, sliceCount);
            rxDstSlicesAll.emplace_back(rxDstPtr, rxOutOffset, sliceSize, sliceCount);
            rxSrcSlicesAll.emplace_back(rxSrcPtr, rxOutOffset, sliceSize, sliceCount);

            HCCL_DEBUG("[InsTempAllGatherSymmetryMemoryMesh1D][RunAllGatherMesh] rankId [%d] connectedRank [%d] rpt [%d] txSrcSlices: "
                        "offset[%d] sliceSize[%d] count[%d].",
                        myRank_, connectedRank, rpt, txOutOffset, sliceSize, sliceCount);

            HCCL_DEBUG("[InsTempAllGatherSymmetryMemoryMesh1D][RunAllGatherMesh] rankId [%d] connectedRank [%d] rpt [%d] txDstSlices: "
                        "offset[%d] sliceSize[%d] count[%d].",
                        myRank_, connectedRank, rpt, txDstOffset, sliceSize, sliceCount);

            HCCL_DEBUG("[InsTempAllGatherSymmetryMemoryMesh1D][RunAllGatherMesh] rankId [%d] connectedRank [%d] rpt [%d] rxSrcSlices: "
                        "offset[%d] sliceSize[%d] count[%d].",
                        myRank_, connectedRank, rpt, rxOutOffset, sliceSize, sliceCount);

            HCCL_DEBUG("[InsTempAllGatherSymmetryMemoryMesh1D][RunAllGatherMesh] rankId [%d] connectedRank [%d] rpt [%d] rxDrcSlices: "
                        "offset[%d] sliceSize[%d] count[%d].",
