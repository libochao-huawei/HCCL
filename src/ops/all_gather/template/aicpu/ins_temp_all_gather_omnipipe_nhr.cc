                                                        ...
                                                        const std::map<u32, std::vector<ChannelInfo>>& channels)
{
    ...
    for (u32 step = 0; step < nSteps; ++step) {
        ...
        if (tempAlgParams_.stepSliceInfo.isFirstStep) {
            CHK_PRT_RET(SendRecvWrite(sendRecvInfo, threads[0]),
                HCCL_ERROR("[InsTempAllGatherNHR] sendrecv failed (step=%u), send to [%u], recv from [%u]", step,
                    stepInfo.toRank, stepInfo.fromRank),
                HcclResult::HCCL_E_INTERNAL);
        } else {
            CHK_PRT_RET(SendRecvWriteWithoutPreSync(sendRecvInfo, threads[0]),
                HCCL_ERROR("[InsTempAllGatherNHR] sendrecv failed (step=%u), send to [%u], recv from [%u]", step,
                    stepInfo.toRank, stepInfo.fromRank),
                HcclResult::HCCL_E_INTERNAL);
        }
