                                                    ...
                                                    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    ...
    for (u32 threadIdx = 0; threadIdx < subCommRanks_[0].size() - 1; threadIdx++) {
        ...
        if (supportSymmetricMemory_) {
            HcclResult ret = HcclSymWinGetPeerPointer(inputSymWindow_, inputOffset_, connectedRank, &remoteInSymmtryPtr);
            CHK_PRT_RET(ret != HCCL_SUCCESS || remoteInSymmtryPtr == nullptr,
                        HCCL_ERROR("[InsTempAllGatherSymmetryMemoryMesh1D] HcclSymWinGetPeerPointer failed, remoteRank[%u] inputRet[%d] in[%p]",
                        connectedRank, ret, remoteInSymmtryPtr), HcclResult::HCCL_E_INTERNAL);

            ret = HcclSymWinGetPeerPointer(outputSymWindow_, outputOffset_, connectedRank, &remoteOutSymmtryPtr);
            CHK_PRT_RET(ret != HCCL_SUCCESS || remoteOutSymmtryPtr == nullptr,
                        HCCL_ERROR("[InsTempAllGatherSymmetryMemoryMesh1D] HcclSymWinGetPeerPointer failed, remoteRank[%u] outputRet[%d] out[%p]",
                        connectedRank, ret, remoteOutSymmtryPtr), HcclResult::HCCL_E_INTERNAL);
            HCCL_INFO("[InsTempAllGatherSymmetryMemoryMesh1D] HcclSymWinGetPeerPointer success, remoteRank[%u] in[%p] out[%p]", connectedRank, remoteInSymmtryPtr, remoteOutSymmtryPtr);
        }
        ...
        for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
            ...
            if (!supportSymmetricMemory_) {
                ...
                txSrcPtr = tempAlgParams_.buffInfo.inputPtr;
