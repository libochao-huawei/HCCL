...
HcclResult InsTempAllGatherMesh1D::GetRes(AlgResourceRequest &resourceRequest) const
{
    ...
    if (supportSymmetricMemory_ && level0RankSize > 1) {
        slaveThreadNum += 1;
    }
                                             ...
                                             TemplateResource &templateResource)
{
    ...
    inputSymWindow_ = param.inSymWinHandle;
    outputSymWindow_ = param.outSymWinHandle;
    inputOffset_ = param.inOffset;
    outputOffset_ = param.outOffset;
    supportSymmetricMemory_ = param.supportSymmetricMemory;
                                                    ...
                                                    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    ...
    for (u32 threadIdx = 0; threadIdx < subCommRanks_[0].size() - 1; threadIdx++) {
        ...
        if (supportSymmetricMemory_) {
            HcclResult inRet = HcclSymWinGetPeerPointer(inputSymWindow_, inputOffset_, connectedRank, &remoteInSymmtryPtr);
            HcclResult outRet = HcclSymWinGetPeerPointer(outputSymWindow_, outputOffset_, connectedRank, &remoteOutSymmtryPtr);
            if (inRet != HCCL_SUCCESS || outRet != HCCL_SUCCESS)
            {
                HCCL_ERROR("[InsTempAllGatherSymmetryMemoryMesh1D] HcclSymWinGetPeerPointer failed, remoteRank[%u] inRet[%d] outRet[%d]",
                            connectedRank, inRet, outRet);
                return HcclResult::HCCL_E_INTERNAL;
            }
            CHK_PRT_RET(remoteIn == nullptr || remoteOut == nullptr,
                        HCCL_ERROR("[InsTempAllGatherSymmetryMemoryMesh1D] HcclSymWinGetPeerPointer failed, remoteRank[%u] in[%p] "
                        "out[%p] is invalid", connectedRank, remoteInSymmtryPtr, remoteOutSymmtryPtr), HcclResult::HCCL_E_INTERNAL);
            HCCL_INFO("[HcclCommAicpu][PrepareSymmetricMemory] remoteRank[%u] in[%p] out[%p]", connectedRank, remoteInSymmtryPtr, remoteOutSymmtryPtr);
        }
        ...
        for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
            ...
            u64 rxOutOffset = 0;
            u64 rxSrcOffset = 0;
            void *txSrcPtr = supportSymmetricMemory_ ? tempAlgParams_.buffInfo.inputPtr : tempAlgParams_.buffInfo.outputPtr;
        ...
        CHK_PRT_RET(SendRecvRead(sendRecvInfo, supportSymmetricMemory_ ? threads[threadIdx + 1] : threads[threadIdx]),
