                                                    ...
                                                    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    ...
    for (u32 threadIdx = 0; threadIdx < subCommRanks_[0].size() - 1; threadIdx++) {
        ...
        for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
            ...
            txSrcSlices.push_back(DataSlice(txSrcPtr, txOutOffset, sliceSize, sliceCount));  // 本地(send)
            txDstSlices.push_back(DataSlice(txDstPtr, txDstOffset, sliceSize, sliceCount));  // 远程(send)
            rxDstSlices.push_back(DataSlice(rxDstPtr, rxOutOffset, sliceSize, sliceCount));  // 本地(recv)
            rxSrcSlices.push_back(DataSlice(rxSrcPtr, rxSrcOffset, sliceSize, sliceCount));  // 远程(recv)
