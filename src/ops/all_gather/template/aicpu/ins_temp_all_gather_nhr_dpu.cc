    ...
    for (uint32_t rpt = 0; rpt < tempAlgParams.repeatNum; ++rpt) {
        ...
        for (uint32_t step = 0; step < nSteps; ++step) {
            ...
            if (txChannel[0].remoteRank == rxChannel[0].remoteRank) {
                // 同一 remoteRank：遵循 SendRecvWrite 的 notify 顺序
                const ChannelInfo& sendCh = txChannel[0];
                const ChannelInfo& recvCh = rxChannel[0];
                const u32 sliceNum = txSrcSlices.size();

                // 前同步：Record 在 recvCh，Wait 在 sendCh
                CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(0, recvCh.handle, NOTIFY_IDX_ACK)));
                CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(0, sendCh.handle, NOTIFY_IDX_ACK, DPU_TIMEOUT)));

                // 数据阶段：Write 在 sendCh，Wait 在 recvCh
                for (u32 i = 0; i < sliceNum; ++i) {
                    ...
                    CHK_RET(static_cast<HcclResult>(
                        HcommWriteWithNotifyNbiOnThread(0, sendCh.handle, dst, src, srcSlice.size_, NOTIFY_IDX_DATA_SIGNAL)));
                    CHK_RET(static_cast<HcclResult>(
                        HcommChannelNotifyWaitOnThread(0, recvCh.handle, NOTIFY_IDX_DATA_SIGNAL, DPU_TIMEOUT)));
                }

                // 后同步：Record(FIN) 在 sendCh → Wait 在 recvCh → Record 在 recvCh → Wait 在 sendCh
                CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(0, sendCh.handle, NOTIFY_IDX_FIN_ACK)));
                CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(0, recvCh.handle, NOTIFY_IDX_FIN_ACK, DPU_TIMEOUT)));
                CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(0, recvCh.handle, NOTIFY_IDX_FIN_ACK)));
                CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(0, sendCh.handle, NOTIFY_IDX_FIN_ACK, DPU_TIMEOUT)));
                CHK_RET(static_cast<HcclResult>(HcommChannelFenceOnThread(0, sendCh.handle)));
                CHK_RET(static_cast<HcclResult>(HcommFenceOnThread(0)));
            } else {
                // 不同 remoteRank：遵循 SendWrite + RecvWrite 的 notify 顺序
                const ChannelInfo& txCh = txChannel[0];
                const ChannelInfo& rxCh = rxChannel[0];
                const u32 sliceNum = txSrcSlices.size();

                // 前同步：各自独立 Record + Wait ACK
                ...
                // 数据阶段：接收方先 Record(DATA)，发送方 Write+Wait(DATA)，接收方 Wait(DATA)
                for (u32 i = 0; i < sliceNum; ++i) {
                    const DataSlice& srcSlice = txSrcSlices[i];
                    const DataSlice& dstSlice = txDstSlices[i];
                    void* dst = static_cast<void*>(static_cast<s8*>(dstSlice.addr_) + dstSlice.offset_);
