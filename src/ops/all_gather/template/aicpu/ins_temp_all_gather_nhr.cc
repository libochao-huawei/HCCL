...
HcclResult InsTempAllGatherNHR::BuildLastStepWriteThenReadSlices(const ChannelInfo &channelSend,
    ...
    std::vector<DataSlice> &rxSrcSlices, std::vector<DataSlice> &rxDstSlices)
{
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        for (u32 i = 0; i < stepInfo.nSlices; ++i) {
            ...
            if (i == 0) {
                txSrcSlices.emplace_back(tempAlgParams_.buffInfo.hcclBuff.addr, txScratchOff, txSliceSize,
...
HcclResult InsTempAllGatherNHR::BuildNormalStepSlices(const ChannelInfo &channelSend,
    const ChannelInfo &channelRecv, const AicpuNHRStepInfo &stepInfo, const u32 &channelIdx,
    std::vector<DataSlice> &txSrcSlices, std::vector<DataSlice> &txDstSlices,
    std::vector<DataSlice> &rxSrcSlices, std::vector<DataSlice> &rxDstSlices)
{
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        for (u32 i = 0; i < stepInfo.nSlices; ++i) {
            ...
            txDstSlices.emplace_back(sendCclBuffAddr, txScratchOff, txSliceSize, txSliceSize / dataTypeSize);
            rxSrcSlices.emplace_back(recvCclBuffAddr, rxScratchOff, rxSliceSize, rxSliceSize / dataTypeSize);
            rxDstSlices.emplace_back(tempAlgParams_.buffInfo.hcclBuff.addr, rxScratchOff, rxSliceSize,
                rxSliceSize / dataTypeSize);
        }
    }
    return HCCL_SUCCESS;
