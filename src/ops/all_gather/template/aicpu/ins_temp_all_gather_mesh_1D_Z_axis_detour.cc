                                           ...
                                           AlgResourceRequest &resourceRequest)
{
    ...
    CHK_RET(CalcChannelRequestMesh1DLevel1(comm, param, topoInfo, subCommRanks_, level0Channels));
    level0ChannelNumPerRank_ = CalcChannelsPerRank(level0Channels);
    level1ChannelNumPerRank_ = CalcChannelsPerRank(level1Channels);
    std::vector<HcclChannelDesc> mergeChannels;
    HCCL_INFO("level0Channels[%d]level1Channels[%d]\n", level0Channels.size(), level1Channels.size());
    mergeChannels.insert(mergeChannels.end(), level0Channels.begin(), level0Channels.end());
    mergeChannels.insert(mergeChannels.end(), level1Channels.begin(), level1Channels.end());
    resourceRequest.channels.push_back(mergeChannels);
    HCCL_INFO("mergeChannels[%d]\n", mergeChannels.size());
    channelsSize = mergeChannels.size();

    if(subCommRanks_.size() <= COMM_LEVEL0) {
        return HCCL_E_PARA;
    }
    auto& ranks = subCommRanks_[COMM_LEVEL0];
    if((ranks.size() -1 ) == mergeChannels.size()) {
        SetIsNewTemp(false);
    } else {
        SetIsNewTemp(true);
    }
...
{
    HCCL_INFO("[InsTempReduceScatterMesh1DZAxisDetour][CalcDataSplitByPortGroup] Run Start[%u][%u][%f]\n", level0ChannelNumPerRank_, level1ChannelNumPerRank_, level0DataRatio_);
                                                    ...
                                                    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    ...
    for (u32 i = 0; i < ranks.size(); i++) {
        ...
        mergedChannels.insert(mergedChannels.end(), channels.at(i).begin(), channel.at(i).end());
    }
    
    u32 threadNum = GetIsNewTemp() ? mergedChannels.size() : (rank.size() - 1);
    HCCL_DEBUG("threadNum %u mergedChannels %u\n", threadNum, mergedChannels.size());
    ...
    for (u32 threadIdx = 0; threadIdx < threadNum; threadIdx++) {
            ...
            const ChannelInfo &linkRemote = GetIsNewTemp() ? channels.at(connectedRank)[idx] : channels.at(connectedRank)[0];
