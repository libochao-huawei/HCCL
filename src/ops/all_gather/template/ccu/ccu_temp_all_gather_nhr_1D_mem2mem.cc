...
HcclResult CcuTempAllGatherNHR1DMem2Mem::FastLaunch(const OpParam& param, const TemplateFastLaunchCtx& tempFastLaunchCtx)
{
    ...
    for (u32 kernelIdx = 0; kernelIdx < kernelNum; kernelIdx++) {
        ...
        HCCL_INFO("[CcuTempReduceScatterMesh1DMem2Mem::KernelRun] args[0][%lu], args[1][%lu], args[2][%lu], args[3][%lu]， args[4][%lu]， args[5][%lu]， args[6][%lu]， args[7][%lu]， args[8][%lu]， args[9][%lu]， args[10][%lu]， args[11][%lu]， args[12][%lu]", args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9], args[10], args[11], args[12]);
                                                       ...
                                                       TemplateResource& templateResource)
{
    ...
    HCCL_INFO("[CcuTempReduceScatterMesh1DMem2Mem::KernelRun] buffInfo_.inBuffBaseOff [%lu], buffInfo_.outBuffBaseOff [%lu], token [%lu], die0Size [%lu], die1Size [%lu], repeatNum [%lu], inputSliceStride [%lu], outputSliceStride [%lu], inputRepeatStride [%lu], outputRepeatStride [%lu], isInputOutputEqual [%lu], die0LastSize [%lu], die1LastSize [%lu]",
    die0Size, die1Size,
            repeatNum, inputSliceStride, outputSliceStride, inputRepeatStride, outputRepeatStride,
            isInputOutputEqual, die0LastSize, die1LastSize);
