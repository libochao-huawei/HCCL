                                                            ...
                                                            std::vector<std::vector<HcclChannelDesc>>& channelsPerDie)
{
    ...
    for (u32 step = 0; step < nSteps; step++) {
        ...
        u32 realFromRankId = subCommRanks_[0][stepInfo.fromRank];
        u32 realToRankId = subCommRanks_[0][stepInfo.toRank];
        if (enableDieNum = DIE_NUM_1) {
            CHK_RET(SelectChannelToVec(comm, myRank_, realFromRankId, rankIdToChannelDesc_, enableDieId, 
                rank2ChannelIdx, channelsPerDie[DIE_0]));
            CHK_RET(SelectChannelToVec(comm, myRank_, realToRankId, rankIdToChannelDesc_, enableDieId, 
                rank2ChannelIdx, channelsPerDie[DIE_1]));
        } else if (enableDieNum = DIE_NUM_2) {
            // 加入fromRank 2个die的链路
            CHK_RET(SelectChannelToVec(comm, myRank_, realFromRankId, rankIdToChannelDesc_, DIE_0, 
                rank2ChannelIdx, channelsPerDie[DIE_0]));
            CHK_RET(SelectChannelToVec(comm, myRank_, realFromRankId, rankIdToChannelDesc_, DIE_1, 
                rank2ChannelIdx, channelsPerDie[DIE_1]));
            // 加入toRank 2个die的链路
            CHK_RET(SelectChannelToVec(comm, myRank_, realToRankId, rankIdToChannelDesc_, DIE_0, 
                rank2ChannelIdx, channelsPerDie[DIE_0]));
            CHK_RET(SelectChannelToVec(comm, myRank_, realToRankId, rankIdToChannelDesc_, DIE_1, 
                rank2ChannelIdx, channelsPerDie[DIE_1]));
        }
    }
