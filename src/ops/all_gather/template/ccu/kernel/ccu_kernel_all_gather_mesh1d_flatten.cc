...
CcuKernelAllGatherMesh1DFlatten::CcuKernelAllGatherMesh1DFlatten(const CcuKernelArg &arg)
    : CcuKernelAlgBase(arg)
{
    const CcuKernelArgAllGatherMesh1DFlatten *kernelArg
        = dynamic_cast<const CcuKernelArgAllGatherMesh1DFlatten *>(&arg);
    rankId_         = kernelArg->rankId_;
    rankSize_       = kernelArg->dimSize_;
    channels_       = kernelArg->channels;

    HCCL_INFO(
        "[CcuKernelAllGatherMesh1DFlatten] Init, KernelArgs are rankId[%u], rankSize_[%u]",
        rankId_, rankSize_);
}

HcclResult CcuKernelAllGatherMesh1DFlatten::InitResource()
{
    localInput_           = CreateVariable();
    uint16_t channelIdx = 0;
    if (channels_.size() == 0) {
        HCCL_ERROR("[CcuKernelAllGatherMesh1DFlatten] channels is empty!");
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            HCCL_DEBUG("[CcuKernelAllGatherMesh1DFlatten] MyRank[%u], PeerId[%u], ChannelId[%u]",
    ...
    for(uint32_t i = 0; i < (rankSize + BIT_NUM_PER_CKE - 1)/BIT_NUM_PER_CKE; i++){
        event_ push_back(CreateCompletedEvent());
    }
    return HcclResult::HCCL_SUCCESS;
}

void CcuKernelAllGatherMesh1DFlatten::LoadArgs()
{
    Load(localInput_);
...
void CcuKernelAllGatherMesh1DFlatten::PreSync()
{
...
void CcuKernelAllGatherMesh1DFlatten::PostSync()
{
    for (ChannelHandle channel : channels_) {
...
void CcuKernelAllGatherMesh1DFlatten::DoAllGather(const hcomm::CcuRep::LocalAddr              &src,
                                                             ...
                                                             const CcuRep::Variable            &sliceSize)
{
    ...
    for (uint64_t rankIdx = 0; rankIdx < rankSize_; rankIdx++) {
        ...
        if (rankIdx == rankId_) {
            LocalCopyNb(remote_src, src_loccopy, sliceSize, event_[eventIdx]);
        } else {
    ...
    for(uint32_t i = 0; i < (rankSize + BIT_NUM_PER_CKE - 1)/BIT_NUM_PER_CKE; i++){
        if (i == (rankSize + BIT_NUM_PER_CKE - 1)/BIT_NUM_PER_CKE - 1) {
...
void CcuKernelAllGatherMesh1DFlatten::DoRepeatAllGather()
{
...
HcclResult CcuKernelAllGatherMesh1DFlatten::Algorithm()
{
    HCCL_INFO("[CcuKernelAllGatherMesh1DFlatten] AllgatherMesh1D run.");
    InitResource();
    LoadArgs();
    PreSync();
    DoRepeatAllGather();
    PostSync();
    HCCL_INFO("[CcuKernelAllGatherMesh1DFlatten] AllgatherMesh1D end.");
    return HcclResult::HCCL_SUCCESS;
}

std::vector<uint64_t> CcuKernelAllGatherMesh1DFlatten::GeneArgs(const CcuTaskArg &arg)
{
    const CcuTaskArgAllGatherMesh1DFlatten *taskArg
        = dynamic_cast<const CcuTaskArgAllGatherMesh1DFlatten *>(&arg);
    ...
    HCCL_INFO("[CcuKernelAllGatherMesh1DFlatten] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
