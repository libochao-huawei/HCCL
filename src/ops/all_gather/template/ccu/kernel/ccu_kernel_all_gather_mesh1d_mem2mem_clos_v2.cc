...
CcuKernelAllGatherMesh1DMem2MemClosV2::CcuKernelAllGatherMesh1DMem2MemClosV2(const CcuKernelArg &arg)
    : CcuKernelAlgBase(arg)
{
    const CcuKernelArgAllGatherMesh1DMem2MemClosV2 *kernelArg
        = dynamic_cast<const CcuKernelArgAllGatherMesh1DMem2MemClosV2 *>(&arg);
    rankId_         = kernelArg->rankId_;
    rankSize_       = kernelArg->dimSize_;
    channels_       = kernelArg->channels;

    HCCL_INFO(
        "[CcuKernelAllGatherMesh1DMem2MemClosV2] Init, KernelArgs are rankId[%u], rankSize_[%u]",
        rankId_, rankSize_);
}

HcclResult CcuKernelAllGatherMesh1DMem2MemClosV2::InitResource()
{
    localInput_           = CreateVariable();
    uint16_t channelIdx = 0;

    CHK_PRT_RET(channels_.size() == 0, HCCL_ERROR("[CcuKernelAllGatherMesh1DMem2MemClosV2] channels is empty!"), HCCL_E_INTERNAL);
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            HCCL_DEBUG("[CcuKernelAllGatherMesh1DMem2MemClosV2] MyRank[%u], PeerId[%u], ChannelId[%u]",
...
void CcuKernelAllGatherMesh1DMem2MemClosV2::LoadArgs()
{
    Load(localInput_);
...
void CcuKernelAllGatherMesh1DMem2MemClosV2::PreSync()
{
...
void CcuKernelAllGatherMesh1DMem2MemClosV2::PostSync()
{
    for (ChannelHandle channel : channels_) {
...
void CcuKernelAllGatherMesh1DMem2MemClosV2::DoAllGather(const hcomm::CcuRep::LocalAddr              &src,
...
void CcuKernelAllGatherMesh1DMem2MemClosV2::DoRepeatAllGather()
{
...
HcclResult CcuKernelAllGatherMesh1DMem2MemClosV2::Algorithm()
{
    HCCL_INFO("[CcuKernelAllGatherMesh1DMem2MemClosV2] AllgatherMesh1D run.");
    InitResource();
    LoadArgs();
    PreSync();
    DoRepeatAllGather();
    PostSync();
    HCCL_INFO("[CcuKernelAllGatherMesh1DMem2MemClosV2] AllgatherMesh1D end.");
    return HcclResult::HCCL_SUCCESS;
}

std::vector<uint64_t> CcuKernelAllGatherMesh1DMem2MemClosV2::GeneArgs(const CcuTaskArg &arg)
{
    const CcuTaskArgAllGatherMesh1DMem2MemClosV2 *taskArg
        = dynamic_cast<const CcuTaskArgAllGatherMesh1DMem2MemClosV2 *>(&arg);
    ...
    HCCL_INFO("[CcuKernelAllGatherMesh1DMem2MemClosV2] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
