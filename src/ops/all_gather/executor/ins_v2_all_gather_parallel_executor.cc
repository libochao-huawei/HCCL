...
HcclResult InsV2AllGatherParallelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::CalculateHierarchyInfo(
    ...
    CHK_PTR_NULL(topoInfo);
    CHK_PRT_RET(
        topoHierarchyInfo.infos.empty() || topoHierarchyInfo.infos[0].empty(),
        HCCL_ERROR("[InsV2AllGatherParallelExecutor] algHierarchyInfo.infos is wrong")
    )
    constexpr u32 TOPO_NUM = 2;
    if(topoInfo->level0Topo == Level0Shape::MESH_1D_CLOS) {
        CHK_PRT_RET(
            topoHierarchyInfo.infos[0].size() < TOPO_NUM || topoHierarchyInfo.infos[0][0].empty() || topoHierarchyInfo.infos[0][1].empty(),
            HCCL_ERROR("[InsV2AllGatherParallelExecutor][MESH_1D_CLOS] algHierarchyInfo.infos[0] is wrong")
        )
        ...
        CHK_PRT_RET(
            topoHierarchyInfo.infos.size() < TOPO_NUM || opoHierarchyInfo.infos[1].empty(),
            HCCL_ERROR("[InsV2AllGatherParallelExecutor] algHierarchyInfo.infos wrong")
        )
