...
{
    HCCL_INFO("[InsV2AllGatherOmniPipe2DExecutor][CalcRes]  Start rank is %u", myRank_);
    ...
    HCCL_INFO("[InsV2AllGatherOmniPipe2DExecutor][CalcRes]  End");
...
HcclResult InsV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::GenTemplateAlgParamsByDimData(
    ...
    // tempAlgParams.buffInfo.inBuffBaseOff = stepSliceInfo.buffInfo.inBuffBaseOff + processedDataCount * dataSize_; //TODO:change
    // tempAlgParams.buffInfo.outBuffBaseOff = stepSliceInfo.buffInfo.outBuffBaseOff + processedDataCount * dataSize_;
    // HCCL_DEBUG("myrank is %u, inBuffBaseOff is %llu, processedDataCount is %llu, end inBuffBaseOff is %llu",
    //     myRank_, stepSliceInfo.buffInfo.inBuffBaseOff, processedDataCount * dataSize_, tempAlgParams.buffInfo.inBuffBaseOff);

    // HCCL_DEBUG("myrank is %u, outBuffBaseOff is %llu, processedDataCount is %llu, end outBuffBaseOff is %llu",
    //     myRank_, stepSliceInfo.buffInfo.outBuffBaseOff, processedDataCount * dataSize_, tempAlgParams.buffInfo.outBuffBaseOff);
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    // std::vector<uint64_t> levelRankSizeVec = {rankSizeLevel0_, rankSizeLevel1_};
    // std::vector<uint64_t> levelRankIdVec = {rankIdxLevel0_, rankIdxLevel1_};

    // OmniPipeSliceInfo alignSliceInfo =
    //     CalcAGOmniPipeSliceInfo(levelRankSizeVec, maxCountPerLoop * dataTypeSize_, endpointAttrBwNew, levelRankIdVec);

    // // 4、计算第n次的loop的slice信息
    // OmniPipeSliceInfo tailSliceInfo;
    // if (dataCount_ % maxCountPerLoop != 0) {
    //     tailSliceInfo = CalcAGOmniPipeSliceInfo(
    //         levelRankSizeVec, (dataCount_ % maxCountPerLoop) * dataTypeSize_, endpointAttrBwNew, levelRankIdVec);

    //     HCCL_INFO("calc tail size");
    // }
