...
#ifndef HCCLV2_INS_V2_ALL_GATHER_PIPELINED_EXECUTOR_H
#define HCCLV2_INS_V2_ALL_GATHER_PIPELINED_EXECUTOR_H

#include "executor_common_ops.h"

namespace ops_hccl {
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
class InsV2AllGatherPipelinedExecutor : public InsCollAlgBase {
public:
    explicit InsV2AllGatherPipelinedExecutor();
...
protected:
    ...
    void GenTemplateAlgParamsIntra1LocalNode(const OpParam &param, const AlgResourceCtxSerializable &resCtx,
                                             const u64 dataOffset, const u64 dataCountPerLoopAixs1,
                                             const u64 scratchOffset, TemplateDataParams &tempAlgParamsIntra1) const;
    void GenTemplateAlgParamsIntra1FromInterScratch(const OpParam &param, const AlgResourceCtxSerializable &resCtx,
                                                    const u32 nodeIdx, const u64 dataOffset,
                                                    const u64 dataCountPerLoopAixs1,
                                                    const u64 intraScratchOffset, const u64 interScratchOffset,
                                                    TemplateDataParams &tempAlgParamsIntra1) const;

    HcclResult PreSyncTemplate(u32 templateIdx);
    HcclResult PostSyncTemplate(u32 templateIdx);
