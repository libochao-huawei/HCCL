...
#include "ins_v2_all_gather_3level_excutor.h"
#include <algorithm>
#include <sstream>
...
InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1,
                               InsAlgTemplate2>::InsV2AllGather3LevelExecutor()
{
}

template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult
InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::InitCommInfo(
    ...
    const AlgHierarchyInfoForAllLevel& algHierarchyInfo)
{
    ...
    HCCL_INFO("[InsV2AllGather3LevelExecutor][InitCommInfo] myRank [%u], rankSize [%u], devType [%u], "
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult
InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::CalcAlgHierarchyInfo(
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::BuildSubCommAndTempMap(
    const OpParam& param,
    const AlgHierarchyInfoForAllLevel& algHierarchyInfo,
    ...
    bool& needFlatten)
{
    ...
    const uint32_t l0Size = rankSizeLevel_[OMNIPIPE_LEVEL0];
    const uint32_t l1Size = rankSizeLevel_[OMNIPIPE_LEVEL1];
    needFlatten = (l0Size >= 2 && l0Size <= 4 && l1Size > 1);

    // 打平
    if (needFlatten) {
        std::vector<u32> flattenRanks;
        uint32_t totalPodSize = l0Size * l1Size;
        rankSizeLevel_[OMNIPIPE_LEVEL0] = 1;
        rankIdxLevel_[OMNIPIPE_LEVEL1] = 0;
        rankSizeLevel_[OMNIPIPE_LEVEL0] = totalPodSize;
        rankIdxLevel_[OMNIPIPE_LEVEL1] = myRank_ % totalPodSize;

        uint32_t startRank = (myRank_ / totalPodSize) * totalPodSize;
        for (uint32_t i = 0; i < totalPodSize; i++) {
            flattenRanks.push_back(startRank + i);
        }
        subCommRanks1.clear();
        subCommRanks1.push_back(flattenRanks);
    }

    // 构造模板map
    tempMap.clear();
    if (!needFlatten && rankSizeLevel_[OMNIPIPE_LEVEL0] > 1) {
    ...
    isFlatten_ = needFlatten;
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::CalcRes(
    ...
    const AlgHierarchyInfoForAllLevel& algHierarchyInfo, AlgResourceRequest& resourceRequest)
{
    ...
    CHK_RET(BuildSubCommAndTempMap(param, algHierarchyInfo,
        subCommRanks0, subCommRanks1, subCommRanks2, tempMap, needFlatten));

    for (auto& temp : tempMap) {
        CHK_RET(CalcResLevel(comm, param, topoInfo, temp.second, resourceRequest));
    }

    HCCL_DEBUG("[InInsV2AllGather3LevelExecutor][CalcRes] myRank[%u], notifyNumOnMainThread[%u], slaveThreadNum[%u], "
               "channels[%u], flatten[%d]",
        myRank_, resourceRequest.notifyNumOnMainThread, resourceRequest.slaveThreadNum,
        resourceRequest.channels.size(), needFlatten);
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult
InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::CalcResLevel(
    HcclComm comm, const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::Orchestrate(
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    HCCL_INFO("[InsV2AllGather3LevelExecutor][Orchestrate] Orchestrate Start");
    ...
    std::map<u32, std::shared_ptr<InsAlgTemplateBase>> tempMap;
    bool needFlatten = false;
    CHK_RET(BuildSubCommAndTempMap(param, resCtx.algHierarchyInfo,
        subCommRanks0, subCommRanks1, subCommRanks2, tempMap, needFlatten));
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR("[InsV2AllGather3LevelExecutor][Orchestrate]errNo[0x%016llx] AllGather excutor kernel run failed",
                   HCCL_ERROR_CODE(ret)),
        ret);
    return HCCL_SUCCESS;
}
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1,
                                          InsAlgTemplate2>::InitExectorInfo(const OpParam& param)
{
    ...
    if (!algHierarchyInfo_.infos[1].empty() && !algHierarchyInfo_.infos[1][0].empty()) {
        rankSizeLevel_[OMNIPIPE_LEVEL1] = algHierarchyInfo_.infos[1][0].size();
    } else {
        rankSizeLevel_[OMNIPIPE_LEVEL1] = 1;
    }
    // Level2
    if (!algHierarchyInfo_.infos[2].empty() && !algHierarchyInfo_.infos[2][0].empty()) {
        rankSizeLevel_[OMNIPIPE_LEVEL2] = algHierarchyInfo_.infos[2][0].size();
    } else {
                                                                               ...
                                                                               StepSliceInfo& stepSliceInfo)
{
    ...
    HCCL_INFO("[InsV2AllGather3LevelExecutor] tempAlgParams.buffInfo.inBuffBaseOff [%u],"
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult
InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::OrchestrateLoop(
    const OpParam& param, const AlgResourceCtxSerializable& resCtx,
    std::map<u32, std::shared_ptr<InsAlgTemplateBase>>& tempMap)
{
    HCCL_INFO("[InsV2AllGather3LevelExecutor][OrchestrateLoop] Start");
    // 1、计算带宽
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        CHK_PRT_RET(omniPipeSliceInfo.dataSliceLevel2.size() == 0,
                    HCCL_ERROR("[InsV2AllGather3LevelExecutor] omniPipeSliceInfo Level2 slice size is 0."),
                    HCCL_E_PARA);
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult
InsV2AllGather3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::RestoreChannelMap(
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_ALLGATHER, InsTestAlgorithm,
                       InsV2AllGather3LevelExecutor, TopoMatch3Level, InsTempAllGatherOmniPipeMesh1D,
