    ...
    const AlgHierarchyInfoForAllLevel& algHierarchyInfo, AlgResourceRequest& resourceRequest)
{
    ...
    const bool needFlatten = (l0Size >= 2 && l0Size <= 4 && l1Size > 1);
    // 打平
    if (needFlatten) {
        std::vector<u32> flattenRanks;
        uint32_t totalPodSize = l0Size * l1Size;
        // 计算当前Rank所在Pod的起始Rank
    ...
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    // Level0
    if (!resCtx.algHierarchyInfo.infos[0].empty() && !resCtx.algHierarchyInfo.infos[0][0].empty()) {
    ...
    // Level1
    if (!resCtx.algHierarchyInfo.infos[1].empty() && !resCtx.algHierarchyInfo.infos[1][0].empty()) {
    ...
    // Level2
    if (!resCtx.algHierarchyInfo.infos[2].empty() && !resCtx.algHierarchyInfo.infos[2][0].empty()) {
        subCommRanks2.push_back(resCtx.algHierarchyInfo.infos[2][0]);
    } else {
        subCommRanks2.emplace_back(std::vector<u32>{myRank_});
    }
    const uint32_t l0Size = rankSizeLevel_[OMNIPIPE_LEVEL0];
