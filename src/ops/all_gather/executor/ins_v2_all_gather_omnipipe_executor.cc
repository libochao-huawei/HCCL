    ...
    std::map<u32, std::shared_ptr<InsAlgTemplateBase>>& tempMap)
{
    ...
    if (param.opMode == OpMode::OPBASE) {
        scratchBoundDataSize = maxTmpMemSize_ / rankSize_ / HCCL_MIN_SLICE_ALIGN * HCCL_MIN_SLICE_ALIGN / dataTypeSize_;
    } else {
        scratchBoundDataSize = dataCount_;
    }
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        if (param.opMode == OpMode::OPBASE) {
            ...
            DataSlice dst(param.outputPtr, myRank_ * dataCount_ * dataTypeSize_, currDataCount * dataTypeSize_,
                          currDataCount);
            CHK_RET(LocalCopy(controlThread_, src, dst));
        }
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_ALLGATHER, InsV2AllGatherOmniPipeMeshNHRNHR,
