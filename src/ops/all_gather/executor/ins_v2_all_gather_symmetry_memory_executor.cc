    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    HCCL_INFO("[InsV2AllGatherSymmetryMemoryExecutor][OrchestrateLoop] myRank[%u], inputPtr[%#llx] outputPtr[%#llx], "
              "channelSize[%lu], threadSize[%lu], ",
              myRank_, param.inputPtr, param.outputPtr, templateAlgRes.channels.size(), templateAlgRes.threads.size());
    // 构建template
    ...
    HCCL_DEBUG("[InsV2AllGatherSymmetryMemoryExecutor] myRank[%u], tempAlgParams.inputSliceStride [%u],"
                "tempAlgParams.outputSliceStride [%u] tempAlgParams.sliceSize [%u]",
                myRank_, tempAlgParams.inputSliceStride, tempAlgParams.outputSliceStride,
                tempAlgParams.sliceSize);
    HCCL_DEBUG("[InsV2AllGatherSymmetryMemoryExecutor] myRank[%u], tempAlgParams.buffInfo.inBuffBaseOff [%u],"
