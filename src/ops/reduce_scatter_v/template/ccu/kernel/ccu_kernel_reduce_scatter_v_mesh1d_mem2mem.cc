...
HcclResult CcuKernelReduceScatterVMesh1DMem2Mem::InitResource()
{
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            CcuRep::Variable inputVar, tokenVar, tokenVar;
...
void CcuKernelReduceScatterVMesh1DMem2Mem::PrepareReduceScatterVData(std::vector<CcuRep::LocalAddr>& reduceScatterVSrc,
    std::vector<CcuRep::LocalAddr>& reduceScatterVDst)
{
    ...
    for (uint32_t rankIdx = 0; rankIdx < rankSize_; rankIdx++) {
        if (rankIdx == rankId_) {
            ...
            reduceScatterVDst[rankIdx].addr = input_[rankIdx];
            reduceScatterVDst[rankIdx].addr += offset_;
            reduceScatterVDst[rankIdx].token = token_[rankIdx];
            continue;
        }
        reduceScatterVSrc[rankIdx].addr = input_[rankIdx];
        reduceScatterVSrc[rankIdx].addr += offset_;
