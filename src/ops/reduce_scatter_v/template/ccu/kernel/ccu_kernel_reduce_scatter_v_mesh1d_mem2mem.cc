...
HcclResult CcuKernelReduceScatterVMesh1DMem2Mem::InitResource()
{
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            input_.push_back(CreateVariable(channels_[channelIdx], INPUT_XN_ID));  // 获取channel中id=0的Var来传递input
            token_.push_back(CreateVariable(channels_[channelIdx], TOKEN_XN_ID));
            channelIdx++;
        }
    }
    ...
    for (uint32_t rankIdx = 0; rankIdx < rankSize_; rankIdx++) {
        reduceScatterVSrc_.push_back(CreateLocalAddr());
...
void CcuKernelReduceScatterVMesh1DMem2Mem::CollectAllRanksSlice(std::vector<CcuRep::LocalAddr>& tmpSrc,
...
void CcuKernelReduceScatterVMesh1DMem2Mem::PrepareReduceScatterVData(std::vector<CcuRep::LocalAddr>& reduceScatterVSrc_,
    std::vector<CcuRep::LocalAddr>& reduceScatterVDst_)
{
    ...
    for (uint32_t rankIdx = 0; rankIdx < rankSize_; rankIdx++) {
        if (rankIdx == rankId_) {
            ...
            reduceScatterVSrc_[rankIdx].addr = input_[rankIdx];
            reduceScatterVSrc_[rankIdx].addr += offset_;
            reduceScatterVSrc_[rankIdx].token = token_[rankIdx];
            reduceScatterVDst_[rankIdx].addr = input_[rankIdx];
            reduceScatterVDst_[rankIdx].addr += offset_;
            reduceScatterVDst_[rankIdx].token = token_[rankIdx];
            continue；
        }
        reduceScatterVSrc_[rankIdx].addr = input_[rankIdx];
        reduceScatterVSrc_[rankIdx].addr += offset_;
        ...
        reduceScatterVDst_[rankIdx].addr  = scratch_;
        reduceScatterVDst_[rankIdx].addr += scratchOffset;
        scratchOffset += scratchInterval_;
        reduceScatterVDst_[rankIdx].token = token_[rankId_];
    }
    return;
}
