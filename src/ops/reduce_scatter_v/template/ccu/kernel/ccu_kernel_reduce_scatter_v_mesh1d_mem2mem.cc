    ...
    std::vector<ccu::CcuLocalAddr>& tmpDst, ReduceScatterVMesh1DMem2MemContext &ctx)
{
    ...
    for (uint32_t rankIdx = 0; rankIdx < arg->rankSize; rankIdx++) {
        if (rankIdx == arg->rankId) {
            ...
            ctx.event.setMask(1 << rankIdx);
            ReadNb(arg->channels[channelId], tmpDst[rankIdx], tmpSrc[rankIdx], arg->sliceSize, ctx.event);
            channelId++;
        }
    }
    // 等读完所有对端
    ...
    std::vector<ccu::CcuLocalAddr>& reduceScatterVDst, ReduceScatterVMesh1DMem2MemContext &ctx)
{
    ...
    for (uint32_t rankIdx = 0; rankIdx < arg->rankSize; rankIdx++) {
        if (rankIdx == arg->rankId) {
            ...
            reduceScatterVDst[rankIdx].addr = ctx.input[rankIdx];
            reduceScatterVDst[rankIdx].addr += ctx.offset;
            reduceScatterVDst[rankIdx].token = ctx.token[rankIdx];
            continue;
        }
        // 从远端的 input 上，读取本卡所需的数据片
        reduceScatterVSrc[rankIdx].addr = ctx.input[rankIdx];
        reduceScatterVSrc[rankIdx].addr += ctx.offset;
        reduceScatterVSrc[rankIdx].token = ctx.token[rankIdx];

        // 将数据放到本卡的 scratch 上
        reduceScatterVDst[rankIdx].addr  = ctx.scratch;
        reduceScatterVDst[rankIdx].addr += scratchOffset;
        scratchOffset += ctx.scratchInterval;
        reduceScatterVDst[rankIdx].token = ctx.token[arg->rankId];
    }
    return;
}
...
static void CcuKernelReduceScatterVMesh1DMem2Mem::DoReduceScatterV(ReduceScatterVMesh1DMem2MemContext &ctx)
{
    ...
    CCU_IF(arg->sliceSize != 0) {
        PrepareReduceScatterVData(reduceScatterVSrc, reduceScatterVDst, ctx);
        CollectAllRanksSlice(reduceScatterVSrc, reduceScatterVDst, ctx);
        ...
        // 执行reduce操作
        GroupLocalReduce(ctx, outDst, reduceScatterVDst_, reduceGosize_,
                        dataType_, outputDataType_, reduceOp_); // 改基类
    }
}
