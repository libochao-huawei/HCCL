...
public:
    ...
    {
        HCCL_DEBUG("[CcuTaskArgReduceScatterVMesh1DMem2Mem] inputAddr: %lu, outputAddr: %lu, token: %lu, scratchAddr: %lu, "
                   "scratchInterval: %lu, sliceSize: %lu, offset: %lu",
                   inputAddr_, outputAddr_, token_, scratchAddr_, scratchInterval, sliceSize, offset);
    }
...
private:
    ...
    void PrepareReduceScatterVData(std::vector<CcuRep::LocalAddr>& reduceScatterVSrc,
        std::vector<CcuRep::LocalAddr>& reduceScatterVDst);
    void CollectAllRanksSlice(std::vector<CcuRep::LocalAddr>& tmpSrc,
    ...
    std::vector<CcuRep::LocalAddr> reduceScatterVSrc_;
