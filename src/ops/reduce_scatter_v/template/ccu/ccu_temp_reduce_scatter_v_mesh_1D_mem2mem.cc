...
uint64_t CcuTempReduceScatterVMesh1DMem2Mem::GetTokenWithFallback(const BuffInfo& buffInfo) {
    if (buffInfo.inputPtr != nullptr) {
        HCCL_INFO("Generate token using input buffer: ptr=%p, size=%llu",
                  buffInfo.inputPtr, buffInfo.inputSize);
        return hcomm::CcuRep::GetTokenInfo(reinterpret_cast<uint64_t>(buffInfo.inputPtr),
