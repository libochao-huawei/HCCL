...
__aicore__ inline void AivBroadcastMesh1D::ProcessBigData(uint64_t curCount, uint64_t sliceId, uint64_t stride)
{
    ...
    bool isMidData16p = rankSize == 16 && curCount * dataTypeSize >= 8 * 1024 * 1024 &&
    ...
    if (rank_ == root_) {
        ...
        // 避免多核同时访问一个flag
        if (isMidData16p) {
            Record(root_, flag_offset, curTag_);
            for (uint32_t i = 0; i < rankSize_; i++) {
                Record(root_, curStageCoreNum + block_idx * rankSize_ + i, curTag_);
            }
        } else {
            for (uint32_t i = 0; i < rankSize_; i++) {
                Record(i, flag_offset, curTag_);
                Record(i, curStageCoreNum + flag_offset, curTag_);
            }
        }
    } else {
        if (peerRank == rank_) {
            ...
            if (isMidData16p) {
                for (uint32_t i = 0; i < rankSize_; i++) {
                    Record(rank_, curStageCoreNum + block_idx * rankSize_ + i, curTag_);
                }
            } else {
                for (uint32_t i = 0; i < rankSize_; i++) {
                    Record(i, curStageCoreNum + flag_offset, curTag_);
    ...
    if (rank_ != root_) {
        // allgather
        if (isMidData16p) {
            WaitFlag(peerRank, curStageCoreNum + block_idx * rankSize_ + rank_, curTag_);
        } else {
            WaitFlag(rank_, curStageCoreNum + flag_offset, curTag_);
        }
        CpGM2GM(inputGM, cclGM, countPerCore);
        PipeBarrier<PIPE_ALL>();
    }
