...
__aicore__ inline void AivBroadcastMesh1D::ProcessBigData(uint64_t curCount, uint64_t sliceId)
{
    ...
    // 每个核开始去读数据
    uint64_t rootInputOffset = reinterpret_cast<uint64_t>(input_) + (rankInnerDisplsStage1 + rankSizeCoreInnerDispls + innerDisplsStage1) * sizeof(T);
    ...
    uint64_t flagRsEndIndex = curStageCoreNum;
    CpGM2GM((__gm__ T *)recvCclOutOffset, (__gm__ T *)rootInputOffset, sendCurCountStage1);
    PipeBarrier<PIPE_ALL>(); 
    Record(rank_, block_idx, curTag_);
    if (coreIndexStage1 == 0) {
        for (uint64_t i = 0; i < rankSize_; i++) {
            WaitFlag(rank_, block_idx + i, curTag_);
        }
        for (uint64_t i = 0; i < rankSize_; i++) {
            Record(i, flagRsEndIndex + rank_ + rankSizeCoreDataIndex * rankSize_, curTag_);
        }
    }
    ...
    if ((rank_ != root_) && (sendCurCount > 0)) {
        ...
        WaitFlag(rank_, flagRsEndIndex + targetRank + coreIndex * rankSize_, curTag_);
