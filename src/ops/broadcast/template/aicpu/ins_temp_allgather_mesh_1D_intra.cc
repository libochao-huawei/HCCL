        ...
        for (u32 threadIdx = 0; threadIdx < subCommRanks_[0].size() - 1; threadIdx++) {
            ...
            if (localSize == 0) { // 发送数据片为0时，只接收数据
                ...
                CHK_PRT_RET(RecvWrite(recvInfo, threads[threadIdx]),
                    ...
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_DEBUG("[InsTempAllGatherMesh1DIntra][RunAllGatherMesh]RecvWrite from rank %u to rank %u "
                ...
                CHK_PRT_RET(SendWrite(sendInfo, threads[threadIdx]),
                    ...
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_DEBUG("[InsTempAllGatherMesh1DIntra][RunAllGatherMesh]SendWrite from rank %u to rank %u "
                          "src offset %u, dst offset %u, "
                          "size %u",
                    ...
                    txDstOffset,
                    localSize);
                HCCL_DEBUG("[InsTempAllGatherMesh1dIntra][RunAllGatherMesh]SendRecvWrite src addr %p, dst addr %p",
                    txSrcPtr,
                    txDstPtr);
            } else {
                ...
                CHK_PRT_RET(SendRecvWrite(sendRecvInfo, threads[threadIdx]),
                    HCCL_ERROR("[InsTempAllGatherMesh1DIntra][RunAllGatherMesh] Send failed"),
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_DEBUG("[InsTempAllGatherMesh1DIntra][RunAllGatherMesh]SendRecvWrite from rank %u to rank %u "
                          "src offset %u, dst offset %u, "
                          "size %u",
                    myRank_, connectedRank,
                    txOutOffset,
