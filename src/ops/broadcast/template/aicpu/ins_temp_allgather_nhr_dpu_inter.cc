            ...
            for (u32 i = 0; i < stepInfo.nSlices; ++i) {
                ...
                if (sendSize > 0 && recvSize > 0) {
                    ...
                    CHK_PRT_RET(SendWrite(sendInfo),
                        ...
                        HcclResult::HCCL_E_INTERNAL);
                    HCCL_DEBUG("[InsTempAllGatherNHRDPUInter][RunNHR]SendWrite from rank %u "
                        ...
                        sendSize);
                    HCCL_DEBUG(
                        "[InsTempAllGatherNHRDPUInter][RunNHR]SendWrite src addr %p, dst addr %p",
                        tempAlgParams.buffInfo.hcclBuff.addr,
                        sendCclBuffAddr);
                } else if (recvSize > 0) {
                    ...
                    CHK_PRT_RET(RecvWrite(recvInfo),
                        ...
                        HcclResult::HCCL_E_INTERNAL);
                    HCCL_DEBUG("[InsTempAllGatherNHRDPUInter][RunNHR]RecvWrite to rank %u "
                              "src offset %u, dst offset %u, "
                              "size %u",
                        myRank_,
                        rxScratchOff,
                        ...
                        recvSize);
                    HCCL_DEBUG(
                        "[InsTempAllGatherNHRDPUInter][RunNHR]RecvWrite src addr %p, dst addr %p",
                        recvCclBuffAddr,
                        tempAlgParams.buffInfo.hcclBuff.addr);
                }
            }
