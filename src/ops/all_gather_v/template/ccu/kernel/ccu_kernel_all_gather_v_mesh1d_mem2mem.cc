    ...
    CCU_IF(mySliceSize != 0) {
        for (uint32_t rankIdx = 0; rankIdx < arg->rankSize; rankIdx++) {
            ...
            if (rankIdx == arg->rankId) {
                ...
                {
                    ctx.event.setMask(1 << rankIdx);
                    WriteNb(channels[channelId], dst[rankIdx], src, mySliceSize, event_); // fix
                }
                channelId++;
            }
        }
