...
static CcuResult InitResource(AllGatherVMesh1DMem2MemContext &ctx)
{
    ...
    CCU_CHK_RET(ccu::Alloc(&ctx.output[arg->rankId]));
    CCU_CHK_RET(ccu::Alloc(&ctx.token[arg->rankId]));
    ...
    CCU_IF(ctx.mySliceSize != 0) {
        for (uint32_t rankIdx = 0; rankIdx < arg->rankSize; rankIdx++) {
            ...
            if (rankIdx == arg->rankId) {
                ...
                {
                    ctx.event.setMask(1 << rankIdx);
                    ccu::WriteNb(arg->channels[channelId], ctx.dst[rankIdx], ctx.src, ctx.mySliceSize, ctx.event); // fix
                }
                channelId++;
            }
        }
