...
void CcuKernelAllGatherVMesh1DMem2Mem::DoAllGatherV()
{
    ...
    for (uint32_t rankIdx = 0; rankIdx < rankSize_; rankIdx++) {
        ...
        if (rankIdx == rankId_) {
            ...
            local_dst.addr = output_[rankId_];
            local_dst.addr += mySliceSizeOutputOffset_;
            local_dst.token = token_[rankId_];
            GroupCopy(local_dst, src, localGoSize_);
            RecordEvent(event_);
        } else {
...
{
    HCCL_INFO("[CcuKernelAllGatherVMesh1DMem2Mem] AllgatherMesh1D run.");
    InitResource();
    LoadArgs();
    PreSync();
    DoAllGatherV();
    PostSync();
    HCCL_INFO("[CcuKernelAllGatherVMesh1DMem2Mem] AllgatherMesh1D end.");
    return HcclResult::HCCL_SUCCESS;
}
