                                                        ...
                                                        const TemplateResource& templateResource)
{
    ...
    uint64_t mySliceSize    = templateDataParams.allRankProcessedDataCount[rankId];
    uint64_t mySliceSizeOutputOffset = tempAlgParams_.allRankDispls[rankId]

    HcclDataType dataType       = param.DataDes.dataType;
    uint64_t dataTypeSize       = DataTypeSizeGet(dataType);
    uint64_t dataCount          = mySliceSize / dataTypeSize;
    if (dataCount == 0) {
        HCCL_INFO("[CcuTempAllGatherVMesh1D] DataCount == 0 &&, Template Run Ends.");
        return HcclResult::HCCL_SUCCESS;
    }
