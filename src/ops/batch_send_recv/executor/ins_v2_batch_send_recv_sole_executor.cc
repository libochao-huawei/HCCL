...
namespace ops_hccl {
InsV2BatchSendRecvSoleExecutor::InsV2BatchSendRecvSoleExecutor()
{
}

HcclResult InsV2BatchSendRecvSoleExecutor::CalcAlgHierarchyInfo(
...
HcclResult InsV2BatchSendRecvSoleExecutor::CalcRes(HcclComm comm, const OpParam &param,
...
HcclResult InsV2BatchSendRecvSoleExecutor::ProcessSelfSendRecvTasks(ThreadHandle &thread)
{
...
bool InsV2BatchSendRecvSoleExecutor::SortSendItems(const HcclSendRecvItem *a, const HcclSendRecvItem *b) const
{
...
bool InsV2BatchSendRecvSoleExecutor::SortRecvItems(const HcclSendRecvItem *a, const HcclSendRecvItem *b) const
{
    u32 aFlag = (a->remoteRank < static_cast<uint32_t>(myRank_)) ? (a->remoteRank + rankSize_) : a->remoteRank;
    u32 bFlag = (b->remoteRank < static_cast<uint32_t>(myRank_)) ? (b->remoteRank + rankSize_) : b->remoteRank;
    if (aFlag != bFlag) {
...
bool InsV2BatchSendRecvSoleExecutor::SortSelfItems(const HcclSendRecvItem *a, const HcclSendRecvItem *b) const
{
    if (a->count != b->count) {
        return a->count > b->count;
    }
...
HcclResult InsV2BatchSendRecvSoleExecutor::GetPairWiseList(const HcclSendRecvItem *sendRecvInfo, u32 itemNum)
{
...
HcclResult InsV2BatchSendRecvSoleExecutor::CalcSendSlices()
{
...
HcclResult InsV2BatchSendRecvSoleExecutor::CalcRecvSlices()
{
...
HcclResult InsV2BatchSendRecvSoleExecutor::GetSendChannel(u32 remoteRank, ChannelInfo &sendChannel) const
{
...
HcclResult InsV2BatchSendRecvSoleExecutor::GetRecvChannel(u32 remoteRank, ChannelInfo &recvChannel) const
{
    auto it = remoteRankToChannelInfo_[0].find(remoteRank);
    if (it == remoteRankToChannelInfo_[0].end()) {
        HCCL_ERROR(
...
HcclResult InsV2BatchSendRecvSoleExecutor::ProcessSendDataSlice(const OpParam &param, SendRecvSlice &sendSlice, BatchSendRecvOpType opType) const
{
...
HcclResult InsV2BatchSendRecvSoleExecutor::ProcessRecvDataSlice(const OpParam &param, SendRecvSlice &recvSlice, BatchSendRecvOpType opType) const
{
...
HcclResult InsV2BatchSendRecvSoleExecutor::RunLoopSendRecv(const OpParam &param)
{
    // TODO: 具体流程可以在这实现?
    for (auto& slice : sendDataSilces_) {
        // 发recv的record，可按ccl循环
        CHK_RET(ProcessRecvDataSlice(param, slice, RECORD));
    }

    while(!sendDataSilces_.empty()) {
        // 发writeWithNotify，然后fence
        CHK_RET(ProcessSendDataSlice(param, sendDataSilces_.front(), SEND));
        sendDataSilces_.pop_front();
    }

    while(!recvDataSilces_.empty()) {
        // waitRecv,按recv接收数据
        CHK_RET(ProcessRecvDataSlice(param, recvDataSilces_.front(), RECV));
...
HcclResult InsV2BatchSendRecvSoleExecutor::Orchestrate(const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
