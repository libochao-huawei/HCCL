    ...
    AlgResourceRequest &resourceRequest)
{
    ...
    for (const u32 &remoteRank : commTargetUserRankSet_) {
        ...
        if (it == algHierarchyInfo.infos[0][0].end()) {
            ...
            HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes]CreateChannelRequestByRankId ready[%u][%u]", myRank_, remoteRank);
            // 1条channel即可
            CHK_RET(
            ...
            HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes]CreateChannelRequestByRankId ok[%u][%u]", myRank_, remoteRank);
        }
    }
...
{
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] GetRecvChannel enter.");
    auto it = remoteRankToChannelInfo_[0].find(remoteRank);
    if (it == remoteRankToChannelInfo_[0].end()) {
        HCCL_ERROR(
    ...
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] GetRecvChannel pass.");
    return HCCL_SUCCESS;
}
    ...
    const OpParam &param, SendRecvSlice &sendSlice, BatchSendRecvOpType opType) const
{
    ...
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessSendDataSlice] sendRecvParam.sendRecvRemoteRank[%u]", sendRecvParam.sendRecvRemoteRank);
    // 构造template
    ...
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessSendDataSlice] algTemplate->KernelRun ready.");
...
{
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] enter.");
    ChannelInfo recvChannel;
    CHK_RET(GetRecvChannel(recvSlice.remoteRank_, recvChannel));
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] GetRecvChannel ok.");
    // 构造param
    OpParam sendRecvParam = param;
    sendRecvParam.sendRecvRemoteRank = recvSlice.remoteRank_;
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] sendRecvParam.sendRecvRemoteRank[%u]", sendRecvParam.sendRecvRemoteRank);
    // 构造template
    ...
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] check algHierarchyInfo_ ok.");
    ...
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] check algHierarchyInfo_.infos[0] ok.");
    std::shared_ptr<InsAlgTemplate> algTemplate
        = std::make_shared<InsAlgTemplate>(sendRecvParam, myRank_, algHierarchyInfo_.infos[0]);
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] algTemplate get.");
    TemplateDataParams tempAlgParams;
    ...
    templateResource.npu2DpuShmemPtr = npu2DpuShmemPtr_;
    templateResource.dpu2NpuShmemPtr = dpu2NpuShmemPtr_;
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][ProcessRecvDataSlice] algTemplate->KernelRun ready.");
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    for (uint32_t rankId = 0; rankId < resCtx.topoInfo.userRankSize; rankId++) {
        ...
        HCCL_INFO("LYQTest[InsV2BatchSendRecvSoleExecutor][Orchestrate] topo rankSize[%u], algHierarchyInfo_ push rankId[%u].", resCtx.topoInfo.userRankSize, rankId);
    }
