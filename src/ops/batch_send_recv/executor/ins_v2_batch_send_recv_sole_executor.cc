    ...
    AlgResourceRequest &resourceRequest)
{
    ...
    for (const u32 &remoteRank : commTargetUserRankSet_) {
        ...
        if (it == algHierarchyInfo.infos[0][0].end()) {
            ...
            CHK_RET(HcclRankGraphGetLinks(comm, 0, myRank, remoteRank, &linkList, &listSize));
            if (listSize == 0) {
                channelNum = 1;
                HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes] 0 layer no listSize, use dpu channelNum = 1");
            }
            HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes] 0 layer find listSize[%u], use ub channelNum = 2", listSize);
        }
