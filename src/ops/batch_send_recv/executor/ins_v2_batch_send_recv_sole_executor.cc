    ...
    HcclComm comm, TopoInfoWithNetLayerDetails *topoInfo, AlgHierarchyInfoForAllLevel &algHierarchyInfo)
{
    ...
    HCCL_INFO("[InsV2BatchSendRecvSoleExecutor][CalcAlgHierarchyInfo] [%u] infos.size[%u], Info.[0].size[%u] Success.", myRank_, algHierarchyInfo.infos.size(), algHierarchyInfo.infos[0].size());
    return HCCL_SUCCESS;
}
    ...
    AlgResourceRequest &resourceRequest)
{
    ...
    for (const u32 &remoteRank : commTargetUserRankSet_) {
        ...
        if (it != algHierarchyInfo.infos[0][0].end()) {
            ...
            HCCL_DEBUG("LYQTest: ready to find infos[1][0].size[%u]", algHierarchyInfo.infos[1][0].size());
