...
namespace ops_hccl {
// template <typename AlgTopoMatch, typename InsAlgTemplate> InsV2BatchSendRecvSoleExecutor<InsAlgTemplate>::InsV2BatchSendRecvSoleExecutor()
// {
// }
    ...
    HcclComm comm, TopoInfoWithNetLayerDetails *topoInfo, AlgHierarchyInfoForAllLevel &algHierarchyInfo)
{
    ...
    // HCCL_INFO("[InsV2BatchSendRecvSoleExecutor][CalcAlgHierarchyInfo] [%u] infos.size[%u], Infos[0].size[%u] infos[0][0].size[%u],  Infos[1].size[%u] infos[1][0].size[%u]Success.",
    //     myRank_, algHierarchyInfo.infos.size(), algHierarchyInfo.infos[0].size(), algHierarchyInfo.infos[0][0].size(),
    //     algHierarchyInfo.infos[1].size(), algHierarchyInfo.infos[1][0].size());
    return HCCL_SUCCESS;
}
    ...
    AlgResourceRequest &resourceRequest)
{
    ...
    for (const u32 &remoteRank : commTargetUserRankSet_) {
        ...
        u32 channelNum = 0;
        HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes]ready to find infos[0][0].size[%u], infos[0][0][0]=%u, infos[0][0][1]=%u",
            algHierarchyInfo.infos[0][0].size(), algHierarchyInfo.infos[0][0][0], algHierarchyInfo.infos[0][0][1]);
        ...
        if (it != algHierarchyInfo.infos[0][0].end()) {
            channelNum = 2;
            HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes] channelNum = %u", channelNum);
        } else {
            HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes]ready to find infos[1][0].size[%u], infos[1][0][0]=%u, infos[1][0][1]=%u",
                algHierarchyInfo.infos[1][0].size(), algHierarchyInfo.infos[1][0][0], algHierarchyInfo.infos[1][0][1]);
            it = std::find(algHierarchyInfo.infos[1][0].begin(), algHierarchyInfo.infos[1][0].end(), remoteRank);
            if (it != algHierarchyInfo.infos[1][0].end()) {
                channelNum = 1;
                HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes] channelNum = %u", channelNum);
            } else {
                HCCL_ERROR(
        ...
        // 1条channel即可(aicpu 通信需要2条?)
        CHK_RET(CreateChannelRequestByRankId(comm, param, myRank_, remoteRank, channelByRank, 1));
        ...
        HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes]CreateChannelRequestByRankId ok[%u][%u] channelNum[%u]", myRank_, remoteRank, channelNum);
    }
    ...
    u32 remoteRank, ChannelInfo &sendChannel) const
{
    ...
    if (it->second[0].locationType == EndpointLocType::ENDPOINT_LOC_TYPE_HOST) {
        ...
        HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][GetSendChannel] Host dpu channel.");
        return HCCL_SUCCESS;
    }
    ...
    u32 remoteRank, ChannelInfo &recvChannel) const
{
    ...
    if (it->second[0].locationType == EndpointLocType::ENDPOINT_LOC_TYPE_HOST) {
        ...
        HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][GetRecvChannel] Host dpu channel.");
        return HCCL_SUCCESS;
    }
    if (remoteRank > myRank_) {
        recvChannel = it->second[0];
    ...
    const OpParam &param, SendRecvSlice &recvSlice, BatchSendRecvOpType opType) const
{
    ...
    tempAlgParams.buffInfo.inputPtr
        = recvChannel.remoteInput.addr; // send端的input 数据来源有send端input和ccl 但是这里的地址实际上不会被使用
...
HcclResult InsV2BatchSendRecvSoleExecutor<AlgTopoMatch, InsAlgTemplate>::RunLoopSendRecv(const OpParam &param)
{
    ...
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][RunLoopSendRecv] first sync success.");

    for (auto &slice : recvDataSilces_) {
        // 发recv的record
        HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][RunLoopSendRecv] RECORD to [%u].", slice.remoteRank_);
        CHK_RET(ProcessRecvDataSlice(param, slice, BatchSendRecvOpType::RECORD));
    }

    for (auto &slice : sendDataSilces_) {
        // 发writeWithNotify
        HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][RunLoopSendRecv] SEND to [%u].", slice.remoteRank_);
        CHK_RET(ProcessSendDataSlice(param, slice, BatchSendRecvOpType::SEND));
    }

    while (!recvDataSilces_.empty()) {
        // waitRecv,按recv接收数据
        HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][RunLoopSendRecv] RECV to [%u].", recvDataSilces_.front().remoteRank_);
    ...
    while (!sendDataSilces_.empty()) {
        // fence
        HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][RunLoopSendRecv] FENCE to [%u].", sendDataSilces_.front().remoteRank_);
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][Orchestrate] algHierarchyInfo_ ok.");
    ...
    HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][Orchestrate] RestoreChannelMap ok.");
...
// REGISTER_EXECUTOR_IMPL_NO_TOPOMATCH(HcclCMDType::HCCL_CMD_BATCH_SEND_RECV, InsBatchSendRecvDPU,
//     InsV2BatchSendRecvSoleExecutor, InsTempBatchSendRecvDpu);TopoMatchMultilevel TopoMatch1D
REGISTER_EXEC_V2(HcclCMDType::HCCL_CMD_BATCH_SEND_RECV, InsBatchSendRecvDPU, InsV2BatchSendRecvSoleExecutor, TopoMatchMultilevel,
