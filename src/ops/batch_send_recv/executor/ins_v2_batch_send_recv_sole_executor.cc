    ...
    AlgResourceRequest &resourceRequest)
{
    ...
    for (const u32 &remoteRank : commTargetUserRankSet_) {
        ...
        if (it == algHierarchyInfo.infos[0][0].end()) {
            ...
            if (listSize == 0) {
                channelNum = 1;
                HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes] 0 layer no listSize, use dpu channelNum = %u", channelNum);
            }
            HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes] 0 layer find listSize[%u], use channelNum = %u", listSize, channelNum);
        }
