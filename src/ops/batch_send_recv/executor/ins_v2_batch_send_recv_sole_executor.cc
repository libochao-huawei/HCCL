...
HcclResult InsV2BatchSendRecvSoleExecutor<AlgTopoMatch, InsAlgTemplate>::RunLoopSendRecv(const OpParam &param)
{
    ...
    while (!recvDataSilces_.empty()) {
        // waitRecv,按recv接收数据
        HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][RunLoopSendRecv] RECV to [%u].", slice.remoteRank_);
    ...
    while (!sendDataSilces_.empty()) {
        // fence
        HCCL_INFO("LYQTest [InsV2BatchSendRecvSoleExecutor][RunLoopSendRecv] FENCE to [%u].", slice.remoteRank_);
