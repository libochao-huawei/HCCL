    ...
    AlgResourceRequest &resourceRequest)
{
    ...
    for (const u32 &remoteRank : commTargetUserRankSet_) {
        ...
        if (it != algHierarchyInfo.infos[0][0].end()) {
            ...
            HCCL_DEBUG("LYQTest: ready to find infos[0][1]")
            it = std::find(algHierarchyInfo.infos[0][1].begin(), algHierarchyInfo.infos[0][1].end(), remoteRank);
            if (it != algHierarchyInfo.infos[0][1].end()) {
                channelNum = 1;
                HCCL_DEBUG("LYQTest: [InsV2BatchSendRecvSoleExecutor][CalcRes] channelNum = %u", channelNum);
            } else {
                HCCL_ERROR(
