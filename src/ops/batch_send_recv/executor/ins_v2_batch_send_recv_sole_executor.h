...
private:
    ...
    // 实现数据发送&接收
    HcclResult ProcessSendDataSlice(const OpParam &param, SendRecvSlice& sendSlice, ThreadHandle& thread) const;
    HcclResult ProcessRecvDataSlice(const OpParam &param, SendRecvSlice& recvSlice, ThreadHandle& thread) const;
