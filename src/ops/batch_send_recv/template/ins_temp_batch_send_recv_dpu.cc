...
namespace ops_hccl
{
    ...
    {
        HCCL_INFO("LYQTest [InsTempBatchSendRecvDpu] KernelRun enter.");
        // 得到thread和channel
        recvRank_ = param.sendRecvRemoteRank;
        threadNum_ = templateResource.threads.size();
        HCCL_INFO("LYQTest [InsTempBatchSendRecvDpu] recvRank_[%u] param.sendRecvRemoteRank[%u]", recvRank_, param.sendRecvRemoteRank);
        if (threadNum_ < 1) {
        ...
        if (sendRecvChannel_.locationType == EndpointLocType::ENDPOINT_LOC_TYPE_HOST) {
            if (tempAlgParams.opType == BatchSendRecvOpType::SEND) {
                ...
                HCCL_INFO("LYQTest [InsTempBatchSendRecvDpu] DPU SEND LocalCopy.");
            ...
            if (tempAlgParams.opType == BatchSendRecvOpType::RECV) {
                ...
                HCCL_INFO("LYQTest [InsTempBatchSendRecvDpu] DPU RECV LocalCopy.");
                DataSlice localCclBuffer(
                    tempAlgParams.buffInfo.hcclBuff.addr, 0, processSize_, count_); // cclbuffer不需要offset
            ...
            if (tempAlgParams.opType == BatchSendRecvOpType::SEND) {
                // 直接发送到对端的cclbuffer上
                HCCL_INFO("LYQTest [InsTempBatchSendRecvDpu] AICPU SendWrite.");
                DataSlice inputBuffer(
                    tempAlgParams.buffInfo.inputPtr, tempAlgParams.buffInfo.inBuffBaseOff, processSize_, count_);
                ...
                DataInfo aicpuSendRecvChannel;
                if (recvRank_ < myRank_) {
                    aicpuSendRecvChannel = sendRecvChannel_;
                } else {
                    aicpuSendRecvChannel = subSendRecvChannel_;
                }
                DataInfo sendInfo(aicpuSendRecvChannel, sendSlicesList);
                ...
                HCCL_INFO("LYQTest [InsTempBatchSendRecvDpu] AICPU RecvWrite.");
                ...
                DataInfo recvInfo(aicpuSendRecvChannel, recvSlicesList);
                                            ...
                                            const std::vector<std::vector<uint32_t>> &subCommRanks)
    {
        ...
        HCCL_INFO("[InsTempBatchSendRecvDpu][DPUKernelRun] Run Recv success!");
#endif
        return HCCL_SUCCESS;
    }
