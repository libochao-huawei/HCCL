...
namespace ops_hccl
{
        ...
        const OpParam &param, const TemplateDataParams &tempAlgParams, TemplateResource &templateResource)
    {
        ...
        {
            if (tempAlgParam.opType == BatchSendRecvOpType::SEND) {
            ...
            if (tempAlgParam.opType == BatchSendRecvOpType::RECV) {
        ...
        {
            HCCL_ERROR("[InsTempBatchSendRecvDpu][Kernel Run] location [%d] is not supported!", sendChannel_.locationType);
                                            ...
                                            const std::vector<std::vector<uint32_t>> &subCommRanks)
    {
        ...
        {
            HCCL_ERROR("[InsTempBatchRecordDpu] [DPUKernelRun] no rank at all!");
            return HCCL_E_PARA;
        }
        u32 sendRank{0};
        for (auto rankId : subCommRanks[0])
        {
            ...
            {
                sendRank = rankId;
                HCCL_INFO("[InsTempBatchRecordDpu] [DPUKernelRun] my rank is [%d],  send rank is [%u].", myRank, sendRank);
            }
        }
        ...
        {
            HCCL_ERROR(
                "[InsTempBatchRecordDpu] [DPUKernelRun] my rank is [%d], send rank [%u] channel not found!", myRank, sendRank);
            return HCCL_E_INTERNAL;
        }
        ...
        HCCL_INFO("[InsTempBatchRecordDpu][DPUKernelRun] Run Recv success!");
#endif
        return HCCL_SUCCESS;
    }

    REGISTER_TEMPLATE_V2("InsTempBatchRecordDpu", InsTempBatchRecordDpu);
} // namespace ops_hccl