...
// ! 已编码完成
HcclResult InsTempSendHostNicDpu::CalcRes(HcclComm comm, const OpParam &param, const TopoInfoWithNetLayerDetails *topoInfo,
    ...
    const std::vector<std::vector<uint32_t>> &subCommRanks)
{
    ...
    for (u32 rankIdx = 0; rankIdx < rankIds.size(); rankIdx++) {
        ...
        for (u64 sizeResidue = inputSize; sizeResidue > 0; sizeResidue -= sizePerRound) {
            ...
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(0, channels.at(rankIdx)[0].handle, 0, 120000)));
            ...
            // 后同步
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(0, channels.at(rankIdx)[0].handle, 2, 120000)));
