...
HcclResult HcclBarrierV2(void *sendBuf, void *recvBuf, uint64_t sendCount, HcclDataType dataType, HcclComm comm, aclrtStream stream)
{
    HcclUs startut = TIME_NOW();
    HCCL_INFO("[HcclBarrierV2] Start, hierarchical barrier");
    ...
    // HCCL_INFO("[HcclBarrierV2] Intra-pod barrier, rankSize[%u]", intraRankSize);

    // if (intraRankSize > 1) {
    //     HcclResult ret = BarrierSingleStep(intraComm, stream);
    //     if (ret != HCCL_SUCCESS) {
    //         HCCL_ERROR("[HcclBarrierV2] Intra-pod barrier failed, ret[0x%016llx]",
    //                     HCCL_ERROR_CODE(ret));
    //         return ret;
    //     }
    // }
    ...
    // HCCL_INFO("[HcclBarrierV2] Inter-pod barrier, rankSize[%u]", interRankSize);

    // if (interRankSize > 1) {
    //     HcclResult ret = BarrierSingleStep(interComm, stream);
    //     if (ret != HCCL_SUCCESS) {
    //         HCCL_ERROR("[HcclBarrierV2] Inter-pod barrier failed, ret[0x%016llx]",
    //                     HCCL_ERROR_CODE(ret));
    //         return ret;
    //     }
    // }

    HCCL_INFO("[HcclBarrierV2] Success, take time[%lld]us",
