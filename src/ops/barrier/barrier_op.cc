...
HcclResult HcclHierarchicalBarrier(HcclComm intraComm, HcclComm interComm, aclrtStream stream)
{
    HcclUs startut = TIME_NOW();
    HCCL_INFO("[HcclBarrier] Start, hierarchical barrier");
    ...
    HCCL_INFO("[HcclBarrier] Intra-pod barrier, rankSize[%u]", intraRankSize);

    if (intraRankSize > 1) {
        HcclResult ret = BarrierSingleStep(intraComm, stream);
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[HcclBarrier] Intra-pod barrier failed, ret[0x%016llx]",
                        HCCL_ERROR_CODE(ret));
            return ret;
        }
    }
    ...
    HCCL_INFO("[HcclBarrier] Inter-pod barrier, rankSize[%u]", interRankSize);

    if (interRankSize > 1) {
        HcclResult ret = BarrierSingleStep(interComm, stream);
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[HcclBarrier] Inter-pod barrier failed, ret[0x%016llx]",
                        HCCL_ERROR_CODE(ret));
            return ret;
        }
    }

    HCCL_INFO("[HcclBarrier] Success, take time[%lld]us",
