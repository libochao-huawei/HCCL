...
HcclResult ScatterRing::RunScatterOnRootRank()
{
    ...
    for (u32 i = 1; i < interRankSize_; i++) {
        ...
        // 等待后一节点同步信号，进行下一轮操作
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelRight_.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT));
        ...
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelRight_.handle, NOTIFY_IDX_DATA_SIGNAL));
    }
    return HCCL_SUCCESS;
}
...
HcclResult ScatterRing::RunScatterOnEndRank()
{
    ...
    CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelLeft_.handle, NOTIFY_IDX_ACK));

    HCCL_DEBUG("last rank[%u] rx data ouputoffset[%llu] size[%llu]", interRank_, scatterOffset, scatterResult);

    CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelLeft_.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT));
