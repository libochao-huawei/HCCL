                                                      ...
                                                      const Slice &txSlice, const Slice &rxSlice, const u32 rankSize)
{
    ...
    // Ack
    if (needReceive) {
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, leftChannel_.handle, NOTIFY_IDX_ACK));
    }
    if (needSend) {
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, rightChannel_.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT));
    }

    // 不同的rank会在不同的step开始持续发送操作，距离root节点越近，越早step开始发送操作
    if (needSend) {
        // TxAsync
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, rightChannel_.handle, NOTIFY_IDX_DATA_SIGNAL));
    }
    // 不同的rank会在不同的step开始持续发送操作，距离root节点越近，越早step开始发送操作
    if (needReceive) {
        void* dst = nullptr;
        ...
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, leftChannel_.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT));
