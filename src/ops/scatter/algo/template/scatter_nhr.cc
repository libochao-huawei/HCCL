...
HcclResult ScatterNHR::RunAsync(const u32 rank, const u32 rankSize, std::vector<ChannelInfo> &channels)
{
    ...
    if (inputMem_.addr != outputMem_.addr) {
        ...
        CHK_RET(HcommLocalCopyOnThread(thread_, outputMem_.addr, src, slices_[targetIdx].size));
    }
...
HcclResult ScatterNHR::SdmaRx(ChannelInfo &channelLeft, ChannelInfo &channelRight, InterServerAlgoStep &stepInfo)
{
    if (channelRight.isValid) {
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelRight.handle, NOTIFY_IDX_ACK));
    }
    if (channelLeft.isValid) {
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelLeft.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT));
        ...
        for (u32 i = 0; i < stepInfo.nSlices; i++) {
            ...
            CHK_RET(HcommReadOnThread(thread_, channelLeft.handle, dst, src, rxSlice.size));
        }
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelLeft.handle, NOTIFY_IDX_DATA_SIGNAL));
    }
    if (channelRight.isValid) {
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelRight.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT));
    }
    return HCCL_SUCCESS;
}
...
HcclResult ScatterNHR::RdmaTxRx(ChannelInfo &channelLeft, ChannelInfo &channelRight, InterServerAlgoStep &stepInfo)
{
    ...
    if (channelLeft.isValid) {
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelLeft.handle, NOTIFY_IDX_ACK));
    }

    if (channelRight.isValid) {
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelRight.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT));
        ...
        for (u32 i = 0; i < stepInfo.nSlices; i++) {
            ...
            CHK_RET(HcommWriteOnThread(thread_, channelRight.handle, dst, src, txSlice.size));
        }
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelRight.handle, NOTIFY_IDX_DATA_SIGNAL)); // rdma数据发完
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelRight.handle, NOTIFY_IDX_FIN_ACK, CUSTOM_TIMEOUT));
    }

    if (channelLeft.isValid) {
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelLeft.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT));  // rdma数据收完
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelLeft.handle, NOTIFY_IDX_FIN_ACK));
    }
...
HcclResult ScatterNHR::ExecuteBarrier(ChannelInfo &channelLeft, ChannelInfo &channelRight)
{
    ...
    if (channelRight.isValid) {
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelRight.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT));
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelRight.handle, NOTIFY_IDX_DATA_SIGNAL));
    }
    if (channelLeft.isValid) {
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelLeft.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT));
        CHK_RET(HcommChannelNotifyRecordOnThread(thread_, channelLeft.handle, NOTIFY_IDX_FIN_ACK));
    }
    if (channelRight.isValid) {
        CHK_RET(HcommChannelNotifyWaitOnThread(thread_, channelRight.handle, NOTIFY_IDX_FIN_ACK, CUSTOM_TIMEOUT));
