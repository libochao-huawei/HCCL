...
namespace ops_hccl {
InsTempScatterNHRDPU::InsTempScatterNHRDPU(const OpParam& param, const u32 rankId, // 传通信域的rankId，userRank
    const std::vector<std::vector<u32>> &subCommRanks)
    : InsAlgTemplateBase(param, rankId, subCommRanks)
{}
 
InsTempScatterNHRDPU::~InsTempScatterNHRDPU()
{}
 
u64 InsTempScatterNHRDPU::GetThreadNum() const
{
    u64 threadNum = 1;
    return threadNum;
}
 
void InsTempScatterNHRDPU::SetRoot(u32 root)
{
    HCCL_INFO("[InsTempScatterNHRDPU][SetRoot] myRank_ [%u], set root_ [%u] ", myRank_, root_);
    root_ = root;
}
 
HcclResult InsTempScatterNHRDPU::CalcRes(HcclComm comm, const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
...
u64 InsTempScatterNHRDPU::CalcScratchMultiple(BufferType inBuffType, BufferType outBuffType)
{
    return templateRankSize_;
}
 
HcclResult InsTempScatterNHRDPU::GetStepInfo(u32 step, u32 nSteps, AicpuNHRStepInfo &stepInfo)
{
#ifndef AICPU_COMPILE
    u32 rankSize = templateRankSize_;
    u32 myAlgRank;
    u32 rootAlgRank;
    HCCL_INFO("[InsTempScatterNHRDPU][GetStepInfo][GetAlgRank]* root_ before getAlgRank [%u] ", root_);
    CHK_RET(GetAlgRank(myRank_, subCommRanks_[0], myAlgRank));
    HCCL_INFO("[InsTempScatterNHRDPU][GetStepInfo][GetAlgRank]* myRank_[%u], myAlgRank[%u]", myRank_, myAlgRank);
    CHK_RET(GetAlgRank(root_, subCommRanks_[0], rootAlgRank));
    HCCL_INFO("[InsTempScatterNHRDPU][GetStepInfo][GetAlgRank]* root_[%u], rootAlgRank[%u]", root_, rootAlgRank);
    ...
    if (deltaRoot < nRanks) {  // 需要发
        HCCL_INFO("[InsTempScatterNHRDPU][GetStepInfo] Need to Send: deltaRoot[%u], nRanks[%d]", deltaRoot, nRanks);
        ...
        HCCL_INFO("[InsTempScatterNHRDPU][GetStepInfo] rankSize[%u], myAlgRank[%d], sendTo Idx[%u]", subCommRanks_[0].size(), myAlgRank, sendTo);
        stepInfo.toRank = subCommRanks_[0].at(sendTo);
        stepInfo.nSlices = nSlices;
    } else if (deltaRoot >= deltaRankPair && deltaRoot < nRanks + deltaRankPair) {  // 需要收
        HCCL_INFO("[InsTempScatterNHRDPU][GetStepInfo] Need to Recv: deltaRoot[%u], nRanks[%d], deltaRankPair[%d]", deltaRoot, nRanks, deltaRankPair);
        ...
        HCCL_INFO("[InsTempScatterNHRDPU][GetStepInfo] rankSize[%u], myAlgRank[%d], recvFrom Idx[%u]", subCommRanks_[0].size(), myAlgRank, recvFrom);
...
HcclResult InsTempScatterNHRDPU::KernelRun(const OpParam& param, const TemplateDataParams &tempAlgParams,
    TemplateResource& templateResource)
{
    HCCL_INFO("[InsTempScatterNHRDPU] Run Start");
    ...
    HCCL_INFO("[InsTempScatterNHRDPU] queNum_ =  [%d], threads size = [%d]", threadNum_, templateResource.threads.size());
    ...
    dpuRunInfo.templateName = "InsTempScatterNHRDPU";
    ...
    HCCL_INFO("[InsTempScatterNHRDPU] Run End");
    return HcclResult::HCCL_SUCCESS;
}

HcclResult InsTempScatterNHRDPU::DPUKernelRun(const TemplateDataParams& tempAlgParams,
...
HcclResult InsTempScatterNHRDPU::LocalDataCopy(const TemplateDataParams& tempAlgParams,
    const TemplateResource& templateResource)
{
    ...
    for (uint64_t rpt = 0; rpt < tempAlgParams.repeatNum; ++rpt) {
        ...
        HCCL_INFO("[InsTempScatterNHRDPU][LocalCopy] LocalDataCopy RankID [%d] algRankIdx [%d] "
...
HcclResult InsTempScatterNHRDPU::PostLocalCopy(const TemplateDataParams& tempAlgParams,
    const TemplateResource& templateResource)
{
    ...
    for (u32 rpt = 0; rpt < tempAlgParams.repeatNum; ++rpt) {
        ...
        for (auto rank : subCommRanks_[0]) {
            ...
            HCCL_INFO("[InsTempScatterNHRDPU][LocalCopy] LocalDataCopy RankID [%d] dataRank [%d] dataAlgRank[%d] "
...
HcclResult InsTempScatterNHRDPU::RunNHR(const std::map<u32, std::vector<ChannelInfo>> &channels,
...
HcclResult InsTempScatterNHRDPU::BatchSend(AicpuNHRStepInfo &stepInfo, const std::map<u32, std::vector<ChannelInfo>> &channels,
    const TemplateDataParams &tempAlgParam, u32 repeat) const
{
#ifndef AICPU_COMPILE
    HCCL_INFO("[InsTempScatterNHRDPU][BatchSend] myRank[%d], toRank[%d]", myRank_, stepInfo.toRank);
    ...
    for (u32 i = 0; i < stepInfo.txSliceIdxs.size(); i++) {
        ...
        u64 srcOffset = repeat * templateRankSize_ * sliceSize + tempAlgParam.buffInfo.hcclBuffBaseOff + sliceOffset;
        u64 dstOffset = repeat * sliceSize + tempAlgParam.buffInfo.hcclBuffBaseOff;
        
        HCCL_INFO("[InsTempScatterNHRDPU][BatchSend]in step [%u]: myRank[%d], toRank[%d], slicesize is [%u] "
                  ",srcOffset is [%u], dstOffset is [%u] ",
            i,
            myRank_,
            stepInfo.toRank,
            sliceSize,
        ...
        DataSlice srcSlice = DataSlice(tempAlgParam.buffInfo.hcclBuff.addr, srcOffset, sliceSize, sliceCount);
        DataSlice dstSlice = DataSlice(remoteCclBuffAddr, dstOffset, sliceSize, sliceCount);
        if (sliceSize != 0) {
    ...
    if (srcSlices.size() != 0) {
        CHK_PRT_RET(static_cast<HcclResult>(SendWrite(sendData)),
            HCCL_ERROR("[InsTempScatterNHRDPU] BatchSend failed"),
...
HcclResult InsTempScatterNHRDPU::BatchRecv(AicpuNHRStepInfo &stepInfo, const std::map<u32, std::vector<ChannelInfo>> &channels,
    const TemplateDataParams &tempAlgParam, u32 repeat) const
{
#ifndef AICPU_COMPILE
    ...
    for (u32 i = 0; i < stepInfo.rxSliceIdxs.size(); i++) {
        ...
        u64 srcDstOffset = repeat * sliceSize + tempAlgParam.buffInfo.hcclBuffBaseOff;

        HCCL_INFO("[InsTempScatterNHRDPU][BatchRecv]in step [%u]: myRank[%d], fromRank[%d],slicesize is [%u] "
                  ",srcDstOffset is [%u] ",
            i,
            myRank_,
            stepInfo.fromRank,
    ...
    if (srcDstSlices.size() != 0) {
        CHK_PRT_RET(static_cast<HcclResult>(RecvWrite(recvData)),
            HCCL_ERROR("[InsTempScatterNHRDPU] BatchRecv failed"),
            HcclResult::HCCL_E_INTERNAL);
    }
#endif
    return HcclResult::HCCL_SUCCESS;
...
HcclResult InsTempScatterNHRDPU::BatchSR(AicpuNHRStepInfo &stepInfo, const std::map<u32, std::vector<ChannelInfo>> &channels,
    const TemplateDataParams &tempAlgParam, u32 repeat) const
{
#ifndef AICPU_COMPILE
    const ChannelInfo &linkSend = channels.at(stepInfo.toRank)[0];
    ...
    for (u32 i = 0; i < stepInfo.txSliceIdxs.size(); i++) {
        ...
        HCCL_INFO("[InsTempScatterNHRDPU][BatchSR]in step [%u]: myRank[%d], toRank[%d], slicesize is [%u] "
                  ",srcOffset is [%u], dstOffset is [%u] ",
            i,
            myRank_,
            stepInfo.toRank,
    ...
    for (u32 i = 0; i < stepInfo.rxSliceIdxs.size(); i++) {
        ...
        HCCL_INFO("[InsTempScatterNHRDPU][BatchSR]in step [%u]: myRank[%d], fromRank[%d],slicesize is [%u] "
                  ",srcDstOffset is [%u] ",
            i,
            myRank_,
            stepInfo.fromRank,
    ...
    if (txSrcSlices.size() != 0) {
        CHK_PRT_RET(SendRecvWrite(sendRecvInfo),
            HCCL_ERROR("[InsTempScatterNHRDPU] RunNHR BatchSendRecv failed"),
            HcclResult::HCCL_E_INTERNAL);
    }
#endif
    return HcclResult::HCCL_SUCCESS;
...
REGISTER_TEMPLATE_V2("InsTempScatterNHRDPU", InsTempScatterNHRDPU);
