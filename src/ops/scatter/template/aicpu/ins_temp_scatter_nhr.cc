...
HcclResult InsTempScatterNHR::GetRes(AlgResourceRequest &resourceRequest) const
{
    ...
    resourceRequest.notifyNumOnMainThread = threadNum - 1;  // 没有从流
    return HCCL_SUCCESS;
}
...
HcclResult SplitDataPerChannel(const DataSlice &srcSlice, const DataSlice &dstSlice, std::vector<ChannelInfo> &channels, std::vector<DataSlice> &srcSliceVec, std::vector<DataSlice> &dstSliceVec) {
    srcSliceVec.clear();
    dstSliceVec.clear();
