...
HcclResult InsTempScatterNHR::PreprareDataSplitForMultiChannel(const TemplateResource &templateResource) {
    ...
    for (u32 channelIdx = 0; channelIdx < channelsPerRank_; channelIdx++) {
        for (u32 step = 0; step < nSteps; step++) {
            ...
            if (stepInfo.txSliceIdxs.size() > 0 && stepInfo.rxSliceIdxs.size() == 0) {
                CHK_RET(BatchSend(stepInfo, channels, threads.at(channelIdx), tempAlgParam, channelIdx));
            }
            // 只有Rx，使用recv指令
            else if (stepInfo.txSliceIdxs.size() == 0 && stepInfo.rxSliceIdxs.size() > 0) {
                CHK_RET(BatchRecv(stepInfo, channels, threads.at(channelIdx), tempAlgParam, channelIdx));
            }
            // 既有Tx又有Rx，使用SendRecv指令
            else if (stepInfo.txSliceIdxs.size() > 0 && stepInfo.rxSliceIdxs.size() > 0) {
                CHK_RET(BatchSR(stepInfo, channels, threads.at(channelIdx), tempAlgParam, channelIdx));
            }
        }
    }
    return HCCL_SUCCESS;
...
HcclResult InsTempScatterNHR::BatchSend(AicpuNHRStepInfo &stepInfo, const std::map<u32, std::vector<ChannelInfo>> &channels,
    const ThreadHandle &thread, const TemplateDataParams &tempAlgParam, u32 channelIdx) const
{
    ...
    u32 curCount = 0;
    const u32 dataTypeSize = DATATYPE_SIZE_TABLE[dataType_];
    for (u32 repeat = 0; repeat < tempAlgParam.repeatNum; repeat++) {
        for (u32 i = 0; i < stepInfo.txSliceIdxs.size(); i++) {
            u32 txId = stepInfo.txSliceIdxs.at(i);
            const u64 txPartialOffset = (txIdx == templateRankSize_ - 1 && tempAlgParams.tailSize != 0) ? dataOffsetTail_[channelIdx]: dataOffset_[channelIdx];
            u64 srcDstOffset = repeat * templateRankSize_ * tempAlgParam.sliceSize + tempAlgParam.buffInfo.hcclBuffBaseOff +
                            txId * tempAlgParam.sliceSize + txPartialOffset;
            curSliceSize = (tempAlgParam.tailSize !=0 && txId == templateRankSize_ - 1) ? dataSplitTail_[channelIdx]: dataSplit_[channelIdx];
            curCount = curSliceSize / dataTypeSize;
    ...
    const ThreadHandle &thread, const TemplateDataParams &tempAlgParam, u32 channelIdx) const
{
    ...
    for (u32 repeat = 0; repeat < tempAlgParam.repeatNum; repeat++) {
        for (u32 i = 0; i < stepInfo.rxSliceIdxs.size(); i++) {
            ...
            const u64 rxPartialOffset = (rxIdx == templateRankSize_ - 1 && tempAlgParams.tailSize != 0) ? dataOffsetTail_[channelIdx]: dataOffset_[channelIdx];
            u64 srcDstOffset = repeat * templateRankSize_ * tempAlgParam.sliceSize + tempAlgParam.buffInfo.hcclBuffBaseOff +
                                rxId * tempAlgParam.sliceSize + rxPartialOffset;
            curSliceSize = (tempAlgParam.tailSize !=0 && rxId == templateRankSize_ - 1) ? dataSplitTail_[channelIdx]: dataSplit_[channelIdx];
            curCount = curSliceSize / dataTypeSize;
