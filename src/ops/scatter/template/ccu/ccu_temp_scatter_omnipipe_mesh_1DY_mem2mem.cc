...
{
    HCCL_DEBUG("[%s] myRank[%u] mySubCommRank_[%u] isStepone[%d] isLastStep[%d] start", __func__, myRank_,
        mySubCommRank_, isStepOne_, isLastStep_);
    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            if (ifRealRoot_) {
                ...
                sliceSize = stepSliceInfo.stepSliceSize[mySubCommRank_+1][rpt];
                inputOmniPipeSliceStride = stepSliceInfo.inputOmniPipeSliceStride[mySubCommRank_+1][rpt];
                outputOmniPipeSliceStride= stepSliceInfo.outputOmniPipeSliceStride[mySubCommRank_+1][rpt];
            }

            HCCL_DEBUG("[%s] myRank[%u] mySubCommRank[%u] subCommRootId_[%u] rpt[%u] sliceSize[%llu] "
