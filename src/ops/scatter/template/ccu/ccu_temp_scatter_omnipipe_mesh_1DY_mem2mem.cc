...
#include "ccu_temp_scatter_omnipipe_mesh_1DY_mem2mem.h"
#include "alg_data_trans_wrapper.h"

namespace ops_hccl {

CcuTempScatterOmniPipeMesh1DYMem2Mem::CcuTempScatterOmniPipeMesh1DYMem2Mem(
...
void CcuTempScatterOmniPipeMesh1DYMem2Mem::SetRoot(u32 root)
{
    HCCL_INFO("[CcuTempScatterOmniPipeMesh1DYMem2Mem][SetRoot] myRank_ [%u], set root [%u] ", myRank_, root);
...
void CcuTempScatterOmniPipeMesh1DYMem2Mem::UnsetRoot(u32 rank)
{
    HCCL_INFO("[CcuTempScatterOmniPipeMesh1DYMem2Mem][UnsetRoot] myRank_ [%u], unset root [%u] ", myRank_, rank);
    if (!ifRealRoot_) {
        subCommRootId_ = 1000;
    }
}

u64 CcuTempScatterOmniPipeMesh1DYMem2Mem::GetThreadNum() const
{
    return 1;
}

HcclResult CcuTempScatterOmniPipeMesh1DYMem2Mem::GetRes(AlgResourceRequest& resourceRequest) const
{
...
uint32_t CcuTempScatterOmniPipeMesh1DYMem2Mem::RemoteRankId2RankId(const u32 remoteRankId) const
{
    u32 subCommRankId = 0;
    std::vector<u32> ranks = subCommRanks_[0];
...
HcclResult CcuTempScatterOmniPipeMesh1DYMem2Mem::CalcRes(HcclComm comm, const OpParam &param,
    const TopoInfoWithNetLayerDetails *topoInfo, AlgResourceRequest &resourceRequest)
{
    ...
    kernelInfo.creator = [](const hcomm::CcuKernelArg &arg) {
        return std::make_unique<CcuKernelScatterOmniPipeMesh1DYMem2Mem>(arg);
    };
    ...
    if (topoInfo->level0Topo != Level0Shape::MESH_1D_CLOS) {
        ...
        CHK_RET(CalcChannelRequestMesh1DWithPriority(comm, param, topoInfo, subCommRanks_, channelDescs, CommTopo::COMM_TOPO_1DMESH));
        for (auto channel : channedlDescs) {
            if (channel.channelProtocol != COMM_PROTOCOL_UBC_CTP) {
                HCCL_ERROR("[CcuTempScatterOmniPipeMesh1DYMem2Mem][%s] channel.channelProtocol[%u]", __func__, channel.channelProtocol);
                return HCCL_E_INTERNAL;
            }
        }
    }

    HCCL_DEBUG("[CcuTempScatterOmniPipeMesh1DYMem2Mem][%s] Get Mesh channels Success.", __func__);
    ...
    //         HCCL_DEBUG("myRank_[%u] mySubCommRank_[%u] channel.remoteRank[%u] ", myRank_, mySubCommRank_, channel.remoteRank);
...
HcclResult CcuTempScatterOmniPipeMesh1DYMem2Mem::KernelRun(
    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            if (ifRealRoot_) {
                ...
                sliceSize = stepSliceInfo.stepSliceSize[myRank_][rpt];
                inputOmniPipeSliceStride = stepSliceInfo.inputOmniPipeSliceStride[myRank_][rpt];
                outputOmniPipeSliceStride= stepSliceInfo.outputOmniPipeSliceStride[myRank_][rpt];
            }
...
HcclResult CcuTempScatterOmniPipeMesh1DYMem2Mem::FastLaunch(const OpParam &param, const TemplateFastLaunchCtx& tempFastLaunchCtx)
{
    HCCL_DEBUG("[%s] end", __func__);
    return HcclResult::HCCL_SUCCESS;
}

u64 CcuTempScatterOmniPipeMesh1DYMem2Mem::CalcScratchMultiple(BufferType inBuffType, BufferType outBuffType)
{
    (void)inBuffType;
    (void)outBuffType;
