...
constexpr uint16_t INPUT_XN_ID = 1;
...
void CcuKernelScatterOmniPipeMesh1D::PreSync()
{
    ...
    for (auto channel : channels_) {                                                           // 遍历所有通道
        NotifyRecord(channel, CKE_IDX_0, INPUT_XN_ID, input_[rankId_], 1 << INPUT_XN_ID);  // 广播input地址
...
HcclResult CcuKernelScatterOmniPipeMesh1D::Algorithm()
{
    ...
    for (uint64_t curId = 0; curId < rankSize_; curId++) {
        ...
        // 使用循环累加代替乘法，因为Variable类型不支持直接乘法运算
    ...
    if (rankId_ == rootId_) {
        // 根节点始终执行发送
        DoScatter();
    } else if (isSameAxisAsRoot_) {
        // 同轴线节点根据currentStep_决定是否执行发送
        // 使用CCU_IF宏处理Variable类型的条件判断
        // 使用两个独立的CCU_IF代替CCU_ELSE，因为CCU_ELSE可能不存在
        CCU_IF(currentStep_ != 0) {
            DoScatter();
        }
        CCU_IF(currentStep_ == 0) {
            // 同轴线节点在第一步等待接收完成
