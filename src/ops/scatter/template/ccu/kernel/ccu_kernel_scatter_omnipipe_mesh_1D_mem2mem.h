...
public:
    explicit CcuKernelArgScatterOmniPipeMesh1DMem2Mem(uint64_t dimSize, uint32_t rankId, uint32_t rootId,
        const OpParam& opParam, const std::vector<std::vector<uint32_t>>& subCommRanks, bool ifRealRoot)
        : dimSize_(dimSize),
          rankId_(rankId),
          rootId_(rootId),
          opParam_(opParam),
    ...
    {
        HCCL_DEBUG("[%s]dimSize: %lu, rankId: %u, dataType: %d rootId: %u, ifRealRoot: %d", 
                   __func__, dimSize_, rankId_, opParam.DataDes.dataType, rootId_, ifRealRoot_);
    }
...
public:
    explicit CcuTaskArgScatterOmniPipeMesh1DMem2Mem(uint64_t inputAddr, uint64_t outputAddr, uint64_t scratchAddr,
        ...
        bool isStepOne, bool isLastStep)
          ...
          isLastStep_(isLastStep)
    {
        HCCL_DEBUG("[%s]inputAddr[%lu] outputAddr[%lu] scratchAddr[%lu] sliceSize[%lu] offSet[%lu] "
                   "localCopyFlag[%lu] isStepOne[%d] isLastStep[%d]",
                   __func__, inputAddr_, outputAddr_, scratchAddr_, sliceSize_, offSet_, 
                   localCopyFlag_, isStepOne_, isLastStep_);
    }
