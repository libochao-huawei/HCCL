...
public:
    explicit CcuKernelArgScatterOmniPipeMesh1DMem2Mem(uint64_t dimSize, uint32_t rankId, 
    ...
    {
        HCCL_DEBUG("[%s]dimSize: %lu, rankId: %u, dataType: %d",
                   __func__, dimSize_, rankId_, opParam.DataDes.dataType);
    }
...
public:
    explicit CcuTaskArgScatterOmniPipeMesh1DMem2Mem(uint64_t inputAddr, uint64_t outputAddr, 
        ...
        : inputAddr_(inputAddr), outputAddr_(outputAddr), sliceSize_(sliceSize), 
          offSet_(offSet), token_(token), localCopyFlag_(localCopyFlag),
