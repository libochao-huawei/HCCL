    ...
    : CcuKernelAlgBase(arg)
{
    ...
    HCCL_INFO("[CcuKernelScatterOmniPipeMesh1DMem2Mem] Init, KernelArgs are rankId[%u], "
        "rankSize_[%u], dataType[%d], rootId[%u], ifRealRoot[%d] ", rankId_, rankSize_, dataType_, rootId_, ifRealRoot_);
}
...
{
    HCCL_DEBUG("[CcuKernelScatterOmniPipeMesh1DMem2Mem] DoRepeatScatter, myRank[%u]", rankId_);
    ...
    for (uint64_t curId = 0; curId < rankSize_; curId++) {
        ...
        outputMem_[remoteIdx].addr = input_[curId];
        outputMem_[remoteIdx].addr += inputSliceStride_;
        outputMem_[remoteIdx].addr += inputOmniPipeSliceStride_;
        remoteIdx++;

        scratchMem_[curId].token = token_[rankId_];
        scratchMem_[curId].addr = scratch_[rankId_];
        scratchMem_[curId].addr += scratchOffset;
        scratchMem_[curId].addr += sliceSize_;
...
{
    HCCL_DEBUG("[CcuKernelScatterOmniPipeMesh1DMem2Mem] DoScatter, myRank[%u]", rankId_);
    ...
    for (uint64_t rankIdx = 0; rankIdx < rankSize_; rankIdx++) {
        ...
        if (rankIdx == rankId_) {
            ...
            CCU_IF(sliceSize_ != 0) {
                WriteNb(channels_[channelId], remoteScratchMem_[rankIdx], inputMem_[rankIdx], sliceSize_, event_);
                channelId++;
            } 
        }
    }
...
void CcuKernelScatterOmniPipeMesh1DMem2Mem::DoScatterFromScratch()
{
    ...
    for (uint64_t rankIdx = 0; rankIdx < rankSize_; rankIdx++) {
        ...
        if (rankIdx == rankId_) {
            ...
            CCU_IF(sliceSize_ != 0) {
                WriteNb(channels_[channelId], outputMem_[rankIdx], scratchMem_[rankIdx], sliceSize_, event_);
                channelId++;
            }
        }
    }
...
std::vector<uint64_t> CcuKernelScatterOmniPipeMesh1DMem2Mem::GeneArgs(const CcuTaskArg &arg)
{
    ...
    HCCL_INFO("[CcuKernelScatterOmniPipeMesh1DMem2Mem] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
        "scratchAddr[%llu], sliceSize[%llu], offSet[%llu], localCopyFlag[%llu], isStepOne[%d], isLastStep[%d]", 
        taskArg.inputAddr_, taskArg.outputAddr_, taskArg.scratchAddr_,
        taskArg.sliceSize_, taskArg.offSet_, taskArg.localCopyFlag_, taskArg.isStepOne_, taskArg.isLastStep_);
