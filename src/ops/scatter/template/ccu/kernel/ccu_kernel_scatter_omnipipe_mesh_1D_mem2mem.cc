...
void CcuKernelScatterOmniPipeMesh1DMem2Mem::DoRepeatScatter()
{
    ...
    for (uint64_t curId = 0; curId < rankSize_; curId++) {
        ...
        outputMem_[curId].addr += inputOmniPipeSliceStride_;        
    }
    if (rankId_ == rootId_) {
        DoScatter();
    } else {
...
std::vector<uint64_t> CcuKernelScatterOmniPipeMesh1DMem2Mem::GeneArgs(const CcuTaskArg &arg)
{
    ...
    HCCL_INFO("[CcuKernelScatterOmniPipeMesh1DMem2Mem] TaskArgs: inputAddr[%llu], outputAddr[%llu],sliceSize[%llu], " 
        "inputSliceStride[%llu], outputSliceStride[%llu], inputOmniPipeSliceStride[%llu], "
        "outputOmniPipeSliceStride[%llu], offSet[%llu], localCopyFlag[%llu], isStepOne[%d], isLastStep[%d]", 
        taskArg.inputAddr_, taskArg.outputAddr_, taskArg.sliceSize_, taskArg.inputSliceStride_, 
        taskArg.outputSliceStride_, taskArg.inputOmniPipeSliceStride_, 
        taskArg.outputOmniPipeSliceStride_, taskArg.offSet_, taskArg.localCopyFlag_, 
        taskArg.isStepOne_, taskArg.isLastStep_);
