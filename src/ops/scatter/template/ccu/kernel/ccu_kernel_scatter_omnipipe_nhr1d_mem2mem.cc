...
{
    CcuRep::Variable curInputOffset = 0;
    CcuRep::Variable curOutputOffset = 0;

    for (u64 i = 0; i < dimSize_; i++) {
        inputOffset_.push_back(CreateVariable());
        inputOffset_[i] = curInputOffset;
        curInputOffset += inputSliceStride_;
    }

    for (u64 i = 0; i < dimSize_; i++) {
        outputOffset_.push_back(CreateVariable());
        outputOffset_[i] = curOutputOffset;
        curOutputOffset += outputSliceStride_;
    }
    ...
    CCU_WHILE(repeatNumVar_ != UINT64_MAX) {
        ...
        CCU_IF(repeatTimeflag_ == 0) {
            if (axisId_ == 1) {
                u32 offsetSize = (rankId_ != dimSize_ - 1) ? die0Size_ : die0TailSize_;
    ...
    CcuRep::RemoteAddr& dst, bool isLastSlice)
{
    ...
    CCU_WHILE(repeatNumVarTemp_ != UINT64_MAX) {
        ...
        CCU_IF(repeatTimeflag_ == 0) {
            if (axisId_ == 1) {
                u32 offsetSize = isLastSlice ? die0TailSize_ : die0Size_;
                src.addr += offsetSize;
                dst.addr += offsetSize;
            }
        }
