...
#include "ccu_kernel_common.h"
#include "nhr_step_info.h"

namespace ops_hccl {
class CcuKernelArgScatterOmniPipeNHR1DMem2Mem : public CcuKernelArg {
public:
    CcuKernelArgScatterOmniPipeNHR1DMem2Mem(std::vector<ChannelHandle> channels,
        const HcclOpParam& opParam,
        ...
        const std::vector<uint32_t>& subCommRanks)
        : CcuKernelArg(channels, opParam),
        rankId_(rankId),
        dimSize_(dimSize),
        rootId_(rootId),
        axisId_(axisId),
