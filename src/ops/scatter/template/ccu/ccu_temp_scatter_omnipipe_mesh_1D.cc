...
HcclResult CcuTempScatterOmniPipeMesh1D::GetRes(AlgResourceRequest &resourceRequest) const
{
    ...
    const OpParam &param, const TemplateDataParams &templateDataParams, TemplateResource &templateResource)
{
    ...
    // Calculate localCopyFlag by comparing input and output addresses
    uint64_t localCopyFlag = (inputAddrBase == outputAddrBase) ? 1 : 0;
    uint64_t offset = 0;
    if (localCopyFlag == 0) {
...
std::string CcuTempScatterOmniPipeMesh1D::Describe() const
{
    return StringFormat("CcuTempScatterOmniPipeMesh1D with templateRankSize [%u].", subCommRanks_[0].size());
}

HcclResult CcuTempScatterOmniPipeMesh1D::FastLaunch(const OpParam &param, const TemplateFastLaunchCtx& tempFastLaunchCtx)
{
    (void)param;
