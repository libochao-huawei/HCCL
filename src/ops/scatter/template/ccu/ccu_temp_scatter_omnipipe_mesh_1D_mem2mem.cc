...
{
    HCCL_DEBUG("[%s] myRank[%u] mySubCommRank_[%u] start", __func__, myRank_, mySubCommRank_);
    ...
    if (localCopyFlag == 0) {
        ...
        if (stepSliceInfo.inputOmniPipeSliceStride.size() == 1) {
            repeatNum = stepSliceInfo.inputOmniPipeSliceStride[0].size();
        } else {
            repeatNum = stepSliceInfo.inputOmniPipeSliceStride[mySubCommRank_].size();
        }

        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            if (stepSliceInfo.stepSliceSize.size() == 1) {
                sliceSize = stepSliceInfo.stepSliceSize[0][rpt];
                inputOmniPipeSliceStride = stepSliceInfo.inputOmniPipeSliceStride[0][rpt];
                outputOmniPipeSliceStride= stepSliceInfo.outputOmniPipeSliceStride[0][rpt];
            } else {
            ...
            HCCL_DEBUG("[%s] myRank[%u] mySubCommRank[%u] subCommRootId_[%u] rpt[%u] sliceSize[%llu] "
                       "inputOmniPipeSliceStride[%llu] "
                       "outputOmniPipeSliceStride[%llu] isStepone[%d] isLastStep[%d]",
                __func__, myRank_, mySubCommRank_, subCommRootId_, rpt, sliceSize, inputOmniPipeSliceStride,
                outputOmniPipeSliceStride, isStepOne_, isLastStep_);
