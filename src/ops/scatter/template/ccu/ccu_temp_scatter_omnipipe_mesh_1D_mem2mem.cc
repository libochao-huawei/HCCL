    ...
    const TopoInfoWithNetLayerDetails *topoInfo, AlgResourceRequest &resourceRequest)
{
    ...
    kernelInfo.kernelArg = std::make_shared<CcuKernelArgScatterOmniPipeMesh1DMem2Mem>(
        subCommRanks_[0].size(), mySubCommRank_, subCommRootId_, param, subCommRanks_, ifRealRoot_);
    ...
    const OpParam &param, const TemplateDataParams &templateDataParams, TemplateResource &templateResource)
{
    ...
    if (localCopyFlag == 0) {
        ...
        HCCL_DEBUG("[%s] myRank[%u] mySubCommRank[%u] repeatNum[%u] inputAddr[%llu] outputAddr[%llu] scratchAddr[%llu], isStepOne[%d], isLastStep[%d]", 
            __func__, myRank_, mySubCommRank_, repeatNum, inputAddr, outputAddr, scratchAddr, isStepOne_, isLastStep_);
