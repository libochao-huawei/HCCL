    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    // 计算loop相关信息
    u64 maxCountPerLoop = param.DataDes.count / dataTypeSize_;
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        for (auto i = 0; i < innerServerStepNum; ++i) {
            ...
            // 这里相当先给x和y维度的模板参数赋下初值，后面会根据具体的步骤和节点角色重新赋值
            ...
            // - 同y轴非root节点：往x轴方向转发数据（mesh算法）
                ...
                if (isRoot) {
                    CHK_RET(GenTempAlgParamsIn2OUT(
                        tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx, param));
                    CHK_RET(GenTempAlgParamsIn2OUT(
                        tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx, param));
                }
                ...
                if (isSameXAxisAsRoot) {
                    ...
                    algTempY.isSameAxisAsRoot_ = true;
                    algTempY.SetRoot(myRank_);
                    CHK_RET(GenTempAlgParamsHCCLBuff2OUT(
                        tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx, param));
                }
                ...
                if (isSameYAxisAsRoot) {
                    HCCL_DEBUG("[%s] set myRank[%u] as root", __func__, myRank_);
                    algTempX.isSameAxisAsRoot_ = true;
            ...
            // 执行X维度通信
            algTempX.currentStep_ = i;
            algTempX.totalStep_ = innerServerStepNum - 1;
            CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
            // 执行Y维度通信
            algTempY.currentStep_ = i;
            algTempY.totalStep_ = innerServerStepNum - 1;
...
REGISTER_EXECUTOR_BY_TWO_TEMPS(HcclCMDType::HCCL_CMD_SCATTER, CcuV2ScatterOmniPipe, CcuV2ScatterOmniPipeExecutor,
    TopoMatchUBX, CcuTempScatterOmniPipeMesh1DMem2Mem, CcuTempScatterOmniPipeMesh1DMem2Mem);
