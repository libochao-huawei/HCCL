    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    maxCountPerLoop = dataSize_/dataTypeSize_;
    ...
    HCCL_INFO("[%s] before call CalcScatterOmniPipeSliceInfo", __func__);
    alignSliceInfo = CalcScatterOmniPipeSliceInfo(sliceParam, param.root);
    HCCL_INFO("[%s] after call CalcScatterOmniPipeSliceInfo", __func__);
