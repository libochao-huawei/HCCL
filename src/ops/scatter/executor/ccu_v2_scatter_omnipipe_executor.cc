    ...
    const AlgHierarchyInfoForAllLevel &algHierarchyInfo)
{
    ...
    isSameXAxisAsRoot = (rankIdxLevel0_ == rootx) && !isRoot;
    isSameYAxisAsRoot = (rankIdxLevel1_ == rooty) && !isRoot;
...
template <typename AlgTopoMatch, typename InsAlgTempLevel0, typename InsAlgTempLevel1>
HcclResult CcuV2ScatterOmniPipeExecutor<AlgTopoMatch, InsAlgTempLevel0, InsAlgTempLevel1>::GenTemplateAlgParamsByStep(
    TemplateDataParams &tempAlgParams, StepSliceInfo &stepSliceInfo, u64 processedDataCount, const AlgResourceCtxSerializable &resCtx)
{
    ...
    stepSliceInfo.buffInfo.hcclBuff = resCtx.hcclBuff;
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        for (auto i = 0; i < innerServerStepNum; ++i) {
            ...
            if (i == 0 && isRoot) {
                ...
                CHK_RET(GenTemplateAlgParamsByStep1(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));

                // 执行X维度通信
                CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                
                // 生成Y维度（快轴）的template参数
                CHK_RET(GenTemplateAlgParamsByStep1(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx));
            ...
            // - 同y轴非root节点：往x轴方向转发数据（mesh算法）
                ...
                if (isRoot) {
                    CHK_RET(GenTemplateAlgParamsByStep3(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));
                    CHK_RET(GenTemplateAlgParamsByStep3(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx));
                    CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                    CHK_RET(algTempY.KernelRun(param, tempAlgParamsY, templateResourceY));
                }
                ...
                if (isSameXAxisAsRoot) {
                    ...
                    algTempY.SetRoot(newRootX);
                    CHK_RET(GenTemplateAlgParamsByStep3Slow(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx));
                    CHK_RET(algTempY.KernelRun(param, tempAlgParamsY, templateResourceY));
                }
                ...
                if (isSameYAxisAsRoot) {
                    ...
                    algTempX.SetRoot(newRootY);
                    CHK_RET(GenTemplateAlgParamsByStep3Fast(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));
                    CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                }
            }
            // 中间步骤：逐步转发阶段
            ...
            // - 同y轴非root节点：往x轴方向发送部分转发数据
                ...
                if (isRoot) {
                    CHK_RET(GenTemplateAlgParamsByStep2(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));
                    CHK_RET(GenTemplateAlgParamsByStep2(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx));
                    CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                    CHK_RET(algTempY.KernelRun(param, tempAlgParamsY, templateResourceY));
                }
...
REGISTER_EXECUTOR_BY_TWO_TEMPS(HcclCMDType::HCCL_CMD_SCATTER, CcuV2ScatterOmniPipe, CcuV2ScatterOmniPipeExecutor, TopoMatchUBX, CcuTempScatterOmniPipeMesh1DMem2Mem, CcuTempScatterOmniPipeNHR1DMem2Mem);
