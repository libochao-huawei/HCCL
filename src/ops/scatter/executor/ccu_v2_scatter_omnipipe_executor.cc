...
template <typename AlgTopoMatch, typename InsAlgTempLevel0, typename InsAlgTempLevel1>
HcclResult CcuV2ScatterOmniPipeExecutor<AlgTopoMatch, InsAlgTempLevel0, InsAlgTempLevel1>::GenTempAlgParamsIn2HCCL(
...
template <typename AlgTopoMatch, typename InsAlgTempLevel0, typename InsAlgTempLevel1>
HcclResult CcuV2ScatterOmniPipeExecutor<AlgTopoMatch, InsAlgTempLevel0, InsAlgTempLevel1>::GenTempAlgParamsHCCL2OUT(
    TemplateDataParams &tempAlgParams, StepSliceInfo &stepSliceInfo, u64 processedDataCount, const AlgResourceCtxSerializable &resCtx)
{
    tempAlgParams.count = 0;
    stepSliceInfo.buffInfo.hcclBuff = resCtx.cclMem;
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        for (auto i = 0; i < innerServerStepNum; ++i) {
            ...
            if (i == 0 && isRoot) {
                ...
                CHK_RET(GenTempAlgParamsIn2HCCL(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));

                // 执行X维度通信
                CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                
                // 生成Y维度（快轴）的template参数
                CHK_RET(GenTempAlgParamsIn2HCCL(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx));
            ...
            // - 同y轴非root节点：往x轴方向转发数据（mesh算法）
                ...
                if (isRoot) {
                    CHK_RET(GenTempAlgParamsIn2OUT(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));
                    CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                    CHK_RET(GenTempAlgParamsIn2OUT(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx)); 
                    CHK_RET(algTempY.KernelRun(param, tempAlgParamsY, templateResourceY));
                }
                ...
                if (isSameXAxisAsRoot) {
                    ...
                    CHK_RET(GenTempAlgParamsHCCL2OUT(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx));
                    CHK_RET(algTempY.KernelRun(param, tempAlgParamsY, templateResourceY));
                }
                ...
                if (isSameYAxisAsRoot) {
                    ...
                    CHK_RET(GenTempAlgParamsHCCL2OUT(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));
                    CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                }
            }
            // 中间步骤：逐步转发阶段
