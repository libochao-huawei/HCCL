    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        for (auto i = 0; i < innerServerStepNum; ++i) {
            ...
            if (i == 0 && isRoot) {
                ...
                CHK_RET(GenTempAlgParamsIn2HCCLBuff(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));

                // 执行X维度通信
                CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                
                // 生成Y维度（快轴）的template参数
                CHK_RET(GenTempAlgParamsIn2HCCLBuff(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx));
            ...
            // - 同y轴非root节点：往x轴方向转发数据（mesh算法）
                ...
                if (isSameXAxisAsRoot) {
                    ...
                    CHK_RET(GenTempAlgParamsHCCLBuff2OUT(tempAlgParamsY, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx));
                    CHK_RET(algTempY.KernelRun(param, tempAlgParamsY, templateResourceY));
                }
                ...
                if (isSameYAxisAsRoot) {
                    ...
                    CHK_RET(GenTempAlgParamsHCCLBuff2OUT(tempAlgParamsX, currentSliceInfo.dataSliceLevel0[i], processedDataCount, resCtx));
                    CHK_RET(algTempX.KernelRun(param, tempAlgParamsX, templateResourceX));
                }
            }
            // 中间步骤：逐步转发阶段
