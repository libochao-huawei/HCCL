...
#include "op_common/template/wrapper/alg_data_trans_wrapper.h"
namespace ops_hccl {
...
HcclResult CcuV2ScatterOmniPipeExecutor<M, X, Y>::GenTemplateAlgParamsByDimData(
...
// 主执行函数
template <typename M, typename X, typename Y>
HcclResult CcuV2ScatterOmniPipeExecutor<M, X, Y>::OrchestrateLoop(const OpParam &param, 
    const AlgResourceCtxSerializable &resCtx)
{
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_SCATTER,
                                CcuV2ScatterOmniPipe,
                                CcuV2ScatterOmniPipeExecutor,
                                TopoMatchMultilevel,
                                CcuTempScatterOmniPipeMesh1D,
                                CcuTempScatterOmniPipeMesh1D);
