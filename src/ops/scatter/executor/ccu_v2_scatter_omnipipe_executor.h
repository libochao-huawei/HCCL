...
#include "template/utils/template_utils.h" // for stepSliceInfo

namespace ops_hccl {

template <typename AlgTopoMatch, typename InsAlgTempLevel0, typename InsAlgTempLevel1>
class CcuV2ScatterOmniPipeExecutor : public ScatterExecutorBase {
public:
    CcuV2ScatterOmniPipeExecutor();
    ~CcuV2ScatterOmniPipeExecutor() override;
    ...
    HcclResult RestoreChannelMap(const AlgResourceCtxSerializable &resCtx,
        std::vector<std::map<u32, std::vector<ChannelInfo>>> &rankIdToChannelInfo) override;
...
protected:
    ...
    // 线程管理
    std::vector<std::thread> level0Threads_;
    std::vector<std::thread> level1Threads_;
    std::thread controlThread_;
    std::vector<std::thread> templateMainThreads_;
    std::vector<std::thread> templateLocalCopyThreads_;
