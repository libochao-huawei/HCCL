            ...
            for (u32 stepIdx = 0; stepIdx < 2; stepIdx++) {
                ...
                if (loopTimes == 1 && param_.engine == CommEngine::COMM_ENGINE_CCU) {
                    if (stageIdx == 0 && stepIdx == 0) {
                        HCCL_INFO("[ReduceParallelExecutor][OrchestrateLoop] tempAlgResArr_.at(0), tempAlgResArr_.at(1)", tempAlgResArr_.at(0).submitInfos.size(), tempAlgResArr_.at(1).submitInfos.size());
                        ...
                        HCCL_INFO("[ReduceParallelExecutor][OrchestrateLoop] ccuKernelLaunchNumInter0_, ccuKernelLaunchNumIntra1_", tempAlgResArr_.at(1).submitInfos.size() - ccuKernelLaunchNumInter1_, tempAlgResArr_.at(0).submitInfos.size() - ccuKernelLaunchNumIntra0_);
                        ...
                        HCCL_INFO("[ReduceParallelExecutor][OrchestrateLoop] ccuKernelLaunchNumInter01_, ccuKernelLaunchNumIntra11_", tempAlgResArr_.at(3).submitInfos.size(), tempAlgResArr_.at(2).submitInfos.size());
