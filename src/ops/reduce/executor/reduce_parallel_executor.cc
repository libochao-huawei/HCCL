...
HcclResult ReduceParallelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::FastLaunchSaveCtx(
	 const OpParam &param, const TemplateResource &templateAlgResIntra, const TemplateResource &templateAlgResInter)
{
	 HCCL_INFO("[ReduceParallelExecutor] loopTimes==1, save fast launch ctx.");
	 ...
	 if (ccuKernelNum < 1) {
	     HCCL_INFO("[ReduceParallelExecutor] ccu kernel num is 0, no need to save.");
	     return HCCL_SUCCESS;
	 }
	 HCCL_INFO("[ReduceParallelExecutor][HcclEngineCtxCreate] threadNum[%llu], ccuKernelNum[%llu]", threadNum, ccuKernelNum);
	 u64 size = CcuFastLaunchCtx::GetCtxSize(threadNum, ccuKernelNum);
	 // 申请ctx
	 void *ctxPtr = nullptr;
	 HCCL_INFO("[ReduceParallelExecutor][HcclEngineCtxCreate] Tag[%s], size[%llu]", param.fastLaunchTag, size);
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
HcclResult ReduceParallelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::FastLaunch(
