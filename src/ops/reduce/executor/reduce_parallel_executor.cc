            ...
            for (u32 stepIdx = 0; stepIdx < 2; stepIdx++) {
                ...
                if (loopTimes == 1 && param_.engine == CommEngine::COMM_ENGINE_CCU) {
                    if (stageIdx == 0 && stepIdx == 0) {
                        HCCL_INFO("[ReduceParallelExecutor][OrchestrateLoop] tempAlgResArr_.at(0)[%llu], tempAlgResArr_.at(1)[%llu]", tempAlgResArr_.at(0).submitInfos.size(), tempAlgResArr_.at(1).submitInfos.size());
                        ccuKernelLaunchNumIntra0_ = tempAlgResArr_.at(0).submitInfos.size();
                        ccuKernelLaunchNumInter1_ = tempAlgResArr_.at(1).submitInfos.size();
                    } else if (stageIdx == 0 && stepIdx == 1) {
                        HCCL_INFO("[ReduceParallelExecutor][OrchestrateLoop] ccuKernelLaunchNumInter0_[%llu], ccuKernelLaunchNumIntra1_[%llu]", tempAlgResArr_.at(1).submitInfos.size() - ccuKernelLaunchNumInter1_, tempAlgResArr_.at(0).submitInfos.size() - ccuKernelLaunchNumIntra0_);
                        ccuKernelLaunchNumInter0_ = tempAlgResArr_.at(1).submitInfos.size() - ccuKernelLaunchNumInter1_;
                        ccuKernelLaunchNumIntra1_ = tempAlgResArr_.at(0).submitInfos.size() - ccuKernelLaunchNumIntra0_;
                    } else if (stageIdx == 1 && stepIdx == 0) {
                        HCCL_INFO("[ReduceParallelExecutor][OrchestrateLoop] ccuKernelLaunchNumInter01_[%llu], ccuKernelLaunchNumIntra11_[%llu]", tempAlgResArr_.at(3).submitInfos.size(), tempAlgResArr_.at(2).submitInfos.size());
                        ccuKernelLaunchNumInter01_ = tempAlgResArr_.at(3).submitInfos.size();
                        ccuKernelLaunchNumIntra11_ = tempAlgResArr_.at(2).submitInfos.size();
    ...
    u32 dataSliceIdx, u32 stageIdx, u32 stepIdx, bool isInter)
{
    ...
    //TemplateResource tempAlgRes = tempAlgResArr_.at(
    //    stageIdx == 0
    //        ? (stepIdx == dataSliceIdx ? 0 : 1)
    //        : (stepIdx == dataSliceIdx ? 3 : 2)
    //);
...
HcclResult ReduceParallelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::FastLaunchSaveCtx()
{
    HCCL_INFO("[ReduceParallelExecutor][FastLaunchSaveCtx] loopTimes==1, save fast launch ctx.");
    ccuKernelLaunchNumIntra01_ = tempAlgResArr_.at(2).submitInfos.size() - ccuKernelLaunchNumIntra11_;
    ccuKernelLaunchNumInter11_ = tempAlgResArr_.at(3).submitInfos.size() - ccuKernelLaunchNumInter01_;
    HCCL_INFO("[ReduceParallelExecutor][FastLaunchSaveCtx] ccuKernelLaunchNumIntra1_[%llu], ccuKernelLaunchNumInter0_[%llu]", ccuKernelLaunchNumIntra1_, ccuKernelLaunchNumInter0_);
    HCCL_INFO("[ReduceParallelExecutor][FastLaunchSaveCtx] ccuKernelLaunchNumIntra0_[%llu], ccuKernelLaunchNumInter1_[%llu]", ccuKernelLaunchNumIntra0_, ccuKernelLaunchNumInter1_);
    HCCL_INFO("[ReduceParallelExecutor][FastLaunchSaveCtx] ccuKernelLaunchNumIntra11_[%llu], ccuKernelLaunchNumInter01_[%llu]", ccuKernelLaunchNumIntra11_, ccuKernelLaunchNumInter01_);
    HCCL_INFO("[ReduceParallelExecutor][FastLaunchSaveCtx] ccuKernelLaunchNumIntra01_[%llu], ccuKernelLaunchNumInter11_[%llu]", ccuKernelLaunchNumIntra01_, ccuKernelLaunchNumInter11_);
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2, typename InsAlgTemplate3>
HcclResult ReduceParallelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::FastLaunch(
        const OpParam &param, const CcuFastLaunchCtx *ctx)
{
    ...
    HCCL_INFO("[ReduceParallelExecutor][FastLaunch] start PrepareResForStage";
    PrepareResForStage(0);
    HCCL_INFO("[ReduceParallelExecutor][FastLaunch] end PrepareResForStage";
    ...
    if (ctx->ccuKernelNum[4] > 0) {
        ...
        CHK_RET(interTempAlg1.FastLaunch(param, tempFastLaunchCtxInter01));
    }
    ...
    if (ctx->ccuKernelNum[6] > 0) {
        ...
        CHK_RET(intraTempAlg1.FastLaunch(param, tempFastLaunchCtxIntra01));
    }
