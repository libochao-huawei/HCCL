...
#elif T_DESC("测试拓扑1*2", false)
    if (userrank == 0) {
        algHierarchyInfo.infos = {{{0}, {0, 1}}};
    } else {
        algHierarchyInfo.infos = {{{1}, {0, 1}}};
    }
#elif T_DESC("测试拓扑2*3", false)
    if (userrank == 0 || userrank == 1) {
        algHierarchyInfo.infos = {{{0, 1}, {0, 1, 2, 3, 4, 5}}};
    } else if (userrank == 2 || userrank == 3) {
        algHierarchyInfo.infos = {{{2, 3}, {0, 1, 2, 3, 4, 5}}};
    } else {
        algHierarchyInfo.infos = {{{4, 5}, {0, 1, 2, 3, 4, 5}}};
    }
#elif T_DESC("测试拓扑3*2", false)
    if (userrank == 0 || userrank == 1 || userrank == 2) {
        algHierarchyInfo.infos = {{{0, 1, 2}, {0, 1, 2, 3, 4, 5}}};
    } else if (userrank == 3 || userrank == 4 || userrank == 5) {
        algHierarchyInfo.infos = {{{3, 4, 5}, {0, 1, 2, 3, 4, 5}}};
    }
...
{
    HCCL_INFO("[InitSubCommRanks] start1");
    subCommRanks0.clear();
    subCommRanks1.clear();
    ...
    for (auto i = 0;i<subCommRanks0[0].size();++i){
        HCCL_INFO("[InitSubCommRanks0] rankid:[%d]", subCommRanks0[0][i]);
    }
    for (auto i = 0;i<subCommRanks1[0].size();++i){
        HCCL_INFO("[InitSubCommRanks1] rankid:[%d]", subCommRanks1[0][i]);
    }
    HCCL_INFO("[InitSubCommRanks] END1");
    return HCCL_SUCCESS;
}
...
//         HCCL_INFO("[InitSubCommRanks0] rankid:[%d]", subCommRanks0[0][i]);
//     }
//     HCCL_INFO("[InitSubCommRanks] rankSizeLevel0_[%d]", rankSizeLevel0_);
//     HCCL_INFO("[InitSubCommRanks] size[%d]", algHierarchyInfo.infos[0][1].size());
//     for (auto i = myRank_ % rankSizeLevel0_; i < algHierarchyInfo.infos[0][1].size(); i += rankSizeLevel0_) {
//         HCCL_INFO("[InitSubCommRanks] 231, i:[%d]", i);
//         HCCL_INFO("[InitSubCommRanks] 231, first[%d] rootYAixs:[%d] second:[%d] myRank_:[%d]  root_:[%d]", algHierarchyInfo.infos[0][1][i] / rankSizeLevel0_,rootYAixs,algHierarchyInfo.infos[0][1][i], myRank_, root_);
//         if (algHierarchyInfo.infos[0][1][i] / rankSizeLevel0_ == rootXAixs || algHierarchyInfo.infos[0][1][i] == myRank_ ||  algHierarchyInfo.infos[0][1][i] != root_) { // TODO:zq 只把当前节点和对端节点加入通信域
//              HCCL_INFO("[InitSubCommRanks] 236, i:[%d]", i);
//             subCommRanks1[0].push_back(algHierarchyInfo.infos[0][1][i]);
//         }
//     }
//     // if (subCommRanks1[0].size() == 1){
//     //     subCommRanks1[0].clear();
//     // }
//     for (auto i = 0;i<subCommRanks1[0].size();++i){
//         HCCL_INFO("[InitSubCommRanks1] rankid:[%d]", subCommRanks1[0][i]);
//     }
//     HCCL_INFO("[InitSubCommRanks] END");
//     HCCL_INFO("size0:[%d] size1:[%d]",subCommRanks0.size(), subCommRanks1.size());
//     return HCCL_SUCCESS;
// }
    ...
    AlgResourceRequest& resourceRequest)
{
    ...
    resourceRequest.slaveThreadNum += 1; // 需要一个主流和一个从流来并行2d   用于同步 多少个从流就需要多少个主流的notify
    resourceRequest.notifyNumOnMainThread += 1; //RSX在主流，RSY在从流
    ...
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    // threads_ = resCtx.threads; // 两条流，两个线程，一个主线程一个子线程
    // // [jjy][todo] 这里没有RestoreChannelMap，会不会有问题？应该没有？
    // // CHK_RET(RestoreChannelMap(resCtx, remoteRankToChannelInfo_));

    // 算法展开
    // HCCL_INFO("[jjy][Reduce]dataSize_[%d], inputSize[%d], outputSize[%d]", dataSize_, param.inputSize, param.outputSize);
            ...
            std::map<u32, TemplateDataParams>& tempAlgParamMap)
{
    ...
    u32 kernelOffset = 0;
    for (int level = 0; level < OMNIPIPE_AR_LEVEL_NUM; level++) { //对4个template挨个初始化参数
        if (tempMap.count(level) > 0) {
            if (level < OMNIPIPE_AG_LEVEL0) {
                // [RS-level0, RS-level1]
            ...
            TemplateDataParams &tempAlgParams, StepSliceInfo &stepSliceInfo, u64 processedDataCount)
{
    ...
    // tempAlgParams.stepSliceInfo.buffInfo.inBuffBaseOff = stepSliceInfo.buffInfo.inBuffBaseOff + processedDataCount * dataTypeSize_;
    // tempAlgParams.stepSliceInfo.buffInfo.outBuffBaseOff = stepSliceInfo.buffInfo.outBuffBaseOff + processedDataCount * dataTypeSize_;
            ...
            const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    CHK_RET(InitSubCommRanks(subCommRanks0, subCommRanks1, algHierarchyInfo_local)); // TODO: zq 在这里初始化子通信域的时候处理一下对端节点？
    ...
    auto allRankSplitData = OmniPipeSplitData(rankSize_, dataCount_, dataTypeSize_);
     HCCL_INFO("[jjy]rankSize_[%d], dataCount_[%d]:%d", rankSize_, dataCount_, dataTypeSize_);
    ...
    u64 maxCountPerLoop = static_cast<u64>(256) / dataTypeSize_;  // 2048 4卡  512
    u32 loopTimes = allRankSplitData[0] / maxCountPerLoop + ((allRankSplitData[0] % maxCountPerLoop == 0) ? 0 : 1); //总的需要传输的数据量 / UB限制 //2次
    ...
    std::vector<u64> dataWholeSize(rankSize_, allRankSplitData[myRank_] * dataTypeSize_);//注意的参数
    OmniPipeSliceParam sliceParam;
    sliceParam.dataSizePerLoop = dataSizePerLoop;
    // sliceParam.dataSizePerLoop = dataWholeSize;
    sliceParam.dataWholeSize = dataWholeSize;
    // sliceParam.endpointAttrBw =  {1, 1, 1};
    sliceParam.endpointAttrBw =  {3, 4, 1};
    sliceParam.levelRankId = {rankIdxLevel0_, rankIdxLevel1_, 0};
    HCCL_INFO("[jjy]rankIdxLevel0_=%d rankIdxLevel1_=%d ", rankIdxLevel0_, rankIdxLevel1_ );
    sliceParam.levelRankSize = {rankSizeLevel0_, rankSizeLevel1_, 1};
    HCCL_INFO("[jjy]rankSizeLevel0_=%d rankSizeLevel1_=%d dataTypeSize_=%d ", rankSizeLevel0_, rankSizeLevel1_,dataTypeSize_ );
    ...
    OmniPipeSliceInfo alignSliceInfoG = CalcGatherOmniPipeSliceInfo(sliceParam);
    
    HCCL_INFO("[zq]align dataSizePerLoop=%d dataSizePerLoop=%d dataSizePerLoop:%d dataSizePerLoop:%d size:%d", sliceParam.dataSizePerLoop[0], sliceParam.dataSizePerLoop[1], sliceParam.dataSizePerLoop[2], sliceParam.dataSizePerLoop[3], sliceParam.dataSizePerLoop.size() );
    HCCL_INFO("[zq]dataWholeSize=%d dataWholeSize=%d dataWholeSize:%d dataWholeSize:%d size:%d", sliceParam.dataWholeSize[0], sliceParam.dataWholeSize[1], sliceParam.dataWholeSize[2], sliceParam.dataWholeSize[3], sliceParam.dataWholeSize.size() );
    ...
    OmniPipeSliceInfo tailSliceInfoG;
    if (allRankSplitData[0] % maxCountPerLoop != 0) {
        ...
        sliceParam.dataWholeSize = dataWholeSize;
        HCCL_INFO("[zq]tail dataSizePerLoop=%d dataSizePerLoop=%d dataSizePerLoop:%d dataSizePerLoop:%d size:%d", sliceParam.dataSizePerLoop[0], sliceParam.dataSizePerLoop[1], sliceParam.dataSizePerLoop[2], sliceParam.dataSizePerLoop[3], sliceParam.dataSizePerLoop.size() );
        tailSliceInfoRS = CalcRSOmniPipeSliceInfo(sliceParam);
        tailSliceInfoG = CalcGatherOmniPipeSliceInfo(sliceParam);
    }

    // 4 进行一次loop的数据处理，开始下发template
    u64 processedDataCount = 0; // TODO:干啥用的？
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {//loopTimes
        ...
        HCCL_INFO("[zq]new loop=%d dataSizePerLoop=%d dataSizePerLoop=%d dataSizePerLoop:%d dataSizePerLoop:%d size:%d", loop,sliceParam.dataSizePerLoop[0], sliceParam.dataSizePerLoop[1], sliceParam.dataSizePerLoop[2], sliceParam.dataSizePerLoop[3], sliceParam.dataSizePerLoop.size() );
        HCCL_INFO("[zq]new loop=%d dataWholeSize=%d dataWholeSize=%d dataWholeSize:%d dataWholeSize:%d size:%d", loop,sliceParam.dataWholeSize[0], sliceParam.dataWholeSize[1], sliceParam.dataWholeSize[2], sliceParam.dataWholeSize[3], sliceParam.dataWholeSize.size() );
        ...
        for(int i=0;i<omniPipeSliceInfoG.dataSliceLevel0.size();i++){
            for(int j=0;j<omniPipeSliceInfoG.dataSliceLevel0[i].inputOmniPipeSliceStride.size();j++){
                for(int k=0;k<omniPipeSliceInfoG.dataSliceLevel0[i].inputOmniPipeSliceStride[j].size();k++){
                    HCCL_INFO("[jjy][dataSliceLevel0]loop[%i]  myRank[%d][inputOmniPipeSliceStride][omniPipeSliceInfoG][%d][%d][%d]:%d SliceSize[%d] ",loop, myRank_,i,j,k,omniPipeSliceInfoG.dataSliceLevel0[i].inputOmniPipeSliceStride[j][k] , omniPipeSliceInfoG.dataSliceLevel0[i].stepSliceSize[j][k]);
                }
            } 
        }
        for(int i=0;i<omniPipeSliceInfoG.dataSliceLevel1.size();i++){
            for(int j=0;j<omniPipeSliceInfoG.dataSliceLevel1[i].inputOmniPipeSliceStride.size();j++){
                for(int k=0;k<omniPipeSliceInfoG.dataSliceLevel1[i].inputOmniPipeSliceStride[j].size();k++){
                    HCCL_INFO("[jjy][dataSliceLevel1]loop[%i]  myRank[%d][inputOmniPipeSliceStride][omniPipeSliceInfoG][%d][%d][%d]:%d SliceSize[%d] ",loop,myRank_,i,j,k,omniPipeSliceInfoG.dataSliceLevel1[i].inputOmniPipeSliceStride[j][k] , omniPipeSliceInfoG.dataSliceLevel1[i].stepSliceSize[j][k]);
                }
            } 
        }

        
        u64 currDataCount = multiLoopAllRankSplitData[loop][myRank_]; // 256B
        ...
        for (auto i = 0; i < level0StepCountRS; ++i) { // op_common计算的通信步数
            // 第一步开始前同步
            CHK_RET(PreSyncInterThreads(mainThread, syncThreads, notifyIdxesMainToSub)); // 主流向从流编号为notifyIdxesMainToSub的位置置位
            ...
            // 第一步做完后回到主流做尾同步
            CHK_RET(PostSyncInterThreads(mainThread, syncThreads, notifyIdxesSubToMain)); // 从流向主流编号为notifyIdxesSubToMain的位置发送置位
        }
        ...
        // 4.5 GATHER for内层2d
        u32 level0StepCountAG = omniPipeSliceInfoG.dataSliceLevel0.size(); // 3步
        ...
        for (u32 i = 0; i < level0StepCountAG; i++) {
            ...
            // 第一步：1. root的同轴节点发送数据，root接收 usrOut->usrOut
            //         2. 斜对角节点发送数据到root节点的同轴节点 usrOut->cclBuff
            // 最后一步：只有root的同轴节点进行发送，只有root节点接收 cclBuff->usrOut
            // 其他步: 1. x轴: root的同轴节点发送数据，root接收 usrOut->usrOut
            //         2. x轴: 斜对角节点发送数据到root节点的同轴节点 usrOut->cclBuff
            //         3. y轴: 只有root的同轴节点进行发送，只有root节点接收 cclBuff->usrOut

            // ----------------第1步----------------
            // 如果当前卡是root的同x轴节点  nhr usrOut->usrOut
            // 如果当前卡是root的同y轴节点  mesh usrOut->usrOut
            // 如果当前卡是斜对角节点 nhr+mesh usrOut->ccl
            tempResMap[OMNIPIPE_AG_LEVEL0].threads.clear();
            tempResMap[OMNIPIPE_AG_LEVEL0].threads.emplace_back(threads_[0]);
            ...
            if (i == 0) {
                ...
                if (isSameXAxis && !isRoot) { // 0,2
                    HCCL_INFO("[%s][isSameXAxis] myRank_[%d] 0.", __func__, myRank_);
                    tempAlgParamMap[OMNIPIPE_AG_LEVEL0].subRoot = rankIdxLevel0_;//0
                    tempAlgParamMap[OMNIPIPE_AG_LEVEL1].subRoot = rootYAixs;//0
                }

                if (isSameYAxis && !isRoot) { // 0,1;
                ...
                if (!isSameXAxis && !isSameYAxis && !isRoot){ // 3
                    ...
                    tempAlgParamMap[OMNIPIPE_AG_LEVEL1].subRoot = GetXRoot();

                }
            }//  else if (i == level0StepCountAG - 1){
            //     tempAlgParamMap[OMNIPIPE_AG_LEVEL1].stepSliceInfo.stepSliceSize[0][0]=0;
            //     tempAlgParamMap[OMNIPIPE_AG_LEVEL1].stepSliceInfo.stepSliceSize[1][0]=0;
            //     tempAlgParamMap[OMNIPIPE_AG_LEVEL0].stepSliceInfo.stepSliceSize[0][0]=0;
            //     tempAlgParamMap[OMNIPIPE_AG_LEVEL0].stepSliceInfo.stepSliceSize[1][0]=0;
            // }
            else if (i == level0StepCountAG - 1) {
            ...
            // 如果当前卡是root的同y轴节点 mesh ccl->usrOut
                ...
                } else if (isSameYAxis && !isRoot) { // 0,1
            ...
            HCCL_INFO("[%s][KernelRun] tempAlgParamMap[OMNIPIPE_AG_LEVEL0].subRoot[%d].", __func__,tempAlgParamMap[OMNIPIPE_AG_LEVEL0].subRoot);
            HCCL_INFO("[%s][KernelRun] tempAlgParamMap[OMNIPIPE_AG_LEVEL1].subRoot[%d].", __func__,tempAlgParamMap[OMNIPIPE_AG_LEVEL1].subRoot);
    ...
    if (myRank_ == param.root) {
        ...
        tempAlgParamLocalCopy.count = dataCount_; // 128 512B
        tempAlgParamLocalCopy.sliceSize = dataCount_ *dataTypeSize_; // 128*4
        tempAlgParamLocalCopy.buffInfo.outBuffBaseOff = 0 ;//i * allRankSplitData[i]; // i * 512
        tempAlgParamLocalCopy.buffInfo.inBuffBaseOff = 0;// i * allRankSplitData[i];  // i * 512
        ...
        HCCL_DEBUG("[%s] myRank[%u] root[%d] localCopy inBuffBaseOff[%lu] outBuffBaseOff[%lu] sliceSize[%lu]", __func__,
        myRank_, param.root, tempAlgParamLocalCopy.buffInfo.inBuffBaseOff, tempAlgParamLocalCopy.buffInfo.outBuffBaseOff,
