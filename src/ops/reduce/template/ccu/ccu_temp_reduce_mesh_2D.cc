...
CcuTempReduceMesh2D::CcuTempReduceMesh2D(const OpParam& param, const u32 rankId,
...
HcclResult CcuTempReduceMesh2D::PartitionChannels(const std::vector<HcclChannelDesc>& channelDescs,
                                                        ...
                                                        std::vector<HcclChannelDesc>& channels_y)
{
    ...
    if (UNLIKELY(subCommRanks_.size() < AXIS_NUM_2)){
        HCCL_ERROR("[CcuTempReduceMesh2D::PartitionChannels] subCommRanks.size() [%d] < 2", subCommRanks_.size());
    ...
    for (u32 rankId: ranks_x) {
        ...
        if (it != rankIdToChannelDesc.end() && !it->second.empty()) {
            ...
            HCCL_ERROR("[CcuTempReduceMesh2D::PartitionChannels] rank[%d]: there's no channel to rank [%d]", myRank_, rankId);
            return HcclResult::HCCL_E_INTERNAL;
        }
    }
...
HcclResult CcuTempReduceMesh2D::CalcRes(HcclComm comm, const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
                                                      AlgResourceRequest& resourceRequest)
{
    ...
    HCCL_DEBUG("[CcuTempReduceMesh2D::CalcRes] notifyNumOnMainThread[%u] slaveThreadNum[%u]",
               resourceRequest.notifyNumOnMainThread, resourceRequest.slaveThreadNum);

    uint32_t dieNum = subCommRanks_.size();
    if (dieNum != 2) { // concurrmesh的topoMatch返回的vTopo大小应当为2，对应X轴和Y轴的大小
        HCCL_ERROR("[CcuTempReduceMesh2D] Rank[%d], Invalid IODieNum[%zu].", myRankId_, subCommRanks_.size());
    ...
    for (uint32_t axisId = 0; axisId < 2; axisId++) { // 2D算法，需要执行两次
        ...
        kernelInfo.creator = [](const hcomm::CcuKernelArg &arg) {
                                return std::make_unique<CcuKernelReduceMesh2D>(arg);
                            };
        
        kernelInfo.kernelArg = std::make_shared<CcuKernelArgReduceMesh2D>(dimSize_,
                                                                          myRankId_,
                                                                          rootId_,
                                                                          axisId,
                                                                          param,
        ...
        HCCL_DEBUG("[CcuTempReduceMesh2D::CalcRes] channelDescs.size()=%llu, dimsize=%llu, "
...
HcclResult CcuTempReduceMesh2D::KernelRun(const OpParam& param,
                                          ...
                                          const TemplateResource& templateResource)
{
    ...
    for (uint32_t axisId = 0; axisId < 2; axisId++) { // 2D算法，需要执行两次
        ...
        std::unique_ptr<hcomm::CcuTaskArg> taskArg = std::make_unique<CcuTaskArgReduceMesh2D>(
        inputAddr, outputAddr, sliceSize, xAxisSize, yAxisSize, offSet, token);

        HCCL_INFO("[CcuTempReduceMesh2D] Run Init: myRankId[%d], dimSize[%llu], inputAddr[%llu],"\
               "outputAddr[%llu], sliceSize[%llu], xAxisSize[%llu], yAxisSize[%llu], offset[%llu], axisId_[%u]",
            myRankId_, dimSize_[axisId], inputAddr, outputAddr, sliceSize, xAxisSize, yAxisSize, offSet, axisId);
    ...
    HCCL_DEBUG("[CcuTempReduceMesh2D::KernelRun] end");
