...
uint32_t CcuTempGatherOmniPipeMesh1DMem2MemY::RemoteRankId2RankId(const uint32_t remoteRankId) const
{
    ...
    if (topoInfo->level0Topo != Level0Shape::MESH_1D_CLOS) {
        ...
        for (auto channel : channelDescs) {
            if (channel.channelProtocol != COMM_PROTOCOL_UBC_CTP) {
                HCCL_ERROR("[CcuTempGatherOmniPipeMesh1DMem2MemY][%s] channel.channelProtocol[%u]", __func__, channel.channelProtocol);
