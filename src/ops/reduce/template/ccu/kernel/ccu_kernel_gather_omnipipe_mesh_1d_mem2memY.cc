...
{
    HCCL_INFO("[CcuKernelGatherOmniPipeMesh1DMem2MemY] STARTS");
    const CcuKernelArgGatherOmniPipeMesh1DMem2MemY* kernelArg =
        dynamic_cast<const CcuKernelArgGatherOmniPipeMesh1DMem2MemY*>(&arg);
    ...
    // HCCL_DEBUG("[CcuKernelGatherOmniPipeMesh1DMem2MemY] rankSize=%llu, rankIdx=%u, rootIdx=%u",
...
HcclResult CcuKernelGatherOmniPipeMesh1DMem2MemY::InitResource()
{
    HCCL_DEBUG("[%s] start", __func__);
    uint16_t channelIdx = 0;
    if (channels_.size() == 0) {
        HCCL_ERROR("[CcuKernelGatherOmniPipeMesh1DMem2MemY] channels is empty!");
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            HCCL_DEBUG("[CcuKernelGatherOmniPipeMesh1DMem2MemY] MyRank[%u], PeerId[%u], ChannelId[%u]",
...
void CcuKernelGatherOmniPipeMesh1DMem2MemY::LoadArgs()
{
    HCCL_DEBUG("[%s] start", __func__);
...
void CcuKernelGatherOmniPipeMesh1DMem2MemY::PreSync()
{
    HCCL_DEBUG("[CcuKernelGatherOmniPipeMesh1DMem2MemY::PreSync] start");
    ...
    HCCL_DEBUG("[CcuKernelGatherOmniPipeMesh1DMem2MemY::PreSync] end");
    return;
}

void CcuKernelGatherOmniPipeMesh1DMem2MemY::PostSync()
{
    HCCL_DEBUG("[CcuKernelGatherOmniPipeMesh1DMem2MemY::PostSync] start");
...
std::vector<uint64_t> CcuKernelGatherOmniPipeMesh1DMem2MemY::GeneArgs(const hcomm::CcuTaskArg& arg)
{
    const auto taskArg = dynamic_cast<const CcuTaskArgGatherOmniPipeMesh1DMem2MemY&>(arg);
    ...
    HCCL_DEBUG("[CcuKernelGatherOmniPipeMesh1DMem2MemY::GeneArgs] taskArg.inputAddr_[%ld] taskArg.outputAddr_[%ld] taskArg.token_[%ld] taskArg.localCopyFlag_[%ld] taskArg.sliceSize_[%ld] taskArg.inputOmniPipeSliceStride_[%ld] taskArg.outputOmniPipeSliceStride_[%ld]"
...
HcclResult CcuKernelGatherOmniPipeMesh1DMem2MemY::Algorithm()
{
...
void CcuKernelGatherOmniPipeMesh1DMem2MemY::DoGather()
{
    HCCL_DEBUG("[CcuKernelGatherOmniPipeMesh1DMem2MemY] DoGather, myRank[%u]", rankId_);
...
void CcuKernelGatherOmniPipeMesh1DMem2MemY::DoRepeatGather()
{
    ...
    for (uint64_t curId = 0; curId < rankSize_; curId++) {
        ...
        outputMem_[curId].addr = output_;
        outputMem_[curId].addr += outputOmniPipeSliceStride_;
    }

    CCU_IF (ifNewRoot_ == true) {
        HCCL_INFO(
                "[%s] DoRepeatGather local rank[%u], root rank[%u], do", __func__, rankId_, rootId_);
        DoGather();
    }
