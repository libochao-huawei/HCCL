...
{
    CCL_DEBUG("[CcuKernelReduceNHR1DMem2Mem] LoadArgs run start");
    ...
    CCL_DEBUG("[CcuKernelReduceNHR1DMem2Mem] LoadArgs run end");
    return CCU_SUCCESS;
}
...
static CcuResult InitResource(ReduceNHR1DMem2MemContext &ctx)
{
    ...
    ctx.output.resize(ctx.localSize);
...
{
    const auto *arg = ctx.arg;
    u32& toRankIdx = arg->rank2ChannelIdx[nhrStepInfo.toRank];
    u32& fromRankIdx = arg->rank2ChannelIdx[nhrStepInfo.fromRank];
    u32  sendSliceIdx = 0;
...
{
    const auto *arg = ctx.arg;
    ChannelHandle sendChannel = arg->channels[arg->rank2ChannelIdx[toRank]];
    bool          islastSlice;
    ...
    const CcuVariable &sliceSize = arg->axisId == 0 ? (islastSlice ? ctx.die0LastSliceSize : ctx.die0SliceSize)
...
static CcuResult DoSendRecvSlice(ReduceNHR1DMem2MemContext &ctx, const u32 &toRank, CcuRep::LocalAddr &src, CcuRep::RemoteAddr &dst,
                                               const u32 &sendSliceIdx, u32 signalIndex)
{
    ...
    islastSlice = (sendSliceIdx + 1 == arg->rankSize);
    const CcuRep::Variable &sliceSize = arg->axisId == 0? (islastSlice? ctx.die0LastSliceSize : ctx.die0SliceSize)
                                                    : (islastSlice? ctx.die1LastSliceSize : ctx.die1SliceSize);
    
    CCU_IF_ONLY(sliceSize != 0) {
        ctx.event.setMask(1 << signalIndex);
        WriteNb(sendChannel, dst, src, sliceSize, ctx.event);
    }
    CCU_IF_ONLY(sliceSize == 0) {
        ctx.event.setMask(1 << signalIndex);
        ccu::RecordEvent(ctx.event);
...
static CcuResult LocalCopySlices(ReduceNHR1DMem2MemContext &ctx)
{
    ...
    CCU_IF_ONLY(ctx.isInputOutputEqual == 0)
    {
        ...
        for (u32 i = 0; i < nonTxSliceIdxList.size(); i++) {
            ...
            DoLocalCopySlice(ctx, nonTxSliceIdx, i);
        }
...
static std::vector<u32> GetNonTxSliceIdxs(ReduceNHR1DMem2MemContext &ctx, const std::vector<u32> &txSliceIdxs) const
{
    const auto *arg = ctx.arg;
...
static void DoLocalCopySlice(ReduceNHR1DMem2MemContext &ctx, const u32 &copySliceIdx, u32 signalIndex)
{
    const auto *arg = ctx.arg;
    bool islastSlice;
    // 添加 die1 偏移
    ...
    CCU_IF_ONLY(sliceSize != 0) {
        ctx.event.setMask(1 << signalIndex);
        LocalCopyNb(ctx.localDst, ctx.localSrc, sliceSize, ctx.event);
    }
    CCU_IF_ONLY(sliceSize == 0) {
        ctx.event.setMask(1 << signalIndex);
        ccu::RecordEvent(ctx.event);
