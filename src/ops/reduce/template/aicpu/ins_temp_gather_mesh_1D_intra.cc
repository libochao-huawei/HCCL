...
HcclResult InsTempGatherMesh1dIntra::CalcRes(HcclComm comm, const OpParam &param, const TopoInfoWithNetLayerDetails *topoInfo,
...
HcclResult InsTempGatherMesh1dIntra::GetRes(AlgResourceRequest &resourceRequest) const
{
...
u64 InsTempGatherMesh1dIntra::GetThreadNum() const
{
    return templateRankSize_ > 1 ? templateRankSize_ - 1: 1;
}

u64 InsTempGatherMesh1dIntra::CalcScratchMultiple(BufferType inBuffType, BufferType outBuffType)
{
    (void)inBuffType;
    (void)outBuffType;
    ...
    const TemplateResource &templateResource)
{
    ...
    HCCL_INFO("[InsTempGatherMesh1dIntra][KernelRun] threadNum:%u",threadNum_);
    tempAlgParams_ = tempAlgParams;

    count_ = tempAlgParams.count;
    dataType_ = param.DataDes.dataType;

    HCCL_DEBUG("[InsTempGatherMesh1dIntra] Rank [%d], get threadNum_[%d].", myRank_, threadNum_);
    ...
    CHK_RET(RunGatherMesh(templateResource.threads, templateResource.channels));
    ...
    HCCL_INFO("[InsTempGatherMesh1dIntra] Run End");
    return HcclResult::HCCL_SUCCESS;
}

HcclResult InsTempGatherMesh1dIntra::RunGatherMesh(const std::vector<ThreadHandle> &threads,
                                                    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    HCCL_INFO("[InsTempGatherMesh1dIntra] RunGatherMesh RankIDs[%d].", myRank_);
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        for (u32 threadIdx = 0; threadIdx < subCommRanks_[0].size() - 1; threadIdx++) {
            ...
            HCCL_INFO("[InsTempGatherMesh1dIntra] RunGatherMesh RankIDs[%d], connectedRank[%d], connectedAlgRank[%d].",
                      myRank_, connectedRank, connectedAlgRank);

            // 异常检查
            CHK_PRT_RET(threadIdx >= threads.size() || !channels.count(connectedRank),
                        HCCL_ERROR("[InsTempGatherMesh1dIntra][RankID]=%u threadIdx=%u, threads.size=%u, "
            ...
            if (myAlgSize == 0) { // 发送数据片为0时，只接收数据
                ...
                CHK_PRT_RET(RecvWrite(recvInfo, threads[threadIdx]),
                    HCCL_ERROR("[InsTempGatherMesh1dIntra] [RunGatherMesh] Recv failed."),
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_INFO("[InsTempGatherMesh1dIntra] [RunGatherMesh]RecvWrite from rank %u to rank %u "
                          "src offset %u, dst offset %u, "
                          "size %u",
                ...
                CHK_PRT_RET(SendWrite(sendInfo, threads[threadIdx]),
                    HCCL_ERROR("[InsTempGatherMesh1dIntra][RunGatherMesh] Send failed."),
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_INFO("[InsTempGatherMesh1dIntra] [RunGatherMesh]SendWrite from rank %u to rank %u "
                          "src offset %u, dst offset %u, "
                          "size %u",
                    ...
                    txDstOffset,
                    myAlgSize);
                HCCL_INFO("[InsTempGatherMesh1dIntra][RunGatherMesh]SendRecvWrite src addr %p, dst addr %p",
                    txSrcPtr,
                    txDstPtr);
            } else {
                ...
                CHK_PRT_RET(SendRecvWrite(sendRecvInfo, threads[threadIdx]),
                    HCCL_ERROR("[InsTempGatherMesh1dIntra][RunGatherMesh] Send failed"),
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_INFO("[InsTempGatherMesh1dIntra] [RunGatherMesh]SendRecvWrite from rank %u to rank %u "
                          "src offset %u, dst offset %u, "
                          "size %u",
                    myRank_, connectedRank,
                    txOutOffset,
...
HcclResult InsTempGatherMesh1dIntra::LocalDataCopy(const std::vector<ThreadHandle> &threads)
{
    HCCL_INFO("[InsTempGatherMesh1D] LocalDataCopy.");
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        HCCL_DEBUG("[InsTempGatherMesh1D][LocalDataCopy] RankID [%d] AlgRank [%d] srcSlice: inBaseOff[%d] inOff[%d] "
                   "sliceSize[%d] count[%d].",
                   myRank_, myAlgRank, inBaseOff, inOff, sliceSize, sliceCount);
        HCCL_DEBUG("[InsTempGatherMesh1D][LocalDataCopy] RankID [%d] AlgRank [%d] dstSlice: outBaseoff[%d] "
...
HcclResult InsTempGatherMesh1dIntra::PostLocalCopy(const std::vector<ThreadHandle> &threads)
{
    HCCL_INFO("[InsTempGatherMesh1dIntra] PostLocalCopy.");
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        for (auto rank : subCommRanks_[0]) {
            ...
            if (myRank_ != tempAlgParams_.root) {
                continue;
            }
            ...
            HCCL_DEBUG("[InsTempGatherMesh1D][PostLocalCopy] LocalDataCopy RankID [%d] dataRank [%d] dataAlgRank[%d] "
...
void InsTempGatherMesh1dIntra::GetNotifyIdxMainToSub(std::vector<u32> &notifyIdxMianToSub)
{
    notifyIdxMianToSub.clear();
    u32 threadNum = threadNum_;
    u32 slaveThreadNum = threadNum - 1;
    HCCL_INFO("[InsTempGatherMesh1dIntra][GetNotifyIdxMainToSub]threadNum: %u, slaveThreadNum: %u",threadNum,slaveThreadNum);
    for (u32 slaveThreadIdx = 0; slaveThreadIdx < slaveThreadNum; slaveThreadIdx++) {
        notifyIdxMianToSub.push_back(0);
    }
}

void InsTempGatherMesh1dIntra::GetNotifyIdxSubToMain(std::vector<u32> &notifyIdxSubToMain)
{
