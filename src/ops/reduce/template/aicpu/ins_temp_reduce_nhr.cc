...
HcclResult InsTempReduceNHR::PreCopy(const TemplateDataParams &tempAlgParams)
{
    ...
    if (buffInfo.inBuffType != BufferType::HCCL_BUFFER) {
        ...
        // DataSlice usrInSlices(buffInfo.inputPtr, buffInfo.inBuffBaseOff, sliceSize, count_);
        // DataSlice scratchSlices(buffInfo.hcclBuff.addr, buffInfo.hcclBuffBaseOff, sliceSize, count_);
