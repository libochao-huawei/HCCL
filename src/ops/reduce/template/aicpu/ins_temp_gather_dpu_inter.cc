            ...
            for (u32 i = 0; i < stepInfo.nSlices; ++i) {
                ...
                if (sendSize > 0 && recvSize >0) {
                    ...
                    CHK_PRT_RET(SendWrite(sendInfo),
                        ...
                        HcclResult::HCCL_E_INTERNAL);
                    HCCL_INFO("[InsTempGatherDpuInter][RunNHR]SendWrite from rank %u "
                    ...
                    CHK_PRT_RET(RecvWrite(recvInfo),
                        ...
                        HcclResult::HCCL_E_INTERNAL);
                    HCCL_INFO("[InsTempGatherDpuInter][RunNHR]RecvWrite to rank %u "
                              "src offset %u, dst offset %u, "
                              "size %u",
                        myRank_,
                        recvOffset,
                        ...
                        recvSize);
                    HCCL_INFO(
                        "[InsTempGatherDpuInter][RunNHR]RecvWrite src addr %p, dst addr %p",
                        recvCclBuffAddr,
                        tempAlgParams.buffInfo.hcclBuff.addr);
                }
            }
