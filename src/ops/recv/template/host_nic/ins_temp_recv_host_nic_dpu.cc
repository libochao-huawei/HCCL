    ...
    const std::vector<std::vector<uint32_t>> &subCommRanks)
{
    ...
    for (u32 rankIdx = 0; rankIdx < rankIds.size(); rankIdx++) {
        ...
        for (u64 sizeResidue = outputSize; sizeResidue > 0; sizeResidue -= sizePerRound) {
            ...
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(0, channels.at(rankIdx)[0].handle, 0, CUSTOM_TIMEOUT)));
            ...
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(0, channels.at(rankIdx)[0].handle, 1, CUSTOM_TIMEOUT)));
