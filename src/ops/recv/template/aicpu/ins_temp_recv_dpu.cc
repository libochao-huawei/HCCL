...
// ! 已编码完成
InsTempRecvDpu::InsTempRecvDpu(const OpParam &param, const u32 rankId,  // 传通信域的rankId，userRank
...
// ! 已编码完成
HcclResult InsTempRecvDpu::CalcRes(HcclComm comm, const OpParam &param, const TopoInfoWithNetLayerDetails *topoInfo,
                                     ...
                                     const TemplateResource &templateResource)
{
    ...
    HCCL_INFO("InsTempRecvDpu HcommRecvRequest run over, sendMsgId[%u]", sendMsgId);
    ...
    const std::vector<std::vector<uint32_t>> &subCommRanks)
{
    ...
    for (u32 rankIdx = 0; rankIdx < rankIds.size(); rankIdx++) {
        ...
        for (u64 sizeResidue = outputSize; sizeResidue > 0; sizeResidue -= sizePerRound) {
            ...
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWait(channels.at(rankIdx)[0].handle, 0, CUSTOM_TIMEOUT)));
            ...
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWait(channels.at(rankIdx)[0].handle, 1, CUSTOM_TIMEOUT)));

            // 后同步，通知发送端数据接收完成
            CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecord(channels.at(rankIdx)[0].handle, 2)));

            CHK_RET(static_cast<HcclResult>(HcommChannelFence(channels.at(rankIdx)[0].handle)));
        }
    }
#endif
    return HCCL_SUCCESS;
