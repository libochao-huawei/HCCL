...
template <typename AlgTopoMatch, typename InsAlgTemplate>
HcclResult InsV2AllReduceSoleExecutor<AlgTopoMatch, InsAlgTemplate>::CreateAlgTemplate(
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    // 构建template
    CHK_PRT_RET(algTemplate_ == nullptr && param.engine == COMM_ENGINE_CCU && param.detourType != HCCL_DETOUR_DISABLE,
