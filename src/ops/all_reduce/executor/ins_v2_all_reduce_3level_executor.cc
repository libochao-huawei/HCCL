...
InsV2AllReduce3LevelExecutor<AlgTopoMatch, InsRsAlgTemplateX, InsRsAlgTemplateY, InsRsAlgTemplateZ, InsAgAlgTemplateX,
                               InsAgAlgTemplateY, InsAgAlgTemplateZ>::InsV2AllReduce3LevelExecutor()
{
}

template <typename AlgTopoMatch, typename InsRsAlgTemplateX, typename InsRsAlgTemplateY, typename InsRsAlgTemplateZ,
          typename InsAgAlgTemplateX, typename InsAgAlgTemplateY, typename InsAgAlgTemplateZ>
HcclResult InsV2AllReduce3LevelExecutor<
...
HcclResult InsV2AllReduce3LevelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::BuildSubCommAndTempMap(
    const OpParam& param,
    ...
    bool& needFlatten)
{
    ...
    const uint32_t l0Size = rankSizeLevel0_;
    const uint32_t l1Size = rankSizeLevel1_;
    ...
    // 构造模板map
    tempMap.clear();
    if (!needFlatten && rankSizeLevel0_ > 1) {
...
template <typename AlgTopoMatch, typename InsRsAlgTemplateX, typename InsRsAlgTemplateY, typename InsRsAlgTemplateZ,
          typename InsAgAlgTemplateX, typename InsAgAlgTemplateY, typename InsAgAlgTemplateZ>
HcclResult InsV2AllReduce3LevelExecutor<AlgTopoMatch, InsRsAlgTemplateX, InsRsAlgTemplateY, InsRsAlgTemplateZ,
                                ...
                                AlgResourceRequest& resourceRequest)
{
    ...
    HCCL_DEBUG("[InsV2AllReduce3LevelExecutor] L0[%u], L1[%u], L2[%u]", rankSizeLevel0_, rankSizeLevel1_,
               rankSizeLevel2_);
...
{
    HCCL_INFO("[InsV2AllReduce3LevelExecutor][Orchestrate] Orchestrate Start");
    // 参数填充
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[InsV2AllReduce3LevelExecutor][Orchestrate]errNo[0x%016llx] Reduce scatter excutor kernel run failed",
            HCCL_ERROR_CODE(ret)),
        ret);
    HCCL_INFO("[InsV2AllReduce3LevelExecutor][Orchestrate] Orchestrate END");
    return HCCL_SUCCESS;
}
                                                                              ...
                                                                              const AlgResourceCtxSerializable& resCtx)
{
    ...
    HCCL_ERROR("[InsV2AllReduce3LevelExecutor][InitExectorInfo] threads_[%u], dataTypeSize_[%u], dataCount_[%d], "
               "L0[%u], L1[%u], L2[%u]",
...
{
    HCCL_INFO("[InsV2AllReduce3LevelExecutor][%s], level[%u]", __func__, level);
                                     ...
                                     const std::vector<std::vector<u32>>& subCommRanks2)
{
    ...
    HCCL_DEBUG("[InsV2AllReduce3LevelExecutor][InitTemplate] tempMap.size()[%u]", tempMap.size());
                                                                              ...
                                                                              const AlgResourceCtxSerializable& resCtx)
{
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        CHK_PRT_RET(
            ...
            HCCL_ERROR("[InsV2AllReduce3LevelExecutor][Orchestrate] multiLoopAllRankSplitData.size() <= loop"),
            HCCL_E_PARA);
        ...
        HCCL_DEBUG(
            "[InsV2AllReduce3LevelExecutor][InitExectorInfo] curloop[%d] loopTimes[%d], RS-interPodStepNum[%d], "
        ...
        HCCL_ERROR(
            "[InsV2AllReduce3LevelExecutor][InitExectorInfo] curloop[%d] loopTimes[%d], AG-interPodStepNum[%d], "
    ...
    HCCL_INFO("[InsV2AllReduce3LevelExecutor][OrchestrateLoop] End.");
    return HCCL_SUCCESS;
}
                                    ...
                                    const std::vector<u64>& curLoopAllRankSplitData)
{
    ...
    CHK_PRT_RET(srcDataSlice.size() != dstDataSlice.size(),
                HCCL_ERROR("[InsV2AllReduce3LevelExecutor][DoLocalCopy] srcDataSlice.size != dstDataSlice.size"),
                HCCL_E_PARA);
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_ALLREDUCE, InsV2AllReduce3Level, InsV2AllReduce3LevelExecutor,
