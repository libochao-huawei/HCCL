    ...
    const AlgHierarchyInfoForAllLevel &algHierarchyInfo, AlgResourceRequest &resourceRequest)
{
    ...
    for (auto &KernelInfo : resReqReduceScatterIntra.kernelInfos) {
        KernelInfo.resGroup = 0;
    }
    for (auto &KernelInfo : resReqReduceScatterInter.kernelInfos) {
        KernelInfo.resGroup = 0;
    }
    for (auto &KernelInfo : resReqAllGatherInter.kernelInfos) {
        KernelInfo.resGroup = 1;
    }
    for (auto &KernelInfo : resReqAllGatherIntra.kernelInfos) {
        KernelInfo.resGroup = 1;
    }
    ...
    TemplateDataParams &tempAlgParamsStepOne) const
{
    ...
    if (engine_ == CommEngine::COMM_ENGINE_CCU) {
        tempAlgParamsStepOne.buffInfo.hcclBuffBaseOff = scratchBuffOffset_;
    } else {
    ...
    const u64 tailSizeLastStep, TemplateDataParams &tempAlgParamsStepTwo) const
{
    ...
    if (engine_ == CommEngine::COMM_ENGINE_CCU) {
        tempAlgParamsStepTwo.buffInfo.hcclBuffBaseOff = scratchBuffOffset_;
    } else {
