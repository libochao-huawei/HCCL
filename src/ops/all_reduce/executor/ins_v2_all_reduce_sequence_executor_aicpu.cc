    ...
    const AlgHierarchyInfoForAllLevel &algHierarchyInfo, AlgResourceRequest &resourceRequest)
{
    ...
    if (param.engine == CommEngine::COMM_ENGINE_CCU) {
        if (resReqReduceScatterIntra.ccuKernelNum.empty() || resReqReduceScatterInter.ccuKernelNum.empty() ||
            resReqAllGatherInter.ccuKernelNum.empty() || resReqAllGatherIntra.ccuKernelNum.empty()) {
            HCCL_ERROR("[InsV2AllReduceSequenceExecutorAicpu][CalcRes] ccuKernelNum is empty");
            return HCCL_E_INTERNAL;
        }
        ...
        if (resReqReduceScatterIntra.channels.empty() || resReqReduceScatterInter.channels.empty()) {
            HCCL_ERROR("[InsV2AllReduceSequenceExecutorAicpu][CalcRes] channels is empty");
            return HCCL_E_INTERNAL;
        }
    ...
    InsAlgTemplate3>::Orchestrate(const OpParam &param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    if (algHierarchyInfo_.infos.size() < SEQUENCE_EXECUTOR_LEVEL_NUM ||
        algHierarchyInfo_.infos[0].empty() || algHierarchyInfo_.infos[1].empty()) {
        HCCL_ERROR("[InsV2AllReduceSequenceExecutorAicpu][Orchestrate] algHierarchyInfo invalid");
        return HCCL_E_INTERNAL;
    }
    ...
    if (param.engine == CommEngine::COMM_ENGINE_CCU) {
        if (resCtx.ccuKernelNum.size() < 4) {
            HCCL_ERROR("[InsV2AllReduceSequenceExecutorAicpu][Orchestrate] ccuKernelNum size[%u] should be at least 4",
                resCtx.ccuKernelNum.size());
            return HCCL_E_INTERNAL;
        }
        u32 offset = 0;
    ...
    InsAlgTemplate3>::OrchestrateLoop(const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    if (param.engine != CommEngine::COMM_ENGINE_CCU) {
        if (remoteRankToChannelInfo_.size() < 2) {
            HCCL_ERROR("[InsV2AllReduceSequenceExecutorAicpu][OrchestrateLoop] remoteRankToChannelInfo size[%u] should be at least 2",
    ...
    if (param.engine == CommEngine::COMM_ENGINE_CCU) {
        maxCountPerLoop = scratchBlockSize_ / HCCL_MIN_SLICE_ALIGN
            * HCCL_MIN_SLICE_ALIGN / dataTypeSize_ / totalRankAlign;
    ...
    InsAlgTemplate3>::FastLaunch(const OpParam &param, const CcuFastLaunchCtx *ctx)
{
    ...
    constexpr u32 STEP_NUM = 4;
    for (u32 i = 0; i < STEP_NUM; i++) {
        if (ctx->ccuKernelNum[i] > 0) {
            HCCL_INFO("[InsV2AllReduceSequenceExecutorAicpu][FastLaunch] Step%u ccuKernelNum[%llu]", i + 1, ctx->ccuKernelNum[i]);
        }
    }
