    ...
    std::shared_ptr<InsAlgTemplateRS> rsTempAlg, TemplateResource &rsTemplateAlgRes)
{
    ...
    for (u32 dataId = 0; dataId < rankSize_; dataId++) {
        // 每个rank在输入数据中的偏移：rankId * rsOutputSliceSize
        u64 userMemInOffset = dataId * rsOutputSliceSize;
        // 输出到outCclBuff的偏移：所有rank归约到同一位置(outCclBuffOffset_)
        u64 outOffset = 0;
        // 临时缓冲区偏移（inCclBuff部分）
        u64 tmpOffset = dataId * rsOutputSliceSize;
    ...
    std::shared_ptr<InsAlgTemplateAG> agTempAlg, TemplateResource &agTemplateAlgRes)
{
    ...
    // AllGather: HCCL_BUFFER (inCclBuff部分) -> OUTPUT
    ...
    agTempAlgParams.buffInfo.inputSize = inCclBuffSize_;
    agTempAlgParams.buffInfo.outputSize = param.outputSize;
    agTempAlgParams.enableRemoteMemAccess = param.opMode == OpMode::OFFLOAD;

    // 输入缓冲区基址偏移：从inCclBuff的偏移位置开始读取
    agTempAlgParams.buffInfo.inBuffBaseOff = inCclBuffOffset_;
    ...
    // 设置输入切片步幅为0：从inCclBuff的固定位置读取归约结果
