...
// 执行ReduceScatter阶段（已整合到OrchestrateLoop中，此函数保留作为备用）
// param: 操作参数
// resCtx: 资源上下文
template <typename AlgTopoMatch, typename InsAlgTemplateRS, typename InsAlgTemplateAG>
HcclResult InsV2AllReduceOrderPreservedExecutor<AlgTopoMatch, InsAlgTemplateRS, InsAlgTemplateAG>::RunReduceScatter(
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    // 打印ReduceScatter阶段开始日志（flat level1模式）
    HCCL_INFO("[RunReduceScatter] Start (flat level1)");

    // 如果只有一个rank，调用单rank处理函数
    if (rankSize_ == 1) {
    ...
    for (u32 dataId = 0; dataId < rankSize_; dataId++) {
        // 计算当前数据块在临时缓冲区中的偏移
        u64 offset = dataId * memInfo_.sizePerBlock;
        // 计算当前数据块在用户输入缓冲区中的偏移
        u64 userMemInOffset = dataId * unitSize * param.DataDes.count;
        // 添加当前数据块的大小
        memBlockInfo.size.push_back(memInfo_.groupSize[dataId]);
        // 添加用户输入偏移
        memBlockInfo.userInputOffsets.push_back(userMemInOffset);
        // 添加输入偏移（临时缓冲区）
        memBlockInfo.inputOffsets.push_back(offset);
...
// 执行AllGather阶段（已整合到OrchestrateLoop中，此函数保留作为备用）
// param: 操作参数
// resCtx: 资源上下文
template <typename AlgTopoMatch, typename InsAlgTemplateRS, typename InsAlgTemplateAG>
HcclResult InsV2AllReduceOrderPreservedExecutor<AlgTopoMatch, InsAlgTemplateRS, InsAlgTemplateAG>::RunAllGather(
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    // 打印AllGather阶段开始日志（使用NHR模板，flat level1模式）
...
// InsTempAllGatherNHR: AllGather模板类型
