    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        // 打印当前循环的调试日志
        HCCL_INFO("[InsV2AllReduceOrderPreservedExecutor] loop[%llu] sliceSize[%llu], "
            "inBuffBaseOff[%llu], outBuffBaseOff[%llu]",
            loop, currGroupSize[myRank_ % rankSize_], tempAlgParams.buffInfo.inBuffBaseOff, tempAlgParams.buffInfo.outBuffBaseOff);

        // 调用ReduceScatter阶段执行函数
        CHK_RET(RunReduceScatter(param, tempAlgParams, currGroupSize, currSizePerBlock, 
            processedDataCount, currDataCount, rsTempAlg, templateAlgRes));
        // 调用AllGather阶段执行函数
        CHK_RET(RunAllGather(param, tempAlgParams, agTempAlg, templateAlgRes));
...
HcclResult InsV2AllReduceOrderPreservedExecutor<AlgTopoMatch, InsAlgTemplateRS, InsAlgTemplateAG>::CalcSizePerBlock(const OpParam &param)
{
    ...
    memInfo_.sizePerBlock = (dataCount_ + rankSize_ - 1) / rankSize_ * dataTypeSize_;
...
HcclResult InsV2AllReduceOrderPreservedExecutor<AlgTopoMatch, InsAlgTemplateRS, InsAlgTemplateAG>::RunReduceScatter(
    const OpParam &param, TemplateDataParams &tempAlgParams, const std::vector<u64> &currGroupSize,
    u64 currSizePerBlock, u64 processedDataCount, u64 currDataCount,
    std::shared_ptr<InsAlgTemplateRS> rsTempAlg, TemplateResource &templateAlgRes)
{
    ...
    for (u32 dataId = 0; dataId < rankSize_; dataId++) {
        ...
        // 添加当前数据块的大小
        memBlockInfo.size.push_back(currGroupSize[dataId]);
        // 添加用户输入偏移
    ...
    tempAlgParams.count = currGroupSize[myRank_ % rankSize_] / dataTypeSize_;
    
    // 将内存块信息设置到ReduceScatter模板
    rsTempAlg->SetMemBlockInfo(memBlockInfo);
    // 调用ReduceScatter模板的KernelRun函数执行ReduceScatter阶段
    CHK_RET(rsTempAlg->KernelRun(param, tempAlgParams, templateAlgRes));
...
HcclResult InsV2AllReduceOrderPreservedExecutor<AlgTopoMatch, InsAlgTemplateRS, InsAlgTemplateAG>::RunAllGather(
    const OpParam &param, TemplateDataParams &tempAlgParams,
    std::shared_ptr<InsAlgTemplateAG> agTempAlg, TemplateResource &templateAlgRes)
{
    // 对于AllGather模板，count同样是每个rank的数据元素个数（由RunReduceScatter已设置）
    // 调用AllGather模板的KernelRun函数执行AllGather阶段
    CHK_RET(agTempAlg->KernelRun(param, tempAlgParams, templateAlgRes));
    // 返回成功
    return HCCL_SUCCESS;
}
