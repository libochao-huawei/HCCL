    ...
    std::shared_ptr<InsAlgTemplateRS> rsTempAlg, TemplateResource &rsTemplateAlgRes)
{
    ...
    // 模板CalcOutputIndex规则：
    // PreLocalCopy: outputIndex = (2*rank) % rankSize
    // AllToAll: outputIndex = (srcRank + dstRank) % rankSize
    // 对于rank k的缓冲区，outputIndex i 存储的数据来源：
    //   i = (j + k) % rankSize => j = (i - k + rankSize) % rankSize
    //   即来自rank j的数据存在outputIndex i
    // 每个outputIndex对应一个rank的数据大小
    
    // 先按outputIndex顺序计算每个位置对应的数据大小和偏移
    // outputIndex i 对应的数据来源rank = (i - rank + rankSize) % rankSize (对任意rank都一致)
    // 实际上，outputIndex i 对应的数据来源rank就是 i (当接收方rank=0时)
    // 因为每个rank的缓冲区布局相同，按outputIndex顺序排列
    ...
    for (u32 outputIndex = 0; outputIndex < rankSize_; outputIndex++) {
        // outputIndex对应的数据来源rank（按线性顺序分配）
        u32 srcRank = outputIndex;
        u64 currSliceSize = (srcRank == rankSize_ - 1) ? rsTailSize : rsSliceSize;
        
        // 设置outputOffsets[outputIndex]
        memBlockInfo.outputOffsets[outputIndex] = inCclBuffOffset_ + cumulativeOffset;
        cumulativeOffset += currSliceSize;
    }
    
    // 设置memBlockInfo其他字段（按rank顺序）
    for (u32 dataId = 0; dataId < rankSize_; dataId++) {
        u64 currSliceSize = (dataId == rankSize_ - 1) ? rsTailSize : rsSliceSize;
        u64 userMemInOffset = dataId * rsSliceSize;
        
        memBlockInfo.size.push_back(currSliceSize);
