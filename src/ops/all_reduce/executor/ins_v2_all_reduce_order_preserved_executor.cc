    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    // 准备临时模板数据参数结构体（仅用于计算ScratchMultiple）
...
// 计算当前循环每个rank的数据大小（字节）
        ...
        CHK_RET(RunReduceScatter(param, resCtx, currGroupSize, currSizePerBlock, 
            processedDataCount, count, rsTempAlg, rsTemplateAlgRes));
        // 调用AllGather阶段执行函数
        CHK_RET(RunAllGather(param, resCtx, currGroupSize, currSizePerBlock,
            processedDataCount, count, agTempAlg, agTemplateAlgRes));
...
{
    // 打印ReduceScatter函数入参
    HCCL_INFO("zdy[%u] [RunReduceScatter] Enter: currGroupSize.size[%u], currSizePerBlock[%llu], "
        "processedDataCount[%llu], currDataCount[%llu]", myRank_, currGroupSize.size(), currSizePerBlock,
        processedDataCount, currDataCount);
    for (u32 i = 0; i < currGroupSize.size(); i++) {
        HCCL_INFO("zdy[%u] [RunReduceScatter] currGroupSize[%u]=[%llu]", myRank_, i, currGroupSize[i]);
    }
    ...
    // 打印ReduceScatter模板参数
    HCCL_INFO("zdy[%u] [RunReduceScatter] count[%llu], sliceSize[%llu], inBuffBaseOff[%llu], outBuffBaseOff[%llu]",
        myRank_, rsTempAlgParams.count, rsTempAlgParams.sliceSize, 
...
{
    // 打印AllGather函数入参
    HCCL_INFO("zdy[%u] [RunAllGather] Enter: currGroupSize.size[%u], currSizePerBlock[%llu], "
        "processedDataCount[%llu], currDataCount[%llu]", myRank_, currGroupSize.size(), currSizePerBlock,
        processedDataCount, currDataCount);
    for (u32 i = 0; i < currGroupSize.size(); i++) {
        HCCL_INFO("zdy[%u] [RunAllGather] currGroupSize[%u]=[%llu]", myRank_, i, currGroupSize[i]);
    }
    ...
    // 打印AllGather模板参数
    HCCL_INFO("zdy[%u] [RunAllGather] count[%llu], sliceSize[%llu], inBuffBaseOff[%llu], outBuffBaseOff[%llu]",
        myRank_, agTempAlgParams.count, agTempAlgParams.sliceSize, 
