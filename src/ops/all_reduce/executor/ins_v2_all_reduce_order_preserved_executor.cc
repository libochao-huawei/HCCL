...
// ReduceScatter: INPUT -> HCCL_BUFFER(outCclBuff)
    ...
    // 新逻辑：切分数据大于等于尾块数据，余数分散到前面的rank
    // 前面的rank处理 (currDataCount/rankSize_ + 1) * unitSize（如果有余数）
    // 最后一个rank处理 currDataCount/rankSize_ * unitSize
    u64 remainderCount = currDataCount % rankSize_;
    u64 baseCount = currDataCount / rankSize_;
    // rsSliceSize：前面rank的大小（有余数时包含一个额外元素）
    u64 rsSliceSize = (baseCount + (remainderCount > 0 ? 1 : 0)) * unitSize;
    // rsTailSize：最后一个rank的大小（基础大小，不含余数）
    u64 rsTailSize = baseCount * unitSize;
    
    // 由于CalcOutputIndex(srcRank, dstRank) = (srcRank+dstRank)%rankSize，
    // 不同destination rank的同一outputIndex存储来自不同source rank的数据。
    // 为避免重叠，outputOffsets使用rsSliceSize作为stride（最大大小），
    // 但memBlockInfo.size保持实际大小（用于正确发送数据量）
    
    memBlockInfo.outputOffsets.resize(rankSize_, 0);
    
    // 设置outputOffsets：使用rsSliceSize作为stride避免重叠
    for (u32 outputIndex = 0; outputIndex < rankSize_; outputIndex++) {
        memBlockInfo.outputOffsets[outputIndex] = inCclBuffOffset_ + outputIndex * rsSliceSize;
    }
    
    // 设置size、userInputOffsets和inputOffsets（按rank顺序）
    // 新逻辑：前面的rank处理更大的数据（含余数），最后一个rank处理基础大小
    for (u32 dataId = 0; dataId < rankSize_; dataId++) {
        // 实际数据大小：最后一个rank使用较小的tailSize，前面rank使用较大的sliceSize
        u64 actualSize = (dataId == rankSize_ - 1) ? rsTailSize : rsSliceSize;
        // 用户输入偏移：已处理数据偏移 + 当前数据块在输入数据中的偏移
        // 输入数据按rsSliceSize步幅排列（使用最大步幅）
    ...
    std::shared_ptr<InsAlgTemplateAG> agTempAlg, TemplateResource &agTemplateAlgRes)
{
    ...
    // 计算sliceSize和tailSize（与ReduceScatter一致）
    // 新逻辑：切分数据大于等于尾块数据，余数分散到前面的rank
    u64 remainderCount = currDataCount % rankSize_;
    u64 baseCount = currDataCount / rankSize_;
    // agSliceSize：前面rank的大小（有余数时包含一个额外元素）
    u64 agSliceSize = (baseCount + (remainderCount > 0 ? 1 : 0)) * dataTypeSize_;
    // agTailSize：最后一个rank的大小（基础大小，不含余数）
    u64 agTailSize = baseCount * dataTypeSize_;
    
    // 设置所有rank的数据切片大小向量（与ReduceScatter输出大小相同）
    std::vector<u64> agSliceSizes;
    for (u32 rankId = 0; rankId < rankSize_; rankId++) {
        // 新逻辑：最后一个rank使用较小的tailSize，前面rank使用较大的sliceSize
    ...
    // 设置输出切片步幅：输出数据中每个rank对应数据块的间隔（使用最大步幅）
