                                ...
                                AlgResourceRequest& resourceRequest)
{
    ...
    HCCL_INFO("[InsV2AllReduceOmniPipeExecutor] L0[%u], L1[%u], L2[%u]", rankSizeLevel0_, rankSizeLevel1_,
               rankSizeLevel2_);
                                                                              ...
                                                                              const AlgResourceCtxSerializable& resCtx)
{
    ...
    HCCL_DEBUG("[InsV2AllReduceOmniPipeExecutor][InitExectorInfo] threads_[%u], dataTypeSize_[%u], dataCount_[%d], "
               "L0[%u], L1[%u], L2[%u]",
...
HcclResult InsV2AllReduceOmniPipeExecutor<
    AlgTopoMatch, InsRsAlgTemplateX, InsRsAlgTemplateY, InsRsAlgTemplateZ, InsAgAlgTemplateX, InsAgAlgTemplateY,
    InsAgAlgTemplateZ>::GenTemplateAlgParamsByDimData(TemplateDataParams& tempAlgParams, StepSliceInfo& stepSliceInfo, bool isFirstStep)
{
    ...
    tempAlgParams.stepSliceInfo.isFirstStep = isFirstStep;
    return HCCL_SUCCESS;
}
                                         ...
                                         std::vector<std::vector<u32>>& subCommRanks2)
{
    ...
    subCommRanks1.push_back(algHierarchyInfo_.infos[1][0]);
                                                                              ...
                                                                              const AlgResourceCtxSerializable& resCtx)
{
    ...
    // 1.计算带宽
    double bw0 = GetExternalInputBandWidthLevel0();
    double bw1 = GetExternalInputBandWidthLevel1();
    double bw2 = GetExternalInputBandWidthLevel2();

    HCCL_INFO("[InsV2AllReduceOmniPipeExecutor][OrchestrateLoop] bw0[%f], bw1[%f], bw2[%f]", bw0, bw1, bw2);

    std::vector<double> endpointAttrBw{bw0, bw1, bw2};
    ...
    HCCL_INFO("maxCountPerLoop[%u], loopTimes[%u]", maxCountPerLoop, loopTimes);
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        for (int stepZ = 0; stepZ < interPodStepNum; stepZ++) {
            if (rankSizeLevel2_ > 1) {
                bool isFirstStepZ = (stepZ == 0) ? true : false;
                GenTemplateAlgParamsByDimData(tempAlgParamMap[OMNIPIPE_RS_LEVEL2],
                                              OmniPipeSliceInfoRS.dataSliceLevel2[stepZ], isFirstStepZ);
            ...
            for (int stepXY = 0; stepXY < intraPodStepNum; stepXY++) {
                // XY前同步
                bool isFirstStepXYZ = (stepXY == 0 && stepZ == 0) ? true : false;
                CHK_RET(PreSyncInterThreads(controlThread_, tempMainThreadsLevel01RS_, ntfIdxCtrlToTempLevel01RS_));
                if (rankSizeLevel0_ > 1) {
                    CHK_RET(GenTemplateAlgParamsByDimData(
                        tempAlgParamMap[OMNIPIPE_RS_LEVEL0],
                        OmniPipeSliceInfoRS.dataSliceLevel0[stepZ * intraPodStepNum + stepXY], isFirstStepXYZ));
                ...
                if (rankSizeLevel1_ > 1) {
                    CHK_RET(GenTemplateAlgParamsByDimData(
                        ...
                        OmniPipeSliceInfoRS.dataSliceLevel1[stepZ * intraPodStepNum + stepXY], isFirstStepXYZ));
        ...
        for (int stepZ = 0; stepZ < interPodStepNum; stepZ++) {
            if (rankSizeLevel2_ > 1) {
                ...
                CHK_RET(GenTemplateAlgParamsByDimData(tempAlgParamMap[OMNIPIPE_AG_LEVEL2],
                                                      OmniPipeSliceInfoAG.dataSliceLevel2[stepZ], isFirstStepZ));
            ...
            for (int stepXY = 0; stepXY < intraPodStepNum; stepXY++) {
                ...
                if (rankSizeLevel0_ > 1) {
                    GenTemplateAlgParamsByDimData(
                        ...
                        OmniPipeSliceInfoAG.dataSliceLevel0[stepZ * intraPodStepNum + stepXY], isFirstStepXYZ);
                ...
                if (rankSizeLevel1_ > 1) {
                    GenTemplateAlgParamsByDimData(
                        ...
                        OmniPipeSliceInfoAG.dataSliceLevel1[stepZ * intraPodStepNum + stepXY], isFirstStepXYZ);
