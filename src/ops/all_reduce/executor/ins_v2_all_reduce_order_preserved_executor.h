...
inline u8 GetLocalDeterministicConfig()
{
    const char* env = std::getenv("HCCL_DETERMINISTIC");
    if (env == nullptr) {
        return static_cast<u8>(DeterministicEnableLevel::DETERMINISTIC_DISABLE);
    }
    std::string val(env);
    std::transform(val.begin(), val.end(), val.begin(), ::toupper);
    if (val == "STRICT") {
        return static_cast<u8>(DeterministicEnableLevel::DETERMINISTIC_STRICT);
    }
    if (val == "TRUE") {
        return static_cast<u8>(DeterministicEnableLevel::DETERMINISTIC_ENABLE);
    }
    return static_cast<u8>(DeterministicEnableLevel::DETERMINISTIC_DISABLE);
}
...
protected:
    ...
    HcclResult RunReduceScatterSingleRank(const OpParam &param, const AlgResourceCtxSerializable &resCtx);
