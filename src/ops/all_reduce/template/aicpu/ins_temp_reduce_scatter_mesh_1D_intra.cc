    ...
    for (u32 queIdx = 1; queIdx < threadNum_; queIdx++) {
        ...
        for (u32 repeatIdx = 0; repeatIdx < tempAlgParam.repeatNum; repeatIdx++) {
            ...
            if (sendSize == 0) { // 发送数据片为0时，只接收数据
                ...
                HCCL_DEBUG("[InsTempReduceScatterMesh1DIntra][RunReduceScatter]RecvWrite from rank %u to rank %u "
                ...
                CHK_PRT_RET(SendWrite(sendInfo, threads[queIdx]),
                    ...
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_DEBUG("[InsTempReduceScatterMesh1DIntra][RunReduceScatter]SendWrite from rank %u to rank %u "
                          "src offset %u, dst offset %u, "
                          "size %u",
                    myRank_,
                    remoteRank,
                    ...
                    tempAlgParam.buffInfo.hcclBuffBaseOff + repeatIdx * tempAlgParam.outputRepeatStride +
                        myAlgRank * sendSize,
                    sendSize);
                HCCL_DEBUG("[InsTempReduceScatterMesh1DIntra][RunReduceScatter]SendRecvWrite src addr %p, dst addr %p",
                    tempAlgParam.buffInfo.inputPtr,
                    remoteCclBuffAddr);
            } else {
                ...
                CHK_PRT_RET(SendRecvWrite(sendRecvInfo, threads[queIdx]),
                    HCCL_ERROR("[InsTempReduceScatterMesh1DIntra][RunReduceScatter] Send failed"),
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_DEBUG("[InsTempReduceScatterMesh1DIntra][RunReduceScatter]SendRecvWrite from rank %u to rank %u "
                          "src offset %u, dst offset %u, "
                          "size %u",
                    myRank_,
                    remoteRank,
