        ...
        for (u32 repeatIdx = 0; repeatIdx < tempAlgParams.repeatNum; repeatIdx++) {
            ...
            if (sendSize > 0 && recvSize >0) {
                ...
                CHK_PRT_RET(SendWrite(sendInfo),
                    ...
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_DEBUG("[InsTempReduceScatterMesh1dDpuInter][DPUKernelRun]SendWrite from rank %u to rank %u"
                    ...
                    tempAlgParams.buffInfo.hcclBuffBaseOff + repeatIdx * tempAlgParams.outputRepeatStride +
                        myAlgRank * sendSize,
                    sendSize);
                HCCL_DEBUG("[InsTempReduceScatterMesh1dDpuInter][DPUKernelRun]SendWrite src addr %p, dst addr %p",
                    tempAlgParams.buffInfo.inputPtr,
                    remoteCclBuffAddr);
            } else if (recvSize > 0) {
                ...
                CHK_PRT_RET(RecvWrite(recvInfo),
                    ...
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_DEBUG("[InsTempReduceScatterMesh1dDpuInter][DPUKernelRun] RecvWrite from rank %u to rank %u "
                          "src offset %u, dst offset %u, "
                          "size %u",
                    remoteRank, myRank,
                    tempAlgParams.buffInfo.inBuffBaseOff + repeatIdx * tempAlgParams.inputRepeatStride + recvOffset,
                    tempAlgParams.buffInfo.hcclBuffBaseOff + repeatIdx * tempAlgParams.outputRepeatStride + rankIdx * recvSize,
                    recvSize);
                HCCL_DEBUG("[InsTempReduceScatterMesh1dDpuInter][DPUKernelRun]RecvWrite src addr %p, dst addr %p",
                    tempAlgParams.buffInfo.inputPtr,//这里应该是远端的，但是A5不支持，先用本端的代替
