...
{
    const CcuKernelArgAllReduceMeshDetour1D *kernelArg = dynamic_cast<const CcuKernelArgAllReduceMeshDetour1D *>(&arg);
    if (kernelArg == nullptr) {
        HCCL_ERROR("CcuKernelAllReduceMesh1DDetour::kernelArg ptr is null");
    }
    rankId_             = kernelArg->rankId_;
    rankSize_           = kernelArg->dimSize_[0];
    dataType_           = kernelArg->op_.dataType;
    outputDataType_     = kernelArg->op_.outputDataType;
    channels_           = kernelArg->channels;
    std::vector<u32> channelsIndexVec = kernelArg->channelsIndexVec_;
    if (outputDataType_ == HcclDataType::HCCL_DATA_TYPE_RESERVED) {
        outputDataType_ = dataType_;
        HCCL_INFO("[CcuKernelAllReduceMesh1DDetour] outputDataType is [HCCL_DATA_TYPE_RESERVED], set outputDataType to[%s]",
            outputDataType_);
    }
    reduceOp_ = kernelArg->op_.reduceOp;
        ...
        for (uint64_t i = 0; i < directPathNum; i++) {
            for (uint32_t j = 0; j < detourChannels_[i].size(); j++) {
                if (detourChannels_[i][j] == nullptr) {
                    HCCL_ERROR("channel is nullptr");
                }
                sems[i].SetMask(1 << j);
                ReadNb(detourChannels_[i][j], bufs[i][j], src[i * rankSize_ + j], lengths[i]);
            }
        }
        // 再读远端绕路的到本地MS
...
void CcuKernelAllReduceMesh1DDetour::GroupReduceDetour(std::vector<CcuRep::Memory> &src,
    std::vector<CcuRep::Memory> &dst, HcclDataType &dataType, HcclDataType &outputDataType, HcclReduceOp &opType)
{
    ...
    CCU_IF(iterNum_ != 0) {
        ...
        loopParam = CcuRep::GetLoopParam(0, singleTransferSize * moConfig.loopCount, 0);  // 下次迭代的偏移是单次总搬运量*loopNum
        loopParam += iterNum_;  // 加上loop的迭代次数构成完整loop参数
        paraCfg = CcuRep::GetParallelParam(moConfig.loopCount - 1, 0, 1);  // loop固定展开到128个
        offsetCfg = CcuRep::GetOffsetParam(singleTransferSize, interLeave, pathNumPerPeer);  // 下一个loop偏移量
...
void CcuKernelAllReduceMesh1DDetour::CreateMultiOpBroadcastDetour()
{
    ...
    CcuRep::LoopBlock lb(this, loopType + "_loop");
    {
        ...
        for (uint64_t i = 0; i < pathNumPerPeer; i++) {
            for (uint64_t j = 0; j < rankSize_ - 1; j++) {
                if (detourChannels_[i][j * 2] == nullptr) { // j * 2是sendOnly Link
                    HCCL_ERROR("channel is nullptr");
                }
                sems[i].SetMask(1 << j);
            ...
            LocalCopy(tmpAddr, bufs[i], lengths[i], sems[i]);
        }
        // 等待给所有远端写完数据
        for (uint64_t i = 0; i < pathNumPerPeer; i++) {
            sems[i].SetMask((1 << rankSize_) - 1);
...
void CcuKernelAllReduceMesh1DDetour::GroupBroadcastDetour(std::vector<CcuRep::Variable> &lengths, std::vector<CcuRep::Memory> &src,
    std::vector<CcuRep::Memory> &dst)
{
    ...
    CCU_IF(iterNum_ != 0) {
        ...
        loopParam = CcuRep::GetLoopParam(0, singleTransferSize * moConfig.loopCount, 0);  // 偏移是单次总搬运量*loopNum
        loopParam += iterNum_;  // 加上loop的迭代次数构成完整loop参数
        paraCfg = CcuRep::GetParallelParam(moConfig.loopCount - 1, 0, 1);  // loop固定展开到128个
        offsetCfg = CcuRep::GetOffsetParam(singleTransferSize, interLeave, pathNumPerPeer);  // 下一个loop偏移量
...
void CcuKernelAllReduceMesh1DDetour::Algorithm()
{
...
std::vector<uint64_t> CcuKernelAllReduceMesh1DDetour::GeneArgs(const CcuTaskArg &arg)
{
    ...
    if (taskArg == nullptr) {
        HCCL_ERROR(StringFormat("CcuKernelAllReduceMesh1DDetour::taskArg ptr is null"));
    }
