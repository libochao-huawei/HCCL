...
public:
    explicit CcuKernelArgAllReduceMesh1DDetour(const std::vector<uint64_t> &dSize, uint32_t rId, const CollAlgOperator &op,
        const std::vector<std::vector<RankId>> &tempVTopo, uint64_t singleTransferSize, uint64_t detourPathNum,
    ...
    CcuKernelSignature GetCtxSignature() const override
    {
