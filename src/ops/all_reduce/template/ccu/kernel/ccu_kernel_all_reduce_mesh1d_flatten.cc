...
void CcuKernelAllReduceMesh1DFlatten::DoGroupReduce()
{
    ...
    for (uint64_t rankIdx = 0; rankIdx < rankSize_ - 1; rankIdx++) {
        reduceSrc.push_back(CreateRemoteAddr());
    }
    CcuRep::LocalAddr reduceDst = CreateLocalAddr();
    CcuRep::LocalAddr LocalSrc = CreateLocalAddr();
    // 填充地址
    uint32_t curId = 0;
    // SRC
    ...
    LocalCopyNb(reduceDst, LocalSrc, sliceSize_, localEvent_);
...
std::vector<uint64_t> CcuKernelAllReduceMesh1DFlatten::GeneArgs(const CcuTaskArg &arg)
{
    ...
    std::vector<uint64_t> taskArgList{inputAddr, outputAddr, tokenInfo, sliceSize};
