    ...
    : CcuKernelAlgBase(arg)
{
    ...
    HCCL_INFO("[CcuKernelAllReduceMesh1DFlatten] Init, CtxArgs are rankId[%u], rankSize[%u], dataType[%u], "
        "outputDataType[%u], reduceOp[%u]", rankId_, rankSize_, dataType_, outputDataType_, reduceOp_);
}

CcuKernelAllReduceMesh1DFlatten::Algorithm()
{
...
CcuKernelAllReduceMesh1DFlatten::InitResource()
{
...
void CcuKernelAllReduceMesh1DFlatten::DoGroupReduce()
{
    ...
    uint32_t srcId = 0;
    for (auto &channel : channels_) {
        event_.SetMask(1 << (srcId % BIT_NUM_PER_CKE));
        HCCL_INFO("srcId % BIT_NUM_PER_CKE=%d",srcId % BIT_NUM_PER_CKE);
        ReadReduceNb(channel, reduceDst, reduceSrc[srcId], sliceSize_, dataType_, reduceOp_, event_);
        srcId++;
    }

    if(srcId % BIT_NUM_PER_CKE == 0){
        CHK_RET(LocalWaitAllEvent((1 << BIT_NUM_PER_CKE) - 1));
        HCCL_INFO("(1 << BIT_NUM_PER_CKE) - 1=%d",(1 << BIT_NUM_PER_CKE) - 1);
    } else {
        CHK_RET(LocalWaitAllEvent((1 << (srcId)% BIT_NUM_PER_CKE) - 1));
        HCCL_INFO("((1 << (srcId % BIT_NUM_PER_CKE) - 1)=%d",((1 << srcId % BIT_NUM_PER_CKE) - 1));
    }
