    ...
    HcclReduceOp opType)
{
    ...
    for (int32_t index = 0; index < 2; index++) { // 需要实例化2个Loop
        ...
        std::vector<CcuBuffer> bufs = {ctx.moRes.ccuBuf.begin() + index * ctx.moConfig.msInterleave,
                                               ctx.moRes.ccuBuf.begin() + index * ctx.moConfig.msInterleave + usedBufNum};
...
static void BcastLocToRmt(AllReduceMeshMem2Mem1DContext &ctx, const CcuVariable &srcAddr,
...
static void ReduceRmtToLoc(AllReduceMeshMem2Mem1DContext &ctx, const std::vector<CcuVariable> &srcAddr,
...
static void DoRepeatAllReduce(AllReduceMeshMem2Mem1DContext &ctx)
{
    const auto *arg = ctx.arg;
    ...
    ReduceRmtToLoc(ctx, ctx.input, ctx.output[arg->rankId]);
    BcastLocToRmt(ctx, ctx.output[arg->rankId], ctx.output);
}
