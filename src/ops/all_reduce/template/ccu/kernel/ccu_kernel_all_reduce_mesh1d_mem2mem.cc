...
static CcuResult PairwiseLocalReduce(AllReduceMeshMem2Mem1DContext &ctx, ccu::LocalAddr myOutput, 
    std::vector<ccu::LocalAddr> &inputVec, CcuVariable sliceSize, 
    HcclDataType dataType, HcclDataType outputDataType, HcclReduceOp opType)
{
    ...
    while (remainPieces > 1) {
        ...
        ccu::LocalReduceNb(inputVec[0], inputVec[srcIdx], len, dataType, opType, ctx.events[0]);
    ...
    ctx.events[0].setMask(1);
    ccu::LocalCopyNb(myOutput, inputVec[0], sliceSize, ctx.events[0]);
    ccu::WaitEvent(ctx.events[0]);
    return CCU_SUCCESS;
}
...
static CcuResult ReduceLoopGroup(AllReduceMeshMem2Mem1DContext &ctx, ccu::LocalAddr srcOrg)
{
    ...
    for (uint32_t idx = 0; idx < size; idx++) {
        ...
        scratch[idx].token = ctx.token[ctx.arg->rankSize];
    }
