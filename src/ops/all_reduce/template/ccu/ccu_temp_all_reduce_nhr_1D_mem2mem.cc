                                                            ...
                                                            std::vector<std::vector<HcclChannelDesc>>& channelsPerDie)
{
    ...
    for (u32 step = 0; step < nSteps; step++) {
        ...
        if (enableDieNum = DIE_NUM_1) {
            ...
            // 加入fromRank 2个die的链路
            CHK_RET(SelectChannelToVec(comm, myRank_, realFromRankId, rankIdToChannelDesc_, DIE0, 
                rank2ChannelIdx, channelsPerDie[DIE0]));
            CHK_RET(SelectChannelToVec(comm, myRank_, realFromRankId, rankIdToChannelDesc_, DIE1, 
                rank2ChannelIdx, channelsPerDie[DIE1]));
            // 加入toRank 2个die的链路
            CHK_RET(SelectChannelToVec(comm, myRank_, realToRankId, rankIdToChannelDesc_, DIE0, 
                rank2ChannelIdx, channelsPerDie[DIE0]));
            CHK_RET(SelectChannelToVec(comm, myRank_, realToRankId, rankIdToChannelDesc_, DIE1, 
                rank2ChannelIdx, channelsPerDie[DIE1]));
        }
    }
    return HcclResult::HCCL_SUCCESS;
