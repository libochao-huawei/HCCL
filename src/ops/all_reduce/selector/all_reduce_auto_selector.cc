...
{
    /* 以下代码 CCU 调试时注意：
    //    因为OpParam中没有inputMem和outputMem, 但AlgResourceCtx中存在cclInputMem和cclOutputMem, 暂不确定是否为同一个
    u64 perDataSize = DATATYPE_SIZE_TABLE[opParam.DataDes.dataType];
    u64 dataSize = opParam.DataDes.count * perDataSize;
    if (topoInfo->level0Topo == Level0Shape::MESH_1D) {
        if (IsSmallData(dataSize) && IsInputOutputOverlap(opParam.inputMem, opParam.outputMem) != true) {
            selectAlgName = "CcuAllReduceMesh1DOneShot";
        } else {
            selectAlgName = "CcuAllReduceMesh1DMultiMission";
        }
    } else if (topoInfo->level0Topo == Level0Shape::MESH_2D) {
        if (IsSmallData(dataSize) && IsInputOutputOverlap(opParam.inputMem, opParam.outputMem) != true) {
            selectAlgName = "CcuAllReduceMesh2DOneShot";
        } else if (IsLargeData(dataSize)) {
            selectAlgName = "CcuAllReduceMesh2DTwoShotMultiMission";
        } else {
            selectAlgName = "CcuAllReduceMesh2DTwoShot";
        }
    }*/
...
{   
    ...
    if (topoInfo->topoLevelNums > 1) {
        ...
        if ((IsDefaultAlg(levle0Algo) || levle0Algo ==  HcclAlgoType::HCCL_ALGO_TYPE_FULLMESH)&&(topoInfo->level0Topo == Level0Shape::MESH_1D)) {
            ...
            selectAlgName = "CcuAllReduceMeshMem2Mem1D";
            return SelectorStatus::MATCH;
        } else if ((IsDefaultAlg(levle0Algo) || (levle0Algo ==  HcclAlgoType::HCCL_ALGO_TYPE_FULLMESH)) &&
            (topoInfo->level0Topo == Level0Shape::MESH_2D)) {
            selectAlgName = "CcuAllReduceMeshTwoShotMem2Mem2D";
            return SelectorStatus::MATCH;
        } else {
