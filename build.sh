...
function build_static() {
    ...
    # 检查静态库是否生成
    local STATIC_LIB=""
    # 首先检查libhccl.a（可能没有_static后缀）
    if [ -f "${BUILD_DIR}/libhccl.a" ]; then
        STATIC_LIB="${BUILD_DIR}/libhccl.a"
    elif [ -f "${OUTPUT_DIR}/lib/libhccl.a" ]; then
        STATIC_LIB="${OUTPUT_DIR}/lib/libhccl.a"
    elif [ -f "${BUILD_DIR}/libhccl_static.a" ]; then
        STATIC_LIB="${BUILD_DIR}/libhccl_static.a"
    elif [ -f "${OUTPUT_DIR}/lib/libhccl_static.a" ]; then
        STATIC_LIB="${OUTPUT_DIR}/lib/libhccl_static.a"
    else
        log "Error: Static library not found in expected locations"
        log "Error: Checked: ${BUILD_DIR}/libhccl.a, ${OUTPUT_DIR}/lib/libhccl.a"
        log "Error: Checked: ${BUILD_DIR}/libhccl_static.a, ${OUTPUT_DIR}/lib/libhccl_static.a"
        exit 1
    fi
    ...
    local AIV_PATTERNS=("hccl_aiv_*_op_*.o" "hccl_aiv_*_op_*_*.o")
    local AIV_FILES=()

    # 在build目录中查找AIV算子文件
    cd ${BUILD_DIR}
    # 使用多个模式查找AIV文件
    for pattern in "${AIV_PATTERNS[@]}"; do
        while IFS= read -r -d '' aiv_file; do
            AIV_FILES+=("${aiv_file}")
            log "Info: Found AIV operator file: ${aiv_file}"
        done < <(find . -name "${pattern}" -type f -print0 2>/dev/null)
    done

    # 如果没有找到文件，尝试在src目录中查找
    if [ ${#AIV_FILES[@]} -eq 0 ]; then
        log "Info: No AIV files found in build directory, searching in source directories"
        local SRC_AIV_DIRS=(
            "${CURRENT_DIR}/src/ops/all_gather/template/aiv/kernel"
            "${CURRENT_DIR}/src/ops/all_reduce/template/aiv/kernel"
            "${CURRENT_DIR}/src/ops/all_to_all_v/template/aiv/kernel"
            "${CURRENT_DIR}/src/ops/broadcast/template/aiv/kernel"
            "${CURRENT_DIR}/src/ops/reduce/template/aiv/kernel"
            "${CURRENT_DIR}/src/ops/reduce_scatter/template/aiv/kernel"
            "${CURRENT_DIR}/src/ops/scatter/template/aiv/kernel"
        )

        for aiv_dir in "${SRC_AIV_DIRS[@]}"; do
            if [ -d "${aiv_dir}" ]; then
                while IFS= read -r -d '' aiv_file; do
                    AIV_FILES+=("${aiv_file}")
                    log "Info: Found AIV operator file in source: ${aiv_file}"
                done < <(find "${aiv_dir}" -name "*.o" -type f -print0 2>/dev/null)
            fi
        done
    fi
