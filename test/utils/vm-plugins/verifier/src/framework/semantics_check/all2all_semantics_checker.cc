    ...
    for (RankId rankId = 0; rankId < rankSize; rankId++) {
        ...
        for (auto &ele : allRankMemSemantics[rankId][BufferType::OUTPUT]) {
            ...
            if (ele.srcBufs.begin()->bufType != BufferType::INPUT) {
                ...
                "cur buffer semantic is %s", rankId, ele.Describe().c_str());
                return HcclResult::HCCL_E_PARA;
            }

            if (ele.srcBufs.begin()->srcAddr != sendOffsets[curRankId] + curDataSize) {
                HCCL_ERROR("[rankId:%u]Cur buffer semantic srcBufs srcAddr should be %llu, "
        ...
        if (curRankId != rankSize) {
            ...
            "accumulated semantic size from rank %u is %llu, while rankSize is %u", rankId, totalSize, curRankId, curDataSize, rankSize);
            return HcclResult::HCCL_E_PARA;
        }
    }
