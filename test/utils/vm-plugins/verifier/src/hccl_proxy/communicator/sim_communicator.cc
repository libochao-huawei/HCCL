...
HcclResult SimCommunicator::Init(const TopoMeta &topoMeta, uint32_t rank)
{
    ...
    CHK_PRT(GenRankTable(topoMeta, rankTable));
    topoModel_ = make_unique<TopoModel>(rankTable);
...
{
    *graph = topoModel_->rankTable_.rankList.data();
    *len = topoModel_->rankTable_.rankList.size() * sizeof(RankInfo_t);
    HCCL_INFO("[%s] len[%u], rankSize[%u], sizeof(RankInfo_t)[%u]",
        __func__, *len, topoModel_->rankTable_.rankList.size(), sizeof(RankInfo_t));
    return HCCL_SUCCESS;
}
