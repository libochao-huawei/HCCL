...
class ST_ALL2ALLVC_TEST : public ::testing::Test {
protected:
    void SetUp() override
    {
...
TEST_F(ST_ALL2ALLVC_TEST, st_all2allVC)
{
    ...
    for (auto rankId = 0; rankId < rankSize; ++rankId) {
        threads.emplace_back([=]() {
            ...
            for (auto i = 0; i < rankSize; ++i) {
                sendDataCount += sendCountMatrix[rankSize * rankId + i];
            }
            u64 recvDataCount = 0;
            for (auto i = 0; i < rankSize; ++i) {
                sendDataCount += sendCountMatrix[i * rankSize + rankId];
            }
            ...
            // 4.算子下发
            CHK_RET(HcclAlltoAllVC(sendBuf,
                sendCountMatrix.data(),
                dataType,
                recvBuf,
                dataType,
                comm,
