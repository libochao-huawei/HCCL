...
TEST_F(ST_REDUCE_SCATTER_TEST_A2A3, st_reduce_scatter_opbase_test_origin)
{
    ...
    auto dataUnitSize = sizeof(float);
...
TEST_F(ST_REDUCE_SCATTER_TEST_A2A3, st_reduce_scatter_cross_server_2x4)
{
    TopoMeta topoMeta;
    std::vector<u32> args {1, 2, 4};
...
TEST_F(ST_REDUCE_SCATTER_TEST_A2A3, st_reduce_scatter_cross_server_2x16)
{
    TopoMeta topoMeta;
    std::vector<u32> args {1, 2, 16};
    for (int i = 0; i < args[0]; i++) {
        SuperPodMeta superPodMeta;
        for (int j = 0; j < args[1]; j++) {
            ServerMeta serverMate;
