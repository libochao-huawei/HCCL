...
struct RegisteredCcuKernel {
    hcomm::KernelCreator *creator = nullptr;
    hcomm::CcuKernelArg *kernelArg = nullptr;
};

std::mutex g_ccuKernelMutex;
std::unordered_map<CcuKernelHandle, std::unique_ptr<RegisteredCcuKernel>> g_ccuKernelRegistry;
}
...
HcclResult HcclCcuKernelRegister(HcclComm comm, CcuKernelHandle *kernelHandle, void *kernelCreator, void *kernelArg)
{
    ...
    auto registeredKernel = std::make_unique<RegisteredCcuKernel>();
    registeredKernel->creator = reinterpret_cast<hcomm::KernelCreator *>(kernelCreator);
    registeredKernel->kernelArg = reinterpret_cast<hcomm::CcuKernelArg *>(kernelArg);
...
HcclResult HcclCcuKernelLaunch(HcclComm comm, const ThreadHandle threadHandle, const CcuKernelHandle KernelHandle, void *taskArgs)
{
    ...
    hcomm::CcuKernelArg *kernelArg = nullptr;
    {
        ...
        kernelArg = it->second->kernelArg;
    }

    if (creator == nullptr || kernelArg == nullptr) {
        return HCCL_E_INTERNAL;
    }

    std::unique_ptr<hcomm::CcuKernel> kernel = (*creator)(*kernelArg);
    if (kernel == nullptr) {
        return HCCL_E_INTERNAL;
    }
