...
void TaskCheckOpSemantics::InitInputBuffer()
{
    for (auto &child : graphHead_->children) {
        ...
        CalcInputOutputSize(opType_, graphHead_->children.size(), dataCount_, dataType_, inputDataSize_, outputDataSize_, rankId, srcRank_, dstRank_, vDataDes_, all2AllDataDes_);
