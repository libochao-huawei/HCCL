...
void CalcInputOutputSize(HcclCMDType opType, uint32_t rankSize, uint64_t count, HcclDataType dataType, u64 &inputSize, u64 &outputSize, RankId myRank, RankId srcRank, RankId dstRank, VDataDesTag vDataDes, All2AllDataDesTag all2AllDataDes)
{
    u32 unitSize = 0;
    ...
    if (opType == HcclCMDType::HCCL_CMD_ALLREDUCE) {
        ...
        for (u32 i = 0; i < rankSize; i++) {
            u64 curSendCounts = all2AllDataDes.sendCountMatrix[myRank * rankSize + i];
            u64 curSendLength = curSendCounts * SIZE_TABLE[all2AllDataDes.sendType];
            curSendOffset += curSendLength;
            u64 curRecvCounts = all2AllDataDes.sendCountMatrix[myRank + rankSize * i];
