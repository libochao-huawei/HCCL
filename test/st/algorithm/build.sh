...
REPO_LIB_DIR="${HCCL_REPO_LIB_DIR:-}"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            cat <<EOF
        Usage: $(basename "$0") [--perf-rs] [--gtest_filter <expr>] [--cpu-list <list>] [--log-file <path>] [--log-level <level>] [--pin-rank-threads] [--rank-detail] [--perf-size <KiB>] [--perf-repeats <N>] [--repo-lib-dir <path>]
...
Options:
  ...
  --gtest_filter <expr>     Pass a custom gtest filter expression.
    ...
    --repo-lib-dir <path>     Prepend a repo-built `libhccl.so` directory before CANN libs.
...
Env:
  HCCL_GTEST_FILTER         Alternative way to set gtest filter.
    ...
    HCCL_REPO_LIB_DIR         Alternative way to prepend a repo-built `libhccl.so` directory.
EOF
            exit 0
            ;;
        *)
...
if [[ -n "${REPO_LIB_DIR}" ]]; then
    export LD_LIBRARY_PATH=${LIBRARY_DIR}${REPO_LIB_DIR}:${LD_LIBRARY_PATH}
    echo "[build.sh] Prepending repo lib dir: ${REPO_LIB_DIR}"
else
