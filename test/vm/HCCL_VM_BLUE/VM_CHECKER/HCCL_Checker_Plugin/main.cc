...
void RunChecker(const std::string& data_id) {
    ...
    HcclResult ret;
    ret = DumpTaskMetaToFile(storage.GetHvmTaskMetaData().task_meta);
    if (ret != HcclResult::HCCL_SUCCESS) {
        HCCL_VM_ERROR("Failed to dump TaskMeta, return {}", static_cast<u32>(ret));
    } else {
        HCCL_VM_INFO("Dump TaskMeta Success.");
    }
