    ...
    : TaskStub(TaskTypeStub::CCU_GRAPH), rankId(rank)
{
    ...
    des = StringFormat("[CcuGraph] : [rank=%u, mission=%u]", rankId, 1);
    if (instrInfo.size() == 0) {
...
void TaskStubCcuGraph::GetSqe(uint32_t queId, uint16_t sqeArgsId, uint64_t& argVal)
{
    ...
    // std::cout<<"zhf-[GetSqe]: queId= "<<queId<<", sqeArgsId= "<<sqeArgsId<<", argVal= "<<argVal<<std::endl;
...
std::string TaskStubCcuGraph::Describe() const
{
    ...
    // }
    return StringFormat("%s", des.c_str());
}
