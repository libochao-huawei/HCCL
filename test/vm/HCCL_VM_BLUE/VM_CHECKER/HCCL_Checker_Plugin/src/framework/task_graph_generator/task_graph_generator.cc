...
HcclResult TaskGraphGenerator::GenGraph(const AllRankTaskQueues &allRankTaskQueues, TaskNodePtr dummyStart)
{
    ...
    */
    ...
    HCCL_VM_INFO("[zhf] start gen ccu graph...{:s}", dummyStart->hasCcuTask);
    // 将CCU微码序列转换为Task子图
    if (dummyStart->hasCcuTask) {
        CHK_RET(HcclSim::GenCcuGraph(dummyStart));
    }
    HCCL_VM_INFO("[zhf] end gen ccu graph...{:s}", dummyStart->hasCcuTask);
