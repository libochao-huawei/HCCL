...
static void CmdTableShow(std::string &tableName)
{
    if (tableName == "Device") {
        ...
        PrintTable<sim::Host>("| id | server_id | ip | arch |", tables, [](const sim::Host &tmp) {
            return "| " + std::to_string(tmp.id) + " | " + std::to_string(tmp.server_id) + " | " +
                   std::to_string(tmp.ip) + " | " + std::to_string(tmp.arch) + " |";
        });
        ...
        PrintTable<sim::CcuResource>("| id | ccu_id | instr_cnt | xn[0] | gsa[0] | cke[0] | ms[0] |", tables,
                                     [](const sim::CcuResource &tmp) {
                                         return "| " + std::to_string(tmp.id) + " | " + std::to_string(tmp.ccu_id) +
                                                " | " + std::to_string(tmp.instr_cnt) + " | " +
                                                std::to_string(tmp.xn[0]) + " | " + std::to_string(tmp.gsa[0]) + " | " +
                                                std::to_string(tmp.cke[0]) + " | " + std::string(tmp.ms) + " |";
                                     });
        ...
        PrintTable<sim::EventSyncTask>(
            "| event_id | excute_time_ms | finish_time_ms | op_timeout_s |", tables, [](const sim::EventSyncTask &tmp) {
                return "| " + std::to_string(tmp.event_id) + " | " + std::to_string(tmp.excute_time_ms) + " | " +
...
void CmdTableHandle(CLI::App &app)
{
