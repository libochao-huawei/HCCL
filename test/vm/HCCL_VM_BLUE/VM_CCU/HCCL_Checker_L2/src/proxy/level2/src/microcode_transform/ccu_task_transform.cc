    ...
    uint32_t queId, bool& isContinue, LoopGroupParam* loopGroupParam)
{
    ...
    CHK_PRT_RET(len == 0, HCCL_ERROR("The size of data transfer is 0."), HcclVmResult::HCCL_SIM_E_INTERNAL);
    CHK_PRT_RET(len > UB_MAX_SIZE,
        HCCL_ERROR("The size of data transfer is more than max transport size of UB."), HcclVmResult::HCCL_SIM_E_INTERNAL);
...
HcclResult GenCcuGraph(TaskNode* dummyStart) {
    ...
    while(!candNode.empty()) {
        ...
        if (unmatchedCnt >= candNode.size()) {
            ...
            return HcclResult::HcclVmResult::HCCL_SIM_E_INTERNAL;
        }
