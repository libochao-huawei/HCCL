...
HcclVmResult CreateSimInstrData(HcclVmInstrData &hvmInstrData)
{
    ...
    for (auto &ccuRes : allCcuRes) {
        ...
        auto device = RunnerDB::GetById<sim::Device>(ccu->device_id);
        if (!device.has_value()) {
            printf("[ERROR][CreateSimInstrData] can not find device by key:%lu\n", ccu->device_id);
            return HcclVmResult::HCCL_SIM_E_INTERNAL;
        }

        mcInstr.desc.rank_id = device->logic_id;
