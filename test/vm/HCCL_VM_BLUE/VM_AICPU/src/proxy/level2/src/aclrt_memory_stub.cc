...
aclError aclrtFreeHost(void *hostPtr)
{
    ...
    HCCL_VM_INFO("[aclrtFreeHost] hostPtr:{:p} offsetPtr:0x{:x} siz:{:d}", hostPtr, phyMemRes->addr, phyMemRes->size);
...
aclError aclrtMalloc(void **devPtr, size_t size, aclrtMemMallocPolicy policy)
{
    ...
    *devPtr = virPtr;
    HCCL_VM_INFO("aclrtMalloc...., addr:{:p}, size:{:d}", virPtr, size);
    return ACL_SUCCESS;
}
...
aclError aclrtFree(void* devPtr)
{
    ...
    HCCL_VM_INFO("[aclrtFree] devPtr:{:p} offsetPtr:0x{:x} siz:{:d}", devPtr, phyMemRes->addr, phyMemRes->size);
    RunnerDB::Delete<sim::VirtualMemBlock>(virMemRes.first.id);
    RunnerDB::Delete<sim::PhyMemBlock>(phyMemRes->id);
