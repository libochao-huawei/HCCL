...
void ReduceMinExecutor::Parser()
{
    ...
    (void)memcpy_s(msId_, sizeof(uint16_t) * CCU_REDUCE_MAX_MS, instr_.v1.min.msId, sizeof(uint16_t) * CCU_REDUCE_MAX_MS);
}
...
// Reduce Min操作
void ReduceMinExecutor::Process(CcuResouceManager &ccuResMgr)
{
    HCCL_DEBUG("[ReduceMinExecutor][Process] Reduce Add info, locCcu[%d:%d], count:[%u], dataType:[%u]", rankId_, dieId_, count_, dataType_);
    ...
    if (ccuSimulator_->GetState() == CcuExecState::EXEC_LOOP_INSTR) {
        ...
        HCCL_DEBUG("[ReduceMinExecutor][Process] locCcu[%d:%d], ms offset=[%u]", rankId_, dieId_, msOffset);
    }
    // 2. reduce操作
...
{
    return Hccl::StringFormat("[Simulation Execute] Wait CKE[%u:%04x], Min %s with Count[%u], DataType[%u] and "
