...
void GenWaitNode(TaskStubCcuGraph *curCcuTask, uint32_t queId, uint16_t waitCKEId, uint16_t waitCKEMask)
{
    ...
    for (auto it = seenPosts.begin(); it != seenPosts.end();) {
        ...
        if (post->task->GetType() == TaskTypeStub::LOCAL_POST_TO) {
            post->children.push_back(localWaitNode);
            localWaitNode->parents.push_back(post);
        } else {
            post->children.push_back(remoteWaitNode);
            remoteWaitNode->parents.push_back(post);
...
{
    HCCL_VM_INFO("[ProcessWaitMask] Enter...rank {:d}, die{:d}, que{:d}, waitCke{:d}, waitMask{:d}",
    ...
    if (waitCKEMask != 0x0000) {
        ...
        if ((ckeValue & waitCKEMask) != waitCKEMask) {
            isContinue = 0;
            HCCL_VM_INFO("[ProcessWaitMask] Does not meet the condition, continue: ckeId: {:d}, ckeValue: {:d}, mask: {:x}", waitCKEId, ckeValue, waitCKEMask);
            return HCCL_SUCCESS;
        } else {
    ...
    uint16_t setCKEId, uint16_t setCKEMask)
{
    if (setCKEMask != 0x0000) {
        ...
        AddLocalPost(rankId, queId, dieId, curCcuTask, setCKEId, setCKEMask);
        ...
        HCCL_VM_INFO("[ProcessSetMask] Get rank: {:d}, die: {:d} CKE: {:d}, value: {:d}, mask: {:x}", rankId, static_cast<uint32_t>(dieId), setCKEId, ckeValue, setCKEMask);
    }
    ...
    uint32_t queId, bool& isContinue, LoopGroupParam* loopGroupParam)
{
    ...
    DataSlice dstSlice;
    RankId rId;
    // zhf-todo: 后续插件通过读取vm的输出文件，重建memlayout的地址类型信息
    ...
    HCCL_VM_INFO("[TransformTransLocMemToRmtMemInstr] curRank= {}, dstRank= {}", rankId, rmtRankId);
    // 说明transportId写错了
    if (rId != rmtRankId) {
    ...
    uint32_t queId, bool& isContinue, LoopGroupParam* loopGroupParam)
{
    ...
    std::cout<<"zhf-locmem2locmem: rankId="<<rankId<<", src= "<<srcSlice.Describe()<<", dst= "<<dstSlice.Describe()<<", size= "<<len<<std::endl;
    ...
    bool& isContinue, LoopGroupParam* loopGroupParam)
{
    ...
    if (locCKEMask != 0x0000) {
        ...
        HCCL_VM_INFO("[TransformSyncCKEInstr] Get loc rank: {:d}, die: {:d} CKE: {:d}, value: {:d}, mask: {:x}", rankId, static_cast<uint32_t>(dieId), locCKEId, localCkeValue, locCKEMask);
        localCkeValue = localCkeValue & locCKEMask;
        CHK_RET(AllRankParamRecorder::Global()->GetCKE(rmtRankId, rmtDieId, rmtCKEId, ckeValue));
        HCCL_VM_INFO("[TransformSyncCKEInstr] Get rmt rank: {:d}, die: {:d} CKE: {:d}, value: {:d}, mask: {:x}", rmtRankId, static_cast<uint32_t>(rmtDieId), rmtCKEId, ckeValue, locCKEMask);
        CHK_RET(AllRankParamRecorder::Global()->SetCKE(rmtRankId, rmtDieId, rmtCKEId, ckeValue | localCkeValue));
        HCCL_VM_INFO("[TransformSyncCKEInstr] Set rank: {:d}, die: {:d} CKE: {:d}, value: {:d}, mask: {:x}", rmtRankId, static_cast<uint32_t>(rmtDieId), rmtCKEId, localCkeValue, locCKEMask);
    }
    ...
    uint32_t queId, bool& isContinue, LoopGroupParam* loopGroupParam)
{
    ...
    RankId rmtRankId = g_allRankChannelInfo[rankId][dieId][channelId].dstRank;
    HCCL_VM_INFO("[TransformTransRmtMemToLocMemInstr] curRank= {}, dstRank= {}", rankId, rmtRankId);
    // 说明transportId写错了
    if (rId != rmtRankId) {
        HCCL_ERROR("remoteRankId calculated by rmtAddr and channelId %u is not equal,"
            "remoteId is %d, but the addr is in rank %d", channelId, rmtRankId, rId);
    ...
    uint32_t queId, bool& isContinue, LoopGroupParam* loopGroupParam, uint32_t loopGroupIdx)
{
    ...
    HCCL_VM_INFO("[ProcessLoopIns] debug...{}, {}", loopGroupIdx, curCcuTask->loopGroupInfo_.size());
    ...
    HCCL_VM_INFO("[LOOP] loop cnt = {}", static_cast<uint64_t>(xm.loopCnt));
    ...
    for (u32 curLoopCnt = 0; curLoopCnt < xm.loopCnt; curLoopCnt++) {
        ...
        HCCL_VM_INFO("[LOOP] loop cnt = {}, cur loop= {}", static_cast<uint64_t>(xm.loopCnt), curLoopCnt);
        for (uint16_t insId = startInstrId; insId <= endInstrId; insId++) {
            ...
            HCCL_INFO("Current process instruction id = %u\n", insId);
    ...
    bool& isContinue, LoopGroupParam* loopInfo)
{
    ...
    HCCL_VM_INFO("[LoopGroup]loop cnt= {}, loop offset= {}, expand cnt= {}",
...
HcclResult TransformInstrQue(TaskNodePtr node, TaskStubCcuGraph *curCcuTask, uint32_t queId)
{
    ...
    while (pos < endInstrId) {
        u32 prePos = pos;
        HCCL_INFO("Current process ccu graph: %s, instruction id = %u\n", curCcuTask->Describe().c_str(), pos);
    ...
    HCCL_VM_INFO("[TransformInstrQue] end...");
    return HCCL_SUCCESS;
}
...
void PrintCcuSingleQue(TaskNodePtr head, u32 rankId, u32 queueIdx)
{
    ...
    // 打印首节点
    HCCL_VM_INFO("{}", head->task->Describe());
    ...
    while(!candNode.empty()) {
        ...
        if (parentsAllPrint) {
            HCCL_VM_INFO("{}", curNode->task->Describe());
...
HcclResult GenCcuGraph(TaskNode* dummyStart) {
    ...
    while(!candNode.empty()) {
        ...
        std::cout<<"[DEBUG] curCcuTask rankId= "<<curCcuTask->Describe()<<std::endl;
