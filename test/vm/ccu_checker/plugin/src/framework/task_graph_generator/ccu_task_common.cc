...
void PrintGraphOneNode(TaskNodePtr curNode)
{
    ...
    if (curNode->task != nullptr) {
        if ((curNode->task->GetType() == TaskTypeStub::CCU_GRAPH || curNode->task->GetType() == TaskTypeStub::SUB_GRAPH_END)) {
            HCCL_VM_INFO("currNode is {}", curNode->task->GetType().Describe());
        } else {
            HCCL_VM_INFO("curNode[{:p}] is {:s}, queueId is {:d}, parent size={:d}, child size={:d}",
                static_cast<void*>(curNode), curNode->task->Describe(), curNode->queIdx, curNode->parents.size(), curNode->children.size());
        }
    }
    ...
    for (int i = 0; i < curNode->parents.size(); i++) {
        ...
        if (curNode->parents[i]->task->GetType() == TaskTypeStub::CCU_GRAPH || curNode->parents[i]->task->GetType() == TaskTypeStub::SUB_GRAPH_END) {
            HCCL_VM_INFO("currNode is {}", curNode->parents[i]->task->GetType().Describe());
            continue;
        }
        if (curNode->parents[i]->task != nullptr) {
            HCCL_VM_INFO("parents[{:d}][{:p}] is {:s}, queueId is {:d}",
                i, static_cast<void*>(curNode->parents[i]), curNode->parents[i]->task->Describe(), curNode->parents[i]->queIdx);
        }
    }
    HCCL_VM_INFO("-----------------------");
    for (int i = 0; i < curNode->children.size(); i++) {
        ...
        if (curNode->children[i]->task->GetType() == TaskTypeStub::CCU_GRAPH || curNode->children[i]->task->GetType() == TaskTypeStub::SUB_GRAPH_END) {
            HCCL_VM_INFO("currNode is {}", curNode->children[i]->task->GetType().Describe());
            continue;
        }
        HCCL_VM_INFO("children[{:d}][{:p}] is {:s}, queueId is {:d}",
            i, static_cast<void*>(curNode->children[i]), curNode->children[i]->task->Describe(), curNode->children[i]->queIdx);
    }
    HCCL_VM_INFO("-----------------------");
    HCCL_VM_INFO("");
}
...
void PrintNodeByTypes(TaskNodePtr curNode, std::set<TaskTypeStub> types)
{
    if (curNode->task != nullptr) {
        if (types.count(curNode->task->GetType()) != 0) {
            HCCL_VM_INFO("-----------------------");
            HCCL_VM_INFO("currNode is: {:p}: {:s}", static_cast<void*>(curNode), curNode->task->Describe());
            HCCL_VM_INFO("-----------------------");
        }
    }
}
    ...
    const std::vector<DataSlice> &srcSlices, const DataSlice &dstSlice, DataType hcclDataType)
{
    TaskStub *localBatchReduce = new TaskStubLocalBatchReduce(srcSlices, dstSlice,
        g_DataType2CheckerDataType_aicpu[hcclDataType], HcclReduceOp::HCCL_REDUCE_SUM);
    ...
    uint16_t setCKEMask)
{
    ...
    TaskStub *localPostTo = new TaskStubLocalPostTo(setCKEMask, queId);
