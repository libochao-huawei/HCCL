...
HcclResult GraphRevampParallel::ProcCcuNode4Async(TaskNodePtr ccuHead, RankId rankId)
{
    ...
    for (auto &asyncNode : curCcuTask->parallelNodes_) {
        CHK_RET(ProcAsyncNode(asyncNode, curCcuTask->queueNum_++));
    }
    return HcclResult::HCCL_SUCCESS;
}

HcclResult GraphRevampParallel::ProcAsyncNode(TaskNodePtr currNode, uint32_t virtualQueId)
{
    ...
    if (locPostNode != nullptr) {
        CHK_RET(RestorRealFlowConnection4Async(locPostNode, localPostHeadNode, locPostAsynChild));
    } else {
    ...
    TaskNodePtr locPostNode, TaskNodePtr localPostHeadNode, std::vector<TaskNodePtr> &locPostAsynChild)
{
    ...
    if (locPostAsynChild.size() == 1) {
        ...
        auto locWait = dynamic_cast<TaskStubLocalWaitFrom*>(locPostAsynChild[0]->task);
        if (locWait->GetNotifyId() != locPost->GetNotifyIdBack()) {
            HCCL_ERROR("[ProcAsyncNode] unexpected error: single async node has no pair localWait child");
            return HcclResult::HCCL_E_INTERNAL;
        }
    ...
    for (auto &child : locPostAsynChild) {
        if (child->task->GetType() == TaskTypeStub::LOCAL_WAIT_FROM) {
            auto locWait = dynamic_cast<TaskStubLocalWaitFrom*>(child->task);
            if (locWait->GetNotifyId() == locPost->GetNotifyIdBack()) {
                // 跳过与locPost配对的localWait
                continue;
            }
        }
    ...
    std::vector<TaskNodePtr> &beforeAsynNodes, std::vector<TaskNodePtr> &locPostAsynChild)
{
    ...
    // todo: async节点可能同时对应多个locPost、post节点，甚至混合节点。  ———— 如何处理？？？
    // 查找async节点对应的{localPost, localWait}节点
    for (auto childIter = currNode->children.begin(); childIter != currNode->children.end(); childIter++) {
        if ((*childIter)->queIdx != currNode->queIdx) {
            continue;
        }
        if ((locAsyncNodes_.count(currNode->task->GetType()) != 0 && (*childIter)->task->GetType() == TaskTypeStub::LOCAL_POST_TO)) {
            locPostNode = (*childIter);
            locPostNode->asynParallel = true;
            break;
        }
    }
    if (locPostNode == nullptr) {
