...
bool ParseYamlTopo(std::string& fileName, TopoMeta& topo, std::vector<std::string> &serverIdx2Ip) {
    try {
        ...
        if (checkPodNum != podNum) {
            std::cerr << "[HVM] [ERROR] YAML : Pod count mismatch! Meta podNum: " << podNum
                      << ", Actual: " << checkPodNum << std::endl;
            isSuccess = false;
        }
        if (checkSerNum != serNum) {
            std::cerr << "[HVM] [ERROR] YAML : Pod count mismatch! Meta serNum: " << serNum
                      << ", Actual: " << checkSerNum << std::endl;
            isSuccess = false;
        }
        if (checkRankNum != rankNum) {
            std::cerr << "[HVM] [ERROR] YAML : Pod count mismatch! Meta rankNum: " << rankNum 
