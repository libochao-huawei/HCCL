...
struct InitParams {
    ...
    bool create;
    InitParams()
        : db("SimShmDB", static_cast<uint64_t>(1024) * 1024 * 1024),
          pool("SimShmPool", static_cast<uint64_t>(8) * 1024 * 1024 * 1024),
          create(false)
    {
    }
};
...
public:
    ...
    void *AllocatePhy(size_t size)
    {
        ...
        printf("[TEST] m_poolMutex:%p %lu \n", m_poolMutex, (uintptr_t)(m_poolMutex));
        try {
