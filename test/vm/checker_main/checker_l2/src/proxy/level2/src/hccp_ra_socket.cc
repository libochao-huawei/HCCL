...
int RaSocketInit(int mode, struct rdev rdevInfo, void **socketHandle)
{
    ...
    *socketHandle = (void *)(uintptr_t)socketfd;
    ...
    HCCL_VM_INFO("[RASOCKET] add socketHandle:{:d} device:{:d} ip:{} portId:{:d}", socketfd, deviceIdx, ipAddr, ipIdx);
    return 0;
}
...
int RaSocketBatchConnect(struct SocketConnectInfoT conn[], unsigned int num)
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        auto ipAddr = Hccl::IpAddress(ba, AF_INET6).GetIpStr().substr(2);
        HCCL_VM_INFO("[RASOCKET] connect ip addr:{}", ipAddr);
        sim::Port port{};
        if (GetPortByIpAddr(ipAddr, port) != 0) {
            HCCL_VM_ERROR("cannot find remote ip{} addr:{:d}", ipAddr, conn[i].remoteIp.addr.s_addr);
            return -1;
        }
        ...
        while (true) {
            ...
            auto socketRes = RunnerDB::GetOneByPred<sim::RaSocket>(
                [device_id, ipIdx](const sim::RaSocket &socket) { return ((socket.device_id == device_id) && (socket.ip_id == ipIdx)); });
            if (!socketRes.second) {
            ...
            HCCL_VM_INFO("[RASOCKET] add socket pair local:{:d}, peer:{:d}", socketHandle, socketRes.first.id);
            break;
        }
    }
    return 0;
...
int RaGetSockets(unsigned int role, struct SocketInfoT conn[], unsigned int num, unsigned int *connectedNum)
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        while (true) {
            ...
            auto ipAddr = Hccl::IpAddress(ba, AF_INET6).GetIpStr().substr(2);
            sim::Port port{};
            HCCL_VM_INFO("[RASOCKET]Get Ip Addr:{}", ipAddr);
            if (GetPortByIpAddr(ipAddr, port) != 0) {
                HCCL_VM_ERROR("cannot find remote ip:{} addr:{:d}", ipAddr, conn[i].remoteIp.addr.s_addr);
                return 0;
            }
...
int RaSocketRecv(const void *fdHandle, void *data, unsigned long long size, unsigned long long *receivedSize)
{
    ...
    while (retryCnt++ <= 5) {
        ipc::scoped_lock<ipc::interprocess_mutex> lock(buf->mutex);
        if (buf->size == 0) {
            HCCL_VM_WARN("localFd :{:d} return  socket eagain key:{}", socketFd, socketKey);
            sleep(1);
            continue;
        }
