...
HcclResult PrepareHostAicpuSyncThreads(HcclComm comm, OpParam &param, aclrtStream stream)
{
    ...
    char *mainThreadFieldPtr = reinterpret_cast<char *>(param.resCtx) + offsetof(AlgResourceCtx, mainThreadHandle);
    ACLCHECK(aclrtMemcpy(&mainThreadHandle,
        sizeof(ThreadHandle),
        mainThreadFieldPtr,
        sizeof(ThreadHandle),
        ACL_MEMCPY_DEVICE_TO_HOST));
    CHK_PRT_RET(mainThreadHandle == 0,
        HCCL_ERROR("mainThreadHandle loaded from resCtx is invalid"),
        HCCL_E_INTERNAL);

    CHK_RET(HcclThreadAcquireWithStream(comm, COMM_ENGINE_CPU_TS, stream, 1, &param.cpuThread));
    CHK_RET(HcclThreadExportToCommEngine(
        comm, 1, &param.cpuThread, COMM_ENGINE_AICPU_TS, &cpuThreadOnAicpu));
    CHK_RET(HcclThreadExportToCommEngine(
        comm, 1, &mainThreadHandle, COMM_ENGINE_CPU_TS, &param.aicpuThreadOnCpu));
...
HcclResult GetAlgRes(HcclComm comm, const OpParam &param, AlgResourceCtx **resCtx)
{
    ...
    const CommEngine engine = COMM_ENGINE_AICPU_TS;
    ...
    const HcclAllGatherItem *items, uint32_t itemCount, HcclComm comm, aclrtStream stream)
{
    ...
    param.resCtx = resCtx;
    param.startThreadNotifyIdx = SubThreadNum;
