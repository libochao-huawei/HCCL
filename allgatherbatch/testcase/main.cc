...
int RunOnDevice(void *arg)
{
    ...
    const size_t totalBytes = token.sendBytes + (ctx->options.scaleCount > 0 ? scale.sendBytes : 0);
    const unsigned long long dataSize = static_cast<unsigned long long>(totalBytes * ctx->rankSize);
